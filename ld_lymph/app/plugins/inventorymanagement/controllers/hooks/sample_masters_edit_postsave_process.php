<?php
	...
	if($is_specimen && $this->data['SampleMaster']['specimen_number'] != $sample_data['SampleMaster']['specimen_number']) {
		$specimen_number = $this->data['SampleMaster']['specimen_number'];
		...
		if(!empty($all_derivatives)) {
			...
			foreach($all_derivatives as $samp_to_update) {
				$all_derivatives_to_update[] = array('SampleMaster' => array('id'=>$samp_to_update['SampleMaster']['id'], 'specimen_number'=>$specimen_number));	
			}

 ...
 * dbo_sqlsrv.php is based on with dbo_mssql.php
 * Update by Frank-Mich on 2011-02-01
 *
 ...
 */
	function __construct($config, $autoConnect = true) {
		...
		if ($autoConnect) {
			...
			sqlsrv_configure("LogSusbystem", SQLSRV_LOG_SYSTEM_ALL);
 ...
 */
	function connect() {
		...
		if ($config['persistent']) {
			die("sqlsrv driver does not support persistent connections");
		} else {
			$this->connection = sqlsrv_connect($config['host'] . $port, $connection_info);
		}
 ...
 */
	function resultSet(&$results) {
		...
		foreach(sqlsrv_field_metadata($this->results) as $fieldMetadata){
			$column_name = $fieldMetadata['Name'];
			if (strpos($column_name, '__')) {
				if (isset($this->__fieldMappings[$column_name]) && strpos($this->__fieldMappings[$column_name], '.')) {
					$map = explode('.', $this->__fieldMappings[$column_name]);
				} elseif (isset($this->__fieldMappings[$column_name])) {
					$map = array(0, $this->__fieldMappings[$column_name]);
				} else {
					$map = array(0, $column_name);
				}
				$this->map[$index++] = $map;
			} else {

...
class AliquotMastersController extends InventoryManagementAppController {
	...
	function deleteSourceAliquot($sample_master_id, $aliquot_master_id, $source) {
		...
		// -> Update volume
		if($deletion_done) {
			if(!$this->AliquotMaster->updateAliquotCurrentVolume($source_data['AliquotMaster']['id'])) { $deletion_done = false; }
		}
		...
		if($initial_display){
			...
			if(!empty($errors)){
				...
				foreach($submitted_data as $parent_id => $children){
					...
					// Update aliquot current volume
					$this->AliquotMaster->updateAliquotCurrentVolume($parent_id, false);
				}
	...
	function deleteRealiquotingData($parent_id, $child_id, $source) {
		...
		// -> Update volume
		if($deletion_done) {
			if(!$this->AliquotMaster->updateAliquotCurrentVolume($realiquoting_data['AliquotMaster']['id'])) { $deletion_done = false; }
		}

    ...
    if(isset($this->request->data[0]['parent']['AliquotMaster'])) {
	    ...
	    foreach($this->request->data as $tmp_procure_new_data_set) {
	        ...
	        if($tmp_procure_new_data_set['parent']['ViewSample']['procure_created_by_bank'] == 's') {
	            $tmp_aliquots_count = $this->AliquotMaster->find('count', array('conditions' => array('AliquotMaster.sample_master_id' => $tmp_procure_new_data_set['parent']['ViewSample']['sample_master_id'])));
	    ...
	    if($data_set_nbr_in_error) {
	        $this->flash(
	            __('no quality control can be created from sample created by system/script to migrate data from the processing site with no aliquot').' '.str_replace('%s', '['.implode('] ,[',$data_set_nbr_in_error).']', __('see # %s')),
	            $cancel_button,
	            5);
	        return;
	    }

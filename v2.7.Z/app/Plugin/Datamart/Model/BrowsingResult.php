...
class BrowsingResult extends DatamartAppModel
{
    ...
    function getModelAndStructureForNode($browsing_result)
    {
        ...
        if ($control_id) {
            ...
            $control_model_data = $control_model->find('first', array(
                ...
                'conditions' => array(
                    $control_model->name . ".id" => $control_id
                )
            ));
            ...
            AppController::buildDetailBinding($model, array(
                $model->name . '.' . $model->getControlForeign() => $control_id
            ), $structure_alias);
    ...
    function countMaxDuplicates($base_node_id, $target_node_id)
    {
        ...
        $data = $base_model->find('first', array(
            'fields' => array(
                'COUNT(' . $final_model->name . '.' . $final_model->primaryKey . ') AS c'
            ),
            'conditions' => array(),
            'joins' => $joins,

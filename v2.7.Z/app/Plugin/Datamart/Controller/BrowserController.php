...
class BrowserController extends DatamartAppController
{
    ...
    private function getIdsAndParentChild($control_id)
    {
        ...
        if (in_array(substr($control_id, - 1), array(
            'c',
            'p'
        ))) {
...
$check_list) // this is a checklist
{
        ...
        if ($check_list) {
            ...
            $this->Browser->initDataLoad($browsing, $merge_to, explode(",", $browsing['BrowsingResult']['id_csv']), $order);
            ...
            $dropdown_options = $this->Browser->getBrowserDropdownOptions($browsing['DatamartStructure']['id'], $node_id, $browsing['DatamartStructure']['plugin'], $this->Browser->checklist_model->name, $browsing['DatamartStructure']['model'], $this->Browser->checklist_use_key, $browsing_model->primaryKey, $this->Browser->checklist_sub_models_id_filter);
            ...
            if ($this->Browser->checklist_model->name != $browsing['DatamartStructure']['model']) {
                $browsing['DatamartStructure']['index_link'] = str_replace($browsing['DatamartStructure']['model'], $this->Browser->checklist_model->name, str_replace($browsing['DatamartStructure']['model'] . "." . $browsing_model->primaryKey, $this->Browser->checklist_model->name . "." . $this->Browser->checklist_use_key, $browsing['DatamartStructure']['index_link']));
            }
            ...
            if ($this->Browser->count <= Configure::read('databrowser_and_report_results_display_limit')) {
                ...
                $this->set("header", array(
                    'title' => __('result'),
                    'description' => $this->Browser->checklist_header
                ));
            } else {
                // overflow
    ...
    function csv($all_fields, $node_id, $merge_to)
    {
        ...
        if ($config['redundancy'] == 'same' && isset($config['singleLineNodes']) && ! empty($config['singleLineNodes'])) {
            ...
            while ($primary_data = $models[$node_id]->find('all', array(
                'conditions' => array(
                    $models[$node_id]->name . '.' . $models[$node_id]->primaryKey => $ids
                ),
                ...
                $base_model_condition = array(
                    $models[$node_id]->name . '.' . $models[$node_id]->primaryKey => array_keys($primary_data)
                );
                foreach ($nodes_info as $key => $node_info) {
                    ...
                    $data = $models[$key]->find('all', array(
                        ...
                        'joins' => $joins[$key],
                        'conditions' => array_merge($base_model_condition, array(
                            $models[$key]->name . '.' . $models[$key]->primaryKey . ' IN (' . $browsing_results[$key]['BrowsingResult']['id_csv'] . ')'
                        ))
                    ));
    ...
    function unusedParent($node_id)
    {
        ...
        if (empty($control)) {
            ...
            $parent_key_used_data = $parent_model->find('all', array(
                'fields' => array(
                    $parent_model->name . '.' . $parent_model->primaryKey
                ),
                'conditions' => array(
                    $parent_model->name . '.' . $control['BrowsingControl']['use_field'] => explode(',', $child_data['BrowsingResult']['id_csv'])
                )
            ));
        } else {
            // load the child model
            ...
            $parent_key_used_data = $child_model->find('all', array(
                'fields' => array(
                    $child_model->name . '.' . $control['BrowsingControl']['use_field']
                ),
                'conditions' => array(
                    $child_model->name . '.' . $child_model->primaryKey => explode(',', $child_data['BrowsingResult']['id_csv'])
                )
            ));
        }
    ...
    function applyBrowsingSteps($starting_node_id, $browsing_step_index_id)
    {
        ...
        $browsing_steps = $this->SavedBrowsingIndex->find('first', array(
            'conditions' => array_merge($this->SavedBrowsingIndex->getOwnershipConditions(), array(
                'SavedBrowsingIndex.id' => $browsing_step_index_id
            ))
        ));
        if (! $browsing_steps) {
            $this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);

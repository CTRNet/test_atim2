...
class CodingIcdAppModel extends AppModel
{
    ...
    function getDescription($id, $is_search_form = false, $data_array = null)
    {
        ...
        if (! $data_array)
            $data_array = $this->find('first', array(
                'conditions' => array(
                    'id' => $id
                )
            ));
        ...
        if (is_array($data_array) && ! empty($data_array)) {
            ...
            foreach ($this->icd_description_table_fields[$is_search_form ? 'search_format' : 'detail_format'] as $field_to_display)
                if (isset($data_array[$lang . "_" . $field_to_display]))
                    $description[] = $data_array[$lang . "_" . $field_to_display];
        }
    ...
    function globalSearch(array $terms, $exact_search, $search_on_id, $limit)
    {
        ...
        $filter_key = $lang . '_';
        $search_fields = array_filter($search_fields, function ($in) use ($filter_key) {
            return strpos($in, $filter_key) === 0;
        });
        if ($search_on_id) {
            $search_fields[] = "id";
        }
        ...
        foreach ($data as &$new_icd_data)
            $new_icd_data['CodingIcd']['generated_detail'] = $this->getDescription($new_icd_data['CodingIcd']['id'], 1, $new_icd_data);
        return $data;
    }

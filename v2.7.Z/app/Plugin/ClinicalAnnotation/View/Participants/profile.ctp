...
foreach ($identifier_controls_list as $new_option) {
    $identifiers_menu[$new_option['MiscIdentifierControl']['misc_identifier_name']] = isset($new_option['reusable']) && $link_availability ? 'javascript:miscIdPopup(' . $atim_menu_variables['Participant.id'] . ' ,' . $new_option['MiscIdentifierControl']['id'] . ');' : '/ClinicalAnnotation/MiscIdentifiers/add/' . $atim_menu_variables['Participant.id'] . '/' . $new_option['MiscIdentifierControl']['id'] . '/';
}

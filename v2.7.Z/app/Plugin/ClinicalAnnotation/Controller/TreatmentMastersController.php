...
class TreatmentMastersController extends ClinicalAnnotationAppController
{
    ...
    function listall($participant_id, $treatment_control_id = null)
    {
        ...
        if (! $treatment_control_id) {
            // 1 - MANAGE DISPLAY
            $treatment_controls = $this->TreatmentControl->find('all', array(
                'conditions' => array(
            ...
            foreach ($treatment_controls as $new_ctrl) {
                if ($new_ctrl['TreatmentControl']['use_detail_form_for_index']) {
                    ...
                    $controls_for_subform_display['-1']['TreatmentControl'] = array(
                        'id' => '-1',
                        'tx_header' => null
                    );
                }
    ...
    function add($participant_id, $tx_control_id, $diagnosis_master_id = null)
    {
        ...
        $this->Structures->set('view_diagnosis', 'diagnosis_structure');
        $this->Structures->set($tx_control_data['TreatmentControl']['form_alias'], 'atim_structure', array(
            'model_table_assoc' => array(
                'TreatmentDetail' => $tx_control_data['TreatmentControl']['detail_tablename']
            )
        ));
        $this->Structures->Set('empty', 'empty_structure');
    ...
    function delete($participant_id, $tx_master_id)
    {
        ...
        if ($arr_allow_deletion['allow_deletion']) {
            if ($this->TreatmentMaster->atimDelete($tx_master_id)) {
                ...
                $this->atimFlashError(__('error deleting data - contact administrator'), '/ClinicalAnnotation/TreatmentMasters/listall/' . $participant_id);
            }
        } else {

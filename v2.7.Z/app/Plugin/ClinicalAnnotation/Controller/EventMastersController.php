        ...
        if (! $event_control_id) {
            ...
            foreach ($event_controls as $new_ctrl) {
                if ($new_ctrl['EventControl']['use_detail_form_for_index']) {
                    ...
                    $controls_for_subform_display['-1']['EventControl'] = array(
                        'id' => '-1',
                        'ev_header' => null
                    );
                }
            }
            if (empty($controls_for_subform_display['-1']))
                unset($controls_for_subform_display['-1']);
    ...
    function delete($participant_id, $event_master_id)
    {
        ...
        if ($arr_allow_deletion['allow_deletion']) {
            if ($this->EventMaster->atimDelete($event_master_id)) {
                ...
                $this->atimFlashError(__('error deleting data - contact administrator'), '/ClinicalAnnotation/EventMasters/listall/' . $event_group . '/' . $participant_id);
            }
        } else {
            $this->atimFlashWarning(__($arr_allow_deletion['msg']), '/ClinicalAnnotation/EventMasters/detail/' . $participant_id . '/' . $event_master_id);
        }
    }
}
...
$final_options = array(
    ...
    'settings' => array(
        'header' => array(
            ...
            'description' => (isset($storage_description) ? __('you are about to create an use/event for %s aliquot(s) contained into %s', substr_count($aliquot_ids, ',') + 1, $storage_description) : __('you are about to create an use/event for %d aliquot(s)', substr_count($aliquot_ids, ',') + 1))
        ),

...
class AliquotMastersController extends InventoryManagementAppController
{
    ...
    function addInit()
    {
        ...
        $init_data = $this->batchInit($this->SampleMaster, $model, $key, 'sample_control_id', $this->AliquotControl, 'sample_control_id', 'you cannot create aliquots with this sample type');
    ...
    function add($sample_master_id = null, $aliquot_control_id = null, $quantity = 1)
    {
        ...
        if ($this->request->data[0]['realiquot_into'] == "") {
            $this->atimFlashWarning__('you need to select an aliquot type'), $url_to_cancel);
            return;
        }
        ...
        $samples = $this->ViewSample->find('all', array(
            'conditions' => array(
                'sample_master_id' => $sample_master_ids
            ),
            'recursive' => - 1
        ));
        ...
        if ($is_batch_process) {
            $this->setBatchMenu(array(
                'SampleMaster' => $sample_master_ids
            ));
        } else {
        ...
        // set structure
        $this->Structures->set($aliquot_control['AliquotControl']['form_alias'], 'atim_structure', array(
            'model_table_assoc' => array(
                'AliquotDetail' => $aliquot_control['AliquotControl']['detail_tablename']
            )
        ));
        if ($is_batch_process) {
        ...
        list ($override_data['AliquotMaster.storage_datetime'], $override_data['AliquotMaster.storage_datetime_accuracy']) = $is_batch_process ? array(
            date('Y-m-d G:i'),
            'c'
     ...
     * @param unknown_type $is_from_tree_view_or_layout
    ...
    function detail($collection_id, $sample_master_id, $aliquot_master_id, $is_from_tree_view_or_layout = 0)
    {
        ...
        // Set storage data
        $this->set('aliquot_storage_data', empty($aliquot_data['StorageMaster']['id']) ? array() : array(
            'StorageMaster' => $aliquot_data['StorageMaster']
        ));
        ...
        $order_item = $this->OrderItem->find('first', array(
            'conditions' => array(
                'OrderItem.aliquot_master_id' => $aliquot_master_id
            )
        ));
        if (! empty($order_item)) {
        ...
        $sample_master = $this->SampleMaster->find('first', array(
            'conditions' => array(
                'SampleMaster.id' => $sample_master_id
            ),
            'recursive' => - 1
        ...
        $ptdsc = $ptdsc_model->find('first', array(
            'conditions' => array(
                'ParentToDerivativeSampleControl.parent_sample_control_id' => $sample_master['SampleMaster']['sample_control_id']
            ),
            'recursive' => - 1
        ));
    ...
    function edit($collection_id, $sample_master_id, $aliquot_master_id)
    {
        ...
        // Set structure
        $this->Structures->set($aliquot_data['AliquotControl']['form_alias'], 'atim_structure', array(
            'model_table_assoc' => array(
                'AliquotDetail' => $aliquot_data['AliquotControl']['detail_tablename']
            )
        ));
        ...
        if (empty($this->request->data)) {
            $aliquot_data['FunctionManagement']['recorded_storage_selection_label'] = $this->StorageMaster->getStorageLabelAndCodeForDisplay(array(
                'StorageMaster' => $aliquot_data['StorageMaster']
            ));
            $aliquot_data['FunctionManagement']['autocomplete_aliquot_master_study_summary_id'] = $this->StudySummary->getStudyDataAndCodeForDisplay(array(
                'StudySummary' => array(
                    'id' => $aliquot_data['AliquotMaster']['study_summary_id']
                )
            ));
    ...
    function delete($collection_id, $sample_master_id, $aliquot_master_id)
    {
        ...
        if ($arr_allow_deletion['allow_deletion']) {
            if ($this->AliquotMaster->atimDelete($aliquot_master_id)) {
                ...
                $this->atimFlashError(__('error deleting data - contact administrator'), '/InventoryManagement/SampleMasters/detail/' . $collection_id . '/' . $sample_master_id);
            }
        } else {
            $this->atimFlashWarning(__($arr_allow_deletion['msg']), '/InventoryManagement/AliquotMasters/detail/' . $collection_id . '/' . $sample_master_id . '/' . $aliquot_master_id);
        }
    }
    ...
    function addAliquotInternalUse($aliquot_master_id = null)
    {
        ...
        if ($aliquot_master_id != null) {
            ...
            $unconsented_aliquots = $this->AliquotMaster->getUnconsentedAliquots(array(
                'id' => $aliquot_ids
            ));
        ...
        if ($initial_display) {
            ...
            if (empty($errors)) {
                ...
                $this->AliquotInternalUse->saveAll($uses_to_save, array(
                    'validate' => false
                ));
    ...
    function editAliquotInternalUse($aliquot_master_id, $aliquot_use_id)
    {
        ...
        if (empty($this->request->data)) {
            ...
            $use_data['FunctionManagement']['autocomplete_aliquot_internal_use_study_summary_id'] = $this->StudySummary->getStudyDataAndCodeForDisplay(array(
                'StudySummary' => array(
                    'id' => $use_data['AliquotInternalUse']['study_summary_id']
                )
            ));
            $this->request->data = $use_data;
    ...
    function addInternalUseToManyAliquots()
    {
        ...
        $studied_aliquot_nbrs = $this->AliquotMaster->find('count', array(
            'conditions' => array(
                'AliquotMaster.id' => $aliquot_ids
            ),
            'recursive' => '-1'
        ...
        $all_volume_units = $this->AliquotControl->find('all', array(
            'conditions' => array(
                'AliquotControl.id' => $aliquot_control_ids
            ),
            'fields' => array(
    ...
    function addSourceAliquots($collection_id, $sample_master_id)
    {
        ...
        $existing_source_aliquots = $this->SourceAliquot->find('all', array(
            'conditions' => array(
        ...
        $criteria = array(
            ...
            'NOT' => array(
                'AliquotMaster.id' => $existing_source_aliquot_ids
            )
        );
        $available_sample_aliquots_wo_volume = $this->AliquotMaster->find('all', array(
            'conditions' => $criteria,
        ...
        $available_sample_aliquots_w_volume = $this->AliquotMaster->find('all', array(
            'conditions' => $criteria,
            'order' => 'AliquotMaster.barcode ASC',
            'recursive' => '0'
        ));
        ...
        if (empty($available_sample_aliquots_w_volume) && empty($available_sample_aliquots_wo_volume)) {
            $this->atimFlashWarning__('no new sample aliquot could be actually defined as source aliquot'), '/InventoryManagement/SampleMasters/detail/' . $collection_id . '/' . $sample_master_id);
        }
        ...
        if (empty($this->request->data)) {
            ...
            if (! $submitted_data_validates) {
                ...
                $this->atimFlash(__('your data has been saved') . '<br>' . __('aliquot storage data were deleted (if required)'), '/InventoryManagement/SampleMasters/detail/' . $collection_id . '/' . $sample_master_id);
            }
        }
    }
    ...
    function realiquotInit($process_type, $aliquot_id = null)
    {
        ...
        if (! empty($aliquot_id)) {
            ...
            if (! is_array($ids) && strpos($ids, ',')) {
                ...
                $this->atimFlashWarning__("batch init - number of submitted records too big"), "javascript:history.back();", 5);
                return;
            }
        ...
        if (count($aliquots) > 1) {
            foreach ($aliquots as $aliquot) {
                if (($aliquot['AliquotMaster']['aliquot_control_id'] != $aliquot_ctrl_id) || ($aliquot['SampleMaster']['sample_control_id'] != $sample_ctrl_id)) {
                    $this->atimFlashWarning__("you cannot realiquot those elements together because they are of different types"), "javascript:history.back();");
                    return;
                }
            }
        }
        ...
        if (empty($possible_ctrl_ids)) {
            $this->atimFlashWarning__("you cannot realiquot those elements"), "javascript:history.back();", 5);
            return;
        }
    ...
    function realiquotInit2($process_type, $aliquot_id = null)
    {
        if (! isset($this->request->data['sample_ctrl_id']) || ! isset($this->request->data['realiquot_from']) || ! isset($this->request->data[0]['realiquot_into']) || ! isset($this->request->data[0]['ids'])) {
            ...
            if ($this->request->data[0]['realiquot_into'] == '') {
                $this->atimFlashWarning__("you must select an aliquot type"), "javascript:history.back();", 5);
                return;
            }
    ...
    function realiquot($aliquot_id = null)
    {
        ...
        if (isset($this->request->data['Realiquoting']) && isset($this->request->data['Realiquoting']['lab_book_master_code']) && (strlen($this->request->data['Realiquoting']['lab_book_master_code']) > 0 || $this->request->data['Realiquoting']['sync_with_lab_book'])) {
            ...
            if (is_numeric($sync_response)) {
                ...
                $this->atimFlashWarning__($sync_response), "javascript:history.back()", 5);
                return;
            }
        }
        ...
        AppModel::$writable_fields = array();
        $this->Structures->set($child_aliquot_ctrl['AliquotControl']['form_alias'] . (empty($parent_aliquot_ctrl['AliquotControl']['volume_unit']) ? ',realiquot_without_vol' : ',realiquot_with_vol'), 'atim_structure', array(
            'model_table_assoc' => array(
                'AliquotDetail' => $child_aliquot_ctrl['AliquotControl']['detail_tablename']
            )
        ));
        ...
        if ($initial_display) {
            ...
            $parent_aliquots = $this->AliquotMaster->find('all', array(
                'conditions' => array(
                    'AliquotMaster.id' => explode(",", $parent_aliquots_ids)
                ),
                'recursive' => 0
            ));
            $display_limit = Configure::read('RealiquotedAliquotCreation_processed_items_limit');
            if (sizeof($parent_aliquots) > $display_limit) {
                $this->atimFlashWarning__("batch init - number of submitted records too big") . " (>$display_limit)", $this->request->data['url_to_cancel'], 5);
                return;
            }
            ...
            foreach ($this->request->data as $parent_id => $parent_and_children) {
                ...
                if ($used_aliquot_data_to_apply_to_all)
                    $parent_and_children = array_replace_recursive($parent_and_children, $used_aliquot_data_to_apply_to_all);
                ...
                $this->AliquotMaster->set(array(
                    "AliquotMaster" => $parent_aliquot_data
                ));
                if (! $this->AliquotMaster->validates()) {
                    foreach ($this->AliquotMaster->validationErrors as $field => $msgs) {
                        $msgs = is_array($msgs) ? $msgs : array(
            ...
            if (empty($errors) && ! empty($lab_book_code)) {
                ...
                foreach ($this->request->data as $key => &$new_data_set) {
                    $lab_book->syncData($new_data_set['children'], array(
                        'Realiquoting'
                    ), $lab_book_code);
                }
            }
            
            $child_writable_fields['aliquot_masters']['addgrid'] = array_merge($child_writable_fields['aliquot_masters']['addgrid'], array(
                'collection_id',
                'sample_master_id',
                'aliquot_control_id',
                'storage_coord_x',
            ...
            $child_writable_fields['realiquotings']['addgrid'] = array_merge($child_writable_fields['realiquotings']['addgrid'], array(
            ...
            if (empty($errors)) {
                ...
                foreach ($this->request->data as $parent_id => $parent_and_children) {
                    ...
                    foreach ($storage_writable_fields as $new_writable_field)
                        AppModel::$writable_fields['aliquot_masters']['edit'][] = $new_writable_field;
    ...
    function defineRealiquotedChildren($aliquot_master_id = null)
    {
        ...
        if ($initial_display) {
            ...
            foreach ($parent_aliquots as $parent_aliquot_data) {
                ...
                $aliquot_to_exclude = array(
                    $parent_aliquot_data['AliquotMaster']['id']
                );
                ...
                $criteria = array(
                    ...
                    'NOT' => array(
                        'AliquotMaster.id' => $aliquot_to_exclude
                    )
                );
            ...
            if (! empty($excluded_parent_aliquot)) {
                ...
                if (empty($this->request->data)) {
                    $this->atimFlashWarning__($msg), "javascript:history.back()", 5);
                    return;
                } else {
                ...
                foreach ($parent_and_children as $tmp_id => &$children_aliquot) {
                    if (is_numeric($tmp_id)) {
                        if ($children_aliquot['FunctionManagement']['use']) {
                            ...
                            $this->Realiquoting->set(array(
                                'Realiquoting' => $children_aliquot['Realiquoting']
                            ));
                            if (! $this->Realiquoting->validates()) {
                                foreach ($this->Realiquoting->validationErrors as $field => $msgs) {
                                    $msgs = is_array($msgs) ? $msgs : array(
            ...
            if (empty($errors)) {
                ...
                foreach ($this->request->data as $parent_id => $parent_and_children) {
                    ...
                    if (! $this->AliquotMaster->save(array(
                        'AliquotMaster' => $parent_data['AliquotMaster']
                    ), false)) {
                        $this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
                    }
                    ...
                    foreach ($parent_and_children['children'] as $children_aliquot_to_save) {
                        if ($children_aliquot_to_save['FunctionManagement']['use']) {
                            ...
                            if (! $this->Realiquoting->save(array(
                                'Realiquoting' => $children_aliquot_to_save['Realiquoting']
                            ), false)) {
                                $this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
                            }
    ...
    function listAllRealiquotedParents($collection_id, $sample_master_id, $aliquot_master_id)
    {
        ...
        $this->request->data = $this->Realiquoting->find('all', array(
            ...
            'conditions' => array(
                'Realiquoting.child_aliquot_master_id' => $aliquot_master_id
            )
        ));
    ...
    function deleteRealiquotingData($parent_id, $child_id, $source)
    {
        ...
        if ($arr_allow_deletion['allow_deletion']) {
            ...
            $this->atimFlashWarning(__($arr_allow_deletion['msg']), $flash_url);
        }
    }
    ...
    function contentTreeView($collection_id, $aliquot_master_id, $is_ajax = false)
    {
        ...
        $realiquoting_data_from_child_ids = array(
            '-1' => array()
        ); // counters Eventum 1353
        foreach ($this->Realiquoting->find('all', array(
            'conditions' => array(
                'Realiquoting.parent_aliquot_master_id' => $aliquot_master_id
            ),
            'recursive' => '-1'
        ...
        foreach ($this->request->data as &$new_children_aliquot_data)
            $new_children_aliquot_data = array_merge($new_children_aliquot_data, $realiquoting_data_from_child_ids[$new_children_aliquot_data['AliquotMaster']['id']]);
        ...
        foreach ($tmp_aliquot_uses as $new_aliquot_use) {
            ...
            $new_aliquot_use = array_merge(array(
                $model => array()
            ), $new_aliquot_use);
        ...
        foreach ($this->request->data as $new_record) {
            ...
            if (isset($new_record['ViewAliquotUse']['use_datetime'])) {
                ...
                $new_record['ViewAliquotUse']['use_datetime_accuracy'] = str_replace(array(
                    '',
                    'c',
                    'i'
                ), array(
                    ...
                    'h',
                    'h'
                ), $new_record['ViewAliquotUse']['use_datetime_accuracy']);
            } else 
                if (isset($new_record['Realiquoting']['realiquoting_datetime'])) {
                    $date_key = $new_record['Realiquoting']['realiquoting_datetime'] . $date_key;
                    $new_record['Realiquoting']['realiquoting_datetime_accuracy'] = str_replace(array(
                        '',
                        'c',
                        'i'
                    ), array(
                        ...
                        'h',
                        'h'
                    ), $new_record['Realiquoting']['realiquoting_datetime_accuracy']);
                } else {
    ...
    function printBarcodes()
    {
        ...
        $at_least_once = false;
        $aliquots_count = $this->AliquotMaster->find('count', array(
        ...
        if ($aliquots_count > $display_limit) {
            $this->atimFlashWarning__("batch init - number of submitted records too big") . " (>$display_limit)", "javascript:history.back();", 5);
            return;
        }
        ...
        if ($at_least_once) {
            ...
            $this->atimFlashWarning__('there are no barcodes to print'), 'javascript:history.back();');
        }
    }
}

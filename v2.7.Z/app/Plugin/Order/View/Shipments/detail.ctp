...
$add_items_to_shipment_link = array(
    'link' => '/Order/Shipments/addToShipment/' . $atim_menu_variables['Order.id'] . '/' . $atim_menu_variables['Shipment.id'] . '/',
...
if ($add_to_shipments_subset_limits) {
    ...
    foreach ($add_to_shipments_subset_limits as $key => $sub_set_data) {
        ...
        $add_items_to_shipment_link[__('batchset') . ' #' . $key] = array(
            'link' => '/Order/Shipments/addToShipment/' . $atim_menu_variables['Order.id'] . '/' . $atim_menu_variables['Shipment.id'] . "/0/$start/$limit",
            'icon' => 'add_to_shipment'
        );
    }
}
$structure_links = array(
    'bottom' => array(
        ...
        'copy for new shipment' => array(
            'link' => '/Order/Shipments/add/' . $atim_menu_variables['Order.id'] . '/' . $atim_menu_variables['Shipment.id'] . '/',
            'icon' => 'copy'
        ),
        ...
        'order items' => array(
            'define order items returned' => array(
                'link' => '/Order/OrderItems/defineOrderItemsReturned/' . $atim_menu_variables['Order.id'] . '/0/' . $atim_menu_variables['Shipment.id'],
...
if (! $is_from_tree_view) {
    ...
    foreach ($all_status as $status) {
        ...
        $final_options = array(
            ...
            'extras' => array(
                'end' => $this->Structures->ajaxIndex('Order/OrderItems/listall/' . $atim_menu_variables['Order.id'] . '/' . $status . '/0/' . $atim_menu_variables['Shipment.id'] . '/Shipment/')
            )
        );
        if ($counter == 1)

...
$parameters);
    ...
    function getSingleStructure($alias = NULL)
    {
        ...
        if (! $return) {
            if ($alias) {
                ...
                $result = $this->Component_Structure->find('first', array(
                    'conditions' => array(
    ...
    function parseSearchConditions($atim_structure = NULL, $auto_accuracy = true)
    {
        ...
        if (isset($atim_structure['Sfs'])) {
            ...
            foreach ($atim_structure['Sfs'] as $value) {
                ...
                $is_float = in_array($value['type'], array(
                    'float',
                    'float_positive'
                ));
                if (in_array($value_type, self::$range_types) || (strpos($value['setting'], "range") !== false) && isset($this->controller->data[$value['model']][$value['field'] . '_start'])) {
                            ...
                            if (is_array($data) && $model != "0") {
                                ...
                                if ($format_data_model->table != $form_fields[$form_fields_key]['tablename'] && strpos($model, 'Detail') !== false) {
                                    ...
                                    if (! empty($form_fields[$form_fields_key]['tablename'])) {
                                        ...
                                        $format_data_model = new AppModel(array(
                                            'table' => $form_fields[$form_fields_key]['tablename'],
                                            'name' => $model_name,
                                            'alias' => $model_name
                                        ));
                                ...
                                if (isset($data)) {
                                    if ($auto_accuracy && in_array($form_fields_key, $accuracy_fields)) {
                                        ...
                                        $tmp_cond[] = array(
                                            ...
                                            $form_fields[$form_fields_key . '_accuracy']['key'] => array(
                                                'c',
                                                ' '
                                            )
                                        );
    ...
    function parse_sql_conditions($sql = NULL, $conditions = NULL)
    {
        ...
        // return BOTH
        return array(
            $sql_with_search_terms,
            $sql_without_search_terms
        );
    }
    ...
    function getFormById($id)
    {
        ...
        $data = $this->Component_Structure->find('first', array(
            'conditions' => array(
                'Structure.id' => $id
            ),
            'recursive' => - 1

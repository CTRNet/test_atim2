...
class ShellHelper extends Helper
{
    ...
    function header($options = array())
    {
        ...
        $menu_array = $this->menu($options['atim_menu'], array(
        ...
        $main_menu = array();
        $menu_array = $this->menu($options['atim_menu_for_header'], array(
            'variables' => $options['atim_menu_variables']
        ));
        $user_for_header = '';
    ...
    function validationHtml()
    {
        ...
        if (isset($_SESSION['ctrapp_core']['warning_trace_msg']) && count($_SESSION['ctrapp_core']['warning_trace_msg'])) {
            ...
            foreach ($_SESSION['ctrapp_core']['warning_trace_msg'] as $trace_msg) {
                if (isset($trace_msg['msg']) && isset($trace_msg['trace'])) {
                    $confirm_msg_html .= $this->getValidationLine('warning', $trace_msg['msg'], $trace_msg['trace']);
                } else {
                    $confirm_msg_html .= '<li><span class="icon16 warning mr5px"></span>' . $trace_msg;
                }
            }
            $confirm_msg_html .= '</ul>';
        ...
        if (isset($_SESSION['ctrapp_core']['error_msg']) && count($_SESSION['ctrapp_core']['error_msg'])) {
            $confirm_msg_html .= '<ul class="error">';
            foreach ($_SESSION['ctrapp_core']['error_msg'] as $trace_msg) {
                $confirm_msg_html .= '<li><span class="icon16 delete mr5px"></span>' . $trace_msg;
            }
            $confirm_msg_html .= '</ul>';
                    ...
                    if (! $is_root) {
                        ...
                        foreach ($menu as &$menu_item) {
                            ...
                            if ($menu_item['Menu']['at']) {
                                ...
                                $summary_item = $menu_item['Menu']['use_summary'] ? NULL : array(
                                ...
                                if ($menu_item['Menu']['use_summary']) {
                                    ...
                                    if ($is_root) {
                                        ...
                                        $active_item = $this->Html->link(html_entity_decode('<span class="icon32 mr5px ' . $class . '" style="vertical-align: bottom;"></span><span style="display: inline-block">' . $menu_item['Menu']['use_summary'] . '<br/><span class="menuSubTitle">&nbsp;&lfloor; ' . $word . '</span></span>', ENT_QUOTES, "UTF-8"), $menu_item['Menu']['use_link'], array(
                                            'escape' => false,
                                            'title' => $title,
                                            'class' => 'mainTitle'
                                        ));
                                    ...
                                    if ($is_root) {
                                        ...
                                        if (! $menu_item['Menu']['allowed']) {
                                            ...
                                            // $html_attributes
                                            $class = ' menu ' . $this->Structures->generateLinkClass('plugin ' . $menu_item['Menu']['use_link']);
                                            $active_item = $this->Html->link(html_entity_decode('<span class="icon32 mr5px ' . $class . '"></span>' . __($menu_item['Menu']['language_title']), ENT_QUOTES, "UTF-8"), $menu_item['Menu']['use_link'], array(
                                                'escape' => false,
                                                'title' => $title,
                                                'class' => 'mainTitle'
                                            ));
                                ...
                                if ($menu_item['Menu']['allowed']) {
                                    $append_menu .= '
											...
											<li class="' . ($menu_item['Menu']['at'] ? 'at ' : '') . 'count_' . $sub_count . '">
												' . $this->Html->link('<span class="icon16 list"></span><span class="menuLabel">' . $title . '</span>', $menu_item['Menu']['use_link'], array(
                                        'escape' => false,
                                        'title' => $title
                                    )) . '
											</li>
            ...
            if ($model) {
                ...
                if ($summary_result) {
                    ...
                    if (isset($summary_result['menu']) && is_array($summary_result['menu'])) {
                        $parts = array(
                            trim($summary_result['menu'][0]) . " ",
                            isset($summary_result['menu'][1]) ? trim($summary_result['menu'][1]) : ''
                        );
                        $total_length = 0;
                        $result_str = "";
                        $max_length = 22;
                    ...
                    if (isset($summary_result['data']) && isset($summary_result['structure alias'])) {
                        ...
                        $summary_long .= $this->Structures->build($structure, array(
                            'type' => 'summary',

...
function initFlyOverCells(scope){
	...
	function openDeleteConfirmPopup(event){
		if($("#deleteConfirmPopup").length == 0){
			...
			buildConfirmDialog('deleteConfirmPopup', STR_DELETE_CONFIRM, [{label : STR_YES, action: yes_action, icon: "detail"}, {label : STR_NO, action: no_action, icon: "delete noPrompt"}]);
		}
	...
	function miscIdPopup(participant_id, ctrl_id){
		buildConfirmDialog('miscIdPopup', STR_MISC_IDENTIFIER_REUSE, [{label : STR_NEW, action: function(){document.location = root_url + "ClinicalAnnotation/MiscIdentifiers/add/" + participant_id + "/" + ctrl_id + "/"; return false;}, icon: "add"},

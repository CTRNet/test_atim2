				...
				if(empty($new_line_data['label']) || in_array($new_line_data['label'], array('x','X'))) {
					...
					switch($new_line_data['Coupe réalisée par']) {
						...
						case  '':
							break;
						case  'tech. Patho':
							$new_line_data['Coupe réalisée par'] = 'tech. patho';
							break;
						case '-CM':
						case 'MO-CM':
...
function loadUrine($participant_id, $participant_identifier, $file_name, $worksheet, $line_counter, $new_line_data) {
	...
	if($new_line_data['urine disponible'] == '1') {
		...
		if($new_line_data['date prélèv. urine']) {
			...
			if(str_replace(array('x','-'),array('',''), $new_line_data['URN-1 vol. ml']) || str_replace(array('x','-'),array('',''), $new_line_data['vol URN-3 non clarifié']) || ($new_line_data == 'V01' && str_replace(array('x','-'),array('',''), $new_line_data['vol URC-1']))) {
				$import_summary['Inventory - Urine']['@@WARNING@@']["No collection date but urine samples defined"][] = "See patient '$participant_identifier'. No collection has been created but urine tubes seam to be banked. Please confirm and create missing collection and samples. [file <b>$file_name</b> (<b>$worksheet</b>), line : <b>$line_counter</b>]";
			}
		}
	} else {
		if(str_replace(array('x','-'),array('',''), $new_line_data['URN-1 vol. ml']) || str_replace(array('x','-'),array('',''), $new_line_data['vol URN-3 non clarifié']) || ($new_line_data == 'V01' && str_replace(array('x','-'),array('',''), $new_line_data['vol URC-1']))) {
			$import_summary['Inventory - Urine']['@@WARNING@@']["Urine defined as not available but urine samples defined"][] = "See patient '$participant_identifier'. No collection has been created but urine tubes seam to be banked. Please confirm and create missing collection and samples. [file <b>$file_name</b> (<b>$worksheet</b>), line : <b>$line_counter</b>]";
		}
	}	
}
	...
	if($prostatectomy_data_from_inv_file) {
		...
		if(!isset($created_prostatectomy[$participant_identifier])) {
			...
			if(array_key_exists($participant_identifier, $prostatectomy_data_from_patho)) {
				$prostatectomy_data_from_inv_file['TreatmentDetail']['procure_chuq_surgeon'] = $prostatectomy_data_from_patho[$participant_identifier]['Chirurgien'];
				$prostatectomy_data_from_inv_file['TreatmentDetail']['procure_chuq_laparotomy'] = $prostatectomy_data_from_patho[$participant_identifier]['laparotomie'];
				$prostatectomy_data_from_inv_file['TreatmentDetail']['procure_chuq_laparoscopy'] = $prostatectomy_data_from_patho[$participant_identifier]['laparoscopie'];
			...
			$import_summary['Inventory - Tissue (V01)']['@@WARNING@@']["The prostatectomy was not defined into treatment file"][] = "Created a prostatectomy on ".$prostatectomy_data_from_inv_file['TreatmentMaster']['start_date']." based on inventory data because prostatectomy date was not defined into treatment excel file. Please confirm and validate surgery data from patho file has been mirgated. See patient '$participant_identifier'.  [field <b>date chirurgie</b> - file <b>$file_name</b> (<b>$worksheet</b>) - line: <b>$line_counter</b>]";
		} else {

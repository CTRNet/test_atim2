...
//TODO Changer ' chirurgie fait' par 'chirurgie fait'
					...
					if(preg_match('/^PS[0-9]P/', $participant_identifier)) {
						if(array_key_exists($participant_identifier, $psp_nbr_to_participant_id_and_patho)) {
							if(!in_array($participant_identifier, $duplicated_participants_check)) {
								...
								//Load Tissue
								if($worksheet == 'V01') {
//TODO									loadTissue($participant_id, $participant_identifier, $psp_nbr_to_blocks_data, $psp_nbr_to_participant_id_and_patho[$participant_identifier]['patho#'], $file_name, $worksheet, $line_counter, $new_line_data);
	...
	if($new_line_data['date prélèv. sang']) {
		...
		if($new_line_data['sérum nb tubes']) {
			...
			for($id=1;$id<6;$id++){
				...
				} else if(strlen(str_replace('x','',($new_line_data['position dans bte de rangement sérum-'.$id].$new_line_data['bte de rangement sérum-'.$id])))) {
					$import_summary['Inventory - Blood']['@@WARNING@@']["No serum volume but storage data set"][] = "Volume of tube $participant_identifier $worksheet -SER".$id." is empty but a storage information is set. Tube won't be created. Please confirm. [file '$file_name :: $worksheet', line : $line_counter']";
				}
			}
			if($created_aliquots != $new_line_data['sérum nb tubes']) {
				$import_summary['Inventory - Blood']['@@WARNING@@']["Numbers of tubes are different"][] = "Based on volume field, the system created $created_aliquots aliquots but the number of aliquots defined into column 'sérum nb tubes' was equak to ".$new_line_data['sérum nb tubes'].". Please confirm. Patient $participant_identifier. [file '$file_name :: $worksheet', line : $line_counter']";
			}
		} else if(str_replace('x','',$new_line_data['ser-1'])) {
			die('ERR2387287328727372');
		}	
		
		// *** Create plasma + wht + BFC ***
		...
		if($new_line_data['plasma nb tubes'] || $new_line_data['FTA Elute WHT'] || $new_line_data['BFC en banque oui-non']) {
			...
			if($new_line_data['plasma nb tubes']) {
				...
				for($id=1;$id<6;$id++){
					if($new_line_data['pla-'.$id]) {
						...
						$created_aliquots++;
						$collection_aliquot_created = true;
					} else if(strlen(str_replace('x','',($new_line_data['position plasma-'.$id.' dans bte rangement'].$new_line_data['bte de rangement plasma-'.$id])))) {
						$import_summary['Inventory - Blood']['@@WARNING@@']["No plasma volume but storage data set"][] = "Volume of tube $participant_identifier $worksheet -PLA".$id." is empty but a storage information is set. Tube won't be created. Please confirm. [file '$file_name :: $worksheet', line : $line_counter']";
					}
				}
				if($created_aliquots != $new_line_data['plasma nb tubes']) {
					$import_summary['Inventory - Blood']['@@WARNING@@']["Numbers of tubes are different"][] = "Based on volume field, the system created $created_aliquots aliquots but the number of aliquots defined into column 'plasma nb tubes' was equak to ".$new_line_data['plasma nb tubes'].". Please confirm. Patient $participant_identifier. [file '$file_name :: $worksheet', line : $line_counter']";
				}
			...
			if($new_line_data['BFC en banque oui-non']) {
				...
				//aliquots
//TODO confirm $storage_date = getDateTimeAndAccuracy($new_line_data, 'date prélèv. sang', "hre de congélation plasma", 'Inventory - Blood', "$file_name ($worksheet)", $line_counter);
$storage_date = $date_of_derivative_creation;
				for($id=1;$id<3;$id++){
					$field = ($id==3)? 'BFC 300ul': 'BFC cong. X2 (ml) '.$id;
					if($new_line_data[$field]) {
						$initial_volume = ($id==3)? '0.3' : getDecimal($new_line_data, $field, 'Inventory - Blood', "$file_name ($worksheet)", $line_counter);
						//Storage data
						...
						$collection_aliquot_created = true;
					} else if(strlen(str_replace('x','',($new_line_data['position BFC-'.$id.' dans bte rangement'].$new_line_data['bte rangement BFC-'.$id])))) {
						$import_summary['Inventory - Blood']['@@WARNING@@']["No BFC volume but storage data set"][] = "Volume of tube $participant_identifier $worksheet -BFC".$id." is empty but a storage information is set. Tube won't be created. Please confirm. [file '$file_name :: $worksheet', line : $line_counter']";
					}
				}
			} else if(str_replace('x','',$new_line_data['bte rangement BFC-1'])) {
				die('ERR238733e233.1');
			}
		} else if(str_replace('x','',$new_line_data['pla-1'])) {
			die('ERR238733222.1.1');
		} else if(str_replace('x','',$new_line_data['bte rangement WHT'])) {
			die('ERR23873322265.2');
		} else if(str_replace('x','',$new_line_data['bte rangement BFC-1'])) {
			die('ERR23873322265.4');
		}		
		...
		if(!$collection_aliquot_created) {
			$import_summary['Inventory - Blood']['@@WARNING@@']["Collection date set with no content"][] = "See patient $participant_identifier. A collection was created but no aliquot can be created then linked to this one based on excel content. [file '$file_name :: $worksheet', line : $line_counter']";	
		}
	}  else if(strlen(str_replace('0','', $new_line_data['sérum nb tubes']).str_replace('0','', $new_line_data['plasma nb tubes']))) {
		$import_summary['Inventory - Blood']['@@WARNING@@']["No collection date but blood samples defined"][] = "See patient '$participant_identifier'. No collection has been created because no date is set but some blood tubes seam to be banked. Please confirm and create missing collection and samples. [file '$file_name :: $worksheet', line : $line_counter']";
	}
}
...
function getPosition($participant_identifier, $excel_postions, $excel_storage_label, $sample_type, $data_type, $file_name, $worksheet, $line_counter) {
	...
	if(empty($excel_postions)) {
		...
		$import_summary[$data_type]['@@ERROR@@']["Storage position but no box label defined"][] = "No position will be set. See patient $participant_identifier. [file '$file_name' :: $worksheet - line: $line_counter] ERR#6 ";
	} else {
		$box_storage_unique_key = getBoxStorageUniqueKey($excel_storage_label, $sample_type);
		...
		switch($storage_type) {
			...
			case 'box81':
				...
				break;
			case 'box49':
				if(preg_match('/^(([1-9])|([1-4][0-9]))$/', $excel_postions)) $positions['x'] = $excel_postions;
				break;
			default:

    ...
    function buildCpcbnSummary($parameters, $display_cores_positions = false, $display_cores_data = false)
    {
        ...
        if (isset($parameters['Browser'])) {
            ...
            if (isset($parameters['ViewStorageMaster']['id'])) {
                ...
                $selected_storages = $storage_model->find('all', array(
                    'conditions' => $criteria,
                    'recursive' => 0
                ));
            ...
            // 1-BANK
            $bank_ids = array();
            foreach ($parameters['Participant']['qc_tf_bank_id'] as $new_bank_id)
                if (strlen($new_bank_id))
                    $bank_ids[] = $new_bank_id;
            if (! empty($bank_ids))
                $conditions[] = 'Participant.qc_tf_bank_id IN (' . implode($bank_ids, ',') . ')';
                
                // 2-STORAGE
            $matching_storage_issue = false;
            $recorded_storage_selection_labels = array();
            foreach ($parameters['FunctionManagement']['recorded_storage_selection_label'] as $new_label)
                if (strlen($new_label))
                    $recorded_storage_selection_labels[] = $new_label;
            if (! empty($recorded_storage_selection_labels)) {
                ...
                foreach ($recorded_storage_selection_labels as $new_recorded_storage_selection_label) {
                    ...
                    } elseif (isset($storage_data['error'])) {
            ...
            if (isset($parameters['Participant']['qc_tf_bank_participant_identifier'])) {
                ...
                foreach ($parameters['Participant']['qc_tf_bank_participant_identifier'] as $new_participant_id)
                    if (strlen($new_participant_id))
                        $participant_ids[] = $new_participant_id;
        ...
        $join_to_display_cores_data = $display_cores_data ? 'LEFT JOIN ad_tissue_cores AS AliquotDetail ON AliquotMaster.id = AliquotDetail.aliquot_master_id
        ...
        foreach ($main_results as &$new_participant) {
            ...
            } elseif (! empty($new_participant['Participant']['qc_tf_suspected_date_of_death'])) {
            ...
            if (! empty($new_participant['DiagnosisMaster']['primary_id']))
                $primary_ids[] = $new_participant['DiagnosisMaster']['primary_id'];
            ...
            if ($display_cores_positions) {
                ...
                if ($set_to_confidential) {
                    if (isset($new_participant['StorageMaster']['qc_tf_bank_id']))
                        $new_participant['StorageMaster']['qc_tf_bank_id'] = CONFIDENTIAL_MARKER;
                    if (isset($new_participant['StorageMaster']['qc_tf_tma_label_site']))
                        $new_participant['StorageMaster']['qc_tf_tma_label_site'] = CONFIDENTIAL_MARKER;
                    if (isset($new_participant['StorageMaster']['qc_tf_tma_name']))
                        $new_participant['StorageMaster']['qc_tf_tma_name'] = CONFIDENTIAL_MARKER;
                }
                ...
                if (isset($new_participant['StorageMaster']['qc_tf_tma_name'])) {
                    ...
                    } elseif ($new_participant['StorageMaster']['qc_tf_bank_id'] == $user_bank_id) {
        ...
        $participant_ids = empty($participant_ids) ? array(
            '-1'
        ) : $participant_ids;
        ...
        if ($primary_ids_condition) {
            ...
            foreach ($metastasis_results as $new_res) {
                ...
                } elseif (! empty($new_res['DiagnosisDetail']['site'])) {
        ...
        foreach ($treatment_results as $new_trt) {
            ...
            if (! isset($treatments_summary[$participant_id]))
                $treatments_summary[$participant_id] = $treatments_summary_template;
        ...
        foreach ($main_results as &$new_participant) {
            ...
            foreach ($date_diff_def as $model_field_data => $model_field_calculated) {
                ...
                if ($new_participant[$model_data][$field_data . '_accuracy'] . $new_participant['TreatmentMaster']['start_date_accuracy'] != 'cc' && $new_participant[$model_data][$field_data] && $new_participant['TreatmentMaster']['start_date'])
        ...
        foreach ($warnings as $new_warning)
            AppController::addWarningMsg($new_warning);

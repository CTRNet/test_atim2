<?php
	...
	for($i = 0; $i < count($value); $i++){
		...
		$query = "INSERT INTO a`structure_permissible_values` SET `value` = '$value[$i]', `language_alias` = '$language[$i]';";
		mysql_query($query);
	}
	...
	$query = "UPDATE `global_lookups` g, `form_fields_global_lookups` l, `structure_fields` s, `structure_value_domains` d SET s.`structure_value_domain_id` = d.`id` 

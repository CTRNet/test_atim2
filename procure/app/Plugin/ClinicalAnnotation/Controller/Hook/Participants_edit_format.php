<?php 
	...
	if (empty($this->request->data)) {
		...
		if(strpos(implode('', $this->passedArgs), 'clinical_file_update_process_step') !== false) {
			//Set default data
			...
			if(preg_match('/^([0-9]{4})\-([0-9]{2})\-([0-9]{2})$/', $participant_data['Participant']['last_chart_checked_date'], $matches)) {
				switch($participant_data['Participant']['last_chart_checked_date_accuracy']) {
					case 'd':
						$last_chart_checked_date = $matches[1].'-'.$matches[2];
						break;
					case 'm':
					case 'y':
						$last_chart_checked_date = $matches[1];
						break;
					default:
						$last_chart_checked_date = $participant_data['Participant']['last_chart_checked_date'];
				}
				$last_chart_checked_date = __("the 'last chart checked date' is currently set to '%s'", $last_chart_checked_date);
			}
			
			if($last_visite_contact) {
				AppController::addWarningMsg(__('set last chart checked date to the date of the visit of the form you compelted today').' '.$last_chart_checked_date);
				...
				AppController::addWarningMsg(__('set last chart checked date to the current date').' '.$last_chart_checked_date);

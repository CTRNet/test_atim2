...
class AliquotMastersControllerCustom extends AliquotMastersController
{
    ...
    function editBarcodeAndLabel()
    {
        ...
        $aliquots_used = $this->AliquotInternalUse->find('all', array(
            'conditions' => array(
                'AliquotInternalUse.aliquot_master_id' => $aliquot_ids,
        ...
        if (! empty($aliquots_used_barcode))
            AppController::addWarningMsg(__('you are editing aliquots that have already been sent to processing site') . '. ' . str_replace('%s', '[' . implode('] ,[', $aliquots_used_barcode) . ']', __('see # %s')));
        ...
        if ($initial_display) {
            ...
            foreach ($this->request->data as $key => $new_studied_aliquot) {
                $record_counter ++;
                // Get order item id
                if (! isset($current_barcode_to_aliquot_master_id[$new_studied_aliquot['ViewAliquot']['barcode']])) {
                ...
                if (! $this->AliquotMaster->validates()) {
                    foreach ($this->AliquotMaster->validationErrors as $field => $msgs) {
                        ...
                        foreach ($msgs as $msg)
                            $errors['AliquotMaster'][$field][$msg][] = $record_counter;
                    }
                }
                if (! isset($current_barcode_to_procure_created_by_bank[$new_studied_aliquot['ViewAliquot']['barcode']])) {
                    $this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
                }
                ...
                if ($barcode_error)
                    $errors['AliquotMaster']['barcode'][__($barcode_error)][] = $record_counter;
                    // Reset data

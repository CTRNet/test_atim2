...
if ($arr_allow_deletion['allow_deletion']) {
    if ($aliquot_data['SampleMaster']['procure_created_by_bank'] == 's' && $aliquot_data['AliquotMaster']['procure_created_by_bank'] != Configure::read('procure_bank_id')) {
        ...
        $returned_nbr = $this->SampleMaster->find('count', array(
            'conditions' => array(
                'SampleMaster.parent_id' => $aliquot_data['SampleMaster']['id']
            ),
            'recursive' => '-1'
        ));
        $linked_recrods_nbr += $returned_nbr;
        $returned_nbr = $this->AliquotMaster->find('count', array(
            'conditions' => array(
                'AliquotMaster.sample_master_id' => $aliquot_data['SampleMaster']['id'],
        ...
        $returned_nbr = $quality_ctrl_model->find('count', array(
            'conditions' => array(
                'QualityCtrl.sample_master_id' => $aliquot_data['SampleMaster']['id']
            ),
            'recursive' => '-1'
        ));
        $linked_recrods_nbr += $returned_nbr;
        ...
        $returned_nbr = $specimen_review_master_model->find('count', array(
            'conditions' => array(
                'SpecimenReviewMaster.sample_master_id' => $aliquot_data['SampleMaster']['id']
            ),
            'recursive' => '-1'
        ));
        $linked_recrods_nbr += $returned_nbr;

...
foreach ($this->request->data as $procure_aliquot_data_set) {
    ...
    foreach ($procure_aliquot_data_set['children'] as $procure_aliquot_data) {
        ...
        if ($barcode_error)
            $errors['barcode'][$barcode_error][$record_counter] = $record_counter;
    }
}
...
if (empty($errors)) {
    ...
    if (! isset($child_writable_fields[$child_aliquot_ctrl['AliquotControl']['detail_tablename']]['addgrid']))
        $child_writable_fields[$child_aliquot_ctrl['AliquotControl']['detail_tablename']]['addgrid'] = array();
    $child_writable_fields[$child_aliquot_ctrl['AliquotControl']['detail_tablename']]['addgrid'] = array_merge($child_writable_fields[$child_aliquot_ctrl['AliquotControl']['detail_tablename']]['addgrid'], array(

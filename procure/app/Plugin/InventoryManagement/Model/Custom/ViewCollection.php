...
Participant.procure_participant_attribution_number,
    ...
    function summary($variables = array())
    {
        ...
        if (isset($variables['Collection.id'])) {
            ...
            $title = empty($collection_data['ViewCollection']['participant_identifier']) ? ($collection_data['ViewCollection']['procure_visit'] == 'Controls' ? 'Controls' : '?') : $collection_data['ViewCollection']['participant_identifier'];
            ...
            $aliquot_count = $aliquot_master_model->find('count', array(
                'conditions' => array(
                    'AliquotMaster.collection_id' => $collection_data['ViewCollection']['collection_id'],
                    "AliquotMaster.barcode NOT LIKE '% " . $collection_data['ViewCollection']['procure_visit'] . " -%'"
                ),
                'recursive' => '-1'
            ));
            if ($aliquot_count) {
    ...
    function getUnconsentedParticipantCollections(array $collection)
    {
        ...
        if (! empty($participants_to_fetch)) {
            ...
            $consent_data = $consent_model->find('all', array(
                ...
                'conditions' => array(
                    'ConsentMaster.participant_id' => array_keys($participants_to_fetch)
                ),
                'recursive' => - 1
            ));
            foreach ($consent_data as $consent_data_unit)
                unset($participants_to_fetch[$consent_data_unit['ConsentMaster']['participant_id']]);
            foreach ($participants_to_fetch as $participant_id => $collection_id)
                $results[$collection_id] = null;
        }
        return $results;
    }
}

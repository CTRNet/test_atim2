<?php
$sd_spe_tissues["app_data"]["pkey"] = "id";
$sd_spe_tissues["app_data"]["file"] = "5";

$sd_spe_tissues["master"]["sample_code"] = "";//Required
$sd_spe_tissues["master"]["collection_id"] = "sample_parent_id";//??

$sd_spe_tissues["app_data"]['additional_queries'][] = "UPDATE sample_masters SET sample_label=CONCAT('S - bankid EDTA'),  sample_code=CONCAT('B - ', id) WHERE id=%%last_master_insert_id%%";
$sd_spe_tissues["app_data"]['additional_queries'][] = "INSERT INTO specimen_details(`id`, `sample_master_id`, `created_by`, `created`, `modified`, `modified_by`) VALUES(%%last_master_insert_id%%, %%last_master_insert_id%%, 1, NOW(), 1, NOW())";

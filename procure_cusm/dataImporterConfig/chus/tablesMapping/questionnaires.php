...
function postQuestionnaireRead(Model $m){
	...
	$event_summary = $m->values[utf8_decode('Questionnaire complet ou incomplet')];
	...
	foreach($working_array as $db_field => $file_field) {
		$studied_date = $m->values[utf8_decode($file_field)];
		if(preg_match('/^[a-zA-Z\ ]+$/',$studied_date,$matches)) {
			if(!$event_summary || (strtolower($event_summary) != strtolower($studied_date))) {
				$event_summary .= " $studied_date";
			}
			$studied_date = '';
		}
		$tmp_date_data = getDateAndAccuracy($studied_date, 'Questionnaire', $file_field, $m->line);
		if($tmp_date_data) {
	...
	if(!empty($version_values)) {	
		$version_values = str_replace(utf8_decode('Français'), 'Francais', $version_values);
		if(preg_match('/^(Anglais|Francais) (20[0-9][0-9])$/', $version_values, $matches)) {
			$version_language = $matches[1];
			$version_date = $matches[2];
		} else if(preg_match('/^(Anglais|Francais)$/', $version_values, $matches)) {
			$version_language = $matches[1];
	...
	$m->values['event_summary'] = "$event_summary";

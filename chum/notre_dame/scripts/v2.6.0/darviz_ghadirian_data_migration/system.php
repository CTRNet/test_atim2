...
//TODO: Add control on detail table based on _control_id	
	if($tables_data) {
		...
		if(isset($main_table_data['data']['sample_control_id']) && $main_table_data['name'] == 'sample_masters') {
		...
		} else if(isset($main_table_data['data']['sample_control_id']) && $main_table_data['name'] == 'sample_masters') {
			$sample_type = $atim_controls['sample_controls']['***id_to_type***'][$main_table_data['data']['sample_control_id']];
			$set_strings = array();
...
$empty_date_time_values = array('-', 'n/a', 'x', '??', 'nd', 'AAAA-MM-JJ');
...
function validateAndGetDateAndAccuracy($date, $summary_section_title, $summary_title_add_in, $summary_details_add_in) {
	...
	} else if(preg_match('/^((19|20)([0-9]{2})\-([01][0-9]))\-((unk)|(JJ))$/',$date,$matches)) {
		return array($matches[1].'-01', 'd');
	} else if(preg_match('/^((19|20)([0-9]{2}))\-((unk)|(MM))-((unk)|(JJ))$/',$date,$matches)) {

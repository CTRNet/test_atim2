...
while(list($line_number, $excel_line_data) = getNextExcelLineData($excel_file_name, $worksheet_name, 1)) {
	...
	if(strlen($excel_line_data['alicot system code'])) {
		...
		// 2- Only Cell Line Defined : Find Cell Line
		...
		if(array_key_exists($cell_culture_key, $cell_culture_key_to_cell_culture_sample_master_id)) {
			...
			list($atim_collection_id, $atim_parent_sample_master_id, $cell_culture_sample_label) = $cell_culture_key_to_cell_culture_sample_master_id[$cell_culture_key];
			...
			if($atim_cell_line_sample_data) {
				if(sizeof($atim_cell_line_sample_data) > 1) {
					//Will create a new cell culture linked to the parent of the first one.
		...
		if($atim_parent_sample_master_id) {
			...
			// Reset value
			$cell_culture_key_to_cell_culture_sample_master_id[$cell_culture_key] = array($atim_collection_id, $atim_parent_sample_master_id, $cell_culture_sample_label);
		}
	}		
	...
	if($atim_source_aliquot_master_id) {
		...
		// Created xenograft and aliquots
		 ...
		 [date injection en souris (annÃ©e/mois/jour)] => 15/03/2013
		[Date rÃ©colte (annÃ©e/mois/jour)] => 40165
		[clones] =>
		[tx] =>
		...
		/*
		 [projet] => Post-Doctorat Laudine Communal (Terry Fox STP-ovaire:Â Validation d anticorps thÃ©rapeutiques dans le cancer)
		 [NumÃ©ro (sur bloc)] => C7-D
		 [Notes] => mÃ©tastases IP
		 [Entreposage] => Tiroir xeno ovaire #1
		 [C11-TD] => 4
		  [utilisation (utilisateur et date)] =>
		 [(passage Ã  l'injection)] =>
		 [cell passage number (Original)] => 515 (1/2)

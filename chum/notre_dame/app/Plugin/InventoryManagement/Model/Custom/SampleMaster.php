...
class SampleMasterCustom extends SampleMaster
{
    ...
    function specimenSummary($variables = array())
    {
        ...
        if (isset($variables['Collection.id']) && isset($variables['SampleMaster.initial_specimen_sample_id'])) {
            ...
            $return = array(
                'menu' => array(
                    null,
                    __($specimen_data['SampleControl']['sample_type'], true) . ' : ' . $specimen_data['SampleMaster']['qc_nd_sample_label']
                ),
                'title' => array(
                    null,
                    __($specimen_data['SampleControl']['sample_type'], true) . ' : ' . $specimen_data['SampleMaster']['qc_nd_sample_label']
    ...
    function derivativeSummary($variables = array())
    {
        ...
        if (isset($variables['Collection.id']) && isset($variables['SampleMaster.initial_specimen_sample_id']) && isset($variables['SampleMaster.id'])) {
            ...
            $return = array(
                'menu' => array(
                    null,
                    __($derivative_data['SampleControl']['sample_type'], true) . ' : ' . $derivative_data['SampleMaster']['qc_nd_sample_label']
                ),
                'title' => array(
                    null,
                    __($derivative_data['SampleControl']['sample_type'], true) . ' : ' . $derivative_data['SampleMaster']['qc_nd_sample_label']
    ...
    function createSampleLabel($collection_id, $sample_data, $bank_participant_identifier = null, $initial_specimen_label = null)
    {
        ...
        if (is_null($sample_category) || ! array_key_exists('sample_type', $sample_data['SampleControl']) || ! array_key_exists('qc_nd_sample_type_code', $sample_data['SampleControl'])) {
            AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
        }
        ...
        if ($sample_category == 'specimen') {
            if (! isset($sample_data['SpecimenDetail']) || ! array_key_exists('type_code', $sample_data['SpecimenDetail']) || ! array_key_exists('sequence_number', $sample_data['SpecimenDetail'])) {
                AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
            }
        ...
        switch ($sample_type) {
            ...
            case 'stool':
            case 'vaginal swab':
                $new_sample_label = $specimen_type_code . ' - ' . $bank_participant_identifier . (empty($specimen_sequence_number) ? '' : ' ' . $specimen_sequence_number);
                break;
            
            case 'blood':
                if (! array_key_exists('blood_type', $sample_data['SampleDetail'])) {
                    AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
                }
                $new_sample_label = $specimen_type_code . ' - ' . $bank_participant_identifier . (empty($specimen_sequence_number) ? '' : ' ' . $specimen_sequence_number) . (empty($sample_data['SampleDetail']['blood_type']) ? ' n/a' : ' ' . $sample_data['SampleDetail']['blood_type']);
                break;
            
            case 'tissue':
                if (! array_key_exists('labo_laterality', $sample_data['SampleDetail'])) {
                    AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
                }
                $new_sample_label = $specimen_type_code . ' - ' . $bank_participant_identifier . (empty($sample_data['SampleDetail']['labo_laterality']) ? ' n/a' : ' ' . $sample_data['SampleDetail']['labo_laterality']) . (empty($specimen_sequence_number) ? '' : ' ' . $specimen_sequence_number);
                break;
            ...
            case 'cell culture':
                ...
                $new_sample_label = $qc_nd_sample_type_code . ' ' . $initial_specimen_label . ((empty($sample_data['SampleDetail']['cell_passage_number']) && (strcmp($sample_data['SampleDetail']['cell_passage_number'], '0') != 0)) ? '' : ' P' . $sample_data['SampleDetail']['cell_passage_number']);
                break;
    ...
    private function validateLabTypeCodeAndLaterality(&$data_to_validate)
    {
        ...
        if ((isset($data_to_validate['SampleControl']['sample_category']) && $data_to_validate['SampleControl']['sample_category'] === 'specimen') || (! isset($data_to_validate['SampleControl']['sample_category']) && array_key_exists('SpecimenDetail', $data_to_validate))) {
            // Load model to control data
            $lab_type_laterality_match = AppModel::getInstance('InventoryManagement', 'LabTypeLateralityMatch', true);
            
            // Get Data
            if (! array_key_exists('sample_type', $data_to_validate['SampleControl']) || ! array_key_exists('SpecimenDetail', $data_to_validate) || ! array_key_exists('type_code', $data_to_validate['SpecimenDetail'])) {
                AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
            }
            ...
            if ($tmp_specimen_type == 'tissue') {
                ...
                if (! (empty($tmp_selected_type_code) && empty($tmp_labo_laterality))) {
                    ...
                    $matching_records = $lab_type_laterality_match->find('all', array(
                        'conditions' => $criteria
                    ));

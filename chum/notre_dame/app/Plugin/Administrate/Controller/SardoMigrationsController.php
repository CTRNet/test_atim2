...
class SardoMigrationsController extends AdministrateAppController
{
    ...
    {
        if (! in_array($message_type, array(
            'all',
            'csv',
        ...
        if ($message_type == 'all') {
            ...
            $process_messages = $this->SardoImportSummary->find('all', array(
            ...
            if (! $this->Session->read('flag_show_confidential')) {
                foreach ($this->request->data as &$new_data) {
                    if ($new_data['SardoImportSummary']['message'] == 'Creation/Update summary')
                        $new_data['SardoImportSummary']['details'] = CONFIDENTIAL_MARKER;
                }
            }
        }
    }

...
//			AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method='.__METHOD__.',line='.__LINE__, null, true);
	...
	function validates($options = array()){
		...
		if(isset($this->data['EventDetail'])) {
			foreach(array('slides', 'blocks') as $item) {
			...
			if(array_key_exists('idc_p_1_intraductal', $this->data['EventDetail'])) {
				if($this->data['EventDetail']['idc_p_1_intraductal'] != 'y') {
					$idc_fields = array('idc_p_2_density_1',
					...
					foreach($idc_fields as $new_field) if(strlen(trim($this->data['EventDetail'][$new_field]))) $icd_field_completed = true;
				...
				foreach(array('idc_p_2_density_1' => 'idc_p_2_density_1_precision', 'idc_p_3_density_2' => 'idc_p_3_density_2_precision') as $new_field => $new_field_precision) {

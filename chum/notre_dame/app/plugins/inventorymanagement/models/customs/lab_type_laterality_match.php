...
class LabTypeLateralityMatch extends InventorymanagementAppModel {
	...
	function getLaboLaterality() {
		...
		foreach($this->find('all', array('conditions' => array("selected_labo_laterality NOT IN ('unknown')"), 'field' => 'DISTINCT selected_labo_laterality', 'order' => 'selected_labo_laterality')) as $new_record) {
			$result[] = array('value' => $new_record['LabTypeLateralityMatch']['selected_labo_laterality'], 'default' => __('lab laterality '.$new_record['LabTypeLateralityMatch']['selected_labo_laterality'] ,true));
		}
		$result[] = array('value' => 'unknown', 'default' => __('unknown', true));
	...
	function getTissueSourcePermissibleValues() {
		...
		foreach($this->find('all', array('conditions' => array("tissue_source_matching NOT IN ('unknown', 'other')"), 'field' => 'DISTINCT tissue_source_matching', 'order' => 'tissue_source_matching')) as $new_record) {

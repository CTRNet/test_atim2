...
class TreatmentMastersControllerCustom extends TreatmentMastersController {
	...
	function preOperativeDetail($participant_id, $tx_master_id){
		...
		switch($surgery_data['TreatmentControl']['detail_tablename']) {
			...
			case 'qc_hb_txd_surgery_pancreas':
				...
				break;
			default:
				$this->flash(__("no pre operative data has to be defined for this type of treatment"), '/ClinicalAnnotation/TreatmentMasters/detail/'.$participant_id.'/'.$tx_master_id.'/');
				return;
		}
		...
		$this->request->data = $surgery_data;
		
		// LOAD EVENT DATA & STRUCTURES (that could be linked to preoperative report)

		$surgeries_events_data = array();
		
		$surgeries_events_data['lab_report_id'] = $this->getEventDataForPreOperativeForm('lab_report_id', 'lab', 'biology', $this->request->data, $participant_id, $this->request->data['TreatmentDetail']['lab_report_id']);
		$surgeries_events_data['imagery_id'] = $this->getEventDataForPreOperativeForm('imagery_id', 'imagery', 'medical imaging%', $this->request->data, $participant_id, $this->request->data['TreatmentDetail']['imagery_id']);
		$surgeries_events_data['fong_score_id'] =  $this->getEventDataForPreOperativeForm('fong_score_id', 'scores', 'fong score',  $this->request->data, $participant_id, $this->request->data['TreatmentDetail']['fong_score_id']);
		$surgeries_events_data['meld_score_id'] = $this->getEventDataForPreOperativeForm('meld_score_id', 'scores', 'meld score', $this->request->data, $participant_id, $this->request->data['TreatmentDetail']['meld_score_id']);
		$surgeries_events_data['gretch_score_id'] = $this->getEventDataForPreOperativeForm('gretch_score_id', 'scores', 'gretch score', $this->request->data, $participant_id, $this->request->data['TreatmentDetail']['gretch_score_id']);
		$surgeries_events_data['clip_score_id'] = $this->getEventDataForPreOperativeForm('clip_score_id', 'scores', 'clip score', $this->request->data, $participant_id, $this->request->data['TreatmentDetail']['clip_score_id']); 
		$surgeries_events_data['barcelona_score_id'] = $this->getEventDataForPreOperativeForm('barcelona_score_id', 'scores', 'barcelona score', $this->request->data, $participant_id, $this->request->data['TreatmentDetail']['barcelona_score_id']);
		$surgeries_events_data['okuda_score_id'] = $this->getEventDataForPreOperativeForm('okuda_score_id', 'scores', 'okuda score', $this->request->data, $participant_id, $this->request->data['TreatmentDetail']['okuda_score_id']);	
	...
	function preOperativeEdit($participant_id, $tx_master_id){
		...
		$this->Structures->set($structure_name);
		$this->set('atim_menu', $this->Menus->get('/ClinicalAnnotation/TreatmentMasters/preOperativeDetail/'));
		$this->set('atim_menu_variables', array('Participant.id'=>$participant_id, 'TreatmentMaster.id' => $tx_master_id));	
		...
		if(empty($this->request->data)) {
			...
			if($this->request->data['TreatmentDetail']['type_of_cirrhosis'] == ""
			&& $this->request->data['TreatmentDetail']['esophageal_varices'] == ""
			&& $this->request->data['TreatmentDetail']['gastric_varices'] == ""
			&& $this->request->data['TreatmentDetail']['tips'] == ""
			&& $this->request->data['TreatmentDetail']['portacaval_gradient'] == ""
			&& $this->request->data['TreatmentDetail']['splenomegaly'] == ""
			&& $this->request->data['TreatmentDetail']['splen_size'] == ""){
			...
			$this->request->data['TreatmentMaster']['id'] = $tx_master_id;
			if($this->TreatmentMaster->save($this->request->data)) {
				$this->flash(__("your data has been saved"), '/ClinicalAnnotation/TreatmentMasters/preOperativeDetail/'.$participant_id.'/'.$tx_master_id.'/');
				return;
			}
		}
		...
		$surgeries_events_data['lab_report_id'] = $this->getEventDataForPreOperativeForm('lab_report_id', 'lab', 'biology', $this->request->data, $participant_id);
		$surgeries_events_data['imagery_id'] = $this->getEventDataForPreOperativeForm('imagery_id', 'imagery', 'medical imaging%', $this->request->data, $participant_id);
		$surgeries_events_data['fong_score_id'] =  $this->getEventDataForPreOperativeForm('fong_score_id', 'scores', 'fong score',  $this->request->data, $participant_id);
		$surgeries_events_data['meld_score_id'] = $this->getEventDataForPreOperativeForm('meld_score_id', 'scores', 'meld score', $this->request->data, $participant_id);
		$surgeries_events_data['gretch_score_id'] = $this->getEventDataForPreOperativeForm('gretch_score_id', 'scores', 'gretch score', $this->request->data, $participant_id);
		$surgeries_events_data['clip_score_id'] = $this->getEventDataForPreOperativeForm('clip_score_id', 'scores', 'clip score', $this->request->data, $participant_id); 
		$surgeries_events_data['barcelona_score_id'] = $this->getEventDataForPreOperativeForm('barcelona_score_id', 'scores', 'barcelona score', $this->request->data, $participant_id);
		$surgeries_events_data['okuda_score_id'] = $this->getEventDataForPreOperativeForm('okuda_score_id', 'scores', 'okuda score', $this->request->data, $participant_id);

...
class ClinicalCollectionLinksController extends ClinicalAnnotationAppController {
	...
	function listall( $participant_id ) {
		...
		$this->request->data = $this->paginate($this->ClinicalCollectionLink, array('ClinicalCollectionLink.participant_id'=>$participant_id));
	...
	function add( $participant_id ) {
		...
		if ( !empty($this->request->data) ) {
			...
			$this->request->data['ClinicalCollectionLink']['participant_id'] = $participant_id;
			$tmp_data = $this->ClinicalCollectionLink->find('first',array('conditions'=>array('ClinicalCollectionLink.collection_id' => $this->request->data['ClinicalCollectionLink']['collection_id'])));
			$this->ClinicalCollectionLink->id = $tmp_data['ClinicalCollectionLink']['id'];
			if(strlen($this->ClinicalCollectionLink->id) == 0){
				$this->request->data['ClinicalCollectionLink']['deleted'] = 1;
			} 
			...
			if ( $submitted_data_validates && $this->ClinicalCollectionLink->save($this->request->data) ) {
	...
	function edit( $participant_id, $clinical_collection_link_id) {
		...
		if ( !empty($this->request->data) ) {
			...
			$this->request->data = $clinical_collection_data;
		}
	}
	...
	function delete( $participant_id, $clinical_collection_link_id ) {
		...
		if($arr_allow_deletion['allow_deletion']) {
			$this->request->data = array('ClinicalCollectionLink' => array(
			...
			$this->ClinicalCollectionLink->id = $clinical_collection_link_id;
			if ($this->ClinicalCollectionLink->save($this->request->data)){

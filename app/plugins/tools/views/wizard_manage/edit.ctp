...
<?php 
	$structures->build($atim_structure, array('links' => array('top' => '/tools/WizardManage/edit/'.$wizard_id)));
?>

<script>
	var modelsData = '<?php echo json_encode($js_data); ?>';
	...
	function bindButtons(scope){
		...
		$(scope).find(".add").unbind('click').click(function(){
			...
			$("#addDialog").data("node", $(getParentElement(this, "LI").find("ul:first")));
	...
	function addNode(treeData, node){
		addButton = treeData.control_id >= 0 ? '<a href="#" class="form add">&nbsp;</a>' : '';
		type = null;
		label = null;
		if(treeData.datamart_structure_id == 2){
			...
			type = 'aliquot';
			label = modelsData.aliquot_controls[-1 * treeData.control_id]["AliquotControl"]["aliquot_type"];
		}else{
			type = 'sample';
		...
		bindButtons(li); 
		return $(li).find("ul:first");
	}

	function getDropdownOptions(parent_id){
		var options = "";
		if(parent_id != ""){
			options += "<optgroup label='derivatives'>";
		}
		...
		if(parent_id != ""){
			options += "</optgroup><optgroup label='aliquots'>";
		}

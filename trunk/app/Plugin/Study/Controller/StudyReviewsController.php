		...
		if ( !empty($this->request->data) ) {
			...
			if($submitted_data_validates) {
				...
				if ( $this->StudyReview->save($this->request->data) ) {
					...
					$this->atimFlash( 'your data has been saved','/Study/StudyReviews/detail/'.$study_summary_id.'/'.$this->StudyReview->id );
					}
				}
			}
 	}
			...
			$this->request->data = $study_reviews_data;
				...
				if($submitted_data_validates) {
					...
					if ( $this->StudyReview->save($this->request->data) ) {
						...
						$this->atimFlash( 'your data has been updated','/Study/StudyReviews/detail/'.$study_summary_id.'/'.$study_reviews_id );
						}
					}
				}
	}
		...
		if( $hook_link ) { require($hook_link); }
			...
			if($arr_allow_deletion['allow_deletion']) {
				...
				if( $this->StudyReview->atim_delete( $study_reviews_id ) ) {
					$this->atimFlash( 'your data has been deleted', '/Study/StudyReviews/listall/'.$study_summary_id );
				} else {
					$this->flash( 'error deleting data - contact administrator.', '/Study/StudyReviews/listall/'.$study_summary_id );
				}
			}else {
					$this->flash($arr_allow_deletion['msg'], '/Study/StudyReviews/detail/'.$study_summary_id.'/'.$study_reviews_id);
			}
	}
}

...
//				if(!empty($fam_link) && !isset($fam_link_matches[$fam_link])) Config::$summary_msg['FAMILY HISTORY']['@@ERROR@@']['Family link value unknown'][] = "The 'Lien parenté' [$fam_link] is not supported by the migration process! [line: $line_counter]";
				...
				if(!empty($tumor_tmp) && isset($tumors_list_matches[$tumor_tmp]) && !empty($family_link_tmp)) {
					...
					$fam_link = '';
					$fam_notes = '';
					if(preg_match('/^(grand-mère) {0,1}(.*)$/',$family_link_tmp,$matches)) {
						$fam_link = 'grandmother';
					} else if(preg_match('/^(arr grand-père) {0,1}(.*)$/',$family_link_tmp,$matches)) {
						$fam_link = '';//TODO
					} else if(preg_match('/^(grand-père) {0,1}(.*)$/',$family_link_tmp,$matches)) {
						$fam_link = 'grandfather';
					...
					if(isset($matches[2])) $fam_notes = $matches[2];
					$tmp_family[utf8_decode($family_link_tmp)] = "$fam_link => $fam_notes";
					//TODO

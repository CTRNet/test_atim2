...
class TreatmentMasterCustom extends TreatmentMaster {
	...
	function summary( $variables=array() ) {
		...
		if ( isset($variables['TreatmentMaster.id']) ) {
			...
			if(isset($result['TreatmentDetail']['donor_number'])) $title .= ' '.$result['TreatmentDetail']['donor_number'];
			$return = array(
	...
	function beforeSave($options = array()){
		...
		if(isset($this->data['TreatmentDetail']['donor_status']) && isset($this->data['TreatmentDetail']['previous_transplant'])) {
			$this->addWritableField(array('donor_number'));
			$this->data['TreatmentDetail']['donor_number'] = (($this->data['TreatmentDetail']['donor_status'] == 'alive')? 'DV' : 'DC').($this->data['TreatmentDetail']['previous_transplant']+1);
		}
		return $ret_val;
	}
}

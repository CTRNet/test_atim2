<?php
	...
	if(isset($this->request->data['AliquotMaster']['ids'])) {
		...
		$sample_aliquot_types = $this->ViewAliquot->find('all', array(
			'conditions'	=> array('ViewAliquot.aliquot_master_id' => explode(",", $this->request->data['AliquotMaster']['ids'])),
			'fields'		=> array("DISTINCT CONCAT(ViewAliquot.sample_type, '-', ViewAliquot.aliquot_type) AS sample_aliquot_types")
		));
		...
		foreach($this->request->data as $procure_key => &$procure_data) {
			if(array_key_exists('AliquotDetail', $procure_data) && array_key_exists('procure_tube_weight_gr', $procure_data['AliquotDetail'])) {
				...
				if(empty($procure_data['AliquotDetail'])) unset($procure_data['AliquotDetail']);
			}
		}
		if($reset_aliquots_volume_structure) $this->Structures->set('used_aliq_in_stock_details,used_aliq_in_stock_detail_volume,procure_tube_weight_for_derivative_creation', 'aliquots_volume_structure');
	}

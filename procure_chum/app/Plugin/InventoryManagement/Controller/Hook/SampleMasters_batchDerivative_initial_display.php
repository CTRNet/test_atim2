<?php
    ...
    if(isset($this->request->data[0]['parent']['AliquotMaster'])) {
		...
		if($data_set_nbr_in_error) {
		    $this->flash(
		        __('no derivative can be created from sample created by system/script to migrate data from the processing site with no aliquot').' '.str_replace('%s', '['.implode('] ,[',$data_set_nbr_in_error).']', __('see # %s')),
		        $url_to_cancel,
		        5);
		    return;
		}

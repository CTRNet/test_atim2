...
$query = "SELECT id, NOW() as modified FROM users WHERE username = 'migration';";
...
While($new_psa = mysqli_fetch_assoc($psa_res)) {
	if(is_null($participant_id) || $participant_id != $new_psa['participant_id']) {
		if($participant_id && $bcr_detected && !$bcr_validated) {
			echo "Warning: BCR not validated.\n";
		}
	...
	if($prostatectomy_date) {
		if($prostatectomy_date < $new_psa['event_date']) {
			...
			if($all_patient_psa[0]['total_ngml'] >= 0.2 && !$bcr_detected) {
				echo "****** Patient ".$new_psa['participant_identifier']." (participant_id = ".$new_psa['participant_id'].") ******\n";
				if($all_patient_psa[0]['biochemical_relapse'] == 'y') {
					echo "BCR already set on ".$new_psa['event_date'].".\n";
				} else {
					echo "Flagged PSA as BCR on ".$new_psa['event_date'].".\n";
					$query = "UPDATE event_masters EventMaster, procure_ed_clinical_followup_worksheet_aps EventDetail
						...
						WHERE EventMaster.id = ".$all_patient_psa[0]['id']." AND EventMaster.id = EventDetail.event_master_id;";
					mysqli_query($db_connection, $query) or die("query failed [".$query."] (line:".__LINE__.") : " . mysqli_error($db_connection)."]");
				}
				$all_patient_psa[0]['biochemical_relapse'] = 'y';	
				$bcr_detected = true;
			}
			if($bcr_detected && sizeof($all_patient_psa) > 1 && $all_patient_psa[0]['total_ngml'] >= 0.2 && $all_patient_psa[1]['total_ngml'] >= 0.2 && $all_patient_psa[1]['biochemical_relapse'] == 'y') {
				echo "BCR validated.\n";
				$bcr_validated = true;
			}
		}

			...
			if(!$participant_id) {
				...
				if(empty(Config::$participants[$jgh_nbr]['rec_worksheet_patient_data'])) {
					...
					if($new_line_data['Prénom']){
						$first_name = Config::$participants[$jgh_nbr]['first_name'];
						if($first_name && $first_name!= $new_line_data['Prénom']) {
							Config::$summary_msg[$summary_msg_title_participant]['@@ERROR@@']['First Names Are Different'][] = "See JGH# ".$new_line_data['No de dossier']." line $excel_line_counter : (atim) $first_name != (file) ".$new_line_data['Prénom'];
						} else {
							Config::$participants[$jgh_nbr]['participant_db_data_to_update']['first_name'] = "first_name = '".$new_line_data['Prénom']."'";
						}
					}
					$last_name = Config::$participants[$jgh_nbr]['last_name'];
					if($new_line_data['Nom']) {
						if($last_name && $last_name!= $new_line_data['Nom']) {
							Config::$summary_msg[$summary_msg_title_participant]['@@ERROR@@']['Last Names Are Different'][] = "See JGH# ".$new_line_data['No de dossier']." line $excel_line_counter : (atim) $last_name != (file) ".$new_line_data['Nom'];
						} else {

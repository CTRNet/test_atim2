...
class ReportsControllerCustom extends ReportsController
{
    ...
    function bankActiviySummary($parameters)
    {
        ...
        if ($conditions) {
            ...
            foreach ($res as $new_record)
                $participant_subset_ids[] = $new_record['Participant']['id'];
        ...
        if ($participant_subset_ids)
            $conditions['Participant.id'] = $participant_subset_ids;
        ...
        if ($participant_subset_ids)
            $conditions['ConsentMaster.participant_id'] = $participant_subset_ids;
        ...
        if ($participant_subset_ids)
            $conditions .= " AND col.participant_id IN (" . implode(',', $participant_subset_ids) . ")";
    ...
    function createTissueAndBuffyCoatDNASummary($parameters)
    {
                ...
                if (array_key_exists('AliquotMaster', $parameters) && array_key_exists('id', $parameters['AliquotMaster'])) {
                    ...
                    if (array_key_exists('ViewCollection', $parameters)) {
                        if (array_key_exists('participant_identifier', $parameters['ViewCollection'])) {
                            ...
                            if ($participant_identifiers)
                                $criteria_array[] = "Participant.participant_identifier IN ('" . implode("','", $participant_identifiers) . "')";
                        } else 
                            if (array_key_exists('participant_identifier_start', $parameters['ViewCollection'])) {
                                ...
                                if (strlen($parameters['ViewCollection']['participant_identifier_start']))
                                    $tmp_conditions[] = "Participant.participant_identifier >= '" . $parameters['ViewCollection']['participant_identifier_start'] . "'";
                                if (strlen($parameters['ViewCollection']['participant_identifier_end']))
                                    $tmp_conditions[] = "Participant.participant_identifier <= '" . $parameters['ViewCollection']['participant_identifier_end'] . "'";
                                if ($tmp_conditions)
                                    $criteria_array[] = implode(" AND ", $tmp_conditions);
                            }
                    }
                    if (array_key_exists('ViewCollection', $parameters)) {
                        if (array_key_exists('misc_identifier_value', $parameters['ViewCollection'])) {
                            ...
                            if ($misc_identifier_values)
                                $criteria_array[] = "MiscIdentifier.identifier_value IN ('" . implode("','", $misc_identifier_values) . "')";
                        } else 
                    ...
                    if (array_key_exists('AliquotMaster', $parameters)) {
                        if (array_key_exists('aliquot_label', $parameters['AliquotMaster'])) {
                            ...
                            if ($aliquot_label_values)
                                $criteria_array[] = "AliquotMaster_TissueBlock_BcTube.aliquot_label IN ('" . implode("','", $aliquot_label_values) . "')";
                        } else 
        ...
        foreach ($all_data as $new_row) {
            ...
            if ($new_row['AliquotDetail_TissueTube']['tissue_tube_aliquot_master_id']) {
                if (is_null($tissues_blocks_and_buffy_coat_data[$block_aliquot_master_id]['AliquotDetail']['tissue_tube_aliquot_master_id'])) {
                    ...
                    if ($tissues_blocks_and_buffy_coat_data[$block_aliquot_master_id]['AliquotDetail']['tissue_tube_aliquot_master_id'] != $new_row['AliquotDetail_TissueTube']['tissue_tube_aliquot_master_id']) {
            ...
            if ($new_row['AliquotReviewDetail']['aliquot_review_master_id']) {
                if (! $tissues_blocks_and_buffy_coat_data[$block_aliquot_master_id]['AliquotReviewDetail']['aliquot_review_master_id']) {
                    ...
                    if ($tissues_blocks_and_buffy_coat_data[$block_aliquot_master_id]['AliquotReviewDetail']['aliquot_review_master_id'] != $new_row['AliquotReviewDetail']['aliquot_review_master_id']) {
                        $tissue_blocks_having_many_reviews[] = $new_row['AliquotMaster_TissueBlock_BcTube']['block_aliquot_label'];
                    }
            }
        }
        if ($tissue_blocks_having_many_reviews)
            AppController::addWarningMsg(__('some blocks have been attached to many reviews') . ' - ' . str_replace('%s', '"' . implode('", "', $tissue_blocks_having_many_reviews) . '"', __('see # %s')));
        if ($tissue_blocks_having_many_aliquots_source)
            AppController::addWarningMsg(__('some blocks are linked to many aliquots source') . ' - ' . str_replace('%s', '"' . implode('", "', $tissue_blocks_having_many_aliquots_source) . '"', __('see # %s')));
        ...
        foreach ($all_data as $new_row) {
            ...
            if (array_key_exists($bc_tube_aliquot_master_id, $tissues_blocks_and_buffy_coat_data))
        ...
        foreach ($dna_rna_sample_data as $new_dna_rna) {
            if ($new_dna_rna['AliquotMaster_DnaRna']['current_volume']) {
                if (! isset($dna_rna_samples_volumes[$new_dna_rna['SampleMaster_DnaRna']['dna_rna_sample_master_id']]))
                    $dna_rna_samples_volumes[$new_dna_rna['SampleMaster_DnaRna']['dna_rna_sample_master_id']] = 0;
            ...
            if (array_key_exists($dna_rna_sample_master_id, $dna_rna_samples_volumes) && strlen($new_qc_data['QualityCtrl_DnaRna']['concentration'])) {
                ...
                switch ($new_qc_data['QualityCtrl_DnaRna']['concentration_unit']) {
                    ...
                    case 'ug/ul':
                        ...
                        if (strlen($new_qc_data['AliquotMaster_TestedDnaRnaTube']['current_volume']))
                            $calculated_quantity_of_tested_aliquot = $new_qc_data['QualityCtrl_DnaRna']['concentration'] * $new_qc_data['AliquotMaster_TestedDnaRnaTube']['current_volume'] / $div;
                        break;
                    default:
                        $calculated_quantity = __('concentration unit missing');
                        if (strlen($new_qc_data['AliquotMaster_TestedDnaRnaTube']['current_volume']))
                            $calculated_quantity_of_tested_aliquot = __('concentration unit missing');
                }
            }
            switch ($new_qc_data['QualityCtrl_DnaRna']['type']) {
                ...
                case 'bioanalyzer':
                    ...
                    if ($dna_rna_sample_type == 'dna' && $new_qc_data['QualityCtrl_DnaRna']['type'] == 'PicoGreen') {
                        ...
                        if ($dna_rna_sample_type == 'rna' && $new_qc_data['QualityCtrl_DnaRna']['type'] == 'bioanalyzer') {
    ...
    function participantIdentifiersSummary($parameters)
    {
        ...
        foreach ($misc_identifiers as $new_ident) {
            ...
            $data[$participant_id]['0'][$ident_key] .= (empty($data[$participant_id]['0'][$ident_key]) ? '' : ' & ') . $new_ident['MiscIdentifier']['identifier_value'];
        }
        return array(
            'header' => $header,
            'data' => $data,

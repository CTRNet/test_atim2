...
foreach($drug_model->find('all') as $new_drug) $drug_from_id[$new_drug['Drug']['id']] = $new_drug['Drug']['generic_name'];

$protocol_from_id = array();
$protcol_model = AppModel::getInstance('Protocol', 'ProtocolMaster', true);
foreach($protcol_model->find('all') as $new_protocol) $protocol_from_id[$new_protocol['ProtocolMaster']['id']] = $new_protocol['ProtocolMaster']['code'];

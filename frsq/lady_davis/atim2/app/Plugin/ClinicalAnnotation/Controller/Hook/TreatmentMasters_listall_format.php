<?php
    ...
    foreach($this->request->data as &$newTreatment) {
        ...
        if ($newTreatment['TreatmentControl']['treatment_extend_control_id']) {
            $allNewTreatmentExtends = $this->TreatmentExtendMaster->find('all', array('conditions' => array('TreatmentExtendMaster.treatment_master_id' => $newTreatment['TreatmentMaster']['id'])));
            ...
            if ($generatedNewTreatmentPrecisions) {
                if($isDrug) {
                    $generatedNewTreatmentPrecisions = $this->Drug->find('all', array(
                        'conditions' => array('Drug.id' => $generatedNewTreatmentPrecisions), 
                        'fields' => array("GROUP_CONCAT(DISTINCT Drug.generic_name SEPARATOR ' & ') as all_drugs"),

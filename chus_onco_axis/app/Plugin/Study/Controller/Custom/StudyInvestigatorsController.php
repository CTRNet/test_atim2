	...
	function add( $study_summary_id ) {
		...
		$this->set( 'atim_menu_variables', array('StudySummary.id'=>$study_summary_id));
       	...
       	if ( !empty($this->request->data) ) {
			...
			if($this->StudyInvestigator->find('count', array('conditions' => array('StudyInvestigator.study_summary_id' => $study_summary_id, 'StudyInvestigator.last_name' => $this->request->data['StudyInvestigator']['last_name'])))) {
				$this->StudyInvestigator->validationErrors['last_name'][] = 'selected value already exists for the study';
			} else if ( $this->StudyInvestigator->save($this->request->data) ) {

<?php
	...
	// Get tissue tube for OCT embedding
	...
	$this->set('tissue_tubes_for_embedding', $this->AliquotMaster->find('all', array('conditions' => $qcroc_conditions, 'joins' => $joins, 'recursive' => '0')));

	// Get tissue block for sampling, etc
	$qcroc_conditions = array(
			'AliquotMaster.collection_id' => $collection_id,
			'SampleControl.sample_type' => 'tissue',
			'AliquotControl.aliquot_type' => array('block'));
	$this->set('tissue_blocks_for_sampling_and_processing', $this->AliquotMaster->find('all', array('conditions' => $qcroc_conditions, 'recursive' => '0')));	

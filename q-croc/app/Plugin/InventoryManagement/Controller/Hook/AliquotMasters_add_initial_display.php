		...
		foreach($this->request->data as $new_data_set){
			...
			switch($tissue_counter) {
				case '1':
					...
					if($full_tissues_data_from_id[$new_data_set['parent']['ViewSample']['sample_master_id']]['SampleDetail']['qcroc_tissue_storage_solution'] == 'rnalater') {
				...
				case '3':
					...
					if($full_tissues_data_from_id[$new_data_set['parent']['ViewSample']['sample_master_id']]['SampleDetail']['qcroc_tissue_storage_solution'] == 'formalin') {
			...
			if($fields_to_duplicate) {
				...
				if(!empty($existing_tissue_tubes)) {

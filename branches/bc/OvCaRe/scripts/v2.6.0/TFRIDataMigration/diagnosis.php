			...
			if($voa) {
				...
				if(in_array($voa, $studied_voas_for_duplicated_data_check)) {
					...
					if($results->num_rows > 1) {
						$summary_msg['Un-migrated VOA#s']['@@WARNING@@']["Reason: More than one Ovarian/Endometrium primary diagnosis into ATiM"]["$participant_id-$voa"] = "The migration process won't be able to match the excel diagnosis and an ATiM diagnosis: See atim patient id $participant_id (VOA#:$voa). [Worksheet: $worksheet_name]";
					...
					if(!in_array($participant_id, $participant_ids_to_skip)) {
						...
						if(empty($atim_diagnosis_data)) {
							if(empty($file_diagnosis_data)) {
								$summary_msg[$worksheet_name]['@@WARNING@@']["No Diagnosis Data in ATiM and Excel"][] = "No diagnosis and progressions will be created. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";						
							} else {
...
//TODO Add data in revs table (end of process)
								...
								$summary_msg['Data Creation/Update Summary'][$participant_id]["New Ovary/Endometrium Primary Diagnosis"][] = "With following values: $file_dx_data_strg. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
							}
						} else {
							...
							if(!empty($file_diagnosis_data)) {
								...
								foreach($file_diagnosis_data as $db_model_and_field => $file_value) {
									...
									if(!strlen($atim_diagnosis_data[$db_model_and_field])) {
										...
										$update_dx = true;
										$summary_msg[$worksheet_name]['@@WARNING@@']["Field '$db_field' Conflict"][] = "ATiM  value [".$atim_diagnosis_data[$db_model_and_field]."] is different than file value [".$file_diagnosis_data[$db_model_and_field]."]. Data will be updated. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
									}
									if($update_dx) {	
...
//TODO Add data in revs table (end of process)
									...
									$summary_msg['Data Creation/Update Summary'][$participant_id]["Ovarian/Endometrium Primary Diagnosis Update Summary"][] = "Updated following information : ".implode(',',$file_dx_data_strg).". See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";	
								}
							}
						}
						...
						if(strlen($new_line_data['Date of Progression/Recurrence::Date'].$new_line_data['Date of Progression/Recurrence::Accuracy'].$new_line_data['Site 1 of Primary Tumor Progression (metastasis)  If Applicable'].$new_line_data['Site 2 of Primary Tumor Progression (metastasis)  If applicable'])) {
							...
							if(!$date_data) {
								if(strlen($new_line_data['Date of Progression/Recurrence::Date'])) $summary_msg[$progression_summary_msg_key]['@@ERROR@@']["Secondary Diagnosis Date Error"][] = "Migration process is unable to get Progression date from value '".$new_line_data['Date of Progression/Recurrence::Date']."'. No date will be associated to the created secondary diagnosis. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
							...
							if(!$site_found && !empty($date_data['date'])) {
								$summary_msg[$progression_summary_msg_key]['@@WARNING@@']["Secondary Diagnosis Site is missing"][] = "But a date is set '".$date_data['date']."'. An 'unknown' secondary diagnosis will be created. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
	...
	foreach($dx_progressions as $voa => $new_voa_progressions) {
		if(!isset($voa_to_diagnosis_master_id[$voa]) || empty($voa_to_diagnosis_master_id[$voa])) {
			$summary_msg[$progression_summary_msg_key]['@@WARNING@@']["Unknown primary diagnosis of a progression"][] = "A diagnosis progression has been defined into the excel file but no primary diagnosis has been created or defined into ATiM. No progression will be created. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name]";
		} else {
			$participant_id = $voas_to_participant_id[$voa];
			...
			if(array_key_exists('progression', $new_voa_progressions)) {
				foreach($new_voa_progressions['progression'] as $new_progression) {
					if(empty($new_progression['date']) && $new_progression['site'] == 'unknown') {
						$summary_msg[$progression_summary_msg_key]['@@WARNING@@']["Unknown site of a diagnosis progression plus no date"][] = "No progression will be created. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name]";
					} else {
						$master_data = array(
							'participant_id' => $participant_id,
							'primary_id' => $primary_id,
		...
		if($excel_line_counter == 1) {
			...
			if($voa) {
				...
				switch($new_line_data['Event Type']) {
					...
					// == chemotherapy ==
					case 'chemotherapy':
						if($ca125_data_set) $summary_msg[$worksheet_name]['@@ERROR@@']["CA125 linked to worng event type"][] = "CA125 are associated to event ".$new_line_data['Event Type'].". CA125 data won't be migrated. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
						if($scan_data_set) $summary_msg[$worksheet_name]['@@ERROR@@']["CTScan result linked to worng event type"][] = "CTScan are associated to event ".$new_line_data['Event Type'].". CTScan data won't be migrated. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
						$chemo_drugs = array();
						for($id=1; $id<5; $id++) {
							$drug = trim(str_replace(array('carboplatinum'), array('carboplatin'), strtolower($new_line_data["Chemotherapy Precision::Drug$id"])));
							if($drug == 'other') {
								$summary_msg[$worksheet_name]['@@WARNING@@']["Chemo Drug Value 'other'"][] = "A chemo drug was defined as 'other' for chemotherapy on on '".$start_data['date']."'. Value won't be migrated. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
							} else if(strlen($drug)) {
						...
						if(isset($participant_id_to_atim_chemos[$participant_id][$start_data['date']])) {
							...
							if($diagnosis_master_id) {
								...
								if(!$atim_tx_data['diagnosis_master_id']) {
									...
									$summary_msg['Data Creation/Update Summary'][$participant_id]["Linked a chemotherapy to diagnosis"][] = "Linked chemotherapy started on '".$start_data['date']."' to a diagnosis. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
								} else if($atim_tx_data['diagnosis_master_id'] != $diagnosis_master_id) {
									$update_diagnosis_master_id = true;
									$summary_msg['Data Creation/Update Summary'][$participant_id]["Changed the diagnosis linked to a chemotherapy"][] = "Linked chemotherapy started on '".$start_data['date']."' to another diagnosis (atim id ".$atim_tx_data['diagnosis_master_id']." to $diagnosis_master_id) . See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
								}
							...
							foreach($chemo_drugs as $drug_name => $drug_id) {
								if(!in_array($drug_name, $atim_tx_data['drugs'])) {
									...
									$summary_msg['Data Creation/Update Summary'][$participant_id]["Added new chemotherapy drug"][] = "Added '$drug_name' to chemotherapy started on '".$start_data['date']."'. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";			
							...
							$summary_msg['Data Creation/Update Summary'][$participant_id]["Chemotherapy Creation"][] = "Created chemotherapy on '".$start_data['date']."' with drugs list {".implode(', ',array_keys($chemo_drugs))."}. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
						}
						break;
					...
					case 'surgery(other)':
					case 'biopsy':
						if($drug_data_set) $summary_msg[$worksheet_name]['@@ERROR@@']["Durgs linked to worng event type"][] = "Drugs are associated to event ".$new_line_data['Event Type'].". Drug data won't be migrated. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
						if($ca125_data_set) $summary_msg[$worksheet_name]['@@ERROR@@']["CA125 linked to worng event type"][] = "CA125 are associated to event ".$new_line_data['Event Type'].". CA125 data won't be migrated. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
						if($scan_data_set) $summary_msg[$worksheet_name]['@@ERROR@@']["CTScan result linked to worng event type"][] = "CTScan are associated to event ".$new_line_data['Event Type'].". CTScan data won't be migrated. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
						...
						if(isset($participant_id_to_atim_surgery_biopsy[$participant_id][$start_data['date']])) {
							if(sizeof($participant_id_to_atim_surgery_biopsy[$participant_id][$start_data['date']]) > 1) die('ERR327328328832 to support');
							...
							if($residual_disease) {
								if(!$atim_tx_data['ovcare_residual_disease']) {
									...
									$summary_msg['Data Creation/Update Summary'][$participant_id]["Recorded residual disease"][] = "Set residual disease '$residual_disease' for $surgical_procedure on '".$start_data['date']."'. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
								} else if($atim_tx_data['ovcare_residual_disease'] != $residual_disease) {
									$update_residual_disease = true;
									$summary_msg['Data Creation/Update Summary'][$participant_id]["Updated residual disease"][] = "Changed residual disease '".$atim_tx_data['ovcare_residual_disease']."'  to '$residual_disease' for $surgical_procedure on '".$start_data['date']."'. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
								}
...
//TODO Add data in revs table (end of process)									
							...
							if($diagnosis_master_id) {
								...
								if(!$atim_tx_data['diagnosis_master_id']) {
									$update_diagnosis_master_id = true;
									$summary_msg['Data Creation/Update Summary'][$participant_id]["Linked a surgery to diagnosis"][] = "Linked $surgical_procedure on '".$start_data['date']."' to a diagnosis. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
								} else if($atim_tx_data['diagnosis_master_id'] != $diagnosis_master_id) {
									$update_diagnosis_master_id = true;
									$summary_msg['Data Creation/Update Summary'][$participant_id]["Changed the diagnosis linked to a surgery"][] = "Linked $surgical_procedure on '".$start_data['date']."' to another diagnosis (atim id ".$atim_tx_data['diagnosis_master_id']." to $diagnosis_master_id) . See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
								}
								if($update_diagnosis_master_id) {
...
//TODO Add data in revs table (end of process)
							...
							if(!$surgical_procedure_already_recorded) {
								...
								$summary_msg['Data Creation/Update Summary'][$participant_id]["Created new Surgery/Biopsy Procedure Performed"][] = "Added '$surgical_procedure' to Procedures Performed list for surgery/biopsy on '".$start_data['date']."'. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
							}
						} else {
							//Add Treatment
							$master_data = array(
...
//TODO Add data in revs table (end of process)				
							...
							$summary_msg['Data Creation/Update Summary'][$participant_id]["Surgery/Biopsy Creation"][] = "Created $surgical_procedure on '".$start_data['date']."'. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
						}
						break;
					// == ct scan ==
					case 'CA125':
						...
						if(!strlen($ca125_value)) {
							$summary_msg[$worksheet_name]['@@ERROR@@']["CA125 value empty"][] = "No CA125 Value is set for an event on '".$start_data['date']."'. No CA125 will be created. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
						} else if(!preg_match('/^[0-9]+\.[0-9]{2}$/', $ca125_value)){
							$summary_msg[$worksheet_name]['@@ERROR@@']["Wrong CA125 value format"][] = "Format of CA125 Value '$ca125_value' is wrong. No CA125 will be created. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
						} else {
							...
							if(isset($participant_id_to_atim_ca125s[$participant_id]) && array_key_exists($ca125_value, $participant_id_to_atim_ca125s[$participant_id])) {
								foreach($participant_id_to_atim_ca125s[$participant_id][$ca125_value] as $key => $event_data) {
									if(empty($event_data['event_date'])) {
										...
										$summary_msg['Data Creation/Update Summary'][$participant_id]["Set date to an existing ATiM CA125"][] = "The test of an existing CA125 = [$ca125_value] has been set to '".$start_data['date']."'. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
							...
							if(isset($ca125_progression_diagnosis_master_id[$voa]) && array_key_exists($start_data['date'], $ca125_progression_diagnosis_master_id[$voa])) {
								...
								$summary_msg['Data Creation/Update Summary'][$participant_id]["Linked a CA125 value to a diagnosis progression"][] = "CA125 on '".$start_data['date']."'. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
							} else if($diagnosis_master_id && $event_master_id_to_update) {
								if(!$diagnosis_master_id_of_event_to_update) {
									$summary_msg['Data Creation/Update Summary'][$participant_id]["Linked existing CA125 to diagnosis"][] = "Linked CA125 on '".$start_data['date']."' to a diagnosis. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
								} else if($diagnosis_master_id_of_event_to_update != $diagnosis_master_id) {
									$summary_msg['Data Creation/Update Summary'][$participant_id]["Changed diagnosis linked to CA125"][] = "Linked CA125 on '".$start_data['date']."' to another diagnosis (atim id $diagnosis_master_id_of_event_to_update to $diagnosis_master_id) . See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
								}
							}
							if($event_master_id_to_update) {
								...
								$summary_msg['Data Creation/Update Summary'][$participant_id]["Added CA125 value"][] = "Recorded CA125 = [$ca125_value] on '".$start_data['date']."'. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
							}
						}
						break;
					// == ct scan ==
					case 'CT scan':
						...
						if(!in_array($new_line_data['CT Scan Precision'], array('', 'negative', 'positive', 'unknown'))) {
							...
							$summary_msg[$worksheet_name]['@@ERROR@@']["Wrong CT-Scan result"][] = "The format of a CT-Scan result '".$new_line_data['CT Scan Precision']."' is wrong. Value won't be recorded. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
						}
						if(strlen($start_data['date'].$new_line_data['CT Scan Precision'])) {
							...
							$summary_msg['Data Creation/Update Summary'][$participant_id]["CT-Scan creation"][] = "CT-Scan with result = [".$new_line_data['CT Scan Precision']."] on '".$start_data['date']."'. See atim patient id $participant_id (VOA#$voa). [Worksheet: $worksheet_name /line: $excel_line_counter]";
						}
						break;
					default:
	...
	foreach($tmp_voa_to_residual_disease as $voa => $residual_disease) {
		if($residual_disease) {
			$summary_msg[$worksheet_name]['@@ERROR@@']["Unrecorded residual disease"][] = "Residual disease '$residual_disease' set for an ovarectomy of the voa#".$voa." has not been recorded. Please check. See atim patient id $participant_id (VOA#$voa).";
		}
	}
}

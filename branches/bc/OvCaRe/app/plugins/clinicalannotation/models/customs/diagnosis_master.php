...
class DiagnosisMasterCustom extends DiagnosisMaster {
	...
	function setInitialSurgeryDate($diagnosis_master_id) {
		...
		if(($diagnosis_data['DiagnosisControl']['category'] == 'primary') && ($diagnosis_data['DiagnosisControl']['controls_type'] == 'ovcare')) {
			...
			$new_min_surgery_date = empty($surgeries_list)? null : $surgeries_list['TreatmentMaster']['start_date'];
			$new_min_surgery_date_accuracy = empty($surgeries_list)? null : $surgeries_list['TreatmentMaster']['start_date_accuracy'];		
	...
	function updateAllSurvivaleTimes($participant_id) {
		...
		foreach($diagnoses_list as $new_diagnosis) $this->updateSurvivalTime($new_diagnosis['DiagnosisMaster']['id'], $new_diagnosis);
	}	
	
	function updateSurvivalTime($diagnosis_master_id, $diagnosis_data = array()) {
		...
		if(($diagnosis_data['DiagnosisControl']['category'] == 'primary') && ($diagnosis_data['DiagnosisControl']['controls_type'] == 'ovcare')) {
			...
			if(!empty($participant_data['Participant']['ovcare_last_followup_date']) && !empty($diagnosis_data['DiagnosisDetail']['initial_surgery_date'])) {
				...
				&& in_array($diagnosis_data['DiagnosisDetail']['initial_surgery_date_accuracy'], array('d','c'))) {
					$initial_surgery_date = new DateTime($diagnosis_data['DiagnosisDetail']['initial_surgery_date']);
					$ovcare_last_followup_date = new DateTime($participant_data['Participant']['ovcare_last_followup_date']);
					$interval = $initial_surgery_date->diff($ovcare_last_followup_date);

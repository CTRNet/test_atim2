<?php 
	...
	*/
	...
	if($dxControlId == $oncologyControlId) {
		...
		$dxPrimaryId = $dx_master_data['DiagnosisMaster']['id'];
		...
		$eventDateAccuracy = 'c';
		$modifiedBy;
		$participantId = $dx_master_data['Participant']['id'];
		...
		*/
		...
		$query = "SELECT id FROM dxd_bcch_oncology WHERE diagnosis_master_id=".$dxPrimaryId." ORDER BY id DESC LIMIT 1;";
		...
		$query = "SELECT version_id FROM dxd_bcch_oncology_revs WHERE diagnosis_master_id=".$dxPrimaryId." ORDER BY version_id DESC LIMIT 1;";
		...
		//Remind user to delete the old event		
		$this->addWarningMsg(__('remember to delete the previous diagnosis code after edit'));

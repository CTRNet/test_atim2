	...
	function updateAllDxAges($date_of_birth) {
		...
		foreach($participant_dx_results as $new_dx) {
			...
			if(!empty($dx_date) && !empty($date_of_birth) && ($dx_date >= $date_of_birth)) {
				...
				// Using updateAll() to update all the relevant diagnosis records
				$this->DiagnosisMaster->updateAll(
						array('DiagnosisMaster.age_at_dx' => $ageInYears, 
				  			  'DiagnosisMaster.ccbr_age_at_dx_months' => $ageInMonths,
				              'DiagnosisMaster.ccbr_age_at_dx_weeks' => $ageInWeeks,
				              'DiagnosisMaster.ccbr_age_at_dx_days' => $ageInDays),
						array('DiagnosisMaster.id' => $new_dx_id,
				   			  'DiagnosisMaster.participant_id' => $this->id)
				);
				...
				// Using updateAll() to update all the relevant diagnosis records			
				$this->DiagnosisMaster->updateAll(
						array('DiagnosisMaster.age_at_dx' => NULL, 
				  			  'DiagnosisMaster.ccbr_age_at_dx_months' => NULL,
				              'DiagnosisMaster.ccbr_age_at_dx_weeks' => NULL,
				              'DiagnosisMaster.ccbr_age_at_dx_days' => NULL),
						array('DiagnosisMaster.id' => $new_dx_id,
				   			  'DiagnosisMaster.participant_id' => $this->id)
				);

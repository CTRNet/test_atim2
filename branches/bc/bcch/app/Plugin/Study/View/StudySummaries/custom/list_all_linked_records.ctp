<?php
	...
	*/
	...
	$final_options = array(
		...
		'links'	=> array(),
		'settings' => array(
			'header' => __('consent masters', null),
			'actions'	=> true), 
		'extras' => $link_permissions['consent masters']? array('end' => $this->Structures->ajaxIndex('Study/StudySummaries/listAllLinkedConsentForms/'.$atim_menu_variables['StudySummary.id'])) : __('You are not authorized to access that location.')
	);
	
	$display_next_form = true; 
	
	// CUSTOM CODE
	$hook_link = $this->Structures->hook('consent_masters');
	if( $hook_link ) { 
		require($hook_link); 
	}

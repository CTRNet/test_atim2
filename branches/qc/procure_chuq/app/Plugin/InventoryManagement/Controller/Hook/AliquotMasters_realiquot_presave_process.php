	...
	foreach($this->request->data as $procure_aliquot_data_set) {
		...
		if(Configure::read('procure_atim_version') == 'PROCESSING') {
			foreach($procure_aliquot_data_set['children'] as $procure_aliquot_data) {
				if(!preg_match('/^[0-9]+$/',$procure_aliquot_data['AliquotMaster']['barcode'])) {
					$errors['barcode']['aliquot barcode format errror - integer value expected'][$record_counter] = $record_counter;
				}
			}
		} else {

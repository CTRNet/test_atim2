...
function postDxRead(Model $m){
	...
	Config::$prostate_dx_fields = array('dx_date' => 'Date of diagnostics Date',
	...
	if(!preg_match('/^[0-9]*$/', $m->values['Age at Time of Diagnosis (yr)'], $matches)) {
	...
	if($m->values['Patient # in biobank'] == $m->custom_data['last_pkey']){
		...
		foreach(Config::$prostate_dx_fields as $field) {
...
function manageManyRows(Model $m){
	...
	foreach(Config::$prostate_dx_fields as $db_field => $new_field) {
				...
				switch($new_field) {				
					...
					case 'hormonorefractory status status':
						...
						if(is_null($m->values[$new_field])){
							printf("WARNING: Invalid [$new_field] value [%s] for dx at line [%d]\n", $m->values[$new_field], $m->line);
						} else {
		...
		if(!empty($data)) {
			$query = "UPDATE $table SET ".implode(', ', $data)." WHERE ".
				(($table == 'diagnosis_masters')? 
					...
					"diagnosis_master_id=(SELECT id FROM diagnosis_masters WHERE participant_id = $db_participant_id AND diagnosis_control_id = 14);");		

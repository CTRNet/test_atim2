...
$detail_fields = array(
	'tool'									=> array('Date of diagnosis  diagnostic tool' => new ValueDomain("qc_tf_dx_tool", ValueDomain::ALLOW_BLANK, ValueDomain::CASE_INSENSITIVE)), 
...
$prostate_dx_fields_to_check = array(
	...
	'tool' => 'Date of diagnosis  diagnostic tool',
...
function preDxWrite(Model $m){
	$m->values['age_at_dx'] = getAgeAtDx($m->parent_model->values['Date of Birth Date'], $m->parent_model->values['Date of Birth Accuracy'], $m->values['Date of diagnosis Date'], $m->values['Date of diagnosis Accuracy'], 'diagnosis: primary', $m->line);
	return true;
}
	...
	foreach($m->custom_data['prostate_dx_fields_to_check'] as $db_field => $new_field) {
		if(strlen($m->values[$new_field])) {
			if(array_key_exists($new_field, $m->custom_data['dx_data_already_recorded'])) {
				...
				switch($new_field) {				
					case 'Date of diagnosis  diagnostic tool':
					case 'pTNM RP':
					case 'RP pTNM':
					case 'hormonorefractory status status':
						...
						break;
					case 'Date of diagnosis Date':
					case 'Date of diagnosis Accuracy':
...
function checkTypeOfBiopsySurgeryValue($values, $lines) {
	...
	switch($values[$type_field]) {
		...
		case 'biopsy':
			$error_fields = array();
			foreach($RP_fields as $new_field) if(strlen($values[$new_field])) $error_fields[] = $new_field;
			if($error_fields) Config::$summary_msg['diagnosis: biopsy/surgery']['@@ERROR@@']['RP data set for Surgery/Biopsy Specification different than RP'][] = "See line $lines fields: ".implode(', ', $error_fields);
			break;
		case 'RP':
			$error_fields = false;
			foreach($Bx_TURP_fields as $new_field) if(strlen($values[$new_field])) $error_fields[] = $new_field;
			if($error_fields) Config::$summary_msg['diagnosis: biopsy/surgery']['@@ERROR@@']['Biopsy data set for Surgery/Biopsy Specification different than Biopsy or turp'][] = "See line $lines fields: ".implode(', ', $error_fields);
			break;
		case '':
			$error_fields = false;
			foreach($Bx_TURP_fields as $new_field) if(strlen($values[$new_field])) $error_fields[] = $new_field;
			if($error_fields) Config::$summary_msg['diagnosis: biopsy/surgery']['@@ERROR@@']['Biopsy data set for undefined Surgery/Biopsy Specification'][] = "See line $lines fields: ".implode(', ', $error_fields);
			$error_fields = false;
			foreach($RP_fields as $new_field) if(strlen($values[$new_field])) $error_fields[] = $new_field;
			if($error_fields) Config::$summary_msg['diagnosis: biopsy/surgery']['@@ERROR@@']['RP data set for undefined Surgery/Biopsy Specification'][] = "See line $lines fields: ".implode(', ', $error_fields);
			break;			
		default:

...
class TreatmentMasterCustom extends TreatmentMaster
{
    ...
    var $dx_biopsy_and_turp_types = array(
        'TURP Dx',
        'Bx Dx',
        'Bx Dx TRUS-Guided'
    );
    ...
    function validates($options = array())
    {
        ...
        if (array_key_exists('qc_tf_disease_free_survival_start_events', $this->data['TreatmentMaster']) && $this->data['TreatmentMaster']['qc_tf_disease_free_survival_start_events']) {
            ...
            if (! in_array($treatment_control['TreatmentControl']['tx_method'], array(
                'hormonotherapy',
                'RP',
                'radiation',
                'chemotherapy'
    ...
    private function getTreatmentControlData()
    {
        ...
        $treatment_control = $treatment_control_model->find('first', array(
            'conditions' => array(
                'TreatmentControl.id' => $treatment_control_id
            )
        ));
    ...
    {
        $deleted_tx_data = $this->find('first', array(
            'conditions' => array(
    ...
    function afterSave($created, $options = Array())
    {
        ...
        if ($this->name == 'TreatmentMaster') {
            ...
            if ($tx['TreatmentControl']['tx_method'] == 'biopsy and turp' || $tx['TreatmentControl']['tx_method'] == 'RP') {
                ...
                $diagnosis_model = AppModel::getInstance('ClinicalAnnotation', 'DiagnosisMaster', true);
                $prostate_dxs = $diagnosis_model->find('all', array(
                    'conditions' => array(
                ...
                foreach ($prostate_dxs as $participant_prostate_dx) {
                    ...
                    $turp_biopsy_dx = $this->find('first', array(
                        'conditions' => $conditions,
                        'joins' => $joins
                    ));
                    if ($turp_biopsy_dx)
                        $tx_gleason_score_biopsy_turp = $turp_biopsy_dx['TreatmentDetail']['gleason_score'];
                    if ($dx_gleason_score_biopsy_turp != $tx_gleason_score_biopsy_turp)
                        $diagnosis_detail_data_tu_update['gleason_score_biopsy_turp'] = $tx_gleason_score_biopsy_turp;
                        // Biopsy/TURP ctnm
                    $dx_ctnm_biopsy_turp = $participant_prostate_dx['DiagnosisDetail']['ctnm'];
                    $tx_ctnm_biopsy_turp = '';
                    if ($turp_biopsy_dx)
                        $tx_ctnm_biopsy_turp = $turp_biopsy_dx['TreatmentDetail']['ctnm'];
                    if ($dx_ctnm_biopsy_turp != $tx_ctnm_biopsy_turp)
                        $diagnosis_detail_data_tu_update['ctnm'] = $tx_ctnm_biopsy_turp;
                        // RP gleason
                    ...
                    if ($rp)
                        $tx_gleason_score_rp = $rp['TreatmentDetail']['qc_tf_gleason_score'];
                    if ($dx_gleason_score_rp != $tx_gleason_score_rp)
                        $diagnosis_detail_data_tu_update['gleason_score_rp'] = $tx_gleason_score_rp;
                        // Dx Update
                    if ($diagnosis_detail_data_tu_update) {
                        $diagnosis_data = array(
                            'DiagnosisMaster' => array(),
                            'DiagnosisDetail' => $diagnosis_detail_data_tu_update
                        );
                        $diagnosis_model->id = $participant_prostate_dx['DiagnosisMaster']['id'];
                        $diagnosis_model->data = null;
                        $diagnosis_model->addWritableField(array(
                            'gleason_score_rp',
                            ''
                        ));

<?php 
	...
	switch($tx['TreatmentControl']['tx_method']) {
		...
		case 'bone specific therapy':
			$allLinkedDrugs = $treatmentExtendModel->find('all', array('conditions' => array('TreatmentExtendMaster.treatment_master_id' => $tx['TreatmentMaster']['id'])));
			$drugs = array();
			...
			if($drugs) $chronologyDetails = implode(' + ', $drugs);
			break;
		case 'other cancer':
	...
	if($chronologyDetails) $chronolgyDataTreatmentStart['chronology_details'] = $chronologyDetails;

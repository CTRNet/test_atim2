...
class TreatmentMasterCustom extends TreatmentMaster
{
    ...
    public function validatesTreatmentToDiagnosisLink($treatmentMasterData, $treatmentControlData)
    {
        ...
        if (! isset($treatmentMasterData['diagnosis_master_id']) || ! $treatmentMasterData['diagnosis_master_id']) {
            ...
            $selectedDx = $DiagnosisMaster->find('first', array(
                'conditions' => array(
                    'DiagnosisMaster.id' => $treatmentMasterData['diagnosis_master_id']
                ),
                'recursive' => 0
            ));
    ...
    public function calculateTimesTo($participantId)
    {
        ...
        $allBreastTxDiagnosisEvent = $this->find('all', array(
            'conditions' => $conditions,
            'recursive' => 0
        ));
        
        if (! $allBreastTxDiagnosisEvent)
            return;
        ...
        if (empty($lastContactOrDeathDate))
            $allWarnings["the last contact or death date is unknown - the 'time to last contact' values cannot be calculated"] = array();
        ...
        $DiagnosisMaster = AppModel::getInstance("ClinicalAnnotation", "DiagnosisMaster", true);
        $progressionWithNoDate = $DiagnosisMaster->find('count', array(
            'conditions' => $conditions,
            'recursive' => 0
        ));
        if ($progressionWithNoDate)
            $allWarnings["at least one breast progression diagnosis date is unknown"] = array();
        ...
        $breastDxEventWithNoDate = $this->find('count', array(
            'conditions' => $conditions,
            'recursive' => 0
        ));
        if ($breastDxEventWithNoDate)
            $allWarnings["at least one breast diagnosis event date is unknown"] = array();
        ...
        foreach ($allBreastTxDiagnosisEvent as $newBreastTx) {
            ...
            if ($newBreastTx['TreatmentMaster']['start_date']) {
                ...
                if (! empty($lastContactOrDeathDate)) {
                    if (in_array($startDateAccuracy . $lastContactOrDeathDateAccuracy, array(
                        'cc',
                        'cd',
                        'dc'
                    ))) {
                        if ($startDateAccuracy . $lastContactOrDeathDateAccuracy != 'cc')
                            $allWarnings["'time to last contact' has been calculated with at least one unaccuracy date"][$startDate] = $startDate;
                ...
                $firstProgression = $DiagnosisMaster->find('first', array(
                    'conditions' => $conditions,
                    'recursive' => 0,
                    'order' => array(
                ...
                if ($firstProgression) {
                    ...
                    if (in_array($startDateAccuracy . $firstProgressionDateAccuracy, array(
                        'cc',
                        'cd',
                        'dc'
                    ))) {
                        if ($startDateAccuracy . $firstProgressionDateAccuracy != 'cc')
                            $allWarnings["'time to first progression' has been calculated with at least one unaccuracy date"][$startDate] = $startDate;
                ...
                $nextBreastDiagnosisEvent = $this->find('first', array(
                    'conditions' => $conditions,
                    'recursive' => 0,
                    'order' => array(
                ...
                if ($nextBreastDiagnosisEvent) {
                    ...
                    if (in_array($startDateAccuracy . $nextBreastDiagnosisEventDateAccuracy, array(
                        'cc',
                        'cd',
                        'dc'
                    ))) {
                        if ($startDateAccuracy . $nextBreastDiagnosisEventDateAccuracy != 'cc')
                            $allWarnings["'time to next breast diagnosis event' has been calculated with at least one unaccuracy date"][$startDate] = $startDate;
            ...
            if ($newTimeToLastContactMonths !== $newBreastTx['TreatmentDetail']['time_to_last_contact_months'])
                $treatmentDetailToUpdate['time_to_last_contact_months'] = $newTimeToLastContactMonths;
            if ($newTimeToFirstProgressionMonths !== $newBreastTx['TreatmentDetail']['time_to_first_progression_months'])
                $treatmentDetailToUpdate['time_to_first_progression_months'] = $newTimeToFirstProgressionMonths;
            if ($newTimeToNextBreastDxEventMonths !== $newBreastTx['TreatmentDetail']['time_to_next_breast_dx_event_months'])
                $treatmentDetailToUpdate['time_to_next_breast_dx_event_months'] = $newTimeToNextBreastDxEventMonths;
            if ($treatmentDetailToUpdate) {
                $allBreastTxDiagnosisEventToUpdate[] = array(
                    'TreatmentMaster' => array(
                        'id' => $newBreastTx['TreatmentMaster']['id']
                    ),
        ...
        foreach ($allWarnings as $newWarning => $allStartDates)
            AppController::getInstance()->addWarningMsg(__($newWarning) . ($allStartDates ? ' - ' . str_replace('%s', implode(', ', $allStartDates), __('see treatment done on %s')) : ''));
    }
}
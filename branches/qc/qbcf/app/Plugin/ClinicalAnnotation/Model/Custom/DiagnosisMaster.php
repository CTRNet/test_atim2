...
class DiagnosisMasterCustom extends DiagnosisMaster {
	...
	public function summary( $diagnosisMasterId = null ) {
		...
		if ( !is_null($diagnosisMasterId) ) {
			$result = $this->find('first', array('conditions'=>array('DiagnosisMaster.id'=>$diagnosisMasterId), 'recursive' => 0));
				
			$structureAlias = 'diagnosismasters';
			switch($result['DiagnosisControl']['category']) {
				case 'primary':
					if($result['DiagnosisControl']['controls_type'] != 'primary diagnosis unknown') $structureAlias .= ',dx_primary';
					break;
	...
	public function setBreastDxLaterality($participantId) {
		...
		$participantBreastDx = $this->find('all', array('conditions' => array(
		...
		foreach($participantBreastDx as $newBrestDx) {
			...
			$breastDxEventLaterality = $TreatmentModel->find('all', array(
				'conditions' => array('TreatmentControl.tx_method' => 'breast diagnostic event', 'TreatmentMaster.diagnosis_master_id' => $newBrestDx['DiagnosisMaster']['id']), 
				'joins' => array($txJoins),
				'fields' => array('DISTINCT TreatmentDetail.laterality')));
			$allDxLaterality = array('laterality_left' => 'n', 'laterality_right' => 'n', 'laterality_bilateral' => 'n');
			foreach($breastDxEventLaterality as $newLaterality) {
				$newLaterality = $newLaterality['TreatmentDetail']['laterality'];
				if(isset($allDxLaterality['laterality_'.$newLaterality])) $allDxLaterality['laterality_'.$newLaterality] = 'y';
			}
			...
			if($allDxLaterality) {
				$allDxToUpdate[] = array(
					'DiagnosisMaster' => array('id' => $newBrestDx['DiagnosisMaster']['id']),

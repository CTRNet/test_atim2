<?php
	...
	if($storage_data['StorageControl']['detail_tablename'] == 'std_tma_blocks') {
		...
		$tissue_core_natures = $StructurePermissibleValuesCustom->getCustomDropdown(array('Tissue Core Natures'));
		...
		$tmp_aliquot_data = $this->AliquotMaster->find('all', array('conditions' => array('AliquotMaster.storage_master_id' => $storage_master_id), 'recursive' => '0'));
		foreach($tmp_aliquot_data as $tmp_new_aliquot) {
			if($tmp_new_aliquot['SampleControl']['sample_type'] == 'tissue' && $tmp_new_aliquot['AliquotControl']['aliquot_type'] == 'core') {
				...
				$new_core_label = 
					($is_conrol? 'Ctrl: ' : '').
					$tmp_new_aliquot['AliquotMaster']['aliquot_label'].
					($is_conrol? '' : ' ['.(array_key_exists($tmp_new_aliquot['AliquotDetail']['qc_nd_core_nature'], $tissue_core_natures)? $tissue_core_natures[$tmp_new_aliquot['AliquotDetail']['qc_nd_core_nature']] : (empty($tmp_new_aliquot['AliquotDetail']['qc_nd_core_nature'])? '?':$tmp_new_aliquot['AliquotDetail']['qc_nd_core_nature'])).']');

...
class AliquotMasterCustom extends AliquotMaster
{
    ...
    function generateDefaultAliquotLabel($view_sample, $aliquot_control_data)
    {
        ...
        if ($view_sample['ViewSample']['bank_id'] == 4) {
            ...
            switch ($view_sample['ViewSample']['sample_type'] . '-' . $aliquot_control_data['AliquotControl']['aliquot_type']) {
                case 'blood-tube':
                    ...
                    $sample_data = $this->SampleMaster->find('first', array(
                        'conditions' => array(
                            'SampleMaster.id' => $view_sample['ViewSample']['sample_master_id']
                        ),
                        'recursive' => '0'
                    ));
                    if ($sample_data) {
            ...
            $tmp_col_data = $this->Collection->find('first', array(
                'conditions' => array(
                    'id' => $view_sample['ViewSample']['collection_id']
                ),
                'recursive' => '-1'
            ));
            ...
            $default_aliquot_label = (empty($view_sample['ViewSample']['qc_nd_sample_label']) ? 'n/a' : $view_sample['ViewSample']['qc_nd_sample_label']) . (empty($aliquot_creation_date) ? '' : ' ' . substr($aliquot_creation_date, 0, strpos($aliquot_creation_date, " ")));
        }

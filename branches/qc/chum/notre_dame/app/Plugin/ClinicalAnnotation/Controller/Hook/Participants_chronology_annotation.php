	...
	switch($annotation['EventControl']['event_type']) {
		...
		case 'other marker':
			if(strlen($annotation['EventDetail']['value'])) {
				if(strlen($lab_marker_types[$annotation['EventDetail']['type']])) $chronolgy_data_annotation['event'] = $lab_marker_types[$annotation['EventDetail']['type']];
				$chronolgy_data_annotation['chronology_details'] =
					$annotation['EventDetail']['value'].
					' '.
					$lab_marker_units[$annotation['EventDetail']['unit']];
			}
			break;
		default:
	}

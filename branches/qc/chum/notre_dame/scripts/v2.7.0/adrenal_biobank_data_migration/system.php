...
function validateAndGetDateAndAccuracy($date, $summary_section_title, $summary_title_add_in, $summary_details_add_in)
{
    ...
    if (empty($date) || in_array(strtolower($date), $empty_date_time_values)) {
        ...
        if (preg_match('/^([0-9]+)$/', $date, $matches)) {
            ...
            if (preg_match('/^(19|20)([0-9]{2})\-([01][0-9])\-([0-3][0-9])$/', $date, $matches)) {
                return array(
                    $date,
                    'c'
                );
            } else 
                if (preg_match('/^(19|20)([0-9]{2})\-([01][0-9])$/', $date, $matches)) {
                    return array(
                        $date . '-01',
                        'd'
                    );
                } else 
                    if (preg_match('/^((19|20)([0-9]{2})\-([01][0-9]))\-((xx)|(unk))$/', $date, $matches)) {
                        return array(
                            $matches[1] . '-01',
                            'd'
                        );
                    } else 
                        if (preg_match('/^((19|20)([0-9]{2}))\-((xx)|(unk))\-((xx)|(unk))$/', $date, $matches)) {
                            return array(
                                $matches[1] . '-01-01',
                                'm'
                            );
                        } else 
                            if (preg_match('/^([0-3][0-9])\/([01][0-9])\/(19|20)([0-9]{2})$/', $date, $matches)) {
                                return array(

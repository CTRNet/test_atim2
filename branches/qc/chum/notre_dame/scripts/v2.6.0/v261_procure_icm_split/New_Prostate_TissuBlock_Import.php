...
// Load data from : Donn�es clinico-pathologiques

$headers = array();
$line_counter = 0;
foreach($XlsReader->sheets[$sheets_nbr['update 2014']]['cells'] as $line => $new_line) {
	$line_counter++;
	if($line_counter == 1) {
		$headers = $new_line;
	} else {
		...
		foreach($block_headers as $column_name => $column_blocks_properties) {			
			...
			foreach($cell_contents as $new_blocks_set) {
				if(strlen($new_blocks_set) && strtolower($new_blocks_set) != 'x') {
					...
					if(preg_match('/^([AB0-9,]*)(\(([A-Za-z�\+]+)\){0,1}){0,1}$/', $new_blocks_set, $matches)) {					
						$notes = '';

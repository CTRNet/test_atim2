...
$excel_file_name = 'nominal_data.xls';
...
$breast_no_labo_control_id = null;
$xls_misc_id_header_to_atim_control_ids = array(
	'RAMQ' => null,
	'# HDM' => null,
	'# HSL' => null,
	'# HND' => null);
foreach(getSelectQueryResult('SELECT * from misc_identifier_controls AS MiscIdControl') as $new_control) {
	switch($new_control['misc_identifier_name']) {
		case 'breast bank no lab':
			$breast_no_labo_control_id = $new_control['id'];
			break;
		case 'ramq nbr':
			$xls_misc_id_header_to_atim_control_ids['RAMQ'] = $new_control['id'];
			break;
		case 'hotel-dieu id nbr':
			$xls_misc_id_header_to_atim_control_ids['# HDM'] = $new_control['id'];
			break;
		case 'notre-dame id nbr':
			$xls_misc_id_header_to_atim_control_ids['# HND'] = $new_control['id'];
			break;
		case 'saint-luc id nbr':
			$xls_misc_id_header_to_atim_control_ids['# HSL'] = $new_control['id'];
			break;
	}
...
$excel_patients_counter = 0;
$excel_patients_found_counter = 0;
$excel_patients_created_counter = 0;
...
while(list($line_number, $excel_line_data) = getNextExcelLineData($excel_file_name, $worksheet_name, 1)) {
	if($line_number > 2) {
		...
		$xls_patient_identifiers_to_create = array();
		foreach($xls_misc_id_header_to_atim_control_ids as $xls_misc_id_header => $misc_identifier_control_id) {
			...
			if($xls_misc_id_value_to_test) {
				...
				if($wrong_format) {
					...
					$xls_patient_identifiers_to_create[$misc_identifier_control_id] = array($xls_misc_id_header, strtoupper($xls_misc_id_value_to_test));
				}
			}
		}
		...
		$xls_patient_definition_for_summary = "Excel (line $line_number) patient <b>$xls_first_name $xls_last_name</b> (".implode(' & ', ($xls_patient_definition_for_summary? $xls_patient_definition_for_summary : array('No idenitfiers into excel'))).")";
		...
		if($sql_conditions) {
			...
			foreach($atim_participant_data as $participant_id => $atim_participant_data_record) $atim_participant_data[$participant_id]['atim_patient_definition_for_summary'] = "ATiM patient <b>".$atim_participant_data_record['first_name'].' '.$atim_participant_data_record['last_name']."</b> (participant_id=".$participant_id.") matching on ".sizeof($atim_participant_data_record['atim_patient_definition_for_summary'])." identifiers (".implode(' & ', $atim_participant_data_record['atim_patient_definition_for_summary']).")";
		}
		if(sizeof($atim_participant_data) > 1) {
			...
			recordErrorAndMessage('Patient', '@@ERROR@@', "More than one ATiM patient matched the Excel data based on identifiers. Excel data won't be migrated. Data to check then to record manually.", "See $xls_patient_definition_for_summary <br>and ".implode(' <br>+ ', $all_atim_patient_defintions));			
			$atim_participant_data = array();
		} else if($atim_participant_data) {
			...
			if($diff_on_names == 1) {
				...
				if(sizeof($atim_participant_data['identifiers']) == 1) {
					if($atim_participant_data['date_of_birth'] == $xls_date_of_birth) {
						...
						recordErrorAndMessage('Patient', '@@ERROR@@', "Patient matched based on one identifier and differences exist on both names plus date of birth. Excel data won't be migrated. Data to check then to record manually.", "See $xls_patient_definition_for_summary <br>and ".$atim_participant_data['atim_patient_definition_for_summary']);
						$atim_participant_data = array();
					}
				} else {
			...
			if($atim_participant_data) {
				$excel_patients_found_counter++;
				//Check profile data are similar and update missing data
				...
				if($xls_date_of_birth) {
					if(!$atim_participant_data['date_of_birth']) {
						...
						recordErrorAndMessage('Patient', '@@WARNING@@', "Date of birth are different", "ATiM : ".$atim_participant_data['date_of_birth'].'('.$atim_participant_data['date_of_birth_accuracy'].") != Xls : $xls_date_of_birth($xls_date_of_birth_accuracy). See $xls_patient_definition_for_summary and ".$atim_participant_data['atim_patient_definition_for_summary']);
					}
				}
				if($xls_date_of_death) {
					if(!$atim_participant_data['date_of_death']) {
						...
						if($atim_participant_data['date_of_death'] != $xls_date_of_death || $atim_participant_data['date_of_death_accuracy'] != $xls_date_of_death_accuracy) {
							recordErrorAndMessage('Patient', '@@WARNING@@', "Date of death are different", "ATiM : ".$atim_participant_data['date_of_death'].'('.$atim_participant_data['date_of_death_accuracy'].") != Xls : $xls_date_of_death($xls_date_of_death_accuracy).See $xls_patient_definition_for_summary and ".$atim_participant_data['atim_patient_definition_for_summary']);
						}
						if($atim_participant_data['vital_status'] != $xls_vital_status) {
							recordErrorAndMessage('Patient', '@@WARNING@@', "Vital status is different", "ATiM : ".$atim_participant_data['vital_status']." != Xls : $xls_vital_status.See $xls_patient_definition_for_summary and ".$atim_participant_data['atim_patient_definition_for_summary']);
						}
					}
				}
				if($data_to_update) {
				...
				foreach($atim_participant_data['identifiers']  as $atim_identifiers_that_matched_an_xls_value) unset($xls_patient_identifiers_to_create[$atim_identifiers_that_matched_an_xls_value['misc_identifier_control_id']]);
				foreach($xls_patient_identifiers_to_create as $misc_identifier_control_id => $new_identifier_to_record) {
					...
					$query = "SELECT participant_id, identifier_value 
						...
						AND (participant_id = ".$atim_participant_data['participant_id']." OR identifier_value LIKE '".$new_identifier_value_to_record."')";
					$matching_identifiers = getSelectQueryResult($query);					
					if(sizeof($matching_identifiers) > 1) {
						recordErrorAndMessage('Patient', '@@ERROR@@', "ATiM Patient $xls_misc_id_header is different than the excel value. Please check data then add correction if required.", "See value '$new_identifier_value_to_record' for $xls_patient_definition_for_summary <br>and ".$atim_participant_data['atim_patient_definition_for_summary']);
					...
					} else if($matching_identifiers && $matching_identifiers[0]['participant_id'] == $atim_participant_data['participant_id']) {
						recordErrorAndMessage('Patient', '@@ERROR@@', "ATiM Patient $xls_misc_id_header is different than the excel value. Please check data then add correction if required", "See value '$new_identifier_value_to_record' for $xls_patient_definition_for_summary <br>and ".$atim_participant_data['atim_patient_definition_for_summary']);
					} else if($matching_identifiers && $matching_identifiers[0]['participant_id'] != $atim_participant_data['participant_id']) {
						recordErrorAndMessage('Patient', '@@ERROR@@', "Excel identifier $xls_misc_id_header is still recorded for another patient into ATiM. Please check data then add correction if required.", "See value '$new_identifier_value_to_record' for $xls_patient_definition_for_summary and ".$atim_participant_data['atim_patient_definition_for_summary']);
					} else {
						if($matching_identifiers) die('ERR23489273982379');
						$misc_identifiers_data = array(
							'misc_identifiers' => array(
								'participant_id' => $atim_participant_data['participant_id'],
			...
			//No ATiM patient found based on identifier values
			//A-Try to find patient based on names (just for information)
			$atim_participants_matching_on_names = getSelectQueryResult("SELECT participants.id AS participant_id, first_name, last_name, misc_identifier_name, identifier_value, misc_identifier_control_id
				FROM participants 
 			...
 			if($atim_participants_matching_on_names) {
 				...
 				foreach($atim_participants_matching_on_names as $new_patient) {
 					$patient_key = 'ATiM patient <b>'.$new_patient['first_name'].' '.$new_patient['last_name'].'</b> (id = '.$new_patient['participant_id'].')';
				...
				recordErrorAndMessage('Patient', '@@MESSAGE@@', "Patient did not match based on identifier but ATiM patient matched based on names. A new patient will be created anyway but please validate.", "See $xls_patient_definition_for_summary <br>and ATiM Patient(s) $atim_patients_with_same_names.");
 			}
		...
		if($atim_participant_data) {		
			...
			if(strlen($excel_line_data['Mutation']) || strlen($excel_line_data['Exon']) || $test_date) {
				$event_data = array(
					...
					$atim_controls['event_controls']['genetic test']['detail_tablename'] => array(
						'test' => 'BRCA 1-2',
						'result' => ($excel_line_data['Mutation'])? $excel_line_data['Mutation'] : 'None',
			...
			foreach($fields as $xls_field => $qc_nd_cancer) {
				$qc_nd_cancer_detail = '';
				$records_to_create = str_replace(array('oui','non'), array('1','0'), $excel_line_data[$xls_field]);
				if($records_to_create) {
						...
						case 'other cancer':
							foreach(explode("\n", $records_to_create) as $new_fam_histo) {
								if(preg_match('/^(.*)[\ ]*:[\ ]*([0-9]+)$/', $new_fam_histo, $matches)) {
									...
									recordErrorAndMessage('Family history', '@@ERROR@@', "Wrong other cancer format", $excel_line_data[$xls_field].". Data won't be recorded. See $xls_patient_definition_for_summary and $atim_patient_definition_for_summary");
								}
							}
							$records_to_create = 0;
							break;
					...
					$records_to_create = validateAndGetInteger($records_to_create, "Family history", "$worksheet_name::$excel_field:", "Family history record won't be created. See Line:$line_number");
					if($records_to_create) {
			...
			list($xls_date, $xls_date_accuracy) = validateAndGetDateAndAccuracy($excel_line_data[$excel_field], 'Nail Collection', "$worksheet_name::$excel_field:", "See Line:$line_number");
			if($xls_date) {
				//Collection
				...
				$sample_data = array(
					'sample_masters' => array(
						...
						'qc_nd_sample_label' => "N - $breast_no_labo",
				...
				$aliquot_data = array(
					'aliquot_masters' => array(
						...
						"aliquot_label" => "N - $breast_no_labo",
						"aliquot_control_id" => $atim_controls['aliquot_controls']['nail-tube']['id'],
						"collection_id" => $collection_id,
						"sample_master_id" => $sample_master_id,
						'in_stock' => 'yes - available',
						'use_counter' => '0',
						'notes' => 'Created by the Dr Ghadirian data import process.'),
					$atim_controls['aliquot_controls']['nail-tube']['detail_tablename'] => array());
				$aliquot_master_id = customInsertRecord($aliquot_data);
				recordErrorAndMessage('Data Creation & Update', '@@MESSAGE@@', "Created nail 'collection' and aliquot", "Aliquot = '"."N - $breast_no_labo"."'. See $xls_patient_definition_for_summary and $atim_patient_definition_for_summary");
			}
			$participant_id_to_collection_data[$participant_id] = array($breast_no_labo, $collection_id);
			...
			// FRSQ
			$shipped = false;
			if(strlen($excel_line_data['FRSQ']) && !in_array($excel_line_data['FRSQ'], array('N/A'))) {
				if(in_array($excel_line_data['FRSQ'], array('Oui', 'oui', 'déjà fait'))) {
					$shipped = true;
				} else {
					recordErrorAndMessage('FRSQ', '@@ERROR@@', "No item shipped to FRSQ but a FRSQ value is set with a wrong value format. No data will be recorded. To record manually if required.", "Value = '".$excel_line_data[$excel_field]."'.See $xls_patient_definition_for_summary and $atim_patient_definition_for_summary");
				}					
			}
...
foreach($all_palb2_data as $palb2_key => $palb2_data) {
	if(!$palb2_data['used']) {
		...
		$participant_id_to_collection_data[$participant_id] = array($misc_identifiers_data['misc_identifiers']['identifier_value'], null);
...
foreach($shipped_items as $short_order_title => $shipped_items_set) {
	foreach($shipped_items_set as $new_shipped_item) {
		...
		list($breast_no_labo, $collection_id) = $participant_id_to_collection_data[$participant_id];
		if(!$collection_id) {
			$collection_data = array(
				'collections' => array(
					'acquisition_label' => 'Dr Darviz Ghadirian',
...
function truncate() {
	...
	$truncate_date = '2016-06-22 01:00:00';

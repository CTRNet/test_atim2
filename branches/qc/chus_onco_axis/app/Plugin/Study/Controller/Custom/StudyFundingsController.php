...
class StudyFundingsControllerCustom extends StudyFundingsController {
	...
	function add( $study_summary_id ) {
       ...
       if ( !empty($this->request->data) ) {
			...
			if($this->StudyFunding->find('count', array('conditions' => array('StudyFunding.study_summary_id' => $study_summary_id, 'StudyFunding.study_sponsor' => $this->request->data['StudyFunding']['study_sponsor'])))) {
				$this->StudyFunding->validationErrors['study_sponsor'][] = 'selected value already exists for the study';
			} else if ( $this->StudyFunding->save($this->request->data) ) {

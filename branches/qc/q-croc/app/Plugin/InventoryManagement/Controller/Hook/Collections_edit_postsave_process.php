<?php
	...
	if($collection_data['Collection']['qcroc_collection_date'] != $new_qcroc_collection_date) {
		...
		foreach($this->AliquotMaster->find('all', array('conditions' => $condtions, 'joins' => $joins, 'recursive' => 0)) as $new_aliquot_to_update) {		
			$time_remained_in_rna_later_days = $this->AliquotMaster->calculateTimeRemainedInRNAlater($new_qcroc_collection_date, $new_aliquot_to_update['SpecimenDetail']['qcroc_collection_time'], $new_aliquot_to_update['AliquotDetail']['date_sample_received']);

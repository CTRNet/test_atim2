...
function loadFamHisto(&$tmp_xls_reader, $sheets_keys, $unmigrated_excel_participant_jgh_nbrs) {
	...
	foreach($tmp_xls_reader->sheets[$sheets_keys[$worksheet_name]]['cells'] as $new_line) {
		...
		if($excel_line_counter == 1) {
			...
			if(!in_array($jgh_nbr, $unmigrated_excel_participant_jgh_nbrs) && isset(Config::$participants[$jgh_nbr])) {
				$participant_id = Config::$participants[$jgh_nbr]['participant_id'];
				if($new_line_data['Antécédents familiaux cancer sein'] != 'Oui') {
					Config::$summary_msg[$summary_msg_title]['@@WARNING@@']["The vlaue for 'Antécédents familiaux cancer sein' is not supported (different than 'Oui'). The data wont be imported into ATiM"][] = "See value '".$new_line_data['Antécédents familiaux cancer sein']."' for JGH# $jgh_nbr line $excel_line_counter";
				} else {
...
function loadReproHisto(&$tmp_xls_reader, $sheets_keys, $unmigrated_excel_participant_jgh_nbrs) {
	$worksheet_name = 'GPA';
	if(!isset($tmp_xls_reader->sheets[$sheets_keys[$worksheet_name]])) die('ERR 838838382');
	...
	foreach($tmp_xls_reader->sheets[$sheets_keys[$worksheet_name]]['cells'] as $new_line) {
		...
		if($excel_line_counter == 1) {
			...
			$jgh_nbr = $new_line_data['No de dossier'];
			if(!in_array($jgh_nbr, $unmigrated_excel_participant_jgh_nbrs) && isset(Config::$participants[$jgh_nbr])) {
				if($new_line_data['Ménopause'] != 'N/A: HOMME') {				
					$participant_id = Config::$participants[$jgh_nbr]['participant_id'];
					//GPA
					$jgh_nbr = $new_line_data['No de dossier'];
					$repro_data = array();

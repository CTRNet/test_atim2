...
function loadReceptors(&$tmp_xls_reader, $sheets_keys, $unmigrated_excel_participant_jgh_nbrs) {
	...
	Config::$tmp_receptor_summary_msg_title = $worksheet_name;
		...
		if($excel_line_counter == 1) {
			...
			if(!in_array($jgh_nbr, $unmigrated_excel_participant_jgh_nbrs)) {
				...
				if(!$participant_id) {
					...
					if(!in_array($new_line_data['Type traitement'], array('CHIR','BIOP'))) {
						if($new_line_data['Type traitement'] == 'REVISION') {
							if($new_line_data['Traitement'] == 'Relecture de lame') {
								...
								$relecture_data = array();
								foreach($rec_data as $key => $value) {
									if(strlen($value) && $value != '-99') $relecture_data[] = "'$key' = [$value]";
								}
								if($relecture_data) {
									Config::$summary_msg[$summary_msg_title]['@@WARNING@@']["'Revision de lame' data not imported into ATiM. Add data manually if required."][] = "See <b>JGH# ".$new_line_data['No de dossier']."</b> line $excel_line_counter  and revision data: ".implode(' & ', $relecture_data);
								}
							} else {
								Config::$summary_msg[$summary_msg_title]['@@WARNING@@']["'Type traitement' not equal to 'CHIR' or 'BIOP'. No data of lines matching these types will be imported into ATiM. You cane delete all lines with these types into the worksheet $worksheet_name"][$new_line_data['Type traitement'].$new_line_data['Traitement']] = "See type '".$new_line_data['Type traitement']." - ".$new_line_data['Traitement'].".";
							}
						} else {
							Config::$summary_msg[$summary_msg_title]['@@WARNING@@']["'Type traitement' not equal to 'CHIR' or 'BIOP'. No data of lines matching these types will be imported into ATiM. You cane delete all lines with these types into the worksheet $worksheet_name"][$new_line_data['Type traitement']] = "See type '".$new_line_data['Type traitement'].".";
						}
					} else {

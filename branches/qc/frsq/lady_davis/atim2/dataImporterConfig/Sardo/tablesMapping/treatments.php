...
function addReceptorData(&$treatment_data, &$dx_receptor_data, $start_date, $treatment_type, $treatment, $jgh_nbr, $excel_line_counter, $summary_msg_title) {
	if(isset($dx_receptor_data[$treatment_type][$start_date][$treatment])) {
		if(sizeof($dx_receptor_data[$treatment_type][$start_date][$treatment]) > 1) {
			Config::$summary_msg[$summary_msg_title]['@@WARNING@@']["Duplicated Receptor Data (No data will be imported - probably bilateral dx - data to import manually)"][] = "See $treatment_type on $start_date for JGH# $jgh_nbr line $excel_line_counter (recpetor worksheet)";
		} else {
			...
			if(strlen($processed_rec_data['tumor_size_mm_x'])) {
				...
				if($treatment_type != 'CHIR') die('ERR 237 63287 632734');
			}
			//FISH
			...
			switch($treatment_receptor_data['HER2NEU FISH']) {
				...
				case 'N/S':
					break;
				default:
					Config::$summary_msg[$summary_msg_title]['@@WARNING@@']["FISH - Value not imported"][] = "Value ".$treatment_receptor_data['HER2NEU FISH'].". See JGH# $jgh_nbr line $excel_line_counter (recpetor worksheet)";
			}
			//Ki67
			...
			if($processed_rec_data['ki67_performed'] == 'y' && !strlen($processed_rec_data['ki67_pct'])) {
				Config::$summary_msg[$summary_msg_title]['@@WARNING@@']["Ki67 - No %"][] = "Ki67 has been performed but no % will be recorded. See JGH# $jgh_nbr line $excel_line_counter (recpetor worksheet)";
			}
			//ER			
			...
			switch($treatment_receptor_data['Récepteurs oestrogènes']) {
				...
				case 'N/S':
					break;
				default:
					Config::$summary_msg[$summary_msg_title]['@@WARNING@@']["Récepteurs oestrogènes - Value not imported"][] = "Value ".$treatment_receptor_data['Récepteurs oestrogènes'].". See JGH# $jgh_nbr line $excel_line_counter (recpetor worksheet)";
			}
			...
			switch($treatment_receptor_data['Récepteurs progestatifs']) {
				...
				case 'N/S':
					break;
				default:
					Config::$summary_msg[$summary_msg_title]['@@WARNING@@']["Récepteurs progestatifs - Value not imported"][] = "Value ".$treatment_receptor_data['Récepteurs progestatifs'].". See JGH# $jgh_nbr line $excel_line_counter (recpetor worksheet)";
			}
			...
			while($i<count($antibodies) && empty($processed_rec_data['her2_receptor_ccl'])) {
				...
				switch($treatment_receptor_data[$antibody]) {
					...
					case 'N/S':
						break;
					default:
						Config::$summary_msg[$summary_msg_title]['@@WARNING@@']["$antibody - Value not imported"][] = "Value ".$treatment_receptor_data[$antibody].". See JGH# $jgh_nbr line $excel_line_counter (recpetor worksheet)";
				}
			...
			switch($treatment_receptor_data['Maladie résiduelle']) {
				...
				case '> 1 cm, <= 2 cm':
				case 'Microscopique':
					Config::$summary_msg[$summary_msg_title]['@@WARNING@@']["Positive Residual Disease Value To approvee"][] = "Value ".$treatment_receptor_data['Maladie résiduelle']." has been considered as 'Yes'. See JGH# $jgh_nbr line $excel_line_counter (recpetor worksheet)";
					$processed_rec_data['residual_disease'] = 'y';
					break;
				case '':
				case 'N/S':

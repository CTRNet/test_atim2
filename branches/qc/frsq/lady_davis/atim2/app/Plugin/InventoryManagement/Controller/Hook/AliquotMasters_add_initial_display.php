<?php
	...
	foreach($this->request->data as $new_data_set){
		...
		if(in_array($new_data_set['parent']['ViewSample']['sample_type'], array('pbmc','serum','plasma')) && $aliquot_control['AliquotControl']['aliquot_type'] == 'tube') {
			...
			$existing_blood_derivative_aliquot = $this->AliquotMaster->find('first', array('conditions' => array('AliquotMaster.collection_id' => $new_data_set['parent']['ViewSample']['collection_id'], 'ViewAliquot.sample_type' => array('pbmc','serum','plasma')), 'recursive' => '0'));

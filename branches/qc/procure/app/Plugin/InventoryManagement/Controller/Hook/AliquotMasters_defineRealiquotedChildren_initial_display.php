...
foreach ($this->request->data as $tmp_procure_key_1 => $tmp_procure_new_data_set) {
    foreach ($tmp_procure_new_data_set['children'] as $tmp_procure_key_2 => $tmp_procure_new_children_aliquot_record) {
        if ($tmp_procure_new_children_aliquot_record['SampleMaster']['procure_created_by_bank'] == 's' && $tmp_procure_new_children_aliquot_record['AliquotMaster']['procure_created_by_bank'] != Configure::read('procure_bank_id')) {

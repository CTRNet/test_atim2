...
foreach ($this->request->data as &$new_sample_record) {
    ...
    if ($new_sample_record['parent']['ViewSample']['procure_created_by_bank'] == 's') {
        $tmp_aliquots_count = $this->AliquotMaster->find('count', array(
            'conditions' => array(
                'AliquotMaster.sample_master_id' => $new_sample_record['parent']['ViewSample']['sample_master_id']
            )
        ));
    ...
    $sample_data = $this->SampleMaster->find('first', array(
        'conditions' => array(
            'SampleMaster.id' => $new_sample_record['parent']['ViewSample']['sample_master_id']
        ),
        'recursive' => '0'
    ));
    ...
    if (in_array($new_sample_record['parent']['ViewSample']['sample_type'], array(
        'serum',
        'pbmc',
        'buffy coat',
        'plasma'
        ...
        $aliquot_control_ids = $this->AliquotControl->find('list', array(
            'conditions' => array(
                'sample_control_id' => $sample_control_ids
            )
        ));
        $collection_blood_derivative_aliquots = $this->AliquotMaster->find('first', array(
            'conditions' => array(
                'AliquotMaster.collection_id' => $new_sample_record['parent']['ViewSample']['collection_id'],
                'AliquotMaster.aliquot_control_id' => $aliquot_control_ids
            ),
            'order' => array(
    ...
    $last_stored_aliquot = $this->AliquotMaster->find('first', array(
        'conditions' => array(
    ...
    if ($last_stored_aliquot)
        $default_storage = $this->StorageMaster->getStorageLabelAndCodeForDisplay(array(
            'StorageMaster' => $last_stored_aliquot['StorageMaster']
        ));
    ...
    switch ($new_sample_record['parent']['ViewSample']['sample_type'] . '-' . $aliquot_control['AliquotControl']['aliquot_type']) {
        ...
        case 'pbmc-tube':
            ...
            $tmp_default_aliquot_data['AliquotDetail.procure_date_at_minus_80_accuracy'] = str_replace(array(
                '',
                'i',
                'h'
            ), array(
        ...
        case 'rna-tube':
            ...
            if (is_null($template_init_id) && sizeof($new_sample_record['children']) == 1)
                $new_sample_record['children'][1] = $new_sample_record['children'][0];
            break;
        // --------------------------------------------------------------------------------
        // DNA
        // --------------------------------------------------------------------------------
    ...
    if ($set_default_value) {
        ...
        if (strlen($default_volume))
            $tmp_default_aliquot_data['AliquotMaster.initial_volume'] = $default_volume;
        if ($default_concentration_unit)
            $tmp_default_aliquot_data['AliquotDetail.concentration_unit'] = $default_concentration_unit;
        if ($default_hemolysis_signs)
            $tmp_default_aliquot_data['AliquotDetail.hemolysis_signs'] = $default_hemolysis_signs;
        if ($default_storage)
            $tmp_default_aliquot_data['FunctionManagement.recorded_storage_selection_label'] = $default_storage;
...
if ($data_set_nbr_in_error) {
    $this->flash(__('no aliquot can be created from sample created by system/script to migrate data from the processing site with no aliquot') . ' ' . str_replace('%s', '[' . implode('] ,[', $data_set_nbr_in_error) . ']', __('see # %s')), $url_to_cancel, 5);
    return;
}

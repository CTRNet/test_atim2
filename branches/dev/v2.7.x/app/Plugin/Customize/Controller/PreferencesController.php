...
class PreferencesController extends CustomizeAppController {
	...
	function index() {
		...
		$config_results = $this->Config->getConfig($_SESSION['Auth']['User']['group_id'],
	...
	function edit() {
		...
		if (!empty($this->request->data)) {
			$this->Config->preSave($config_results, $this->request->data, $_SESSION['Auth']['User']['group_id'],
				$_SESSION['Auth']['User']['id']);
			...
			$this->request->data = $this->User->find('first',
				array('conditions' => array('User.id' => $_SESSION['Auth']['User']['id'])));
			$this->request->data['Config'] = $config_results['Config'];
		}
	}
}

...
class StorageCoordinatesController extends StorageLayoutAppController {
	...
	function listAll($storage_master_id) {
		...
		$this->request->data = $this->paginate($this->StorageCoordinate,
			array('StorageCoordinate.storage_master_id' => $storage_master_id));
	...
	function add($storage_master_id) {
		...
		if (!empty($this->request->data)) {
			...
			if ($this->StorageCoordinate->isDuplicatedValue($storage_master_id,
				$this->request->data['StorageCoordinate']['coordinate_value'])
			) {
				$submitted_data_validates = false;
			}

			if ($this->StorageCoordinate->isDuplicatedOrder($storage_master_id,
				$this->request->data['StorageCoordinate']['order'])
			) {
				$submitted_data_validates = false;
			}
			...
			if ($submitted_data_validates) {
				...
				if ($this->StorageCoordinate->save($this->request->data['StorageCoordinate'])) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/StorageLayout/StorageCoordinates/listAll/' . $storage_master_id);
				}
			}
		}
	}
	...
	function delete($storage_master_id, $storage_coordinate_id) {
		...
		$storage_coordinate_data = $this->StorageCoordinate->find('first', array(
			'conditions' => array(
				'StorageCoordinate.id' => $storage_coordinate_id,

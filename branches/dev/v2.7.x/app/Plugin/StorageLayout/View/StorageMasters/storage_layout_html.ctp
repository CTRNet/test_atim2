    ...
    <div style="display: table-cell; vertical-align: top;">
        <ul style='margin-right: 10px' ;>
            <li><span class="button RecycleStorage" style='width: 80%;'><span
                            class="ui-icon ui-icon-refresh"></span><?php echo(__("unclassify all storage's items")); ?></span>
            </li>
            <li><span class="button TrashStorage" style='width: 80%;'><span
                            class="ui-icon ui-icon-close"></span><?php echo(__("remove all storage's items")); ?></span>
            </li>
        </ul>
    </div>
				...
				if ($data['parent']['StorageControl']['coord_x_type'] == 'list') {
					...
					if ((strlen($x_size) == 0 || strlen($y_size) == 0) && ($data['parent']['StorageControl']['display_x_size'] > 0 || $data['parent']['StorageControl']['display_y_size'] > 0)) {
						...
						if (max(1, $data['parent']['StorageControl']['coord_x_size']) * max(1,
							...
							echo("Real storage cells: " . (($data['parent']['StorageControl']['coord_x_size']) * max(1,
										$data['parent']['StorageControl']['coord_y_size'])) . "<br/>");
					...
					$j = null;
					while (axisLoopCondition($j, $data['parent']['StorageControl']['reverse_y_numbering'],
						$use_height)) {
						echo("<tr>");
						if (!$twoAxis) {
						...
						$i = null;
						while (axisLoopCondition($i, $data['parent']['StorageControl']['reverse_x_numbering'],
							$use_width)) {
							if ($twoAxis) {
                ...
                <div style="width: 100%; border:solid 1px; display: inline-block; vertical-align: top;">
                    <h4 class="ui-widget-header" style="white-space: nowrap;">
                        <span class="ui-icon ui-icon-refresh"
                              style="float: left;"></span><?php echo(__("unclassified")); ?>
                        <span class='help storage'>
                    ...
                    <div class="droppable" style="padding-top: 5px; border: solid 1px transparent;">
                        <ul id="s_<?php echo $atim_menu_variables['StorageMaster.id']; ?>_c_u_u" class="unclassified"
                            style="margin-right: 5px;"></ul>
                        <span class="button TrashUnclassified"><span class="ui-icon ui-icon-close"
                                                                     style="float: left;"></span><?php echo(__("remove all unclassified")); ?></span>
                    </div>
                </div>
            </li>
            <li class='trash_n_unclass'>
                <div style="width: 100%; border:solid 1px; display: inline-block; vertical-align: top;">
                    ...
                    <div class="droppable" style="padding-top: 5px; border: solid 1px transparent;">
                        ...
                        <span class="button RecycleTrash"><span class="ui-icon ui-icon-refresh"
                                                                style="float: left;"></span><?php echo(__("unclassify all removed")); ?></span>
                    </div>
                </div>
            </li>
        </ul>

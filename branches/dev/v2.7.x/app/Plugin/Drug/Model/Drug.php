...
class Drug extends DrugAppModel {
	...
	function allowDeletion($drug_id) {
		...
		$returned_nbr = $TreatmentExtendMaster->find('count', array(
			'conditions' => array('TreatmentExtendDetail.drug_id' => $drug_id),
			'joins' => $joins,
			'recursive' => '1'
		));
		if ($returned_nbr > 0) {
			return array(
				...
				'msg' => 'drug is defined as a component of at least one participant chemotherapy'
			);
		}
		...
		$returned_nbr = $ProtocolExtendMaster->find('count', array(
			'conditions' => array('ProtocolExtendDetail.drug_id' => $drug_id),
			'joins' => $joins,
			'recursive' => '1'
		));
		if ($returned_nbr > 0) {

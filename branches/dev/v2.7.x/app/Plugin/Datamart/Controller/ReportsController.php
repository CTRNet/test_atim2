...
class ReportsController extends DatamartAppController {
	...
	function compareToBatchSetOrNode(
		$type_of_object_to_compare,
		$batch_set_or_node_id_to_compare,
		$csv_creation = false,
		$previous_current_node_id = null
	) {
		...
		switch ($type_of_object_to_compare) {
			case 'batchset':
				...
				if (!AppController::checkLinkPermission($selected_batchset['DatamartStructure']['index_link'])) {
					$this->atimFlash(__("You are not authorized to access that location."),
		...
		if ($previous_current_node_id) {
			...
			if (empty($this->request->data)) {
				...
				if (array_key_exists('Config', $this->request->data) && $csv_creation) {
					// Export data in csv
					$config = array_merge($this->request->data['Config'],
						(array_key_exists(0, $this->request->data) ? $this->request->data[0] : array()));
		...
		if ($datamart_structure['control_master_model']) {
			if (isset($this->request->data[$datamart_structure['model']])) {
				$model_instance = AppModel::getInstance($datamart_structure['plugin'], $datamart_structure['model'],
					true);
				$model = $datamart_structure['model'];
				$lookup_key_name = $model_instance->primaryKey;
			} else {
				$model_instance = AppModel::getInstance($datamart_structure['plugin'],
					$datamart_structure['control_master_model'], true);
		...
		if ($csv_creation) {
			...
			$this->request->data[$model][$lookup_key_name] = explode(",",
				$_SESSION['compareToBatchSetOrNode']['selected_elements_ids']);
			...
			if ($tmp_node_of_selected_elements && ($previous_current_node_id || $this->request->data[$model][$lookup_key_name] == 'all')) {
				...
				$this->request->data[$model][$lookup_key_name] = explode(",",
					$tmp_node_of_selected_elements['BrowsingResult']['id_csv']);
			} else {
				if (empty($this->request->data)) {
					// Sort data
					$this->request->data[$model][$lookup_key_name] = explode(",",
		...
		foreach ($all_studied_elements as $new_studied_element) {
			if ($csv_creation && !in_array($new_studied_element[$model][$lookup_key_name],
					$studied_element_ids_to_export)
			) {
				continue;
			}
			...
			if (in_array($new_studied_element[$model][$lookup_key_name], $elements_ids_just_in_selected_object)) {
				$new_studied_element['Generated']['batchset_and_node_elements_distribution_description'] = str_replace('%s_2',
					$selected_object_title, __('data of selected %s_2 only (2)'));
				$sorted_all_studied_elements[3][] = $new_studied_element;
			} else {
				if (in_array($new_studied_element[$model][$lookup_key_name],
					$elements_ids_just_in_previously_disp_object)) {
					$new_studied_element['Generated']['batchset_and_node_elements_distribution_description'] = str_replace('%s_1',
						$previously_displayed_object_title, __("data of previously displayed %s_1 only (1)"));
					...
					$new_studied_element['Generated']['batchset_and_node_elements_distribution_description'] = str_replace(array(
						'%s_1',
						'%s_2'
					), array($previously_displayed_object_title, $selected_object_title),
						__('data both in previously displayed %s_1 and selected %s_2 (1 & 2)'));
					$sorted_all_studied_elements[1][] = $new_studied_element;
				}
			}
		}
		$diff_results_data = array_merge($sorted_all_studied_elements[1], $sorted_all_studied_elements[2],
			$sorted_all_studied_elements[3]);
		...
		if ($control_master_ids && (sizeof($control_master_ids) == 1)) {
			...
			$alternate_info = Browser::getAlternateStructureInfo($datamart_structure['plugin'],
				$model_instance->getControlName(), $control_master_id);
			...
			$atim_structure_data = $this->Structure->find('first', array(
				'conditions' => array('Structure.id' => $datamart_structure['structure_id']),
				'recursive' => '-1'
			));
			$structure_alias = $atim_structure_data['structure']['Structure']['alias'];
		}
		$this->set('atim_structure_for_results',
			$this->Structures->get('form', 'batchset_and_node_elements_distribution,' . $structure_alias));
		$this->set('datamart_structure_id', $datamart_structure['id']);
		...
		$this->set('header_1',
			str_replace('%s_1', $previously_displayed_object_title, __('data of previously displayed %s_1 (1)')));
		...
		if ($csv_creation) {
			...
			$datamart_structure_actions = $this->DatamartStructure->getDropdownOptions($datamart_structure['plugin'],
				$datamart_structure['model'], $model_instance->primaryKey, null, $datamart_structure['model'],
			...
			$sort_args = array_key_exists('sort',
				$this->passedArgs) ? 'sort:' . $this->passedArgs['sort'] . '/direction:' . $this->passedArgs['direction'] : '';
	...
	function index() {
		...
		$this->request->data = $this->paginate($this->Report,
			array('Report.flag_active' => '1', 'Report.limit_access_from_datamart_structrue_function' => '0'));
	...
	function manageReport($report_id, $csv_creation = false) {
		...
		$report = $this->Report->find('first',
			array('conditions' => array('Report.id' => $report_id, 'Report.flag_active' => '1')));
		...
		if ($report['Report']['limit_access_from_datamart_structrue_function'] && empty($this->request->data) && (!$csv_creation) && !array_key_exists('sort',
				$this->passedArgs)
		) {
			$this->atimFlash(__('the selected report can only be launched from a batchset or a databrowser node'),
				"/Datamart/Reports/index", 5);

		} else {
			if (empty($this->request->data) && (!empty($report['Report']['form_alias_for_search'])) && (!$csv_creation) && !array_key_exists('sort',
					$this->passedArgs)
			) {
				...
				if ($report['Report']['form_type_for_results'] == 'index' && $report['Report']['associated_datamart_structure_id']) {
					...
					$linked_datamart_structure = $this->DatamartStructure->find('first',
						array('conditions' => array("DatamartStructure.id" => $report['Report']['associated_datamart_structure_id'])));
					if (empty($linked_datamart_structure)) {
						$this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__,
							null, true);
					}
					$LinkedModel = AppModel::getInstance($linked_datamart_structure['DatamartStructure']['plugin'],
						$linked_datamart_structure['DatamartStructure']['model'], true);
					...
					if (array_key_exists('sort', $this->passedArgs)) {
									...
									if (preg_match('/^(.+)_with_file_upload$/', $field, $matches)) {
										...
										if (strlen($parameters['tmp_name'])) {
											if (!preg_match('/((\.txt)|(\.csv))$/', $parameters['name'])) {
												...
												if ($handle) {
													while (($csv_data = fgetcsv($handle, 1000, CSV_SEPARATOR,
															'"')) !== false) {
						...
						if (array_key_exists('node', $criteria_to_build_report)) {
							$browsing_result = $this->BrowsingResult->find('first',
								array('conditions' => array('BrowsingResult.id' => $criteria_to_build_report['node']['id'])));
							...
							if ($criteria_to_build_report[$model][$lookup_key_name] == 'all') {
								$criteria_to_build_report[$model][$lookup_key_name] = explode(",",
				...
				$data_returned_by_fct = call_user_func_array(array($this, $report['Report']['function']),
					array($criteria_to_build_report));
				if (empty($data_returned_by_fct) || (!array_key_exists('header',
						$data_returned_by_fct)) || (!array_key_exists('data',
						$data_returned_by_fct)) || (!array_key_exists('columns_names',
						$data_returned_by_fct)) || (!array_key_exists('error_msg', $data_returned_by_fct))
				) {
					...
					if (!empty($data_returned_by_fct['error_msg'])) {
						...
						$this->set('display_new_search',
							(empty($report['Report']['form_alias_for_search']) || $report['Report']['limit_access_from_datamart_structrue_function']) ? false : true);
						...
						if (sizeof($data_returned_by_fct['data']) > Configure::read('databrowser_and_report_results_display_limit') && !$csv_creation) {
							...
							$this->request->data = AppModel::sortWithUrl($data_returned_by_fct['data'],
								$criteria_to_sort_report);
							$this->Structures->set($report['Report']['form_alias_for_results'],
								'result_form_structure');
							...
							if ($csv_creation) {
								...
								if ($linked_datamart_structure) {
									...
									$this->set('linked_datamart_structure_model_name',
									...
									if ($linked_datamart_structure['DatamartStructure']['index_link']) {
										$this->set('linked_datamart_structure_links',
											$linked_datamart_structure['DatamartStructure']['index_link']);
									}
									$linked_datamart_structure_actions = $this->DatamartStructure->getDropdownOptions($linked_datamart_structure['DatamartStructure']['plugin'],
										$linked_datamart_structure['DatamartStructure']['model'],
										$LinkedModel->primaryKey, null, null, null, null, false);
	...
	function bankActiviySummary($parameters) {
		...
		// 1- Build Header
		$start_date_for_display = AppController::getFormatedDateString($parameters[0]['report_date_range_start']['year'],
			$parameters[0]['report_date_range_start']['month'], $parameters[0]['report_date_range_start']['day']);
		$end_date_for_display = AppController::getFormatedDateString($parameters[0]['report_date_range_end']['year'],
			$parameters[0]['report_date_range_end']['month'], $parameters[0]['report_date_range_end']['day']);
		$header = array(
		...
		// 2- Search data
		$start_date_for_sql = AppController::getFormattedDatetimeSQL($parameters[0]['report_date_range_start'],
			'start');
	...
	function sampleAndDerivativeCreationSummary($parameters) {
		...
		// 1- Build Header
		$start_date_for_display = AppController::getFormatedDateString($parameters[0]['report_datetime_range_start']['year'],
			$parameters[0]['report_datetime_range_start']['month'],
			$parameters[0]['report_datetime_range_start']['day']);
		$end_date_for_display = AppController::getFormatedDateString($parameters[0]['report_datetime_range_end']['year'],
			$parameters[0]['report_datetime_range_end']['month'], $parameters[0]['report_datetime_range_end']['day']);
		...
		$start_date_for_sql = AppController::getFormattedDatetimeSQL($parameters[0]['report_datetime_range_start'],
			'start');
	...
	function participantIdentifiersSummary($parameters) {
		...
		// 		}
		$misc_identifiers = $misc_identifier_model->find('all',
			array('conditions' => $conditions, 'order' => array('MiscIdentifier.participant_id ASC')));
		$data = array();
		foreach ($misc_identifiers as $new_ident) {
			...
			$data[$participant_id]['0'][str_replace(array(' ', '-'), array('_', '_'),
	...
	function getAllDerivatives($parameters) {
		...
		$tmp_res_count = $sample_master_model->find('count', array(
			...
			'fields' => array('SampleMaster.*', 'SampleControl.*'),
			'order' => array('SampleMaster.sample_code ASC'),
			'recursive' => '0'
		));
		...
		foreach ($studied_samples as $new_studied_sample) {
			$all_derivatives_samples = $this->getChildrenSamples($view_sample_model,
				array($new_studied_sample['SampleMaster']['id']));
			if ($all_derivatives_samples) {
				foreach ($all_derivatives_samples as $new_derivative_sample) {
					if (array_key_exists('SelectedItemsForCsv',
							$parameters) && !in_array($new_derivative_sample['ViewSample']['sample_master_id'],
							$parameters['SelectedItemsForCsv']['ViewSample']['sample_master_id'])
					) {
						continue;
					}
	...
	function getChildrenSamples($view_sample_model, $parent_sample_ids = array()) {
		...
		if (!empty($parent_sample_ids)) {
			...
			$children_samples = $view_sample_model->find('all', array(
				'conditions' => array('ViewSample.parent_id' => $parent_sample_ids),
				'fields' => array('ViewSample.*, DerivativeDetail.*'),
				'order' => array('ViewSample.sample_code ASC'),
				'recursive' => '0'
			));
	...
	function getAllSpecimens($parameters) {
		...
		foreach ($studied_samples as $new_studied_sample) {
			$initial_specimen = isset($tmp_initial_specimens[$new_studied_sample['SampleMaster']['initial_specimen_sample_id']]) ? $tmp_initial_specimens[$new_studied_sample['SampleMaster']['initial_specimen_sample_id']] : $view_sample_model->find('first',
				array(
					'conditions' => array('ViewSample.sample_master_id' => $new_studied_sample['SampleMaster']['initial_specimen_sample_id']),
					'fields' => array('ViewSample.*, SpecimenDetail.*'),
					'order' => array('ViewSample.sample_code ASC'),
					'recursive' => '0'
				));
			...
			if ($initial_specimen) {
				if (!(array_key_exists('SelectedItemsForCsv',
						$parameters) && !in_array($initial_specimen['ViewSample']['sample_master_id'],
						$parameters['SelectedItemsForCsv']['ViewSample']['sample_master_id']))
				) {
	...
	function getAllChildrenStorage($parameters) {
		...
		$tmp_res_count = $storage_master_model->find('count', array(
			'conditions' => $conditions,
			'fields' => array('StorageMaster.*'),
			'order' => array('StorageMaster.selection_label ASC'),
			'recursive' => '-1'
		));
		// *** NOTE: Has to control the number of record because the next report code lines can be really time and memory consuming ***
		if ($tmp_res_count > Configure::read('databrowser_and_report_results_display_limit')) {
		...
		foreach ($studied_storages as $new_studied_storage) {
			$children_storage_masters = $storage_master_model->children($new_studied_storage['StorageMaster']['id'],
				false, array('StorageMaster.*'));
			if ($children_storage_masters) {
				foreach ($children_storage_masters as $new_child) {
					if (array_key_exists('SelectedItemsForCsv',
							$parameters) && !in_array($new_child['StorageMaster']['id'],
							$parameters['SelectedItemsForCsv']['ViewStorageMaster']['id'])
					) {
						continue;
					}
					$res[] = array_merge($new_studied_storage,
						array('ViewStorageMaster' => $new_child['StorageMaster']));
				}
			}
		}
		return array(
	...
	function getAllRelatedDiagnosis($parameters) {
		...
		$tmp_res_count = $diagnosis_master_model->find('count',
			array('conditions' => $conditions, 'fields' => array('DISTINCT primary_id'), 'recursive' => '0'));
		// *** NOTE: Has to control the number of record because the next report code lines can be really time and memory consuming ***
		if ($tmp_res_count > Configure::read('databrowser_and_report_results_display_limit')) {
			return array(
				'header' => null,
		...
		$tmp_primary_ids = $diagnosis_master_model->find('all',
			array('conditions' => $conditions, 'fields' => array('DISTINCT primary_id'), 'recursive' => '0'));
		$primary_ids = array();
		...
		$res = $diagnosis_master_model->find('all', array(
			...
			'fields' => array('Participant.*', 'DiagnosisMaster.*', 'DiagnosisControl.*'),
			'order' => array(
	...
	function countNumberOfElementsPerParticipants($parameters) {
		...
		$models_list = array(
			'MiscIdentifier' => array('id', array('misc_identifiers'), 'misc identifiers'),
			...
			'SpecimenReviewMaster' => array(
				'id',
				array('specimen_review_masters', 'sample_masters', 'collections'),
				'specimen review'
			),
			'ViewAliquotUse' => array(
				'id',
				array('view_aliquot_uses', 'aliquot_masters', 'collections'),
				'aliquot uses and events'
			),
			'AliquotReviewMaster' => array(
				'id',
				array('aliquot_review_masters', 'aliquot_masters', 'sample_masters', 'collections'),
				'aliquot review'
			)
		);
		$model_name = null;
		...
		if ($model_name) {
			...
			$query = "SELECT count(*) AS nbr_of_elements, Participant.*
				FROM participants AS Participant " . implode(' ',
					$joins) . " WHERE Participant.deleted <> 1 AND " . implode(' AND ', $delete_constraints) . " AND ModelLevel$model_levels.id IN ($ids)

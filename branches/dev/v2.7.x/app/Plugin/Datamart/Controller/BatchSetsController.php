...
class BatchSetsController extends DatamartAppController {
	...
	function index($type_of_list = 'user') {
		...
		$tmp_batch = $this->BatchSet->find('all', array(
			'conditions' => array(
				'BatchSet.user_id' => $_SESSION['Auth']['User']['id'],
				'BatchSet.flag_tmp' => true
			),
			'order' => array('BatchSet.created DESC')
		));
	...
	function listall($batch_set_id) {
		...
		$this->ModelToSearch = AppModel::getInstance($batch_set['BatchSet']['plugin'], $batch_set['BatchSet']['model'],
			true);
		...
		if (count($lookup_ids) > 0) {
			$criteria = $batch_set['BatchSet']['model'] . '.' . $batch_set['BatchSet']['lookup_key_name'] . " IN ('" . implode("', '",
					$lookup_ids) . "')";
		}
		...
		if ($batch_set['DatamartStructure']['control_master_model']) {
			...
			$results = $this->ModelToSearch->find('all', array(
				'fields' => array($this->ModelToSearch->getControlForeign()),
				'conditions' => $criteria,
				'recursive' => 0,
			...
			if (count($results) == 1) {
				...
				$alternate_info = Browser::getAlternateStructureInfo($datamart_structure['plugin'],
					$this->ModelToSearch->getControlName(),
					$results[0][$datamart_structure['model']][$this->ModelToSearch->getControlForeign()]);
				...
				$this->ModelToSearch = AppModel::getInstance($datamart_structure['plugin'],
				...
				$batch_set['BatchSet']['form_links_for_results'] = Browser::updateIndexLink($batch_set['BatchSet']['form_links_for_results'],
					$datamart_structure['model'], $datamart_structure['control_master_model'], $prev_pkey,
					$this->ModelToSearch->primaryKey);
				$batch_set['BatchSet']['form_links_for_results'] = substr($batch_set['BatchSet']['form_links_for_results'],
					strpos($batch_set['BatchSet']['form_links_for_results'], '/'));
		...
		$actions = array();
		if (count($results)) {
			$actions = $this->BatchSet->getDropdownOptions($batch_set['BatchSet']['plugin'],
				$batch_set['BatchSet']['model'], $batch_set['BatchSet']['lookup_key_name'],
				$batch_set['BatchSet']['structure_alias'], $lookup_model_name, $lookup_key_name, $batch_set_id);
	...
	function add($target_batch_set_id = 0) {
		...
		if (!$target_batch_set_id) {
			...
			if (array_key_exists('node', $this->request->data)) {
				...
				if (array_key_exists('BatchSet',
						$this->request->data) && isset($this->request->data['BatchSet']['datamart_structure_id'])
				) {
					...
					if (array_key_exists('Report',
							$this->request->data) && isset($this->request->data['Report']['datamart_structure_id'])
					) {
						...
						if (array_key_exists('BatchSet', $this->request->data)) {
							$batch_set_tmp = $this->BatchSet->find('first', array(
								'conditions' => array('BatchSet.id' => $this->request->data['BatchSet']['id']),
								'recursive' => 0
							));
							...
							if (isset($this->request->params['_data'])) {
		...
		if (isset($this->request->data[$model])) {
			...
			if (is_array($this->request->data[$model][$lookup_key_name])) {
				...
				if ($this->request->data[$model][$lookup_key_name] == 'all' && array_key_exists('node',
						$this->request->data)
				) {
					if (!$browsing_result) {
						$browsing_result = $this->BrowsingResult->find('first',
					...
					$batch_set_ids = array_merge(explode(",", $browsing_result['BrowsingResult']['id_csv']),
						$batch_set_ids);
				} else {
					$batch_set_ids = array_merge(explode(",", $this->request->data[$model][$lookup_key_name]),
						$batch_set_ids);
				}
			}
	...
	function remove($batch_set_id) {
		...
		$datamart_structure = $this->DatamartStructure->findById($batch_set['BatchSet']['datamart_structure_id']);
		$datamart_structure = $datamart_structure['DatamartStructure'];
		$batch_set_model_instance = AppModel::getInstance($datamart_structure['plugin'], $datamart_structure['model'],
			true);

		if (count($this->request->data[$batch_set_model_instance->name][$batch_set_model_instance->primaryKey])) {
			// START findall criteria
			$criteria = 'set_id="' . $batch_set_id . '" ' . 'AND ( lookup_id="' . implode('" OR lookup_id="',
					$this->request->data[$batch_set_model_instance->name][$batch_set_model_instance->primaryKey]) . '" )';

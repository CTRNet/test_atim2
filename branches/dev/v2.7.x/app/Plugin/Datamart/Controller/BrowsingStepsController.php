...
class BrowsingStepsController extends DatamartAppController {
	...
	function listall() {
		...
		foreach ($tmp_datamart_structures as $new_datamart_structure) {
			$datamart_structures[$new_datamart_structure['DatamartStructure']['id']] = $new_datamart_structure['DatamartStructure'];
		}
		...
		foreach ($this->request->data as &$data) {
			...
			foreach ($data['SavedBrowsingStep'] as $new_step) {
				...
				$new_step_model = AppModel::getInstance($new_step_datamart_structure['plugin'],
					$new_step_datamart_structure['model'], true);
				...
				if ((isset($search['search_conditions']) && count($search['search_conditions'])) || $adv_search || isset($search['counters'])) {
					...
					if ($new_step_model->getControlName() && $new_step['datamart_sub_structure_id'] > 0) {
						...
						$tmp_model = AppModel::getInstance($new_step_datamart_structure['plugin'],
							$new_step_datamart_structure['model'], true);
						AppModel::getInstance("Datamart", "Browser", true);
						$alternate_alias = Browser::getAlternateStructureInfo($new_step_datamart_structure['plugin'],
							$tmp_model->getControlName(), $new_step['datamart_sub_structure_id']);
						...
						$tmp_model = AppModel::getInstance($new_step_datamart_structure['plugin'],
							$new_step_datamart_structure['control_master_model'], true);
						$tmp_data = $tmp_model->find('first', array(
							'conditions' => array($tmp_model->getControlName() . ".id" => $new_step['datamart_sub_structure_id']),
							'recursive' => 0
						));
					...
					if (isset($search['counters'])) {
						foreach ($search['counters'] as $counter) {
							...
							$addon_params[] = array(
								'field' => __('counter') . ': ' . __($browsing_structure['display_name']),
					...
					if (count($search['search_conditions']) || $adv_search || $addon_params) {//count might be zero if the only condition was the sub type
						...
						if ($new_step_datamart_structure['adv_search_structure_alias']) {
							$adv_structure = StructuresComponent::$singleton->get('form',
								$new_step_datamart_structure['adv_search_structure_alias']);
						}
	...
	function edit($id) {
		$this->Structures->set('datamart_saved_browsing');
		$browsing_index = $this->SavedBrowsingIndex->find('first', array(
			'conditions' => array_merge($this->SavedBrowsingIndex->getOwnershipConditions(),
				array('SavedBrowsingIndex.id' => $id))
		));
		if (!$browsing_index) {

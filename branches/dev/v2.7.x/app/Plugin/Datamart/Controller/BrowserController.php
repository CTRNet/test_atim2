...
class BrowserController extends DatamartAppController {
	...
	function delete($index_id) {
		...
		if (!empty($this->request->data)) {
			...
			$this->atimFlash(__('error deleting data - contact administrator'), '/Datamart/Browser/index/');
		}
	}
	...
	/**
	 * Core of the databrowser, handles all browsing requests. Searches, normal display, merged display and overflow
	 * display.
	 *
	 ...
	 * the string will separate the structure id from the substructure id with an underscore. It will be of the form
	 *     id_sub-id
	 * @param int $merge_to If a merged display is required, the node id to merge to. The merge direction is always
	 *     from node_id to merge_to
	 */
	function browse($node_id = 0, $control_id = 0, $merge_to = 0) {
			...
			if ($node_id == 0) {
				if ($control_id) {
					...
					$this->set("dropdown_options",
						$this->Browser->getBrowserDropdownOptions(0, $node_id, null, null, null, null, null, null));
				...
				if ($control_id) {
					...
					if ($browsing['BrowsingResult']['user_id'] != CakeSession::read('Auth.User.id')) {
						$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null,
							true);
					}
					$check_list = true;
				}
		...
		if ($check_list) {
			...
			$browsing_model = AppModel::getInstance($browsing['DatamartStructure']['plugin'],
				$browsing['DatamartStructure']['model'], true);

			$this->set('top', "/Datamart/Browser/browse/" . $node_id . "/");
			$this->set('node_id', $node_id);
			$this->set('type', "checklist");
			$this->set('checklist_key',
				$this->Browser->checklist_model->name . "." . $this->Browser->checklist_use_key);
			$this->set('checklist_key_name',
				$browsing['DatamartStructure']['model'] . "." . $browsing_model->primaryKey);
			$this->set('is_root', $browsing['BrowsingResult']['parent_id'] == 0);

			$dropdown_options = $this->Browser->getBrowserDropdownOptions($browsing['DatamartStructure']['id'],
				$node_id, $browsing['DatamartStructure']['plugin'], $this->Browser->checklist_model->name,
				$browsing['DatamartStructure']['model'], $this->Browser->checklist_use_key, $browsing_model->primaryKey,
				$this->Browser->checklist_sub_models_id_filter);
			foreach ($dropdown_options as $key => $option) {
				if (isset($option['value']) && strpos($option['value'],
						'javascript:setCsvPopup(\'Datamart/Csv/csv') === 0
				) {
			...
			if ($this->Browser->checklist_model->name != $browsing['DatamartStructure']['model']) {
				$browsing['DatamartStructure']['index_link'] = str_replace($browsing['DatamartStructure']['model'],
					$this->Browser->checklist_model->name,
					str_replace($browsing['DatamartStructure']['model'] . "." . $browsing_model->primaryKey,
						$this->Browser->checklist_model->name . "." . $this->Browser->checklist_use_key,
			...
			if ($browsing) {
				...
				//search screen
				$tmp_model = AppModel::getInstance($browsing['DatamartStructure']['plugin'],
					$browsing['DatamartStructure']['model'], true);
				if (isset($sub_structure_id) && $ctrl_name = $tmp_model->getControlName()) {
					$alternate_info = Browser::getAlternateStructureInfo($browsing['DatamartStructure']['plugin'],
						$ctrl_name, $sub_structure_id);
					...
					$this->set("atim_structure",
						$this->Structures->getFormById($browsing['DatamartStructure']['structure_id']));
					$this->set("header", array(
						"title" => __("search"),
				...
				if ($browsing['DatamartStructure']['adv_search_structure_alias']) {
					...
					$advanced_structure = $this->Structures->get('form',
				...
				if ($path) {
					while ($parent_node = array_pop($path)) {
						if ($this->BrowsingControl->find1ToN($current_structure_id,
							$parent_node['BrowsingResult']['browsing_structures_id'])
						) {
							//valid parent
				...
				if ($counters_structure_fields) {
					$this->set('counters_structure_fields',
						array('Structure' => array('alias' => 'custom'), 'Sfs' => $counters_structure_fields));
				}
	...
	function csv($all_fields, $node_id, $merge_to) {
		...
		if ($config['redundancy'] == 'same' && isset($config['singleLineNodes']) && !empty($config['singleLineNodes'])) {
			...
			$valid_ids = array_merge(array_keys($mergeable_nodes['parents']),
				array_keys($mergeable_nodes['flat_children']));
			...
			$browsing_results = $this->BrowsingResult->find('all', array(
				'conditions' => array(
					'BrowsingResult.id' => array_merge(array($node_id), $config['singleLineNodes'])
				)
			));
			...
			foreach ($browsing_results as $browsing_result) {
				...
				if ($browsing_result['BrowsingResult']['id'] != $node_id) {
					$joins[$browsing_result['BrowsingResult']['id']] = $this->BrowsingResult->getJoins($browsing_result['BrowsingResult']['id'],
						$node_id);
				}
			}
			...
			while ($primary_data = $models[$node_id]->find('all', array(
				'conditions' => array($models[$node_id]->name . '.' . $models[$node_id]->primaryKey => $ids),
				'limit' => $base_fetch_limit,
				'offset' => $offset
			))) {
				$primary_data = AppController::defineArrayKey($primary_data, $models[$node_id]->name,
					$models[$node_id]->primaryKey, true);
				$this->request->data = array();
				$this->request->data[$node_id] = AppController::defineArrayKey($primary_data, $models[$node_id]->name,
					$models[$node_id]->primaryKey);
				...
				foreach ($nodes_info as $key => $node_info) {
					...
					$data = $models[$key]->find('all', array(
						'fields' => array('*'),
						'joins' => $joins[$key],
						'conditions' => array_merge($base_model_condition,
							array($models[$key]->name . '.' . $models[$key]->primaryKey . ' IN (' . $browsing_results[$key]['BrowsingResult']['id_csv'] . ')'))
					));
					$this->request->data[$key] = AppController::defineArrayKey($data, $models[$node_id]->name,
						$models[$node_id]->primaryKey);
				}
	...
	function batchToDatabrowser($model, $source = 'batchset') {
		...
		if (array_key_exists($dm_structure['DatamartStructure']['model'], $this->request->data)) {
			$model = AppModel::getInstance($dm_structure['DatamartStructure']['plugin'],
				$dm_structure['DatamartStructure']['model'], true);
		} else {
			if (array_key_exists($dm_structure['DatamartStructure']['control_master_model'], $this->request->data)) {
				$model = AppModel::getInstance($dm_structure['DatamartStructure']['plugin'],
					$dm_structure['DatamartStructure']['control_master_model'], true);
			} else {
	...
	function unusedParent($node_id) {
		...
		$control = $this->BrowsingControl->find('first', array(
			'conditions' => array(
				'BrowsingControl.id1' => $child_data['DatamartStructure']['id'],
				'BrowsingControl.id2' => $parent_data['DatamartStructure']['id']
			)
		));
		...
		if (empty($control)) {
			$control = $this->BrowsingControl->find('first', array(
				'conditions' => array(
					'BrowsingControl.id2' => $child_data['DatamartStructure']['id'],
					'BrowsingControl.id1' => $parent_data['DatamartStructure']['id']
				)
			));
			...
			$parent_key_used_data = $parent_model->find('all', array(
				...
				'conditions' => array(
					$parent_model->name . '.' . $control['BrowsingControl']['use_field'] => explode(',',
						$child_data['BrowsingResult']['id_csv'])
				)
			));
		} else {
			//load the child model
			...
			$parent_key_used_data = $child_model->find('all', array(
				...
				'conditions' => array(
					$child_model->name . '.' . $child_model->primaryKey => explode(',',
						$child_data['BrowsingResult']['id_csv'])
				)
			));
		}
	...
	function applyBrowsingSteps($starting_node_id, $browsing_step_index_id) {
		$this->BrowsingResult->check_writable_fields = false;
		$browsing_steps = $this->SavedBrowsingIndex->find('first', array(
			'conditions' => array_merge($this->SavedBrowsingIndex->getOwnershipConditions(),
				array('SavedBrowsingIndex.id' => $browsing_step_index_id))
		));
		if (!$browsing_steps) {

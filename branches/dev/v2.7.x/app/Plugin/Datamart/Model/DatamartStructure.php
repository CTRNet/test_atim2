...
class DatamartStructure extends DatamartAppModel {
	...
	function getIdByModelName($model_name) {
		$data = $this->find('first', array(
			'conditions' => array('OR' => array('model' => $model_name, 'control_master_model' => $model_name)),
			'recursive' => -1,
			'fields' => array('id')
		));
		if (!empty($data)) {
	...
	function getModel($id, $model_name = null) {
		$d = $this->findById($id);
		return AppModel::getInstance($d['DatamartStructure']['plugin'],
			$model_name ?: $d['DatamartStructure']['model']);
	}
}
...
class BrowsingResult extends DatamartAppModel {
	...
	function getModelAndStructureForNode($browsing_result) {
		...
		$model = AppModel::getInstance($browsing_result['DatamartStructure']['plugin'],
			$browsing_result['DatamartStructure']['model']);
		...
		if ($control_id) {
			$model = AppModel::getInstance($browsing_result['DatamartStructure']['plugin'],
				$browsing_result['DatamartStructure']['control_master_model']);
			$control_model = AppModel::getInstance($browsing_result['DatamartStructure']['plugin'],
				$model->getControlName());
			$control_model_data = $control_model->find('first', array(
					'fields' => array(
						$control_model->name . '.form_alias',
						$control_model->name . '.databrowser_label'
					),
			...
			AppController::buildDetailBinding($model,
				array($model->name . '.' . $model->getControlForeign() => $control_id), $structure_alias);

			$structure = $structures_component->get('form', $structure_alias);
		}

		return array(
			'specific' => $control_id != null,
			'model' => $model,
			'structure' => $structure,
			'header_sub_type' => $header_sub_type
		);
	}
	...
	function getSingleLineMergeableNodes($starting_node_id) {
		...
		while ($parent = array_pop($parents_nodes)) {
			...
			if ($datamart_controls_model->findNTo1($current_ctrl_id,
				$parent['BrowsingResult']['browsing_structures_id'])
			) {
				...
				if ($datamart_controls_model->find1ToN($current_ctrl_id,
					$parent['BrowsingResult']['browsing_structures_id'])
				) {
				...
				foreach ($child_node['nodes'] as $k => &$node) {
					if ($node['BrowsingResult']['browsing_type'] == 'drilldown') {
						...
						if (!in_array($node['BrowsingResult']['browsing_structures_id'], $encountered_ctrls)) {
							if ($datamart_controls_model->findNTo1($current_ctrl_id,
								$node['BrowsingResult']['browsing_structures_id'])
							) {
	...
	function getJoins($base_node_id, $target_node_id) {
		...
		foreach ($merge_on as $merge_unit) {
			...
			$joins[] = $browsing_control_model->getInnerJoinArray($current_model, $to_model,
				explode(',', $merge_unit['BrowsingResult']['id_csv']));

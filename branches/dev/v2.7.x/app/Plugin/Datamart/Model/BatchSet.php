...
class BatchSet extends DatamartAppModel {
		...
		if (isset($variables['BatchSet.id'])) {
			if (isset($variables['BatchSet.temporary_batchset']) && $variables['BatchSet.temporary_batchset']) {
				...
				if (!empty($variables['BatchSet.id'])) {
					$batchset_data = $this->find('first',
						array('conditions' => array('BatchSet.id' => $variables['BatchSet.id'])));
	...
	public function isUserAuthorizedToRw(array $batchset, $must_be_unlocked) {
		if (empty($batchset) || (!(array_key_exists('user_id', $batchset['BatchSet']) && array_key_exists('group_id',
					$batchset['BatchSet']) && array_key_exists('sharing_status', $batchset['BatchSet'])))
		) {
		...
		if (!$allowed) {
			AppController::getInstance()->flash(__('you are not allowed to work on this batchset'),
	...
	function saveWithIds(array $batch_set_data, array $ids) {
		...
		if (!$this->save($batch_set_data)) {
			$this->redirect('/Pages/err_plugin_system_error?method=' . $bt[1]['function'] . ',line=' . $bt[1]['line'],
				null, true);
		}
		...
		if (!$batch_id_model->saveAll($batch_ids)) {
			$this->redirect('/Pages/err_plugin_system_error?Bmethod=' . $bt[1]['function'] . ',line=' . $bt[1]['line'],
				null, true);
		}

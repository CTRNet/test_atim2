...
$settings = array(
	'header' => array('title' => __('temporary batch sets') . ' (' . __($filter_value) . ')',
		'description' => __('unsaved batch sets that are automatically deleted when there are more than %d',
			BatchSetsController::$tmp_batch_set_limit)
	),
	'actions' => false,
	'pagination' => false
);
$this->Structures->build($atim_structure,
	array('data' => $tmp_batch, 'links' => $structure_links, 'settings' => $settings));

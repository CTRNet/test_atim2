...
$settings = array(
	'header' => array('title' => __('temporary browsing'),
		'description' => __('unsaved browsing trees that are automatically deleted when there are more than %d',
			$tmp_browsing_limit)
	),
	'form_bottom' => false,
	'actions' => false,
...
$this->Structures->build($atim_structure,
	array('data' => $tmp_browsing, 'type' => 'index', 'links' => $structure_links, 'settings' => $settings));
...
$this->Structures->build($atim_structure,
	array('data' => $this->request->data, 'type' => 'index', 'links' => $structure_links, 'settings' => $settings));
?>
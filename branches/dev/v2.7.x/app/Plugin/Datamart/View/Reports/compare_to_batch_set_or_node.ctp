...
if ($csv_creation) {
	...
	$settings = array();
	$this->Structures->build($atim_structure_for_results,
		array('type' => 'csv', 'data' => $diff_results_data, 'settings' => $settings));
	exit;
	...
	if (isset($datamart_structure_links)) {
		$structure_links['index'] = array('details' => $datamart_structure_links);
	}

	$add_to_batchset_hidden_field = $this->Form->input('Report.datamart_structure_id',
		array('type' => 'hidden', 'value' => $datamart_structure_id));

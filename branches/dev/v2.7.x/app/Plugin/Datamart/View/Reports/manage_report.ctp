...
if (isset($search_form_structure)) {
	...
	if ($csv_creation) {
		...
		$this->Structures->build($result_form_structure,
			array('type' => 'csv', 'data' => $this->request->data, 'settings' => $settings));
		exit;

	} else {
		if (isset($linked_datamart_structure_model_name) && isset($linked_datamart_structure_key_name)) {
			...
			if (isset($linked_datamart_structure_links)) {
				$structure_links['index'] = array('details' => $linked_datamart_structure_links);
			}

			$add_to_batchset_hidden_field = $this->Form->input('Report.datamart_structure_id',
				array('type' => 'hidden', 'value' => $linked_datamart_structure_id));
			...
			$settings = array(
				...
				'actions' => false,
				'pagination' => false,
				'sorting' => array($atim_menu_variables['Report.id'], '0')
			);
			...
			if ($display_new_search) {
				$structure_links['bottom']['new search'] = array(
					'link' => '/Datamart/Reports/manageReport/' . $atim_menu_variables['Report.id'],
					'icon' => 'search'
				);
			}
			...
			$structure_links = array(
				'bottom' => array(
					'export as CSV file (comma-separated values)' => sprintf("javascript:setCsvPopup('Datamart/Reports/manageReport/" . $atim_menu_variables['Report.id'] . "/1/');",
						0)
				)
			);
			if ($display_new_search) {
...
if (isset($linked_datamart_structure_actions)) {
	$linked_datamart_structure_actions = json_encode(Sanitize::clean($linked_datamart_structure_actions));
} else {

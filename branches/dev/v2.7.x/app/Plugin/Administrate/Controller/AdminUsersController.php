...
class AdminUsersController extends AdministrateAppController {
	...
	public $paginate = array('User' => array('limit' => PAGINATION_AMOUNT, 'order' => 'User.username ASC'));
	...
	public function listall($group_id) {
	...
	public function detail($group_id, $user_id) {
	...
	public function add($group_id) {
		...
		if ($this->Group->hasPermissions($group_id)) {
				...
				if ($submitted_data_validates) {
					if ($this->User->save($this->request->data)) {
						...
						$this->atimFlash(__('your data has been saved'),
							'/Administrate/AdminUsers/detail/' . $group_id . '/' . $this->User->getLastInsertId() . '/');
					}
				}
			...
			$this->atimFlash(__('you cannot create a user for that group because it has no permission'),
				"/Administrate/AdminUsers/listall/" . $group_id . "/");
		}
	}
		...
		if (empty($this->request->data)) {
			...
			if ($submitted_data_validates) {
				if ($this->User->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/Administrate/AdminUsers/detail/' . $group_id . '/' . $user_id . '/');
					return;
				}
			}
	...
	public function changeGroup($group_id, $user_id) {
		...
		if ($this->request->data) {
			...
			$this->atimFlash(__('your data has been saved'),
				'/Administrate/AdminUsers/detail/' . $this->request->data['Group']['id'] . '/' . $user_id . '/');
		} else {

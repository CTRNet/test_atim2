...
class PermissionsController extends AdministrateAppController {
	...
	public function update($aro_id, $aco_id, $state) {

		$this->autoRender = false;

		$aro = $this->Aro->find('first',
			array('conditions' => 'Aro.id = "' . $aro_id . '"', 'order' => 'alias ASC', 'recursive' => -1));
	...
	private function updatePermission($aro_id, $aco_id, $state) {
		...
		try {
			...
			$bt = debug_backtrace();
			AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method=' . $bt[1]['function'] . ',line=' . $bt[0]['line'],
				null, true);
		}

	}

	public function tree($group_id = 0, $user_id = 0) {

		$this->set('atim_menu_variables', array('Group.id' => $group_id, 'User.id' => $user_id));
		$aro = $this->Aro->find('first',
			array('conditions' => 'Aro.alias = "Group::' . $group_id . '"', 'order' => 'alias ASC', 'recursive' => 1));
		...
		if ($this->request->data) {
			...
			$dependent_fcts = $dm_struct_fct_model->find('all',
				array('conditions' => array('NOT' => array('DatamartStructureFunction.ref_single_fct_link' => ''))));
			...
			$this->atimFlash(__('your data has been updated'),
				'/Administrate/Permissions/tree/' . $group_id . '/' . $user_id);
			return;
		}
		...
		$this->set("group_data",
			$this->Group->find('first', array('conditions' => array('id' => $group_id), 'recursive' => 0)));
	}

	public function addPermissionStateToThreadedData($threaded_data = array()) {
	...
	public function savePreset() {
		...
		if (!empty($this->request->data)) {
			$permission_preset = $this->PermissionsPreset->find('first',
				array('conditions' => array('PermissionsPreset.name' => $this->request->data['PermissionsPreset']['name'])));

...
class DropdownsController extends AdministrateAppController {
	...
	public function subIndex($filter = 'all') {
		if (!in_array($filter, array('all', 'empty', 'not_empty'))) {
	...
	public function view($control_id) {
		...
		$this->request->data = $this->StructurePermissibleValuesCustom->find('all', array(
			'conditions' => array('StructurePermissibleValuesCustom.control_id' => $control_id),
			'order' => array('display_order', 'value')
		));
		$this->Structures->set("administrate_dropdown_values", 'administrate_dropdown_values');
	}

	public function add($control_id) {
		$control_data = $this->StructurePermissibleValuesCustomControl->getOrRedirect($control_id);
		$this->set("control_data", $control_data);
		...
		if (empty($this->request->data)) {
			...
			$max_length = min($this->StructurePermissibleValuesCustom->_schema['value']['length'],
				$control_data["StructurePermissibleValuesCustomControl"]["values_max_length"]);
			$break = false;
			$tmp = $this->StructurePermissibleValuesCustom->find('all',
				array('conditions' => array('control_id' => $control_id), 'recursive' => -1));
			...
			foreach ($this->request->data as &$data_unit) {
				...
				$data_unit['StructurePermissibleValuesCustom'] = array_map("trim",
					$data_unit['StructurePermissibleValuesCustom']);
				if (array_key_exists($data_unit['StructurePermissibleValuesCustom']['value'], $existing_values)) {
					$errors_tracking['value'][__('a specified %s already exists for that dropdown',
						__("value"))][] = $row_counter;
				}
				if (array_key_exists($data_unit['StructurePermissibleValuesCustom']['value'], $current_values)) {
					$errors_tracking['value'][__('you cannot declare the same %s more than once',
						__("value"))][] = $row_counter;
				}
				if (strlen($data_unit['StructurePermissibleValuesCustom']['value']) > $max_length) {
					$errors_tracking['value'][__('%s cannot exceed %d characters', __("value"),
						$max_length)][] = $row_counter;
				}
				...
				if (!(is_null($data_unit['StructurePermissibleValuesCustom']['en']) || ($data_unit['StructurePermissibleValuesCustom']['en'] == ''))) {
					if (array_key_exists($data_unit['StructurePermissibleValuesCustom']['en'], $existing_en)) {
						$errors_tracking['en'][__('a specified %s already exists for that dropdown',
							__("english translation"))][] = $row_counter;
					}
					if (array_key_exists($data_unit['StructurePermissibleValuesCustom']['en'], $current_en)) {
						$errors_tracking['en'][__('you cannot declare the same %s more than once',
							__("english translation"))][] = $row_counter;
					}
				}
				if (strlen($data_unit['StructurePermissibleValuesCustom']['en']) > $this->StructurePermissibleValuesCustom->_schema['en']['length']) {
					$errors_tracking['en'][__('%s cannot exceed %d characters', __("english translation"),
						$this->StructurePermissibleValuesCustom->_schema['en']['length'])][] = $row_counter;
				}
				...
				if (!(is_null($data_unit['StructurePermissibleValuesCustom']['fr']) || ($data_unit['StructurePermissibleValuesCustom']['fr'] == ''))) {
					if (array_key_exists($data_unit['StructurePermissibleValuesCustom']['fr'], $existing_fr)) {
						$errors_tracking['fr'][__('a specified %s already exists for that dropdown',
							__("french translation"))][] = $row_counter;
					}
					if (array_key_exists($data_unit['StructurePermissibleValuesCustom']['fr'], $current_fr)) {
						$errors_tracking['fr'][__('you cannot declare the same %s more than once',
							__("french translation"))][] = $row_counter;
					}
				}
				if (strlen($data_unit['StructurePermissibleValuesCustom']['fr']) > $this->StructurePermissibleValuesCustom->_schema['fr']['length']) {
					$errors_tracking['fr'][__('%s cannot exceed %d characters', __("french translation"),
						$this->StructurePermissibleValuesCustom->_schema['fr']['length'])][] = $row_counter;
				}
			...
			if (empty($errors_tracking)) {
				...
				foreach ($errors_tracking as $field => $msg_and_lines) {
					foreach ($msg_and_lines as $msg => $lines) {
						$this->StructurePermissibleValuesCustomControl->validationErrors[$field][] = $msg . ' - ' . str_replace('%s',
	...
	public function edit($control_id, $value_id) {
		$control_data = $this->StructurePermissibleValuesCustomControl->getOrRedirect($control_id);
		$this->set("control_data", $control_data);
		...
		if (empty($this->request->data)) {
			...
			if (!(is_null($this->request->data['StructurePermissibleValuesCustom']['en'])
				...
				if (!empty($tmp)) {
					$this->StructurePermissibleValuesCustom->validationErrors['en'][] = __('a specified %s already exists for that dropdown',
				...
				if (strlen($this->request->data['StructurePermissibleValuesCustom']['en']) > $this->StructurePermissibleValuesCustom->_schema['en']['length']) {
					$this->StructurePermissibleValuesCustom->validationErrors['en'][] = __('%s cannot exceed %d characters',
						__("english translation"), $this->StructurePermissibleValuesCustom->_schema['en']['length']);
					$skip_save = true;
				}
			}
			...
			if (!(is_null($this->request->data['StructurePermissibleValuesCustom']['fr'])
				...
				if (!empty($tmp)) {
					$this->StructurePermissibleValuesCustom->validationErrors['fr'][] = __('a specified %s already exists for that dropdown',
						__("french translation"));
					$skip_save = true;
				}
				if (strlen($this->request->data['StructurePermissibleValuesCustom']['fr']) > $this->StructurePermissibleValuesCustom->_schema['fr']['length']) {
					$this->StructurePermissibleValuesCustom->validationErrors['fr'][] = __('%s cannot exceed %d characters',
						__("french translation"), $this->StructurePermissibleValuesCustom->_schema['fr']['length']);
					$skip_save = true;
				}
			}
	...
	public function configure($control_id) {
		...
		if (empty($this->request->data)) {
			...
			$this->set('alpha_order',
				$this->request->data[0]['StructurePermissibleValuesCustom']['display_order'] == 0);
		} else {
			$data = array();

...
class Order extends OrderAppModel {
	...
	function allowDeletion($order_id) {
		...
		$returned_nbr = $order_item_model->find('count',
			array('conditions' => array('OrderItem.order_id' => $order_id), 'recursive' => '-1'));
		if ($returned_nbr > 0) {
		...
		$returned_nbr = $order_ling_model->find('count',
			array('conditions' => array('OrderLine.order_id' => $order_id), 'recursive' => '-1'));
		if ($returned_nbr > 0) {
		...
		$returned_nbr = $shipment_model->find('count',

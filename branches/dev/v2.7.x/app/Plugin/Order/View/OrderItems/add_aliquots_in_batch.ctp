...
$final_options = array(
	...
	'settings' => array(
		'actions' => false,
		'pagination' => false,
		'header' => array('title' => __('add aliquots to order'), 'description' => __('studied aliquots'))
	),
...
$extras = $this->Form->input('0.aliquot_ids_to_add',
		array('type' => 'hidden', 'value' => $aliquot_ids_to_add)) . $this->Form->input('url_to_cancel',
		array('type' => 'hidden', 'value' => $url_to_cancel));
...
if (Configure::read('order_item_to_order_objetcs_link_setting') != 2) {
	$linked_objects[] = __('order');
}
if (Configure::read('order_item_to_order_objetcs_link_setting') != 3) {
	$linked_objects[] = __('order line');
}
$header = '2 - ' . str_replace('%%order_objects%%', implode(' ' . __('or') . ' ', $linked_objects),
		__('%%order_objects%% selection', null));
if (Configure::read('order_item_to_order_objetcs_link_setting') == 3) {

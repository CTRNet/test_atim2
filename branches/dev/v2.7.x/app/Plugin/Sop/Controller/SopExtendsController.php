...
class SopExtendsController extends SopAppController {
	...
	function listall($sop_master_id) {
		...
		$sop_master_data = $this->SopMaster->find('first',
			array('conditions' => array('SopMaster.id' => $sop_master_id)));

		$this->SopExtend = AppModel::atimInstantiateExtend($this->SopExtend,
			$sop_master_data['SopControl']['extend_tablename']);
		...
		$material_list = $this->Material->find('all',
			array('fields' => array('Material.id', 'Material.item_name'), 'order' => array('Material.item_name')));
	...
	function detail($sop_master_id = null, $sop_extend_id = null) {
		...
		$this->request->data = $this->SopExtend->find('first',
			array('conditions' => array('SopExtend.id' => $sop_extend_id)));
		...
		if (!empty($this->request->data)) {
			...
			if ($this->SopExtend->save($this->request->data)) {
				$this->atimFlash(__('your data has been updated'),
					'/Sop/SopExtends/detail/' . $sop_master_id . '/' . $sop_extend_id);
			}
		} else {

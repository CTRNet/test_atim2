...
class SopMastersController extends SopAppController {
	...
	function listall() {
		...
		$this->set('sop_controls',
			$this->SopControl->find('all', array('conditions' => array('SopControl.flag_active' => '1'))));
	...
	function add($sop_control_id) {
		...
		$this_data = $this->SopControl->find('first',
			array('conditions' => array('SopControl.id' => $sop_control_id, 'SopControl.flag_active' => '1')));
		if (empty($this_data)) {
			...
			if ($submitted_data_validates) {
				if ($this->SopMaster->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/Sop/SopMasters/detail/' . $this->SopMaster->getLastInsertId());
				}
			}
		}
	}
		...
		if (empty($this->request->data)) {
			...
			if ($submitted_data_validates) {
				if ($this->SopMaster->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/Sop/SopMasters/detail/' . $sop_master_id . '/');
				}
			}
		}
	}

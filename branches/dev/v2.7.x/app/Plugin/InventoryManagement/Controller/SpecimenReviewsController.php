...
class SpecimenReviewsController extends InventoryManagementAppController {
	...
	function listAll($collection_id, $sample_master_id) {
		...
		$this->request->data = $this->paginate($this->SpecimenReviewMaster,
			array('SpecimenReviewMaster.sample_master_id' => $sample_master_id));

		// Set list of available review
		$review_controls = $this->SpecimenReviewControl->find('all', array(
			'conditions' => array(
		...
		$this->set('atim_menu',
			$this->Menus->get('/InventoryManagement/SpecimenReviews/listAll/%%Collection.id%%/%%SampleMaster.initial_specimen_sample_id%%'));
	...
	function add($collection_id, $sample_master_id, $specimen_review_control_id) {
		...
		$review_control_data = $this->SpecimenReviewControl->find('first',
			array('conditions' => $criteria, 'recursive' => '2'));
		...
		if (array_key_exists('flag_active',
				$review_control_data['AliquotReviewControl']) && $review_control_data['AliquotReviewControl']['flag_active']
		) {
			$is_aliquot_review_defined = true;
		}
		$this->set('is_aliquot_review_defined', $is_aliquot_review_defined);

		// Set available aliquot
		if ($is_aliquot_review_defined) {
			$this->set('aliquot_list', $this->AliquotReviewMaster->getAliquotListForReview($sample_master_id,
				(($review_control_data['AliquotReviewControl']['aliquot_type_restriction'] == 'all') ? null : $review_control_data['AliquotReviewControl']['aliquot_type_restriction'])));
		}
		...
		$this->Structures->set($review_control_data['SpecimenReviewControl']['form_alias'],
			'specimen_review_structure');
		if ($is_aliquot_review_defined) {
			$this->Structures->set('empty', 'empty_structure');
			$this->Structures->set($review_control_data['AliquotReviewControl']['form_alias'],
				'aliquot_review_structure');
		}
		...
		if (empty($this->request->data)) {
			...
			$specimen_review_data['SpecimenReviewDetail'] = array_key_exists('SpecimenReviewDetail',
				$this->request->data) ? $this->request->data['SpecimenReviewDetail'] : array();
			...
			if ($is_aliquot_review_defined) {
				...
				foreach ($aliquot_review_data as &$new_aliquot_review) {
					...
					$all_aliquot_review_master_errors = array_merge($all_aliquot_review_master_errors,
						$this->AliquotReviewMaster->validationErrors);
			...
			if ($submitted_data_validates) {
				...
				$this->atimFlash(__('your data has been saved'),
					'/InventoryManagement/SpecimenReviews/detail/' . $collection_id . '/' . $sample_master_id . '/' . $specimen_review_master_id);
			}
		}
	}
	...
	function detail($collection_id, $sample_master_id, $specimen_review_id, $aliquot_master_id_from_tree_view = false) {
		...
		$specimen_review_data = $this->SpecimenReviewMaster->find('first',
			array('conditions' => $criteria, 'recursive' => '2'));
		...
		$is_aliquot_review_defined = false;
		if (array_key_exists('flag_active',
				$specimen_review_data['SpecimenReviewControl']['AliquotReviewControl']) && $specimen_review_data['SpecimenReviewControl']['AliquotReviewControl']['flag_active']
		) {
			$is_aliquot_review_defined = true;
		}
		$this->set('is_aliquot_review_defined', $is_aliquot_review_defined);
		...
		if ($is_aliquot_review_defined) {
			...
			if ($aliquot_master_id_from_tree_view) {
				$criteria['AliquotReviewMaster.aliquot_master_id'] = $aliquot_master_id_from_tree_view;
			}
		...
		$this->Structures->set($specimen_review_data['SpecimenReviewControl']['form_alias'],
			'specimen_review_structure');
		if ($is_aliquot_review_defined) {
			$this->Structures->set('empty', 'empty_structure');
			$this->Structures->set($specimen_review_data['SpecimenReviewControl']['AliquotReviewControl']['form_alias'],
				'aliquot_review_structure');
		}
	...
	function edit($collection_id, $sample_master_id, $specimen_review_id, $undo = false) {
		...
		$initial_specimen_review_data = $this->SpecimenReviewMaster->find('first',
			array('conditions' => $criteria, 'recursive' => '2'));
		...
		$is_aliquot_review_defined = false;
		if (array_key_exists('flag_active',
				$initial_specimen_review_data['SpecimenReviewControl']['AliquotReviewControl']) && $initial_specimen_review_data['SpecimenReviewControl']['AliquotReviewControl']['flag_active']
		) {
			$is_aliquot_review_defined = true;
		}
		$this->set('is_aliquot_review_defined', $is_aliquot_review_defined);
		...
		if ($is_aliquot_review_defined) {
			...
			$initial_aliquot_review_data_list = $this->AliquotReviewMaster->find('all',
				array('conditions' => $criteria));

			// Set available aliquot
			$this->set('aliquot_list', $this->AliquotReviewMaster->getAliquotListForReview($sample_master_id,
				(($initial_specimen_review_data['SpecimenReviewControl']['AliquotReviewControl']['aliquot_type_restriction'] == 'all') ? null : $initial_specimen_review_data['SpecimenReviewControl']['AliquotReviewControl']['aliquot_type_restriction'])));
		}
		...
		$this->Structures->set($initial_specimen_review_data['SpecimenReviewControl']['form_alias'],
			'specimen_review_structure');
		if ($is_aliquot_review_defined) {
			$this->Structures->set('empty', 'empty_structure');
			$this->Structures->set($initial_specimen_review_data['SpecimenReviewControl']['AliquotReviewControl']['form_alias'],
				'aliquot_review_structure');
		}
		...
		if (empty($this->request->data) || $undo) {
			...
			if ($submitted_data_validates) {
				...
				$this->atimFlash(__('your data has been saved'),
					'/InventoryManagement/SpecimenReviews/detail/' . $collection_id . '/' . $sample_master_id . '/' . $specimen_review_id);
			}
		}
	}
	...
	function delete($collection_id, $sample_master_id, $specimen_review_id) {
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			if (!$this->SpecimenReviewMaster->atimDelete($specimen_review_id)) {
				$this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null,
					true);
			}

...
class CollectionsController extends InventoryManagementAppController {
	...
	function search($search_id = 0, $is_ccl_ajax = false) {
		if ($is_ccl_ajax && $this->request->data) {
			...
			$this->request->data = $this->ViewCollection->find('all',
				array('conditions' => $conditions, 'limit' => $limit + 1));
			...
			if (isset($this->passedArgs['unlinkedParticipants'])) {
				...
				$group = $group_model->find('first',
					array('conditions' => array('Group.id' => $this->Session->read('Auth.User.group_id'))));
				...
				$this->searchHandler($search_id, $this->ViewCollection, 'view_collection',
					'/InventoryManagement/Collections/search');
			}
		}
		...
		if (empty($search_id)) {
			...
			if (!isset($this->request->query['nolatest'])) {
				$this->request->data = $this->ViewCollection->find('all', array(
						'conditions' => array('Collection.created_by' => $this->Session->read('Auth.User.id')),
	...
	function detail($collection_id, $hide_header = false) {
		...
		if (!$this->request->is('ajax')) {
			$this->Structures->set('sample_masters_for_collection_tree_view',
				'sample_masters_for_collection_tree_view');
			$sample_data = $this->SampleMaster->find('all', array(
				'conditions' => array(
	...
	function add($collection_id = 0, $copy_source = 0) {
		...
		if ($collection_id > 0) {
			$collection_data = $this->Collection->find('first', array(
				'conditions' => array('Collection.id' => $collection_id, 'Collection.deleted' => 1),
				'recursive' => '1'
			));
		}
		// MANAGE FORM, MENU AND ACTION BUTTONS
		...
		if (!empty($this->request->data) && !array_key_exists('collection_property',
	...
	function template($collection_id, $template_id) {
		...
		$aliquot_controls = $aliquot_control_model->find('all', array(
			'fields' => array('id', 'sample_control_id', 'aliquot_type'),
		...
		$parent_to_derivative_sample_control_model = AppModel::getInstance("InventoryManagement",
			"ParentToDerivativeSampleControl", true);
		$samples_relations = $parent_to_derivative_sample_control_model->find('all',
			array('conditions' => array('flag_active' => 1), 'recusrive' => -1));
		...
		$samples_relations = AppController::defineArrayKey($samples_relations, 'ParentToDerivativeSampleControl',
		...
		$js_data = array(
			...
			'aliquot_controls' => AppController::defineArrayKey($aliquot_controls, 'AliquotControl', 'id', true),
			'aliquot_relations' => AppController::defineArrayKey($aliquot_controls, "AliquotControl",
				"sample_control_id")
		);
		...
		$this->set('structure_header', array(

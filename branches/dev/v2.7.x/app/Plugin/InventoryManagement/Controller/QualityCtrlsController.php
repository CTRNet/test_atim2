...
class QualityCtrlsController extends InventoryManagementAppController {
	...
	function listAll($collection_id, $sample_master_id) {
		...
		$this->request->data = $this->paginate($this->QualityCtrl,
			array('QualityCtrl.sample_master_id' => $sample_master_id));
		...
		$this->set('atim_menu',
			$this->Menus->get('/InventoryManagement/QualityCtrls/listAll/%%Collection.id%%/' . $sample_id_parameter));
	...
	function add($sample_master_id = null) {
		...
		if ($sample_master_id != null) {
			...
			$menu_data = $this->SampleMaster->find('first',
			...
			if (array_key_exists('ViewAliquot', $this->request->data)) {
				...
				if (array_key_exists('ViewSample', $this->request->data)) {
					if (isset($this->request->data['node']) && $this->request->data['ViewSample']['sample_master_id'] == 'all') {
						$this->BrowsingResult = AppModel::getInstance('Datamart', 'BrowsingResult', true);
						$browsing_result = $this->BrowsingResult->find('first',
							array('conditions' => array('BrowsingResult.id' => $this->request->data['node']['id'])));
						$this->request->data['ViewSample']['sample_master_id'] = explode(",",
							$browsing_result['BrowsingResult']['id_csv']);
					}
			...
			if (!empty($this->request->data)) {
				...
				foreach ($this->request->data as $key => $data_unit) {
					...
					if (isset($data_unit['AliquotMaster'])) {
						...
						$display_data[] = array(
							'parent' => array_merge($aliquot_data, array('ViewSample' => $sample_data)),
							'children' => $data_unit
						);
						...
						$display_data[] = array(
							'parent' => array('ViewSample' => $sample_data),
							'children' => $data_unit
						);
					}
				...
				if (empty($errors) && !empty($qc_data_to_save)) {
					...
					if ($sample_master_id != null) {
						...
						$batch_set_model->saveWithIds($batch_set_data,
							range($last_qc_id - count($qc_data_to_save) + 1, $last_qc_id));
	...
	function detail($collection_id, $sample_master_id, $quality_ctrl_id, $is_from_tree_view = false) {
		...
		$this->set('atim_menu',
			$this->Menus->get('/InventoryManagement/QualityCtrls/detail/%%Collection.id%%/' . $sample_id_parameter . '/%%QualityCtrl.id%%'));
		...
		if (empty($this->request->data)) {
			...
			if ($submitted_data_validates && $this->QualityCtrl->save($this->request->data)) {
				...
				$this->atimFlash(__('your data has been saved'),
					'/InventoryManagement/QualityCtrls/detail/' . $collection_id . '/' . $sample_master_id . '/' . $quality_ctrl_id . '/');
			}
		}
		...
		if ($arr_allow_deletion['allow_deletion']) {
			if ($this->QualityCtrl->atimDelete($quality_ctrl_id)) {
				if ($qc_data['QualityCtrl']['aliquot_master_id'] != null) {
					$this->AliquotMaster->updateAliquotUseAndVolume($qc_data['QualityCtrl']['aliquot_master_id'], true,
						true, false);
				}
				$this->atimFlash(__('your data has been deleted'),
					'/InventoryManagement/QualityCtrls/listAll/' . $qc_data['SampleMaster']['collection_id'] . '/' . $qc_data['QualityCtrl']['sample_master_id'] . '/');
			} else {
				$this->atimFlash(__('error deleting data - contact administrator'),
					'/InventoryManagement/QualityCtrls/listAll/' . $collection_id . '/' . $sample_master_id);
			}
		} else {

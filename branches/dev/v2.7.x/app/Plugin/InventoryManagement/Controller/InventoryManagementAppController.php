	...
	function setBatchMenu(array $data) {
		if (array_key_exists('SampleMaster', $data) && !empty($data['SampleMaster'])) {
			...
			if (is_string($data['SampleMaster'])) {
				...
				if (array_key_exists(0, $data['SampleMaster']) && is_numeric($data['SampleMaster'][0])) {
					$id = $data['SampleMaster'];
				} else {
					if (!array_key_exists('initial_specimen_sample_id', $data['SampleMaster'])) {
						$id = $data['SampleMaster']['id'];
					}
				}
			}
			if ($id != null) {
				$data = $this->SampleMaster->find('all',
					array('conditions' => array('SampleMaster.id' => $id), 'recursive' => -1));
			} else {
				if (array_key_exists('SampleMaster', $data)) {
			...
			if (count($data) == 1) {
				...
				if ($data['initial_specimen_sample_id'] == $data['id']) {
					$this->set('atim_menu',
						$this->Menus->get('/InventoryManagement/SampleMasters/detail/%%Collection.id%%/%%SampleMaster.initial_specimen_sample_id%%'));
				} else {
					$this->set('atim_menu',
				...
				if (!empty($data)) {
					...
					if ($collection_id == null) {
						...
						$this->set('atim_menu',
							$this->Menus->get('/InventoryManagement/Collections/detail/%%Collection.id%%'));
						$this->set('atim_menu_variables', array(

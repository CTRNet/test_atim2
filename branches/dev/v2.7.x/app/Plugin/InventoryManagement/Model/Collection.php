...
class Collection extends InventoryManagementAppModel {
	...
	function hasChild(array $collection_ids) {
		...
		return array_filter($sample_master->find('list', array(
			'fields' => array("SampleMaster.collection_id"),
			'conditions' => array('SampleMaster.collection_id' => $collection_ids, 'SampleMaster.parent_id IS NULL'),
	...
	function allowDeletion($collection_id) {
		...
		$returned_nbr = $sample_master_model->find('count',
			array('conditions' => array('SampleMaster.collection_id' => $collection_id), 'recursive' => '-1'));
		if ($returned_nbr > 0) {

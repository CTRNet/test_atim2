...
class StudySummary extends StudyAppModel {
	...
	function summary($variables = array()) {
		...
		if (isset($variables['StudySummary.id'])) {
			...
			$result = $this->find('first',
				array('conditions' => array('StudySummary.id' => $variables['StudySummary.id'])));
	...
	function allowDeletion($study_summary_id) {
		...
		$ctrl_value = $ctrl_model->find('count',
			array('conditions' => array('TmaSlide.study_summary_id' => $study_summary_id), 'recursive' => '-1'));
		if ($ctrl_value > 0) {
		...
		$ctrl_value = $ctrl_model->find('count',
			array('conditions' => array('MiscIdentifier.study_summary_id' => $study_summary_id), 'recursive' => '-1'));
		if ($ctrl_value > 0) {
		...
		$ctrl_value = $ctrl_model->find('count',
			array('conditions' => array('ConsentMaster.study_summary_id' => $study_summary_id), 'recursive' => '-1'));
		if ($ctrl_value > 0) {
		...
		$ctrl_value = $ctrl_model->find('count',
			array('conditions' => array('Order.default_study_summary_id' => $study_summary_id), 'recursive' => '-1'));
		if ($ctrl_value > 0) {
		...
		$returned_nbr = $order_ling_model->find('count',
			array('conditions' => array('OrderLine.study_summary_id' => $study_summary_id), 'recursive' => '-1'));
		if ($returned_nbr > 0) {
		...
		$ctrl_value = $ctrl_model->find('count',
			array('conditions' => array('AliquotMaster.study_summary_id' => $study_summary_id), 'recursive' => '-1'));
		if ($ctrl_value > 0) {
		...
		$ctrl_value = $ctrl_model->find('count', array(
			'conditions' => array('AliquotInternalUse.study_summary_id' => $study_summary_id),
			'recursive' => '-1'
		));
		if ($ctrl_value > 0) {
			return array('allow_deletion' => false, 'msg' => 'study/project is assigned to an aliquot');

...
class StudyRelatedController extends StudyAppController {
	...
	function listall($study_summary_id) {
		...
		// MANAGE DATA
		$study_related_data = $this->StudySummary->find('first',
			array('conditions' => array('StudySummary.id' => $study_summary_id), 'recursive' => '-1'));
		if (empty($study_related_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		}

		$this->request->data = $this->paginate($this->StudyRelated,
			array('StudyRelated.study_summary_id' => $study_summary_id));
	...
	function detail($study_summary_id, $study_related_id) {
		...
		// MANAGE DATA
		$study_related_data = $this->StudyRelated->find('first', array(
			'conditions' => array(
				'StudyRelated.id' => $study_related_id,
				'StudyRelated.study_summary_id' => $study_summary_id
			)
		));
		if (empty($study_related_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		...
		$this->set('atim_menu_variables',
			array('StudySummary.id' => $study_summary_id, 'StudyRelated.id' => $study_related_id));
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyRelated->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/Study/StudyRelated/detail/' . $study_summary_id . '/' . $this->StudyRelated->id);
				}
			}
		}
	}
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyRelated->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/Study/StudyRelated/detail/' . $study_summary_id . '/' . $study_related_id);
				}
			}
		}
	}
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			if ($this->StudyRelated->atimDelete($study_related_id)) {
				...
				$this->atimFlash(__('error deleting data - contact administrator.'),
					'/Study/StudyRelated/listall/' . $study_summary_id);
			}
		} else {

...
class StudyEthicsBoardsController extends StudyAppController {
	...
	function listall($study_summary_id) {
		...
		// MANAGE DATA
		$study_ethics_board_data = $this->StudySummary->find('first',
			array('conditions' => array('StudySummary.id' => $study_summary_id), 'recursive' => '-1'));
		if (empty($study_ethics_board_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		}

		$this->request->data = $this->paginate($this->StudyEthicsBoard,
			array('StudyEthicsBoard.study_summary_id' => $study_summary_id));
	...
	function detail($study_summary_id, $study_ethics_board_id) {
		...
		$this->set('atim_menu_variables',
			array('StudySummary.id' => $study_summary_id, 'StudyEthicsBoard.id' => $study_ethics_board_id));
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyEthicsBoard->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/Study/StudyEthicsBoards/detail/' . $study_summary_id . '/' . $this->StudyEthicsBoard->id);
				}
			}
		}
	}
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyEthicsBoard->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/Study/StudyEthicsBoards/detail/' . $study_summary_id . '/' . $study_ethics_board_id);
				}
			}
		}
	}
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			if ($this->StudyEthicsBoard->atimDelete($study_ethics_board_id)) {
				$this->atimFlash(__('your data has been deleted'),
					'/Study/StudyEthicsBoards/listall/' . $study_summary_id);
			} else {

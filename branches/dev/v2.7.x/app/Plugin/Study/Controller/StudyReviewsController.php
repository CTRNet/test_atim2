...
class StudyReviewsController extends StudyAppController {
	...
	function listall($study_summary_id) {
		...
		// MANAGE DATA
		$study_reviews_data = $this->StudySummary->find('first',
			array('conditions' => array('StudySummary.id' => $study_summary_id), 'recursive' => '-1'));
		if (empty($study_reviews_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		}

		$this->request->data = $this->paginate($this->StudyReview,
			array('StudyReview.study_summary_id' => $study_summary_id));
	...
	function detail($study_summary_id, $study_reviews_id) {
		...
		// MANAGE DATA
		$study_reviews_data = $this->StudyReview->find('first', array(
			'conditions' => array(
				'StudyReview.id' => $study_reviews_id,
				'StudyReview.study_summary_id' => $study_summary_id
			)
		));
		if (empty($study_reviews_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		...
		$this->set('atim_menu_variables',
			array('StudySummary.id' => $study_summary_id, 'StudyReview.id' => $study_reviews_id));
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyReview->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/Study/StudyReviews/detail/' . $study_summary_id . '/' . $this->StudyReview->id);
				}
			}
		}
	}
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyReview->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/Study/StudyReviews/detail/' . $study_summary_id . '/' . $study_reviews_id);
				}
			}
		}
	}
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			if ($this->StudyReview->atimDelete($study_reviews_id)) {
				...
				$this->atimFlash(__('error deleting data - contact administrator.'),
					'/Study/StudyReviews/listall/' . $study_summary_id);
			}
		} else {

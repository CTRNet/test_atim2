...
class MiscIdentifiersController extends ClinicalAnnotationAppController {
	...
	public function search($search_id = '') {
		...
		$this->searchHandler($search_id, $this->MiscIdentifier, 'miscidentifiers_for_participant_search',
			'/ClinicalAnnotation/MiscIdentifiers/search');
	...
	public function add($participant_id, $misc_identifier_control_id) {
		...
		if ($controls['MiscIdentifierControl']['flag_once_per_participant']) {
			...
			if ($already_exist) {
				$this->atimFlash(__('this identifier has already been created for this participant'),
		...
		$this->set('atim_menu_variables',
			array('Participant.id' => $participant_id, 'MiscIdentifierControl.id' => $misc_identifier_control_id));
		...
		if (empty($this->request->data) && $display_add_form) {
			...
			if (!$is_incremented_identifier) {
				$this->request->data['MiscIdentifier']['identifier_value'] = str_pad($this->request->data['MiscIdentifier']['identifier_value'],
					$controls['MiscIdentifierControl']['pad_to_length'], '0', STR_PAD_LEFT);
			}
			...
			if ($submitted_data_validates) {
				...
				if ($is_incremented_identifier) {
					$new_identifier_value = $this->MiscIdentifierControl->getKeyIncrement($controls['MiscIdentifierControl']['autoincrement_name'],
						$controls['MiscIdentifierControl']['misc_identifier_format'],
						$controls['MiscIdentifierControl']['pad_to_length']);
	...
	public function edit($participant_id, $misc_identifier_id) {
		$this->Participant->getOrRedirect($participant_id);
		...
		$this->set('atim_menu', $this->Menus->get('/ClinicalAnnotation/Participants/profile'));
		$this->set('atim_menu_variables',
			array('Participant.id' => $participant_id, 'MiscIdentifier.id' => $misc_identifier_id));
		...
		if (empty($this->request->data)) {
			...
			if (!$is_incremented_identifier && $misc_identifier_data['MiscIdentifierControl']['pad_to_length']) {
				$this->request->data['MiscIdentifier']['identifier_value'] = str_pad($this->request->data['MiscIdentifier']['identifier_value'],
					$misc_identifier_data['MiscIdentifierControl']['pad_to_length'], '0', STR_PAD_LEFT);
			}
	...
	public function delete($participant_id, $misc_identifier_id) {
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			if (empty($misc_identifier_data['MiscIdentifierControl']['autoincrement_name'])) {
				...
				$this->MiscIdentifier->updateAll(array('participant_id' => null, 'tmp_deleted' => 1, 'deleted' => 1),
					array('MiscIdentifier.id' => $misc_identifier_id));
				$this->MiscIdentifier->afterDelete();
				$this->Participant->id = $participant_id;
				$this->Participant->data = array();
				$this->Participant->save(array('Participant.modified' => date("Y-m-d H:i:s")));//trigger last modification
			}
	...
	public function reuse($participant_id, $misc_identifier_ctrl_id, $submited = false) {
		$this->Participant->getOrRedirect($participant_id);
		$this->MiscIdentifierControl->getOrRedirect($misc_identifier_ctrl_id);
		$this->set('atim_menu', $this->Menus->get('/ClinicalAnnotation/Participants/profile'));
		$this->set('atim_menu_variables',
			array('Participant.id' => $participant_id, 'MiscIdentifierControl.id' => $misc_identifier_ctrl_id));

...
class TreatmentExtendMastersController extends ClinicalAnnotationAppController {
	...
	public function add($participant_id, $tx_master_id) {
		// Get treatment data
		...
		if (!$tx_master_data['TreatmentControl']['treatment_extend_control_id']) {
			$this->atimFlash(__('no additional data has to be defined for this type of treatment'),
		...
		$this->set('atim_menu',
			$this->Menus->get('/ClinicalAnnotation/TreatmentMasters/detail/%%Participant.id%%/%%TreatmentMaster.id%%'));
	...
	public function edit($participant_id, $tx_master_id, $tx_extend_id) {
	...
	public function delete($participant_id, $tx_master_id, $tx_extend_id) {
		// Get treatment extend data
		$tx_extend_data = $this->TreatmentExtendMaster->getOrRedirect($tx_extend_id);
		if ($tx_extend_data['TreatmentMaster']['id'] != $tx_master_id) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
	...
	public function importDrugFromChemoProtocol($participant_id, $tx_master_id) {
		$tx_master_data = $this->TreatmentMaster->getOrRedirect($tx_master_id);
		if ($tx_master_data['TreatmentMaster']['participant_id'] != $participant_id) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		}
		...
		if (is_numeric($tx_master_data['TreatmentMaster']['protocol_master_id'])) {
			...
			$prot_extend_data = $this->ProtocolExtendMaster->find('all',
				array('conditions' => array('ProtocolExtendMaster.protocol_master_id' => $tx_master_data['TreatmentMaster']['protocol_master_id'])));
			$data = array();
			if (empty($prot_extend_data)) {
				$this->atimFlash(__('there is no drug defined in the associated protocol'),
					'/ClinicalAnnotation/TreatmentMasters/detail/' . $participant_id . '/' . $tx_master_id);
			} else {
				...
				if ($this->TreatmentExtendMaster->saveAll($data)) {
					$this->atimFlash(__('drugs from the associated protocol were imported'),
						'/ClinicalAnnotation/TreatmentMasters/detail/' . $participant_id . '/' . $tx_master_id);
				} else {
					$this->atimFlash(__('unknown error'),
						'/ClinicalAnnotation/TreatmentMasters/detail/' . $participant_id . '/' . $tx_master_id);
				}
			}
		} else {
			$this->atimFlash(__('there is no protocol associated with this treatment'),
				'/ClinicalAnnotation/TreatmentMasters/detail/' . $participant_id . '/' . $tx_master_id);
		}
	}
}
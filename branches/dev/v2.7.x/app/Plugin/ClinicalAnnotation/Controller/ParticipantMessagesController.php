...
class ParticipantMessagesController extends ClinicalAnnotationAppController {
	...
	public function listall($participant_id) {
		...
		$this->request->data = $this->paginate($this->ParticipantMessage,
			array('ParticipantMessage.participant_id' => $participant_id));
	...
	public function detail($participant_id, $participant_message_id) {
		// MANAGE DATA
		...
		$this->set('atim_menu_variables',
			array('Participant.id' => $participant_id, 'ParticipantMessage.id' => $participant_message_id));
			...
			if ($submitted_data_validates) {
				...
				if ($this->ParticipantMessage->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/ClinicalAnnotation/ParticipantMessages/detail/' . $participant_id . '/' . $this->ParticipantMessage->id);
				}
			}
		}
	}

	public function edit($participant_id, $participant_message_id) {
		...
		if (empty($this->request->data)) {
			...
			if ($submitted_data_validates) {
				...
				if ($this->ParticipantMessage->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/ClinicalAnnotation/ParticipantMessages/detail/' . $participant_id . '/' . $participant_message_id);
				}
			}
		}
	}

	public function delete($participant_id, $participant_message_id) {
		// MANAGE DATA
		$participant_message_data = $this->ParticipantMessage->find('first', array(
			'conditions' => array(
				'ParticipantMessage.id' => $participant_message_id,
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			if ($this->ParticipantMessage->atimDelete($participant_message_id)) {
				$this->atimFlash(__('your data has been deleted'),
					'/ClinicalAnnotation/ParticipantMessages/listall/' . $participant_id);
			} else {
	...
	public function search($search_id = 0) {
		$this->set('atim_menu', $this->Menus->get('/ClinicalAnnotation/Participants/search'));
		$this->searchHandler($search_id, $this->ParticipantMessage, 'participantmessages',
			'/ClinicalAnnotation/ParticipantMessages/search');

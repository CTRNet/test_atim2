...
class ParticipantContactsController extends ClinicalAnnotationAppController {
	...
	public function listall($participant_id) {
		...
		$this->request->data = $this->paginate($this->ParticipantContact,
			array('ParticipantContact.participant_id' => $participant_id));
	...
	public function detail($participant_id, $participant_contact_id) {
		...
		$this->set('atim_menu_variables',
			array('Participant.id' => $participant_id, 'ParticipantContact.id' => $participant_contact_id));
			...
			if ($submitted_data_validates) {
				if ($this->ParticipantContact->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/ClinicalAnnotation/ParticipantContacts/detail/' . $participant_id . '/' . $this->ParticipantContact->id);
				}
			}
		}
	}

	public function edit($participant_id, $participant_contact_id) {
		if (!$participant_id && !$participant_contact_id) {
			$this->redirect('/Pages/err_plugin_funct_param_missing?method=' . __METHOD__ . ',line=' . __LINE__, null,
				true);
		}
		...
		if (empty($this->request->data)) {
			...
			if ($submitted_data_validates) {
				...
				if ($this->ParticipantContact->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/ClinicalAnnotation/ParticipantContacts/detail/' . $participant_id . '/' . $participant_contact_id);
				}
			}
		}
	}

	public function delete($participant_id, $participant_contact_id) {
		if (!$participant_id && !$participant_contact_id) {
			$this->redirect('/Pages/err_plugin_funct_param_missing?method=' . __METHOD__ . ',line=' . __LINE__, null,
				true);
		}
		...
		if ($arr_allow_deletion['allow_deletion']) {
			if ($this->ParticipantContact->atimDelete($participant_contact_id)) {
				$this->atimFlash(__('your data has been deleted'),
					'/ClinicalAnnotation/ParticipantContacts/listall/' . $participant_id);
			} else {

...
class Participant extends ClinicalAnnotationAppModel {
	...
	function summary($variables = array()) {
		...
		if (isset($variables['Participant.id'])) {
			$result = $this->find('first',
				array('conditions' => array('Participant.id' => $variables['Participant.id'])));
	...
	function allowDeletion($participant_id) {
		...
		$nbr_linked_collection = $collection_model->find('count',
			array('conditions' => array('Collection.participant_id' => $participant_id)));
		...
		$nbr_consents = $consent_master_model->find('count',
		...
		$nbr_diagnosis = $diagnosis_master_model->find('count',
		...
		$nbr_familyhistory = $family_history_model->find('count',
			array('conditions' => array('FamilyHistory.participant_id' => $participant_id)));
		...
		$nbr_reproductive = $reproductive_history_model->find('count',
			array('conditions' => array('ReproductiveHistory.participant_id' => $participant_id)));
		...
		$nbr_contacts = $participant_contact_model->find('count', array(
			'conditions' => array(
				...
				array('OR' => array('ParticipantContact.confidential != 1', 'ParticipantContact.confidential = 1'))
			)
		));
		if ($nbr_contacts > 0) {
			$arr_allow_deletion['allow_deletion'] = false;
		...
		$nbr_identifiers = $misc_identifier_model->find('count',
			array('conditions' => array('MiscIdentifier.participant_id' => $participant_id)));
		...
		$nbr_messages = $participant_message_model->find('count',
			array('conditions' => array('ParticipantMessage.participant_id' => $participant_id)));
		if ($nbr_messages > 0) {
			$arr_allow_deletion['allow_deletion'] = false;
	...
	function beforeSave($options = array()) {
		if ($this->whitelist && !in_array('last_modification', $this->whitelist)) {
			$this->whitelist = array_merge($this->whitelist, array('last_modification', 'last_modification_ds_id'));
		}

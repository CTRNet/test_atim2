...
class EventMaster extends ClinicalAnnotationAppModel {
	...
	function summary($variables = array()) {
		...
		if (isset($variables['EventMaster.id'])) {
			...
			$result = $this->find('first',
				array('conditions' => array('EventMaster.id' => $variables['EventMaster.id'])));

			$return = array(
				'menu' => array(
					null,
					__($result['EventControl']['event_type'],
						true) . (empty($result['EventControl']['disease_site']) ? '' : ' - ' . __($result['EventControl']['disease_site'],
							true))
				),
	...
	static function generateDxCompatWarnings(array $diagnosis_data, array $event_data) {
		...
		foreach ($to_check as $field => $language_label) {
			if (array_key_exists($field, $event_data) && $diagnosis_data[$field] != $event_data[$field]) {
				AppController::addWarningMsg(sprintf(__('the diagnosis value for %s does not match the cap report value'),
						__($language_label)));
			}
		}
	}
	...
	function allowDeletion($event_master_id) {
		...
		if ($collection_model->find('first',
			array('conditions' => array('Collection.event_master_id' => $event_master_id)))
		) {
	...
	function calculatedDetailFields(array &$data) {
		if (($data['EventControl']['detail_tablename'] == 'ed_all_lifestyle_smokings') && array_key_exists('started_on',
				$data['EventDetail']) && array_key_exists('stopped_on', $data['EventDetail'])
		) {
			...
			if (!empty($data['EventDetail']['started_on']) && $data['EventDetail']['started_on_accuracy'] == 'c' && !empty($data['EventDetail']['stopped_on']) && $data['EventDetail']['stopped_on_accuracy'] == 'c') {
				$data['EventDetail']['smoked_for'] = AppModel::manageSpentTimeDataDisplay(AppModel::getSpentTime($data['EventDetail']['started_on'] . ' 00:00:00',
					$data['EventDetail']['stopped_on'] . ' 00:00:00'), false);
			...
			if (!empty($data['EventDetail']['stopped_on']) && $data['EventDetail']['stopped_on_accuracy'] == 'c') {
				$data['EventDetail']['stopped_since'] = AppModel::manageSpentTimeDataDisplay(AppModel::getSpentTime($data['EventDetail']['stopped_on'] . ' 00:00:00',

...
class ClinicalAnnotationAppModel extends AppModel {
	...
	function afterSave($created, $options = Array()) {
		if ($this->name != 'Participant') {
			...
			if (isset($this->data[$this->name]['participant_id'])) {
				...
				if ($this->name == 'TreatmentExtendMaster') {
					...
					$tx_data = $treatment_master->find('first', array(
						'conditions' => array('TreatmentMaster.id' => $this->data['TreatmentExtendMaster']['treatment_master_id']),
						'fields' => array('TreatmentMaster.participant_id')
					));
			...
			$datamart_structure = $datamart_structure_model->find('first',
				array('conditions' => array('DatamartStructure.model' => $name)));
			...
			if ($participant_id) {
				...
				$participant_model->save(array(
					'last_modification' => $this->data[$this->name]['modified'],
					'last_modification_ds_id' => $datamart_structure['DatamartStructure']['id']
				));

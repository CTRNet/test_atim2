...
if ($use_addgrid) {
	...
	$final_options['settings'] = array_merge($final_options['settings'],
		array('pagination' => false, 'add_fields' => true, 'del_fields' => true));
...
$final_options = array(
	...
	'links' => $structure_links,
	'extras' => array(
		'start' => '<input type="radio" name="data[TreatmentMaster][diagnosis_master_id]" ' . ($radio_checked ? 'checked="checked"' : '') . ' value=""/>' . __('n/a',
				null)
	)
);

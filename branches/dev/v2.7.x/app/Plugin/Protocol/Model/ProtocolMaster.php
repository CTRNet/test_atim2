...
class ProtocolMaster extends ProtocolAppModel {
	...
	function summary($variables = array()) {
		...
		if (isset($variables['ProtocolMaster.id'])) {
			...
			$result = $this->find('first',
				array('conditions' => array('ProtocolMaster.id' => $variables['ProtocolMaster.id'])));
	...
	function getProtocolPermissibleValuesFromId($protocol_control_id = null) {
		...
		if (!self::$protocol_dropdown_set) {
			...
			foreach ($this->find('all',
				array('conditions' => $criteria, 'order' => 'ProtocolMaster.code')) as $new_protocol) {
	...
	function isLinkedToTreatment($protocol_master_id) {
		...
		$nbr_trt_masters = $this->TreatmentMaster->find('count', array(
			'conditions' => array('TreatmentMaster.protocol_master_id' => $protocol_master_id),
			'recursive' => '-1'
		));

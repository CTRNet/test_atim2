...
class ProtocolExtendMastersController extends ProtocolAppController {
	...
	function listall($protocol_master_id){
		...
		if(!$protocol_master_data['ProtocolControl']['protocol_extend_control_id']){
			$this->atimFlash(__('no additional data has to be defined for this type of protocol'), '/Protocol/ProtocolMasters/detail/'.$protocol_master_id);
			return;
		}
	...
	function delete($protocol_master_id, $protocol_extend_master_id) {
		...
		if ($arr_allow_deletion['allow_deletion']) {
			if( $this->ProtocolExtendMaster->atimDelete( $protocol_extend_master_id ) ) {
				...
				$this->atimFlash(__('error deleting data - contact administrator'), '/Protocol/ProtocolMasters/detail/'.$protocol_master_id);
			}
		} else {
			$this->atimFlash(__($arr_allow_deletion['msg']), '/Protocol/ProtocolExtendMaster/detail/'.$protocol_master_id.'/'.$protocol_extend_master_id);
		}

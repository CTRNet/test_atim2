 ...
 * @var SampleMaster
 */
	public $previous_model = null; //some model, only provides accc
	...
	public static $writableFields = array();//whether to prevent data to be saved if the data array contains a pkey different than model->id

	protected $_registeredModels;
	...
	protected static $locked_views_update = false;

	//tablename -> flag suffix -> fields
	protected static $cached_views_update = array();

	//The values in this array can trigger magic actions when applied to a field settings
	protected static $cached_views_delete = array();

	protected static $cached_views_insert = array();//use related to views

	protected static $cached_views_model = null;

/**
 * This is convenient for search based on master/detail detail table.
 *
 * @param int $id (see parent::__construct)
 * @param string $table (see parent::__construct)
 * @param string $ds (see parent::__construct)
 ...
 * @desc If $base_model_name and $detail_table are not null, a new hasOne relationship is created before calling
...
/**
 * get Magic Coding Icd Trigger Array
 *
 * @return array
 */
 ...
 *
 * @param Model $class The class to instantiate
 * @param string $table_name The table to use
 *
 * @return Model The instantiated class
 */
 ...
 *
 * @return array Return an array that contains the spent time
 ...
 */
	public static function getSpentTime($start_date, $end_date) {
		...
		if (empty($start_date) || empty($end_date) || (strcmp($start_date, $empty_date) == 0) || (strcmp($end_date,
					$empty_date) == 0)
		) {
 ...
 *
 * @return int|false Return unix time stamp of the date.
 *
 * @author N. Luc
 * @since 2007-06-20
 */
...
/**
 * Manage Spent Time Data Display
 *
 * @param array $spent_time_data Should contain a key 'message'
 * @param bool $with_time if message is null and time should be output
 *
 * @return mixed|string
 */
		...
		if (!empty($spent_time_data)) {
			if (!is_null($spent_time_data['message'])) {
				if ($spent_time_data['message'] == '0') {
					...
					if (strcmp('error in the date definitions', $spent_time_data['message']) == 0) {
				...
				if ($with_time) {
					$spent_time_msg = AppModel::translateDateValueAndUnit($spent_time_data,
							'days') . AppModel::translateDateValueAndUnit($spent_time_data,
							'hours') . AppModel::translateDateValueAndUnit($spent_time_data, 'minutes');
				} else {
					$spent_time_data['days'] = $spent_time_data['days_mod_years'];
					$spent_time_msg = AppModel::translateDateValueAndUnit($spent_time_data,
							'years') . AppModel::translateDateValueAndUnit($spent_time_data, 'days');
				}
			}
		}
 ...
 */
	public static function translateDateValueAndUnit($spent_time_data, $time_unit) {
		if (array_key_exists($time_unit, $spent_time_data)) {
			return (((!empty($spent_time_data[$time_unit])) && ($spent_time_data[$time_unit] != '00')) ? ($spent_time_data[$time_unit] . ' ' . ($spent_time_data[$time_unit] == 1 ? __(substr($time_unit,
					0, -1)) : __($time_unit)) . ' ') : '');
		}
		return '#err#';
	}
 ...
 *
 * @return array The data sorted if the passed_args were compatible with it
 */
	public static function sortWithUrl(array $data, array $passedArgs) {
		$order = array();
		if (isset($passedArgs['sort'])) {
			$result = array();
			list($sort_model, $sort_field) = explode(".", $passedArgs['sort']);
			$i = 0;
 ...
 */
	public static function manageViewUpdate($model_table, $foreign_key, $ids, $query_part) {
		if (self::$locked_views_update) {
			...
			self::$cached_views_update[$model_table][$foreign_key][$query_part]["ids"] = array_merge(self::$cached_views_update[$model_table][$foreign_key][$query_part]["ids"],
				$ids);
		} else {
			$table_query = str_replace('%%WHERE%%', 'AND ' . $foreign_key . ' IN (' . implode(',', $ids) . ')',
				$query_part);
...
/**
 * release Batch Views Update Lock
 */
	public static function releaseBatchViewsUpdateLock() {
		...
		foreach (self::$cached_views_update as $model_table => $models) {
			foreach ($models as $foreign_key => $query_parts) {
				foreach ($query_parts as $query_part => $details) {
					$table_query = str_replace('%%WHERE%%',
						'AND ' . $foreign_key . ' IN(' . implode(",", array_unique($details['ids'])) . ')',
						$query_part);
					$query = sprintf('REPLACE INTO %s (%s)', $model_table, $table_query);
					$pages->tryCatchquery($query);
				}
		...
		foreach (self::$cached_views_delete as $model_table => $models) {
			foreach ($models as $primary_key => $details) {
				$query = sprintf('DELETE FROM %s  WHERE %s IN (%s)', $model_table, $primary_key, implode(',',
					array_unique($details['pkeys_for_deletion'])));    //To fix issue#2980: Edit Storage & View Update
				$pages->tryCatchquery($query);
			}
		}
		foreach (self::$cached_views_insert as $model_table => $base_models) {
			foreach ($base_models as $base_model => $query_parts) {
				foreach ($query_parts as $query_part => $details) {
					$table_query = str_replace('%%WHERE%%',
						'AND ' . $base_model . '.id IN(' . implode(", ", array_unique($details['ids'])) . ')',
						$query_part);
 ...
 */
	public function save($data = null, $validate = true, $fieldList = array()) {
		if ($this->pkeySafeguard && ((isset($data[$this->name][$this->primaryKey]) && $this->id != $data[$this->name][$this->primaryKey]) || (isset($data[$this->primaryKey]) && $this->id != $data[$this->primaryKey]))) {
			...
			AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__,
				null, true);
			return false;
		}
		...
		if ((!isset($data[$this->name]) || empty($data[$this->name])) && isset($this->Behaviors->MasterDetail->settings[$this->name]['is_master_model']) && $this->Behaviors->MasterDetail->settings[$this->name]['is_master_model'] && isset($data[$this->Behaviors->MasterDetail->settings[$this->name]['detail_class']])) {
 ...
 * @return array
 **/
	protected function filter_move_files(&$data) {
		$move_files = array();
		if (!is_array($data)) {
			return $move_files;
		}
 ...
 * @param int $pad_to_length - The min length of the key increment part. If the retrieved key is too short, 0 will
 *     be prepended.
 *
 ...
 */
	public function getKeyIncrement($key, $str, $pad_to_length = 0) {
		...
		$this->query('UNLOCK TABLES');
		return str_replace("%%key_increment%%",
			str_pad($result[0]['key_increments']['key_value'], $pad_to_length, '0', STR_PAD_LEFT), $str);
	}
 ...
 */
	private function checkWritableFields() {
		if (isset(AppModel::$writableFields[$this->table])) {
			...
			$real_fields = isset($this->data[$this->name]) ? array_intersect(array_keys($this->data[$this->name]),
				$schema_keys) : array();
			...
			if (!empty($invalid_fields)) {
				...
				if (Configure::read('debug') > 0) {
					AppController::addWarningMsg('Non authorized fields have been removed from the data set prior to saving. (' . implode(',',
							$invalid_fields) . ')', true);
				}
			}
		} else {
		...
		foreach ($this->_schema as $field_name => $field_properties) {
			...
			if ($tmp_type == "float" || $tmp_type == "number" || $tmp_type == "float_positive") {
				...
				if (isset($this->data[$this->alias][$field_name])) {
					$this->data[$this->alias][$field_name] = str_replace(",", ".",
						$this->data[$this->alias][$field_name]);
					$this->data[$this->alias][$field_name] = str_replace(" ", "",
						$this->data[$this->alias][$field_name]);
					$this->data[$this->alias][$field_name] = str_replace("+", "",
						$this->data[$this->alias][$field_name]);
					if (is_numeric($this->data[$this->alias][$field_name])) {
						...
						if (strpos($this->data[$this->alias][$field_name], "-.") === 0) {
							$this->data[$this->alias][$field_name] = "-0" . substr($this->data[$this->alias][$field_name],
									1);
						}
					}
				}
...
/**
 * ATiM 2.0 function - used instead of Model->delete, because SoftDelete Behaviour will always return a FALSE
 *
 * @param $model_id
 * @param bool $cascade
 *
 ...
 */
	private function updateWritableField($field, $tablename = null, $add) {
		...
		AppModel::$writableFields[$tablename][$add_into] = array_unique(array_merge(AppModel::$writableFields[$tablename][$add_into],
			$field));

		if (isset(AppModel::$writableFields[$this->table][$remove_from])) {
			AppModel::$writableFields[$this->table][$remove_from] = array_diff(AppModel::$writableFields[$this->table][$remove_from],
				$field);
		}
	}
	...
	private function updateRegisteredModels() {
		foreach ($this->_registeredModels as $registered_model) {
			...
			if (self::$locked_views_update) {
				...
				self::$cached_views_delete[$model->table][$model->primaryKey]["pkeys_for_deletion"] = array_merge(self::$cached_views_delete[$model->table][$model->primaryKey]["pkeys_for_deletion"],
					$registered_model['pkeys_for_deletion']);
			} else {
				$query = sprintf('DELETE FROM %s  WHERE %s IN (%s)', $model->table, $model->primaryKey, implode(',',
					$registered_model['pkeys_for_deletion']));    //To fix issue#2980: Edit Storage & View Update
			...
			foreach (explode("UNION ALL", $model::$table_query) as $query_part) {
				...
				if ($ids) {
					if (self::$locked_views_update) {
						...
						self::$cached_views_insert[$model->table][$base_model][$query_part]["ids"] = array_merge(self::$cached_views_insert[$model->table][$base_model][$query_part]["ids"],
							$ids);
					} else {
						$table_query = str_replace('%%WHERE%%',
							'AND ' . $base_model . '.id IN(' . implode(", ", $ids) . ')', $query_part);
...
/**
 * ATiM 2.0 function - acts like find('all') but returns array with ID values as arrays key values
 *
 * @param array $options
 *
 * @return array|bool
...
/**
 * Deconstructs a complex data type (array or object) into a single field value. Copied from CakePHP core since
		...
		if (in_array($type, array('datetime', 'timestamp', 'date', 'time'))) {
			...
			if (strlen($data['year']) > 0 || strlen($data['month']) > 0 || strlen($data['day']) > 0 || strlen($data['hour']) > 0 || strlen($data['min']) > 0) {
				...
				if ($got_date && $got_time) {
					$result = sprintf("%s-%s-%s %s:%s:%s", $data['year'], $data['month'], $data['day'], $data['hour'],
						$data['min'], $data['sec']);
				} else {
					if ($got_date) {
 ...
 */
	public function validates($options = array()) {
		...
		if (!isset(self::$autoValidation[$this->name]) && isset($this->Behaviors->MasterDetail) && (strpos($this->name,
					'Detail') === false || !array_key_exists(str_replace('Detail', 'Master', $this->name),
					$this->Behaviors->MasterDetail->settings))
		) {
		...
		if (isset($this->Behaviors->MasterDetail->settings[$this->name]['is_master_model']) && $this->Behaviors->MasterDetail->settings[$this->name]['is_master_model']) {
			...
			if (isset($this->data[$master_class][$control_foreign]) && $this->data[$master_class][$control_foreign]) {
				...
				$associated = $this->$control_class->find('first',
					array('conditions' => array($control_class . '.id' => $this->data[$master_class][$control_foreign])));
			} else {
				if (isset($this->id) && is_numeric($this->id)) {
					// else, if EDIT, use MODEL.ID to get row and find CONTROL_ID that way...
					$associated = $this->find('first',
						array('conditions' => array($master_class . '.id' => $this->id)));
				} else {
					if (isset($this->data[$master_class]['id']) && is_numeric($this->data[$master_class]['id'])) {
						...
						$associated = $this->find('first',
							array('conditions' => array($master_class . '.id' => $model->data[$this]['id'])));
					}
				}
			}
			...
			if ($associated == null || empty($associated)) {
				...
				AppController::getInstance()->redirect('/Pages/err_internal?p[]=' . __CLASS__ . " @ line " . __LINE__ . " (the detail control id was not found for " . $master_class . ")",
					null, true);
				exit;
			}
			...
			if ($use_form_alias) {
				...
				if (isset(AppController::getInstance()->{$detail_class}) && (!isset($params['validate']) || $params['validate'])) {
					...
					foreach (self::$autoValidation[$auto_validation_name] as $field_name => $rules) {
						...
						$detail_class_instance->validate[$field_name] = array_merge($detail_class_instance->validate[$field_name],
							$rules);
					}
					...
					if (!$valid_detail_class) {
						...
						$this->validationErrors = array_merge($this->validationErrors,
							$detail_class_instance->validationErrors);
					}
				}
			}
		}
				...
				switch ($field_data['type']) {
					...
					case 'float':
						...
						if ($field_data['length']) {
							...
							if ($field_data['type'] == 'float unsigned') {
								$min = "0";
							}
						}
						$auto_validation[$field_name][] = array(
					...
					case 'integer':
						$rule = null;
						if (strpos($field_data['type'], 'int') === 0) {
							if (strpos($field_data['type'], 'unsigned') === false) {
								$rule = array('range', -2147483649, 2147483648);
							} else {
								$rule = array('range', -1, 4294967295);
							}
						} else {
							if (strpos($field_data['type'], 'tinyint') === 0) {
								if (strpos($field_data['type'], 'unsigned') === false) {
								...
								if (strpos($field_data['type'], 'smallint') === 0) {
									if (strpos($field_data['type'], 'unsigned') === false) {
									...
									if (strpos($field_data['type'], 'mediumint') === 0) {
										if (strpos($field_data['type'], 'unsigned') === false) {
										...
										if (strpos($field_data['type'], 'bigint') === 0) {
											if (strpos($field_data['type'], 'unsigned') === false) {
											...
											AppController::addWarningMsg('Unknown integer type for field [' . $model->name . '.' . $field_name,
												true);
										}
									}
								}
 ...
 */
	public function getPluginName() {
		...
		$matches = array();
		if (preg_match('#' . str_replace('/', '[\\\/]', '/app/Plugin/([\w\d]+)/') . '#', $class->getFileName(),
			$matches)) {
			return $matches[1];
		}
		return null;
 ...
 */
	public function redirectIfNonExistent($id, $method, $line, $return = false) {
		...
		if ($result = $this->read()) {
			...
			AppController::getInstance()->redirect('/Pages/err_plugin_no_data?method=' . $method . ',line=' . $line,
				null, true);
		}
		return null;
	}
 ...
 *
 * @return array The model data if it succeeds
 */
	public function getOrRedirect($id) {
		...
		$bt = debug_backtrace();
		AppController::getInstance()->redirect('/Pages/err_plugin_no_data?method=' . $bt[1]['function'] . ',line=' . $bt[0]['line'],
			null, true);
		return null;
	}
			...
			if (!isset(Browser::$cache['path'])) {
				...
				while ($current = array_pop($path)) {
					if ($current['BrowsingResult']['raw']) {
						if (array_key_exists($current['DatamartStructure']['model'],
							Browser::$cache['allowed_models'])) {
							break;
						}
			...
			foreach ($this->browsing_search_dropdown_info[$field_name] as $key => $option) {
				if (array_key_exists($option['model'], Browser::$cache['allowed_models'])) {
					$sfs = $sfs_model->find('first', array(
						'fields' => array('language_label'),
						'conditions' => array('Sfs.field' => $option['field'], 'Sfs.model' => $option['model']),
						'recursive' => -1
					));
				...
				foreach ($current_fields_replace as $field_name => $options) {
					if (isset($options['msg'][$result[$this->name][$field_name]])) {
						...
						if ($options['type'] == 'spentTime') {
							...
							$spent_time = AppModel::translateDateValueAndUnit($time,
									'days') . '' . AppModel::translateDateValueAndUnit($time,
									'hours') . AppModel::translateDateValueAndUnit($time, 'minutes');
				...
				foreach ($foreign_keys as $foreign_key) {
					...
					foreach (explode("UNION ALL", $model::$table_query) as $query_part) {
						...
						if (self::$locked_views_update) {
							...
							array_push(self::$cached_views_update[$model->table][$foreign_key][$query_part]["ids"],
								$this->id);
						} else {
							$table_query = str_replace('%%WHERE%%', 'AND ' . $foreign_key . '=' . $this->id,
								$query_part);

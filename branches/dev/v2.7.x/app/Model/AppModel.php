				...
				switch($field_data['type']){
					...
					case 'float':
						...
						if($field_data['length']){
							...
							$min = -1 * $max;
						}else if($field_data['type'] == 'float unsigned'){
							$min = "0";
						}
						$auto_validation[$field_name][] = array(
					...
					case 'integer':
						...
						}else if(strpos($field_data['type'], 'tinyint') === 0){
							if(strpos($field_data['type'], 'unsigned') === false){
								$rule = array('range', -129, 128);
							}else{
								$rule = array('range', -1, 256);
							}
						}else if(strpos($field_data['type'], 'smallint') === 0){
							if(strpos($field_data['type'], 'unsigned') === false){
								$rule = array('range', -32769, 32768);
							}else{
								$rule = array('range', -1, 65536);
							}
						}else if(strpos($field_data['type'], 'mediumint') === 0){
							if(strpos($field_data['type'], 'unsigned') === false){
								$rule = array('range', -8388609, 8388608);
							}else{
								$rule = array('range', -1, 16777216);
							}
						}else if(strpos($field_data['type'], 'bigint') === 0){
							if(strpos($field_data['type'], 'unsigned') === false){

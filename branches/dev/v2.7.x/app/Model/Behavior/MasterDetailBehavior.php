...
class MasterDetailBehavior extends ModelBehavior {
	...
	public function setup(Model $model, $config = array()) {
		...
		if (strpos($model->alias, 'Master') || strpos($model->alias,
				'Control') || (isset($model->base_model) && strpos($model->base_model, 'Master'))
		) {
			$modelToUse = $model;
			...
			if ($isControlModel) {
				...
				if (isset($schema['detail_form_alias'])) {
					$aliasName = isset($model->master_form_alias) ? 'CONCAT("' . $model->master_form_alias . ',",' . $model->alias . '.detail_form_alias)' : $model->alias . '.detail_form_alias';
		...
		if ($is_master_model) {
			...
			if ($primary && isset($results[0][$control_class][$detail_field]) && $model->recursive > 0) {
				...
				foreach ($results as $key => &$result) {
					if (!isset($results[$key][$detail_class])) {//the detail model is already defined if it was a find on a specific control_id
						...
						if (!isset($grouping[$detail_model_cache_key])) {
							...
							$grouping[$detail_model_cache_key]['model'] = new AppModel(array(
								'table' => $result[$control_class][$detail_field],
								'name' => $detail_class,
								'alias' => $detail_class . '_' . $result[$control_class][$detail_field]
							));
				...
				foreach ($grouping as $group) {
					...
					$detail_data = $group['model']->find('all',
						array('conditions' => array($master_foreign => array_keys($id_to_index)), 'recursive' => -1));
				...
				if (isset($results[$control_class][$detail_field]) && !isset($results[$detail_class])) {
					...
					$associated = $detail_model->find(array($master_foreign => $results[0][$model->alias]['id']), null,
						null, -1);
	...
	public function getSingleControlIdCondition(Model $model, $query) {
		extract($this->settings[$model->alias]);
		if (is_array($query['conditions']) && array_key_exists($model->name . "." . $control_foreign,
				$query['conditions']) && count($query['conditions'][$model->name . "." . $control_foreign]) == 1
		) {
	...
	public function getDetailJoin(Model $model, $control_id, $alternate_model_name = null) {
		...
		$detail_info = $detail_control->find('first',
			array('conditions' => array($detail_control->name . ".id" => $control_id)));
	...
	public function afterSave(Model $model, $created, $options = Array()) {
		...
		if (!$is_view && ($is_master_model || $is_control_model)) {
			...
			$associated = $model->find('first', array(
				'conditions' => array(
					$master_class . '.id' => $model->id,
					$master_class . '.deleted' => array('0', '1')
				),
				'recursive' => 0
			));
			...
			$detail_model = new AppModel(array(
				'table' => $table,
				'name' => $detail_class,
				'alias' => $detail_class . '_' . $table,
				'primaryKey' => $master_foreign
			));
			...
			if ((isset($detail_model->id) && $detail_model->id && !$created) || $created) {
				$result = $detail_model->save($model->data[$detail_class],
					false);//validation should have already been done
			} else {
				$result = true;
			}
	...
	function getDetailModel($model, $control_id) {
		...
		$detail_control = AppModel::getInstance($plugin, $detail_control_name, true);
		$detail_info = $detail_control->find('first', array(
			'conditions' => array($detail_control->name . ".id" => $control_id)
		));

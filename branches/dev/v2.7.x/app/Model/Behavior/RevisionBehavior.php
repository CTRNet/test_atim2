...
class RevisionBehavior extends ModelBehavior {
	...
	private function createShadowModel(&$Model) {
		...
		$model_name = $this->settings[$Model->alias]['model'];
		if (is_string($model_name)) {
			if (App::import('model', $model_name)) {
				$Model->ShadowModel = new $model_name(false, $shadow_table, $dbConfig);
			} else {
	 ...
	 * @example $this->Post->id = 4; $today = $this->Post->revisions(array('conditions'=>array('version_create
	 *     >'=>'2008-12-10')));
	 *
	 * @param object $Model
	 * @param array $options
	...
	public function revisions(&$Model, $options = array(), $include_current = false) {
		...
		if (isset($options['conditions'])) {
			$options['conditions'] = am($options['conditions'],
		...
		if ($include_current == false) {
			$current = $this->newest($Model,
				array('fields' => array($Model->alias . '.version_id', $Model->primaryKey)));
	...
	public function revertToDate(&$Model, $datetime, $cascade = false, $force_delete = false) {
		...
		if ($cascade) {
			...
			foreach ($associated as $assoc => $data) {
				...
				$children = $Model->$assoc->find('list',
					array('conditions' => array($data['foreignKey'] => $Model->id), 'recursive' => -1));
				if (!empty($children)) {
	...
	public function beforeDelete(Model $model, $cascade = true) {
		...
		foreach ($model->hasAndBelongsToMany as $assocAlias => $a) {
			if (isset($model->{$assocAlias}->ShadowModel->_schema[$model->alias])) {
				...
				$this->deleteUpdates[$model->alias][$assocAlias] = Set::extract($joins,
					'/' . $a['with'] . '/' . $a['associationForeignKey']);
			}
		}
		return true;
	}

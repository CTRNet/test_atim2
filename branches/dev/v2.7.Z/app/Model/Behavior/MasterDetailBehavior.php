...
class MasterDetailBehavior extends ModelBehavior
{
        ...
        if ($is_master_model) {
            ...
            if ($primary && isset($results[0][$control_class][$detail_field]) && $model->recursive > 0) {
                ...
                foreach ($grouping as $group) {
                    ...
                    $detail_data = $group['model']->find('all', array(
                        'conditions' => array(
                            $master_foreign => array_keys($id_to_index)
                        ),
                        'recursive' => - 1
                    ));
    ...
    {
        extract($this->__settings[$model->alias]);
        if (is_array($query['conditions']) && array_key_exists($model->name . "." . $control_foreign, $query['conditions']) && count($query['conditions'][$model->name . "." . $control_foreign]) == 1) {
    ...
    public function getDetailJoin(Model $model, $control_id, $alternate_model_name = null)
    {
        ...
        $detail_info = $detail_control->find('first', array(
            'conditions' => array(
                $detail_control->name . ".id" => $control_id
            )
        ));
    ...
    public function beforeFind(Model $model, $query)
    {
        ...
        if ($is_master_model) {
            ...
            if ($control_id !== false) {
                ...
                foreach ($query['joins'] as $new_join_to_check)
                    if ($detail_join['alias'] == $new_join_to_check['alias'])
                        $detail_join = null;
                if ($detail_join)
                    $query['joins'][] = $detail_join;
            }
        }
        return $query;
    }

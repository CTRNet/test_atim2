...
class User extends AppModel
{
    ...
    {
        $all_users_data = $this->find('all', array(
            'recursive' => '-1'
        ));
        $result = array();
    ...
    function savePassword(array $data, $modified_by_user = true)
    {
        ...
        if ($this->validatePassword($data)) {
            ...
            if (array_key_exists('force_password_reset', $data['User']))
                $data_to_save['User']['force_password_reset'] = $data['User']['force_password_reset'];
            if ($modified_by_user)
                $data_to_save['User']['force_password_reset'] = '0';
    ...
    function validatePassword(array $data, $created_user_name = null)
    {
        ...
        $db_user_data = $this->id ? $this->find('first', array(
            'conditions' => array(
                'User.id' => $this->id
            )
        )) : null;
        ...
        if ($password_format_error)
            $validation_errors['password'][] = 'password_format_error_msg_' . $password_security_level;

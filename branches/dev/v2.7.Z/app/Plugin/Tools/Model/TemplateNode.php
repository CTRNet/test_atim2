...
class TemplateNode extends AppModel 
{
    ...
    public function getStructureNodeDefaultValue($datamartStructureId, $controlId) {
        $nodeDatamartStructureId = $datamartStructureId;
        $nodeControlId = $controlId;
        ...
        foreach ($nodeLinkedStructures['Sfs'] as $k => $newLinkedStructuresField) {
            $mainFieldKey = $newLinkedStructuresField['model'] . '.' . $newLinkedStructuresField['field'];
            if (
                    !isset($this->collectionTemplateNodesStructuresFields[$mainFieldKey]) &&
                    (
                    ($newLinkedStructuresField['flag_add'] && !$newLinkedStructuresField['flag_add_readonly']) ||
                    ($newLinkedStructuresField['flag_addgrid'] && !$newLinkedStructuresField['flag_addgrid_readonly']) //||
//                    ($newLinkedStructuresField['flag_editgrid'] && !$newLinkedStructuresField['flag_editgrid_readonly']) ||
//                    ($newLinkedStructuresField['flag_edit'] && !$newLinkedStructuresField['flag_edit_readonly'])
                    )
            ) {
                // Manage Label
                $nodeLinkedStructures['Sfs'][$k]['display_column'] = 0;
                ...
                unset($nodeLinkedStructures['Sfs'][$k]);
            }
        }
        return $nodeLinkedStructures;
    }

    /**
     * Format the default values defined for each node of a collection template

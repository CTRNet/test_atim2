...
class SopMastersController extends SopAppController
{
    ...
    function add($sop_control_id)
    {
        $this->set('atim_menu_variables', array(
            'SopControl.id' => $sop_control_id
        ));
        $this_data = $this->SopControl->find('first', array(
            'conditions' => array(
                'SopControl.id' => $sop_control_id,
                'SopControl.flag_active' => '1'
            )
    ...
    function delete($sop_master_id)
    {
        ...
        if ($arr_allow_deletion['allow_deletion']) {
            ...
            $this->atimFlashWarning(__($arr_allow_deletion['msg']), '/Sop/SopMasters/detail/' . $sop_master_id);
        }
    }
}
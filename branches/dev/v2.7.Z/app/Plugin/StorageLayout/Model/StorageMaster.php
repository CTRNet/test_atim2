...
class StorageMaster extends StorageLayoutAppModel
{
    ...
    function isDuplicatedStorageBarCode($storage_data)
    {
        ...
        $storage_having_duplicated_barcode = $this->find('all', array(
            'conditions' => $criteria,
            'recursive' => - 1
        ));
    ...
    function buildAllowedStoragePosition($storage_data, $coord)
    {
        ...
        return array(
            'array_to_display' => $array_to_display,
            'array_to_order' => array_flip($array_to_order)
        );
    }
    ...
    function getStorageDataFromStorageLabelAndCode($storage_label_and_code)
    {
        ...
        if (! isset($this->storage_selection_labels_already_checked[$storage_label_and_code])) {
            ...
            if (preg_match_all("/([^\b]+)\[([^\[]+)\]/", $storage_label_and_code, $matches, PREG_SET_ORDER) > 0) {
                ...
                $selected_storages = $this->find('all', array(
                    'conditions' => array(
                        'StorageMaster.selection_label' => $storage_label_and_code
                    )
                ));
            }
     ...
     *
     * @param $studied_storage_master_id ID
    ...
    /**
     *
     * @param array $storage_master_ids
    ...
    function hasChild(array $storage_master_ids)
    {
        ...
        $result = array_unique(array_filter($this->find('list', array(
            ...
            'conditions' => array(
                'StorageMaster.parent_id' => $storage_master_ids
            )
        ))));
        ...
        return array_merge($result, array_unique(array_filter($aliquot_master->find('list', array(
            'fields' => array(
                'AliquotMaster.storage_master_id'
            ),
            'conditions' => array(
                'AliquotMaster.storage_master_id' => $storage_master_ids
            )
        )))), array_unique(array_filter($tma_slide->find('list', array(
            'fields' => array(
                'TmaSlide.storage_master_id'
            ),
            'conditions' => array(
                'TmaSlide.storage_master_id' => $storage_master_ids
            )
        )))));
    }
    ...
    function allowDeletion($storage_master_id)
    {
        ...
        $nbr_children_storages = $this->find('count', array(
            'conditions' => array(
        ...
        $nbr_tma_slides = $tma_slide_model->find('count', array(
            'conditions' => array(
                'TmaSlide.tma_block_storage_master_id' => $storage_master_id
            ),
            'recursive' => '-1'
        ));
        if ($nbr_tma_slides > 0) {
        ...
        $nbr_children_storages = $tma_slide_model->find('count', array(
            'conditions' => array(
    ...
    {
        $arr_studied_parents_data = array(
            $parent_storage_id => $parent_storage_data
        );
        ...
        while (! empty($arr_studied_parents_data)) {
            ...
            $children_storage_to_update = $this->find('all', array(
     ...
     *
     *
     * @param $storage_master_id Storage
     *            master id of the studied storage.
     * @param $storage_data Storage
     *            data including storage master, storage control, etc.
     * @param $storage_control_data Control
     ...
     * be updated.
     *
     * @param $parent_storage_master_id Id
     *            of the parent storage.
     * @param $parent_temperature Parent
     *            storage temperature.
     * @param $parent_temp_unit Parent
    ...
    function updateChildrenSurroundingTemperature($parent_storage_master_id, $parent_temperature, $parent_temp_unit)
    {
        $studied_parent_storage_ids = array(
            $parent_storage_master_id => $parent_storage_master_id
        );
     ...
     *            storage_control
     */
    function updateAndSaveDataArray($data_array, $type, $x_key, $y_key, $storage_parent_key, $rcv_data, $updater_model, array $storage_controls, &$updated_record_counter)
    {
        $error_found = false;
        foreach ($data_array as &$init_data_unit) {
            $init_data_id = $init_data_unit[$type]['id'];
            $storage_control = in_array($rcv_data[$type][$init_data_id]['s'], array(
                'u',
                't'
            )) ? null : $storage_controls[$rcv_data[$type][$init_data_id]['s']]['StorageControl'];
            if (($init_data_unit[$type][$x_key] != $rcv_data[$type][$init_data_id]['x'] && ! (in_array($rcv_data[$type][$init_data_id]['x'], array(
                'u',
                't'
            )) && $init_data_unit[$type][$x_key] == '') && ! ($storage_control && $storage_control['coord_x_size'] == null && $storage_control['coord_x_type'] != 'list' && $rcv_data[$type][$init_data_id]['x'] == '1')) || ($init_data_unit[$type][$y_key] != $rcv_data[$type][$init_data_id]['y'] && ! (in_array($rcv_data[$type][$init_data_id]['y'], array(
                'u',
                't'
            )) && $init_data_unit[$type][$y_key] == '') && ! ($storage_control && $storage_control['coord_y_size'] == null && $storage_control['coord_y_type'] != 'list' && $rcv_data[$type][$init_data_id]['y'] == '1')) || $init_data_unit[$type][$storage_parent_key] != $rcv_data[$type][$init_data_id]['s']) {
                ...
                if ($type == "StorageMaster") {
                    ...
                    if ($this->insideItself()) {
                        ...
                        $error_found = true;
                        $save_new_postion = false;
                    }
                }
                
                if ($storage_control && $storage_control['is_tma_block']) {
                    if ($type != 'AliquotMaster' || $init_data_unit['SampleControl']['sample_type'] != 'tissue' || $init_data_unit['AliquotControl']['aliquot_type'] != 'core') {
                        $data = $updater_model->findById($init_data_id);
                        if ($type == 'StorageMaster') {
    ...
    function positionStatusQuick($storage_master_id, array $position, array $exception = array())
    {
        ...
        $aliquot_master_model = AppModel::getInstance("InventoryManagement", "AliquotMaster", true);
        $tmp = $aliquot_master_model->find('first', array(
            'conditions' => $conditions,
            'recursive' => - 1
        ));
        if (! empty($tmp)) {
        ...
        $tma_slide_model = AppModel::getInstance("StorageLayout", "TmaSlide", true);
        $tmp = $tma_slide_model->find('first', array(
            'conditions' => $conditions,
            'recursive' => - 1
        ));
        if (! empty($tmp)) {
        ...
        if (isset($data[$model_name])) {
            foreach ($data[$model_name] as &$model_data) {
                ...
                if (isset($cumul_storage_data[$storage_id]['pos'][$model_data['x']][$model_data['y']])) {
                    $msg = __('conflict detected in storage [%s] at position [%s, %s]', $cumul_storage_data[$storage_id]['printed_label'], $model_data['x'], $model_data['y']);
                    // react

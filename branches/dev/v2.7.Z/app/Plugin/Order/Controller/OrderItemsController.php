...
class OrderItemsController extends OrderAppController
{
     ...
     *
     * @param unknown $object_model_name
    ...
    {
        if (! in_array($object_model_name, array(
            'AliquotMaster',
            'TmaSlide'
        )))
            $this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
        $foreign_key_field = str_replace(array(
            'AliquotMaster',
            'TmaSlide'
        ), array(
            'aliquot_master_id',
    ...
    function add($order_id, $order_line_id = 0, $object_model_name = 'AliquotMaster')
    {
        ...
        if (empty($this->request->data)) {
            ...
            if (sizeof($this->request->data) > $display_limit) {
                $this->atimFlashWarning__("batch init - number of submitted records too big") . " (>$display_limit)", ($order_line_id ? '/Order/OrderLines/detail/' . $order_id . '/' . $order_line_id . '/' : '/Order/Orders/detail/' . $order_id), 5);
                return;
            }
            ...
            foreach ($this->request->data as &$data_unit) {
                ...
                if ($object_model_name == 'AliquotMaster') {
                    ...
                    $aliquot_data = $this->AliquotMaster->find('first', array(
                        'conditions' => array(
                            'AliquotMaster.barcode' => $data_unit['AliquotMaster']['barcode']
                        ),
                        'recursive' => '-1'
                    ));
                    if (! $aliquot_data) {
                    ...
                    $slide_data = $this->TmaSlide->find('first', array(
                        'conditions' => array(
                            'TmaSlide.barcode' => $data_unit['TmaSlide']['barcode']
                        ),
                        'recursive' => '-1'
                    ));
                    if (! $slide_data) {
                    ...
                    if (in_array($data_unit['TmaSlide']['barcode'], $barcodes_recorded))
                        $errors_tracking['barcode']['a tma slide can only be added once to an order'][] = $row_counter;
    ...
    function addOrderItemsInBatch($object_model_name, $object_id = null)
    {
        ...
        $new_items_data = $this->{$object_model_name}->find('all', array(
            'conditions' => array(
                "$object_model_name.id" => $object_ids_to_add
            )
        ));
        ...
        foreach ($new_items_data as $new_item) {
            if ($object_model_name == 'AliquotMaster') {
                if (! $this->OrderItem->checkOrderItemStatusCanBeSetToPendingOrShipped('aliquot_master_id', $new_item['AliquotMaster']['id'])) {
                    $this->atimFlashWarning__("an aliquot cannot be added twice to orders as long as this one has not been first returned"), $url_to_cancel, 5);
                    return;
                }
            } else {
                if (! $this->OrderItem->checkOrderItemStatusCanBeSetToPendingOrShipped('tma_slide_id', $new_item['TmaSlide']['id'])) {
                    $this->atimFlashWarning__("a tma slide cannot be added twice to orders as long as this one has not been first returned"), $url_to_cancel, 5);
                    return;
                }
            }
        }
        ...
        if (! $order_data_tmp) {
            $this->atimFlashWarning__('no order to complete is actually defined'), $url_to_cancel);
            return;
        }
        foreach ($order_data_tmp as $new_order) {
            ...
            $order_and_order_line_data[$order_id] = array(
                'order' => array(
                    $new_order
                ),
                'lines' => array()
        ...
        $order_line_data_tmp = $this->OrderLine->find('all', array(
            'conditions' => array(
                'OrderLine.order_id' => array_keys($order_and_order_line_data)
            ),
    ...
    function edit($order_id, $order_item_id, $main_form_model = null)
    {
        ...
        if ($order_item_data['OrderItem']['status'] == 'shipped')
            $this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
    ...
    function editInBatch()
    {
        ...
        if ($initial_display) {
            $intial_order_items_data = $this->OrderItem->find('all', array(
                'conditions' => $criteria,
                'recursive' => '0'
            ));
            if (empty($intial_order_items_data)) {
                $this->atimFlashWarning__('no item to update'), $url_to_cancel);
                return;
            }
            if ($order_item_ids)
                $this->OrderItem->sortForDisplay($intial_order_items_data, $order_item_ids);
        ...
        if (empty($statuses)) {
            ...
            if (sizeof($statuses) != 1) {
                $this->atimFlashWarning__('items should have the same status to be updated in batch'), $url_to_cancel);
                return;
            } else 
                if ($statuses[0]['OrderItem']['status'] == 'shipped') {
                    $this->atimFlashWarning__('items should have a status different than shipped to be updated in batch'), $url_to_cancel);
                    return;
                }
        ...
        if ($initial_display) {
            ...
            foreach ($this->request->data as $key => &$new_studied_item) {
                $record_counter ++;
                // Get id
                if (! isset($new_studied_item['OrderItem']['id']))
                    $this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
            ...
            if ($this->OrderItem->find('count', array(
                'conditions' => array(
                    'OrderItem.id' => $updated_item_ids
                ),
                'recursive' => '-1'
            )) != sizeof($updated_item_ids)) {
                // In case an order item has just been deleted by another user before we submitted updated data
                $this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
            }
        ...
        if ($arr_allow_deletion['allow_deletion']) {
            ...
            if ($this->OrderItem->atimDelete($order_item_id)) {
                ...
                if ($order_line_id) {
                    // Update order line status
                    $new_status = 'pending';
                    $order_item_count = $this->OrderItem->find('count', array(
                        'conditions' => array(
    ...
    function defineOrderItemsReturned($order_id = 0, $order_line_id = 0, $shipment_id = 0, $order_item_id = 0)
    {
        ...
        if ($initial_display) {
            $initial_order_items_data = $this->OrderItem->find('all', array(
                'conditions' => $criteria,
                'order' => array(
            ...
            if (empty($initial_order_items_data)) {
                $this->atimFlashWarning__('no order items can be defined as returned'), $url_to_cancel);
                return;
            }
            ...
            if (sizeof($initial_order_items_data) > $display_limit) {
                $this->atimFlashWarning__("batch init - number of submitted records too big") . " (>$display_limit). " . __('use databrowser to submit a sub set of data'), $url_to_cancel, 5);
                return;
            }
            if ($order_item_ids) {
                $this->OrderItem->sortForDisplay($initial_order_items_data, $order_item_ids);
            } else {
                foreach ($initial_order_items_data as $new_order_item)
                    $order_item_ids[] = $new_order_item['OrderItem']['id'];
            }
        }
        ...
        if ($this->OrderItem->find('count', array(
            ...
            $this->atimFlashWarning__('only shipped items can be defined as returned'), $url_to_cancel);
            return;
        }
        ...
        if ($initial_display) {
            ...
            foreach ($this->request->data as &$new_studied_item) {
                ...
                if (! isset($new_studied_item['OrderItem']['id']))
                    $this->redirect('/Pages/err_plugin_record_err?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
                if (! isset($new_studied_item['OrderItem']['aliquot_master_id']) || ! isset($new_studied_item['OrderItem']['tma_slide_id']))
                    $this->redirect('/Pages/err_plugin_record_err?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
    ...
    function removeFlagReturned($order_id, $order_item_id, $main_form_model = null)
    {
        ...
        if (! $error) {
            ...
            $this->atimFlashWarning__($error), $redirect_url);
        }
    }
}
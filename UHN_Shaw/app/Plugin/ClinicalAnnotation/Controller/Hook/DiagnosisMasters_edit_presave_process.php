<?php
	...
	if($dx_master_data['DiagnosisControl']['controls_type'] == 'primary diagnosis unknown') {
		$this->request->data['DiagnosisMaster']['uhn_site'] = 'unknown';
		$this->DiagnosisMaster->addWritableField(array('uhn_site'));		
	} else if($dx_master_data['DiagnosisControl']['controls_type'] == 'ovary') {
		$this->request->data['DiagnosisMaster']['uhn_site'] = 'female genital-ovary';
		$this->DiagnosisMaster->addWritableField(array('uhn_site'));		
	} else if($this->request->data['DiagnosisMaster']['uhn_site']) && $this->request->data['DiagnosisMaster']['uhn_site'] == 'female genital-ovary') {

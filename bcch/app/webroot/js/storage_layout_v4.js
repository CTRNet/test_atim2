...
function moveItem(draggable, droparea){
	if($(draggable).parent()[0] != $(droparea).children("ul:first")[0]){
		/** BB-150 **/
		// Check if a cell already has user typed barcode
		// If it already has a barcode, prevent moving to the cell
		if($(droparea).children("ul:first").children(".barcode-input").attr("contentEditable") == "true") {
			droparea = draggable;
...
function preparePost(){
	...
	if(gotConflicts){
		...
		// BB-150
		var new_cells = ''; // For storing the JSON-like string objects
		var new_elements = $(".barcode-input");
		for(var i = new_elements.length - 1; i >= 0; --i){
			var info = $(new_elements[i]).parent().prop("id").match(/s\_([^\_]+)\_c\_([^\_]+)\_([^\_]+)/);
			var barcode = $.trim($(new_elements[i]).text());
			// JSON like string with the new barcode and coordinates
			new_cells += '{"barcode" : "' + barcode + '", "type" : "' + "Unknown" + '", "s" : "' + info[1] + '", "x" : "' + info[2] + '", "y" : "' + info[3] + '"},'; 
		}
		//Truncate the last comma at the end
		if (new_cells.length > 0) {
			new_cells = new_cells.substr(0, new_cells.length - 1);
		}
		...
		// Condition if there are new barcodes added by the user
		if(new_cells.length > 0) {
			// Check if the box is initially empty.
			// If are adding the first item to an empty box by direct barcode input, the cells array would be empty
			if(cells.length > 0) {
				$(form).append("<input type='hidden' name='data' value='[" + cells + "," + new_cells + "]'/>").submit();
			} else {
				// The cells array is empty in the empty box, only need to append the new_cells array for posting
				$(form).append("<input type='hidden' name='data' value='[" + new_cells + "]'/>").submit();
			}			 
		} else {

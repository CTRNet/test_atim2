...
$db_connection = @mysqli_connect($db_ip . (! empty($db_port) ? ":" . $db_port : ''), $db_user, $db_pwd) or migrationDie("ERR_DATABASE_CONNECTION: Could not connect to MySQL");
...
foreach (getSelectQueryResult($query) as $new_aliquot_control)
    $atim_controls['aliquot_controls'][$new_aliquot_control['sample_type'] . '-' . $new_aliquot_control['aliquot_type']] = $new_aliquot_control;
    // *** Control : consent_controls ***
$atim_controls['consent_controls'] = array();
foreach (getSelectQueryResult("SELECT id, controls_type, detail_tablename FROM consent_controls WHERE flag_active = 1") as $new_control)
    $atim_controls['consent_controls'][$new_control['controls_type']] = $new_control;
    // *** Control : event_controls ***
$atim_controls['event_controls'] = array();
foreach (getSelectQueryResult("SELECT id, disease_site, event_type, detail_tablename FROM event_controls WHERE flag_active = 1") as $new_control)
    $atim_controls['event_controls'][(strlen($new_control['disease_site']) ? $new_control['disease_site'] . '-' : '') . $new_control['event_type']] = $new_control;
...
foreach (getSelectQueryResult("SELECT id, category, controls_type, detail_tablename FROM diagnosis_controls WHERE flag_active = 1") as $new_control) {
    ...
    if ($new_control['category'] == 'primary')
        $primary_control_ids[] = $new_control['id'];
}
...
foreach (getSelectQueryResult("SELECT * FROM misc_identifier_controls WHERE flag_active = 1") as $new_control)
    $atim_controls['misc_identifier_controls'][$new_control['misc_identifier_name']] = $new_control;
    // *** Control : sop_controls ***
$atim_controls['sop_controls'] = array();
foreach (getSelectQueryResult("SELECT id, sop_group, type, detail_tablename, extend_tablename, extend_form_alias FROM sop_controls WHERE flag_active = 1") as $new_control)
    $atim_controls['sop_controls'][$new_control['sop_group'] . '-' . $new_control['type']] = $new_control;
    // *** Control : storage_controls ***
$atim_controls['storage_controls'] = array();
foreach (getSelectQueryResult("SELECT id, storage_type, coord_x_title, coord_x_type, coord_x_size, coord_y_title, coord_y_type, coord_y_size, display_x_size, display_y_size , set_temperature, is_tma_block, detail_tablename FROM storage_controls WHERE flag_active = 1") as $new_control)
    $atim_controls['storage_controls'][$new_control['storage_type']] = $new_control;
...
foreach (getSelectQueryResult($query) as $new_control)
    $atim_controls['protocol_controls'][$new_control['tumour_group'] . '-' . $new_control['type']] = $new_control;
...
foreach (getSelectQueryResult($query) as $new_control)
    $atim_controls['treatment_controls'][(strlen($new_control['disease_site']) ? $new_control['disease_site'] . '-' : '') . $new_control['tx_method']] = $new_control;
...
foreach (getSelectQueryResult($query) as $new_control)
    $atim_controls['specimen_review_controls'][$new_control['review_type']] = $new_control;
ksort($atim_controls);
 ...
 *
 * @param string $title
 *            Title of the migration process
 * @param array $display_file_names
 *            List of files names
 */
 ...
 * - $summary_details ...
 *
 * @param string $summary_section_title
 *            Title of a section gathering error or message linked to the same task, data type, etc.
 * @param string $summary_type
 *            @@ERROR@@ or @@WARNING@@ or @@MESSAGE@@
 * @param string $summary_title
 *            Error type
 * @param string $summary_details
 *            Details of the error (to inlude excel line number or a patient identifier, etc)
 * @param string $message_detail_key
 *            Key to not duplicate summary details for a same error.
 */
 ...
 *            True for any $query being an INSERT statement
 *            
...
{
    global $db_connection;
    $query_res = mysqli_query($db_connection, $query) or migrationDie(array(
        "ERR_QUERY",
 ...
 *            Data to insert. See format above.
 *            
    ...
    if ($tables_data) {
        ...
        foreach ($tables_data as $table_name => $table_fields_and_data) {
            foreach ($table_fields_and_data as $field => $data) {
                if (! strlen($data))
                    unset($tables_data[$table_name][$field]);
            }
        }
        // --1-- Check data
        switch (sizeof($tables_data)) {
            case '1':
                ...
                if (preg_match('/_masters$/', $table_name))
                    migrationDie("ERR_FUNCTION_customInsertRecord(): Detail table is missing to record data into $table_name");
                $main_table_data = array(
                    'name' => $table_name,
            ...
            case '3':
                ...
                if (empty($main_table_data))
                    migrationDie("ERR_FUNCTION_customInsertRecord(): Table sample_masters is missing (See table names: " . implode(' & ', $tables_data_keys) . ")");
                if (empty($details_tables_data))
                    migrationDie("ERR_FUNCTION_customInsertRecord(): Table 'specimen_details' or 'derivative_details' is missing (See table names: " . implode(' & ', $tables_data_keys) . ")");
                if (sizeof($tables_data) != 1)
                    migrationDie("ERR_FUNCTION_customInsertRecord(): Wrong 3 tables names for a new sample (See table names: " . implode(' & ', $tables_data_keys) . ")");
                $details_tables_data[] = array(
                    'name' => $details_table_name,
                    'data' => $tables_data[$details_table_name]
                );
                break;
            case '2':
                $details_table_name = '';
                ...
                if (empty($main_table_data))
                    migrationDie("ERR_FUNCTION_customInsertRecord(): Table %%_masters is missing (See table names: " . implode(' & ', $tables_data_keys) . ")");
                if (sizeof($tables_data) != 1)
                    migrationDie("ERR_FUNCTION_customInsertRecord(): Wrong 2 tables names for a master/detail model record (See table names: " . implode(' & ', $tables_data_keys) . ")");
                $details_tables_data[] = array(
                    'name' => $details_table_name,
                    'data' => $tables_data[$details_table_name]
                );
        ...
        if (isset($main_table_data['data']['sample_control_id'])) {
            if (! isset($atim_controls['sample_controls']['***id_to_type***'][$main_table_data['data']['sample_control_id']]))
                migrationDie('ERR_FUNCTION_customInsertRecord(): Unsupported sample control id.');
            ...
            if ($atim_controls['sample_controls'][$sample_type]['sample_category'] == 'specimen') {
                if (isset($main_table_data['data']['initial_specimen_sample_id']))
                    unset($main_table_data['data']['initial_specimen_sample_id']);
                $main_table_data['data']['initial_specimen_sample_type'] = $sample_type;
                if (isset($main_table_data['data']['parent_id']))
                    unset($main_table_data['data']['parent_id']);
                if (isset($main_table_data['data']['parent_sample_type']))
                    unset($main_table_data['data']['parent_sample_type']);
                if (! isset($main_table_data['data']['sample_code']))
                    $main_table_data['data']['sample_code'] = "@@@TO_GENERATE@@";
            } else {
                if (! isset($main_table_data['data']['initial_specimen_sample_id']))
                    migrationDie('ERR_FUNCTION_customInsertRecord(): Missing sample information : initial_specimen_sample_id.');
                if (! isset($main_table_data['data']['initial_specimen_sample_type']))
                    migrationDie('ERR_FUNCTION_customInsertRecord(): Missing sample information : initial_specimen_sample_type.');
                if (! isset($main_table_data['data']['parent_id']))
                    migrationDie('ERR_FUNCTION_customInsertRecord(): Missing sample information : parent_id.');
                if (! isset($main_table_data['data']['parent_sample_type']))
                    migrationDie('ERR_FUNCTION_customInsertRecord(): Missing sample information : parent_sample_type.');
                if (! isset($main_table_data['data']['sample_code']))
                    $main_table_data['data']['sample_code'] = "@@@TO_GENERATE@@";
            }
        }
        $main_table_data['data'] = array_merge($main_table_data['data'], array(
            "created" => $import_date,
        ...
        if (isset($main_table_data['data']['diagnosis_control_id'])) {
            ...
            if (isset($main_table_data['data']['sample_control_id'])) {
                ...
                $set_strings = array();
                if ($atim_controls['sample_controls'][$sample_type]['sample_category'] == 'specimen')
                    $set_strings[] = "initial_specimen_sample_id=id";
                if ($main_table_data['data']['sample_code'] == "@@@TO_GENERATE@@")
                    $set_strings[] = "sample_code=id";
                if ($set_strings) {
        ...
        if (isset($main_table_data['data']['sample_control_id'])) {
            if (sizeof($details_tables_data) != 2)
                migrationDie("ERR_FUNCTION_customInsertRecord(): Table 'specimen_details', 'derivative_details' or 'SampleDetail' is missing (See table names: " . implode(' & ', $tables_data_keys) . ")");
        } else {
            if (sizeof($details_tables_data) > 2)
                migrationDie("ERR_FUNCTION_customInsertRecord(): Too many tables are declared (>2) (See table names: " . implode(' & ', $tables_data_keys) . ")");
        }
        ...
        if ($details_tables_data) {
            ...
            foreach ($details_tables_data as $detail_table) {
                $detail_table['data'] = array_merge($detail_table['data'], array(
                ...
                if (! in_array($detail_table['name'], array(
                    'specimen_details',
                    'derivative_details'
                )))
                    $tmp_detail_tablename = $detail_table['name'];
            }
        }
 ...
 * @param array $tables_data
 *            Data to update (see format above)
 */
    ...
    if ($tables_data) {
        ...
        switch (sizeof($tables_data)) {
            case '1':
                ...
                if (! empty($tables_data[$main_or_master_tablename]))
                    $to_update = true;
                break;
            case '2':
            case '3':
                foreach ($tables_data_keys as $table_name) {
                    if (preg_match('/_masters$/', $table_name)) {
                        if (! is_null($main_or_master_tablename))
                            migrationDie("ERR_FUNCTION_updateTableData(): 2 Master tables passed in arguments: " . implode(', ', $tables_data_keys) . ".");
                        $main_or_master_tablename = $table_name;
                    }
                    if (! empty($tables_data[$table_name]))
                        $to_update = true;
                }
                if (is_null($main_or_master_tablename))
                    migrationDie("ERR_FUNCTION_updateTableData(): Master table not passed in arguments: " . implode(', ', $tables_data_keys) . ".");
                break;
            default:
        ...
        if ($to_update) {
            ...
            foreach (array_merge($table_data, array(
                'modified' => $import_date,
                'modified_by' => $imported_by
            )) as $key => $value)
            ...
            foreach ($tables_data as $table_name => $table_data) {
                if (! empty($table_data)) {
                    ...
                    if (! in_array($table_name, array(
                        ...
                        $tmp_detail_tablename = $table_name;
                }
            }
...
function addToModifiedDatabaseTablesList($main_table_name, $detail_table_name)
{
    ...
    $modified_database_tables_list[$key] = array(
        $main_table_name,
        $detail_table_name
    );
}
 ...
 * - updateTableData()
 *
 * @param unknown $main_table_name
 *            Name of the main table
 * @param unknown $detail_table_name
 *            Name of the detail table name
 */
function insertIntoRevsBasedOnModifiedValues($main_tablename = null, $detail_tablename = null)
{
    ...
    foreach ($initial_tables_sets_to_update as $key => $tmp) {
        if (preg_match('/^([a-z]+_masters)\-$/', $key, $matches)) {
            ...
            if (! isset($atim_controls[$control_table_name]))
                migrationDie("ERR 778894003 #$master_table_name.$control_table_name");
            foreach ($atim_controls[$control_table_name] as $control_data) {
                if (is_array($control_data) && isset($control_data['detail_tablename'])) {
                    ...
                    if (! isset($tables_sets_to_update[$new_key]))
                        $tables_sets_to_update[$new_key] = array(
                            "$master_table_name",
                            "$detail_table_name"
                        );
                }
    ...
    foreach ($tables_sets_to_update as $new_tables_set) {
        ...
        if (! $detail_tablename) {
            ...
            // *** MASTER DETAIL MODEL ***
            if (! preg_match('/^.+\_masters$/', $main_tablename))
                migrationDie("ERR_FUNCTION_insertIntoRevsBasedOnModifiedValues(): '$main_tablename' is not a 'Master' table of a MasterDetail model.");
            ...
            // Detail table
            $all_detail_tablenames = ($main_tablename != 'sample_masters') ? array(
                $detail_tablename
            ) : array(
                $detail_tablename,
                'specimen_details',
            ...
            foreach ($all_detail_tablenames as $detail_tablename) {
                ...
                while ($row = $results->fetch_assoc())
                    $table_fields[] = $row['Field'];
                if (! in_array($foreign_key, $table_fields))
                    migrationDie("ERR_FUNCTION_insertIntoRevsBasedOnModifiedValues(): Foreign Key '$foreign_key' defined based on 'Master' table name '$main_tablename' is not a field of the 'Detail' table '$detail_tablename'.");
 ...
 * @param unknown $value
 *            Value to validate
 * @param unknown $domain_name
 *            Domain name of the list
 * @param unknown $summary_section_title
 *            Section title if an error is generated by the function (See recordErrorAndMessage() description)
 * @param string $summary_title_add_in
 *            Additional information to add to summary title if an error is generated by the function (See recordErrorAndMessage() description)
 * @param string $summary_details_add_in
 *            Additional information to add to seummary detail if an error is generated by the function (See recordErrorAndMessage() description)
 *            
            ...
            if ($domain_data_results['source']) {
                if (preg_match('/getCustomDropdown\(\'(.*)\'\)/', $domain_data_results['source'], $matches)) {
                    ...
                    foreach (getSelectQueryResult($query) as $domain_value)
                        $domains_values[$domain_name][strtolower($domain_value['value'])] = $domain_value['value'];
                } else {
 ...
 * @param string $value
 *            Value to test
 * @param array $values_matches
 *            List of good values (key being a value you can find in excel and value being the value to record in database (ex: array('yes' => 'y', 'y' => 'y', 'unknown' => 'u', '?' => 'u', etc)
 * @param string $str_to_lower
 *            Should the case of the value to test change to lower for test
 * @param unknown $summary_section_title
 *            Section title if an error is generated by the function (See recordErrorAndMessage() description)
 * @param string $summary_title_add_in
 *            Additional information to add to summary title if an error is generated by the function (See recordErrorAndMessage() description)
...
function validateAndGetDateAndAccuracy($date, $summary_section_title, $summary_title_add_in, $summary_details_add_in)
{
    ...
    $date = str_replace(
        array('Jan', 'Fev', 'Feb', 'Jul', 'Aug', 'Nov'),
        array('01', '02', '02', '07', '08', '11'),
        $date);
    ...
    } elseif (preg_match('/^([0-9]+)$/', $date, $matches)) {
    ...
    } elseif (preg_match('/^((19|20)([0-9]{2})\-([01][0-9]))\-((xx)|(unk))$/', $date, $matches)) {
        return array(
            $matches[1] . '-01',
            'd'
        );
    } elseif (preg_match('/^((19|20)([0-9]{2}))\-((xx)|(unk))\-((xx)|(unk))$/', $date, $matches)) {
        return array(
            $matches[1] . '-01-01',
            'm'
        );
    } elseif (preg_match('/^([0-3][0-9])\/([01][0-9])\/(19|20)([0-9]{2})$/', $date, $matches)) {
        return array(
            $matches[3] . $matches[4] . '-' . $matches[2] . '-' . $matches[1],
            'c'
        );
    } elseif (preg_match('/^([0-3][0-9])\-([01][0-9])\-(19|20)([0-9]{2})$/', $date, $matches)) {
        return array(
    ...
    if (! $formatted_date) {
        ...
        if (empty($time) || in_array(strtolower($time), $empty_date_time_values)) {
            return array(
                ...
                str_replace('c', 'h', $formatted_date_accuracy)
            );
        } else {
            if ($formatted_date_accuracy != 'c') {
                ...
                if (preg_match('/^(0{0,1}[0-9]|1[0-9]|2[0-3]):([0-5][0-9])$/', $time, $matches)) {
                    return array(
                        $formatted_date . ' ' . ((strlen($time) == 5) ? $time : '0' . $time),
                        'c'
                    );
                } else 
                    if (preg_match('/^0\.[0-9]+$/', $time)) {
                        ...
                        if ($hour > 23)
                            migrationDie("ERR_GET_DATETIME: Hour value > 24.");
                        $time = $hour . ':' . $mn;
                        return array(
                            $formatted_date . ' ' . ((strlen($time) == 5) ? $time : '0' . $time),
                            'c'
...
function validateAndGetDecimal($decimal_value, $summary_section_title, $summary_title_add_in, $summary_details_add_in)
{
    global $import_summary;
    global $empty_number_values;
    $decimal_value = str_replace(array(
        ' ',
        ','
    ), array(
        '',
...
function validateAndGetInteger($integer_value, $summary_section_title, $summary_title_add_in, $summary_details_add_in)
{
    global $import_summary;
    global $empty_number_values;
    $integer_value = str_replace(array(
        ' '
    ), array(
        ''
    ), $integer_value);
 ...
 *            List of excel files names to test
 *            
 ...
 */
/**
 * Get the next excel line data.
 * The process will restart at the begining of a file worksheet
 ...
 *            Studied worksheet
 * @param string $header_lines_nbr
 *            The number of header lines
 * @param string $file_xls_offset
 *            Serial number $windows_xls_offset = 36526 & $mac_xls_offset = 35064 (Use for excel date parsing)
 *            
...
function getNextExcelLineData($excel_file_name, $worksheet_name, $header_lines_nbr = 1, $file_xls_offset = '36526')
{
    ...
    if (is_null($studied_excel_file_name_properties) || $studied_excel_file_name_properties['file_name'] != $excel_file_name) {
        ...
        $studied_excel_file_name_properties = array(
            'file_name' => $excel_file_name,
            'file_worksheets' => array()
        );
        foreach ($XlsReader->boundsheets as $key => $tmp)
            $studied_excel_file_name_properties['file_worksheets'][$tmp['name']] = $key;
    }
    ...
    if (! array_key_exists('worksheet_name', $studied_excel_file_name_properties) || $studied_excel_file_name_properties['worksheet_name'] != $worksheet_name) {
        if (! array_key_exists($worksheet_name, $studied_excel_file_name_properties['file_worksheets'])) {
            ...
            if (! isset($XlsReader->sheets[$studied_excel_file_name_properties['file_worksheets'][$worksheet_name]]['cells']) || empty($XlsReader->sheets[$studied_excel_file_name_properties['file_worksheets'][$worksheet_name]]['cells'])) {
                // Empty worksheet
    ...
    if ($studied_excel_file_name_properties['headers'] == '-1') {
        ...
        if (empty($studied_excel_file_name_properties['headers'])) {

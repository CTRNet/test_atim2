...
class TmaSlideCustom extends TmaSlide{
	...
	private $sectionIdsCheck = array(); 	//used for validations
	...
	public function validates($options = array()){
		...
		if(isset($this->data['TmaSlide']['qbcf_section_id'])) {
			...
			if(isset($this->data['TmaSlide']['tma_block_storage_master_id'])) {
				...
				$tmpSlide = $this->find('first', array('conditions' => array('TmaSlide.id' => $this->data['TmaSlide']['id']), 'fields' => array('TmaSlide.tma_block_storage_master_id'), 'recursive' => -1));
				if($tmpSlide) {
					$tmaBlockStorageMasterId = $tmpSlide['TmaSlide']['tma_block_storage_master_id'];
				}
			}
			if(!$tmaBlockStorageMasterId) AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method='.__METHOD__.',line='.__LINE__, null, true);
			...
			$criteria = array('TmaSlide.tma_block_storage_master_id' => $tmaBlockStorageMasterId, 'TmaSlide.qbcf_section_id' => $qbcfSectionId);
			$slidesHavingDuplicatedId = $this->find('all', array('conditions' => $criteria, 'recursive' => -1));;

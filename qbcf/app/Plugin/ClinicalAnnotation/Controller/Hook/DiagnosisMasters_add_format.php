<?php 
	...
	switch($dxControlData['DiagnosisControl']['controls_type']) {
		case 'breast':
			if($this->DiagnosisMaster->find('count', array('conditions' => array('DiagnosisControl.controls_type' => 'breast', 'DiagnosisMaster.participant_id' => $participantId)))) {
				$this->atimFlashError(__('you can not create a breast diagnosis twice'), 'javascript:history.back();');
			}
			break;
		case 'breast progression':
			if(!$parentDx)  $this->redirect('/Pages/err_plugin_system_error?method='.__METHOD__.',line='.__LINE__, null, true); 

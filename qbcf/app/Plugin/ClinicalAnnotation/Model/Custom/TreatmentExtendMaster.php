...
class TreatmentExtendMasterCustom extends TreatmentExtendMaster
{
    ...
    public function validates($options = array())
    {
        ...
        if (array_key_exists('drug_id', $this->data['TreatmentExtendMaster']) && $this->data['TreatmentExtendMaster']['drug_id']) {
            ...
            if (! $drugType || ! $treatmentExtendControlType)
            ...
            switch ($drugType) {
                case 'chemotherapy':
                case 'immunotherapy':
                    if (strcmp("$drugType drug", $treatmentExtendControlType) !== 0)
                        $valTypes = false;
                    break;
                case 'bone specific':
                    if (strcmp("bone specific therapy drug", $treatmentExtendControlType) !== 0)
                        $valTypes = false;
                    break;
                case 'hormonal':
                    if (strcmp("hormonotherapy drug", $treatmentExtendControlType) !== 0)
                        $valTypes = false;
                    break;
                case 'other':
                    if (strcmp("other (breast cancer systemic treatment) drug", $treatmentExtendControlType) !== 0)
                        $valTypes = false;
                    break;
            }
            if (! $valTypes) {

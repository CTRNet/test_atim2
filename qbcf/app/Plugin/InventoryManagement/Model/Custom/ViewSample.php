...
SampleMaster.qbcf_tma_sample_control_code,
    ...
    public function beforeFind($queryData)
    {
        if (($_SESSION['Auth']['User']['group_id'] != '1') && is_array($queryData['conditions'])) {
            if (AppModel::isFieldUsedAsCondition("ViewSample.qbcf_bank_participant_identifier", $queryData['conditions']) || AppModel::isFieldUsedAsCondition("ViewSample.qbcf_pathology_id", $queryData['conditions']) || AppModel::isFieldUsedAsCondition("ViewSample.bank_id", $queryData['conditions'])) {
    ...
    public function afterFind($results, $primary = false)
    {
        ...
        if ($_SESSION['Auth']['User']['group_id'] != '1') {
            ...
            $groupData = $GroupModel->findById($_SESSION['Auth']['User']['group_id']);
            $userBankId = $groupData['Group']['bank_id'];
            if (isset($results[0]['ViewSample']['bank_id']) || isset($results[0]['ViewSample']['qbcf_bank_participant_identifier']) || isset($results[0]['ViewSample']['qbcf_pathology_id'])) {
                foreach ($results as &$result) {
                    if ((! isset($result['ViewSample']['bank_id'])) || $result['ViewSample']['bank_id'] != $userBankId) {
                        if (isset($result['ViewSample']['bank_id']))
                            $result['ViewSample']['bank_id'] = CONFIDENTIAL_MARKER;
                        if (isset($result['ViewSample']['qbcf_bank_participant_identifier']))
                            $result['ViewSample']['qbcf_bank_participant_identifier'] = CONFIDENTIAL_MARKER;
                        if (isset($result['ViewSample']['qbcf_pathology_id']))

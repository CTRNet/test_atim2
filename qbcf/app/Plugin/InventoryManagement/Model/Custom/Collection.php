        ...
        if (isset($this->data['Collection']['qbcf_pathology_id'])) {
            ...
            if ($this->data['Collection']['collection_property'] == 'independent collection') {
                ...
                if ($this->find('count', array(
                    'conditions' => array(
                        'Collection.collection_property' => 'independent collection',
            ...
            foreach ($results as &$result) {
                ...
                if (array_key_exists('qbcf_pathology_id', $result['Collection'])) {
                    ...
                    if (! isset($result['ViewCollection'])) {
                        if (! $ViewCollectionModel)
                            $ViewCollectionModel = AppModel::getInstance("InventoryManagement", "ViewCollection", true);
                        $collectionViewData = $ViewCollectionModel->find('first', array(
                            'conditions' => array(
                                'ViewCollection.collection_id' => $result['Collection']['id']
                            ),
                            'recursive' => - 1
                        ));
                    } else {
                        $collectionViewData = array(
                            'ViewCollection' => $result['ViewCollection']
                        );
                    }

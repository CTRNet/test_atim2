...
LEFT JOIN treatment_masters AS TreatmentMaster ON TreatmentMaster.id = Collection.treatment_master_id AND TreatmentMaster.deleted <> 1
    ...
    public function beforeFind($queryData)
    {
        if (($_SESSION['Auth']['User']['group_id'] != '1') && is_array($queryData['conditions'])) {
            if (AppModel::isFieldUsedAsCondition("ViewCollection.qbcf_bank_participant_identifier", $queryData['conditions']) || AppModel::isFieldUsedAsCondition("ViewCollection.qbcf_pathology_id", $queryData['conditions']) || AppModel::isFieldUsedAsCondition("ViewCollection.bank_id", $queryData['conditions'])) {
    ...
    public function afterFind($results, $primary = false)
    {
        ...
        if ($_SESSION['Auth']['User']['group_id'] != '1') {
            ...
            $groupData = $GroupModel->findById($_SESSION['Auth']['User']['group_id']);
            $userBankId = $groupData['Group']['bank_id'];
            if (isset($results[0]['ViewCollection']['bank_id']) || isset($results[0]['ViewCollection']['qbcf_bank_participant_identifier']) || isset($results[0]['ViewCollection']['qbcf_pathology_id'])) {
                foreach ($results as &$result) {
                    if ((! isset($result['ViewCollection']['bank_id'])) || $result['ViewCollection']['bank_id'] != $userBankId) {
                        if (isset($result['ViewCollection']['bank_id']))
                            $result['ViewCollection']['bank_id'] = CONFIDENTIAL_MARKER;
                        if (isset($result['ViewCollection']['qbcf_bank_participant_identifier']))
                            $result['ViewCollection']['qbcf_bank_participant_identifier'] = CONFIDENTIAL_MARKER;
                        if (isset($result['ViewCollection']['qbcf_pathology_id']))

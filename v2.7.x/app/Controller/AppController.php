 ...
 * @property SessionAcl $SessionAcl
    ...
    public function beforeRender(){
		...
		if(isset($this->passedArgs['batchsetVar'])){
			...
			if(empty($data)){
				...
				$this->Flash->error(__('there is no data to add to a temporary batchset'), 'javascript:history.back()');
				return false;
			}
...
// 		echo("Exec time (sec): ".(AppController::microtime_float() - $start_time));
	 ...
	 *
	 * @param array $datetime_array Array gathering date data into following structure:
	 * 	array('month' => string, '
	 * 		'day' => string,
	 * 		'year' => string,
	 * 		'hour' => string,
	 * 		'min' => string)
	 * @param string $date_type  Specify the type of date ('normal', 'start', 'end')

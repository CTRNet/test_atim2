 ...
 */
	public function login() {
		...
		if ($this->request->is('post')) {
			...
			$loginTemporarilyDisabled = $this->User->shouldLoginFromIpBeDisabledAfterFailedAttempts();

			if (!$loginTemporarilyDisabled && $this->Auth->login() && (!isset($this->passedArgs['login']))) {
 ...
 * @return void
 */
	protected function _setSessionSearchId() {
 ...
 */
	public function reset_password() {
		...
		if ($this->request->is('post')) {
			$userId = $this->User->field('id', array(
				'User.username' => $this->request->data['User']['username']
			));

			if (!$userId) {
				return $this->validationErrors['username'][] = __('Username does not match');
			}

			$this->User->id = $userId;

			$passwordValid = $this->User->validatePassword($this->request->data);
			...
			$this->User->checkWritableFields = false;
			if ($passwordValid && $this->User->save($data)) {
 ...
 * @return void
 */
	protected function _initializeNotificationSessionVariables() {

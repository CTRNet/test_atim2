	...
	function csv() {
		...
		if (isset($this->passedArgs['popup'])) {
			...
			if (!is_array($this->request->data[$data_model][$data_pkey])) {
				$this->request->data[$data_model][$data_pkey] = explode(",",
					$this->request->data[$data_model][$data_pkey]);
			}
			...
			$this->request->data = $this->ModelToSearch->find('all',
				array('conditions' => $model_name . "." . $model_pkey . " IN ('" . implode("', '", $ids) . "')"));

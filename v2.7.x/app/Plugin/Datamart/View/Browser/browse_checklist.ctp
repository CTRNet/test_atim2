...
if (isset($is_root) && !$is_root) {
	$links['bottom']['save browsing steps'] = array(
		'link' => AppController::checkLinkPermission('/Datamart/BrowsingSteps/save/') ? 'javascript:openSaveBrowsingStepsPopup("Datamart/BrowsingSteps/save/' . $node_id . '");' : '/underdev/',
		'icon' => 'disk'
	);
}
...
if ($type == "checklist") {
	...
	if (is_array($this->request->data)) {
        ...
        <ul class="warning">
            <li>
                <span class="icon16 warning mr5px"></span><?php echo(__("the query returned too many results") . ". " . __("try refining the search parameters") . ". " . __("for any action you take (%s, %s, csv, etc.), all matches of the current set will be used",
						__('browse'), __('batchset'))); ?>.
            </li>
        </ul>
		<?php
		$this->Structures->build($empty, array(
			...
			'type' => 'add',
			'links' => $links,
			'settings' => array('actions' => false, 'form_bottom' => false)
		));
...
$header_description = __('link to databrowser wiki page %s  + datamart structures relationship diagram access',
		$help_url) . '<a href="' . 'javascript:dataBrowserHelp();' . '" >' . __('data types relationship diagram') . '</a>';
if (isset($header)) {
	if (!is_array($header)) {
		$header_title = $header;
	} else {
...
"); ?>";
    ...
    var STR_LANGUAGE = "<?php echo((Configure::read('Config.language') == 'eng') ? 'en' : 'fr'); ?>";
</script>

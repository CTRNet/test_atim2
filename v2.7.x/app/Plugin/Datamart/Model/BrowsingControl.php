...
class BrowsingControl extends DatamartAppModel {
	...
	function completeData(array &$data) {
		...
		$datamart_structures = $datamart_structure_model->find('all', array(
			'conditions' => array(
	 ...
	 *
	 * @return If the value is a model name (string), return the id of the associated DatamartStructure. Otherwise
	...
	function getInnerJoinArray($a, $b, array $ids_filter = null) {
		...
		$data = $this->find('first', array(
			'conditions' => array(
				'BrowsingControl.id1' => $browsing_structure_id_a,
				'BrowsingControl.id2' => $browsing_structure_id_b
			)
		));
		if ($data) {
			//n to 1
			$this->completeData($data);
			$model_n = AppModel::getInstance($data['DatamartStructure1']['plugin'],
				$a == $data['DatamartStructure1']['control_master_model'] ? $a : $data['DatamartStructure1']['model']);
			$model_1 = AppModel::getInstance($data['DatamartStructure2']['plugin'],
				$b == $data['DatamartStructure2']['control_master_model'] ? $b : $data['DatamartStructure2']['model']);
			$model_b = &$model_1;
		} else {
			//1 to n
			$data = $this->find('first', array(
				'conditions' => array(
					'BrowsingControl.id2' => $browsing_structure_id_a,
					'BrowsingControl.id1' => $browsing_structure_id_b
				)
			));
			if (empty($data)) {
			...
			$this->completeData($data);
			$model_n = AppModel::getInstance($data['DatamartStructure1']['plugin'],
				$b == $data['DatamartStructure1']['control_master_model'] ? $b : $data['DatamartStructure1']['model']);
			$model_1 = AppModel::getInstance($data['DatamartStructure2']['plugin'],
				$a == $data['DatamartStructure2']['control_master_model'] ? $a : $data['DatamartStructure2']['model']);
			$model_b = &$model_n;
		}

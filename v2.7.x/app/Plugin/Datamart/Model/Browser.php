...
class Browser extends DatamartAppModel {
	...
	/**
	 * @param int $starting_ctrl_id The control id to base the dropdown on. If zero, all will be displayed without
	 *     export options
	 ...
	 */
	function getBrowserDropdownOptions(
		$starting_ctrl_id,
		$node_id,
		$plugin_name,
		$model_name,
		...
		$model_pkey,
		$data_pkey,
		array $sub_models_id_filter = null
	) {
		...
		if ($starting_ctrl_id != 0) {
			if ($plugin_name == null || $model_name == null || $data_model == null || $model_pkey == null || $data_pkey == null) {
				$app_controller->redirect('/Pages/err_internal?p[]=missing parameter for getBrowserDropdownOptions',
					null, true);
			}
			...
			$browsing_structures = $DatamartStructure->find('all', array(
				'conditions' => array(
					'DatamartStructure.id IN (0, ' . implode(", ", $active_structures_ids) . ')'
				)
			));
			$tmp_arr = array();
			...
			$rez = Browser::buildBrowsableOptions($options, $starting_ctrl_id, $browsing_structures,
			...
			$result = array_merge($result,
				parent::getDropdownOptions($plugin_name, $model_name, $model_pkey, null, $data_model, $data_pkey));
			...
			if (AppController::checkLinkPermission('/Datamart/Browser/applyBrowsingSteps/')) {
				...
				$data = $saved_browsing_index_model->find('all', array(
					'conditions' => array_merge($saved_browsing_index_model->getOwnershipConditions(),
						array('SavedBrowsingIndex.starting_datamart_structure_id' => $starting_ctrl_id)),
				...
				if ($data) {
					...
					foreach ($data as $data_unit) {
						...
						$first_step_model = AppModel::getInstance($first_step_datamart_structure['plugin'],
							$first_step_datamart_structure['model'], true);
						...
						$last_step_model = AppModel::getInstance($last_step_datamart_structure['plugin'],
							$last_step_datamart_structure['model'], true);
			...
			foreach ($to_sort as $k => $foo) {
				...
				$tmp_model = AppModel::getInstance($data_unit['DatamartStructure']['plugin'],
					$data_unit['DatamartStructure']['model'], true);
				if ($ctrl_name = $tmp_model->getControlName()) {
					...
					if (!empty($children)) {
						array_unshift($children,
							array('label' => __('all'), 'value' => $data_unit['DatamartStructure']['id']));
	...
	function buildBrowsableOptionsRecur(
		array $from_to,
		$current_id,
		array $browsing_structures,
		array $sub_models_id_filter = null,
		array $stack
	) {
		$result = null;
		if (isset($from_to[$current_id]) && isset($browsing_structures[$current_id])) {
			...
			if (Browser::$hierarchical_dropdown) {
				foreach ($to_arr as $to) {
					$tmp_result = $this->buildBrowsableOptions($from_to, $to, $browsing_structures,
						$sub_models_id_filter, $stack);
					if ($tmp_result != null) {
	 ...
	 * @param array $browsing_structures An array containing data about all available browsing structures. Used to get
	 *     the displayed value
	 * @param array $sub_models_id_filter An array with ControlModel => array(ids) to filter the sub models id
	 *
	 * @return An array representing the browsable portion of the action menu
	 */
	function buildBrowsableOptions(
		array $from_to,
		$current_id,
		array $browsing_structures,
		array $sub_models_id_filter = null
		...
		if (Browser::$hierarchical_dropdown) {
			return $this->buildBrowsableOptionsRecur($from_to, $current_id, $browsing_structures, $sub_models_id_filter,
				array());
		}
	...
	function buildItemOptions(array &$result, array &$browsing_structures, &$current_id, array &$sub_models_id_filter) {
		...
		$browsing_model = AppModel::getInstance($browsing_structures[$current_id]['plugin'],
			$browsing_structures[$current_id]['model'], true);
		if ($control_name = $browsing_model->getControlName()) {
			...
			$result['children'] = array_merge($result['children'],
				self::getSubModels(array("DatamartStructure" => $browsing_structures[$current_id]), $result['value'],
					$id_filter));
		}
	}
	...
	static function getSubModels(array $main_model_info, $prepend_value, array $ids_filter = null) {
		...
		$main_model = AppModel::getInstance($main_model_info['DatamartStructure']['plugin'],
			$main_model_info['DatamartStructure']['model'], true);
		$control_model = AppModel::getInstance($main_model_info['DatamartStructure']['plugin'],
			$main_model->getControlName(), true);
		$conditions = array();
		if ($main_model->getControlName() == "SampleControl") {
			//hardcoded SampleControl filtering
			$parentToDerivativeSampleControl = AppModel::getInstance("InventoryManagement",
				"ParentToDerivativeSampleControl", true);
		...
		$children_data = $control_model->find('all', array(
			'order' => $control_model->name . '.databrowser_label',
	 ...
	 * @param Array $linked_types_down Should be left blank when calling the function. Internally used to know when to
	 *     stop to display the "merge" button
	 * @param Array $linked_types_up Should be left blank when calling the function. Internally used to know when to
	 *     stop to display the "merge" button
	 *
	...
	static function getTree(
		...
		$active_node,
		$merged_ids,
		array &$linked_types_down = array(),
		array &$linked_types_up = array()
	) {
		$BrowsingResult = new BrowsingResult();
		$result = $BrowsingResult->find('all', array(
			'conditions' => 'BrowsingResult.id=' . $node_id . ' OR BrowsingResult.parent_id=' . $node_id,
			'order' => array('BrowsingResult.id'),
			'recursive' => 1
		));
		$tree_node = null;
		if ($tree_node = array_shift($result)) {
			...
			//going down the tree
			$drilldown_allow_merge = !$tree_node['BrowsingResult']['raw'] && in_array($tree_node['BrowsingResult']['browsing_structures_id'],
					$linked_types_down);
			...
			$merge = (count($linked_types_down) > 0 || $tree_node['active'] || $drilldown_allow_merge) && !in_array($tree_node['BrowsingResult']['browsing_structures_id'],
					$linked_types_down);
			if ($merge) {
			...
			foreach ($children as $child) {
				if ($merge) {
					$child_node = Browser::getTree($child, $active_node, $merged_ids, $linked_types_down,
						$linked_types_up);
				} else {
					$foo = array();
				...
				if (!isset($tree_node['merge']) && (($child_node['merge'] && $node_id != $active_node) || $child_node['BrowsingResult']['id'] == $active_node)) {
					...
					if (!in_array($tree_node['BrowsingResult']['browsing_structures_id'],
							$linked_types_up) || !$child_node['BrowsingResult']['raw']
					) {
	...
	static function getPrintableTree($current_node, array $merged_ids, $webroot_url) {
		...
		do {
			...
			if ($current_node == $br['BrowsingResult']['id']) {
				$current_node_datamart_structure_id = $br['DatamartStructure']['id'];
			}
			assert($br);
		...
		foreach ($tree as $y => $line) {
			...
			foreach ($line as $x => $cell) {
				...
				if (is_array($cell)) {
					$cell_model = AppModel::getInstance($cell['DatamartStructure']['plugin'],
						$cell['DatamartStructure']['model'], true);
					if ($cell_model->table == 'tma_slides') {
						AppController::addWarningMsg(__('links between storage and tma slides are limited to tma blocks and slides created from these blocks - different than slide storage'));
					}
					$class = '';
					if ($cell['active']) {
						$class .= " active ";
					...
					$count = strlen($cell['BrowsingResult']['id_csv']) ? count(explode(",",
						$cell['BrowsingResult']['id_csv'])) : 0;
						...
						if ($cell['BrowsingResult']['raw']) {
							...
							if ((isset($search['search_conditions']) && count($search['search_conditions'])) || $adv_search || isset($search['counters'])) {
								...
								if ($cell_model->getControlName() && $cell['BrowsingResult']['browsing_structures_sub_id'] > 0) {
									...
									$tmp_model = AppModel::getInstance($cell['DatamartStructure']['plugin'],
										$cell['DatamartStructure']['model'], true);
									$alternate_alias = self::getAlternateStructureInfo($cell['DatamartStructure']['plugin'],
										$tmp_model->getControlName(),
										$cell['BrowsingResult']['browsing_structures_sub_id']);
									...
									$tmp_model = AppModel::getInstance($cell['DatamartStructure']['plugin'],
										$cell['DatamartStructure']['control_master_model'], true);
								...
								if (isset($search['counters'])) {
									foreach ($search['counters'] as $counter) {
										...
										$addon_params[] = array(
											'field' => __('counter') . ': ' . __($browsing_structure['DatamartStructure']['display_name']),
								...
								if (count($search['search_conditions']) || $adv_search || $addon_params) {//count might be zero if the only condition was the sub type
									...
									if ($cell['DatamartStructure']['adv_search_structure_alias']) {
										$adv_structure = StructuresComponent::$singleton->get('form',
											$cell['DatamartStructure']['adv_search_structure_alias']);
									}
					...
					if (isset($cell['merge']) && $cell['merge'] && !isset($cell['hide_merge_icon'])) {
						$controls = sprintf($controls,
							"<a class='icon16 link' href='" . $link . $current_node . "/0/" . $cell['BrowsingResult']['id'] . "' title='" . __("link to current view") . "'/>&nbsp;</a>");
					} else {
						if ($current_node_datamart_structure_id == $cell['DatamartStructure']['id'] && $cell['BrowsingResult']['id'] != $current_node) {
							$controls = sprintf($controls,
								"<a class='icon16 data_diff' href='" . $webroot_url . "Datamart/Reports/compareToBatchSetOrNode/node/" . $cell['BrowsingResult']['id'] . "/0/$current_node' title='" . __("compare contents") . "'/>&nbsp;</a>");
						} else {
							$controls = sprintf($controls, "");
						}
					}
	...
	static function formatSearchToPrint(array $params, $html_format = true) {
		...
		foreach ($search_conditions as $key => $value_element) {
						...
						if (!isset($sf_unit['StructureValueDomain']['StructurePermissibleValue'])) {
							if (isset($sf_unit['StructureValueDomain']['source']) && strlen($sf_unit['StructureValueDomain']['source']) > 0) {
								...
								$tmp_dropdown_result = $structure_value_domain_model->find('first', array(
									'recursive' => 2,
									'conditions' => array('StructureValueDomain.id' => $sf_unit['StructureValueDomain']['id'])
								));
			...
			if (count($values) > 6 && $html_format) {
				$html_result .= '<span class="databrowserShort">' . stripslashes(implode(", ", array_slice($values, 0,
						6))) . '</span>' . '<span class="databrowserAll hidden">, ' . stripslashes(implode(", ",
						array_slice($values,
							6))) . '</span>' . '<br/><a href="#" class="databrowserMore">' . __('and %d more',
						count($values) - 6) . '</a>';
			} else {
		...
		foreach ($params['adv_search'] as $key => $value) {
			...
			foreach ($params['adv_structure']['Sfs'] as $sfs) {
				if ($sfs['field'] == $key) {
					...
					$dm_structure = $dm_structure_model->find('first',
						array('conditions' => array('DatamartStructure.model' => $option['model'])));
					$sfs2 = $sfs_model->find('first', array(
						'conditions' => array('model' => $option['model'], 'field' => $option['field']),
						'recursive' => -1
					));
					$html_result .= sprintf("<tr><th>%s %s</th><td>%s %s</td>\n", __($sfs['language_label']),
						$option['relation'], __($dm_structure['DatamartStructure']['display_name']),
						__($sfs2['Sfs']['language_label']));
					$txt_result .= sprintf("%s %s %s %s\n", __($sfs['language_label']), $option['relation'],
						__($dm_structure['DatamartStructure']['display_name']), __($sfs2['Sfs']['language_label']));
				}
			}
		}
	...
	static function updateIndexLink($link, $prev_model, $new_model, $prev_pkey, $new_pkey) {
		return str_replace("%%" . $prev_model . ".", "%%" . $new_model . ".",
			str_replace("%%" . $prev_model . "." . $prev_pkey . "%%", "%%" . $new_model . "." . $new_pkey . "%%",
				$link));
	}
		...
		if ($browsing['DatamartStructure']['id'] == 5) {
			...
			if ($sm_data) {
				...
				$data = $ac->find('all', array(
					'conditions' => array(
						"AliquotControl.sample_control_id" => array_keys(AppController::defineArrayKey($sm_data,
							'SampleMaster', 'sample_control_id', true)),
			...
			if ($browsing['DatamartStructure']['id'] == 1) {
				...
				$ac = AppModel::getInstance("InventoryManagement", "AliquotControl", true);
				$data = $ac->find('all', array(
					'conditions' => array("AliquotControl.id" => $ctrl_ids, "AliquotControl.flag_active" => 1),
					'recursive' => -1
				));
				$ids = array();
				foreach ($data as $unit) {
	...
	/**
	 * @desc Databrowser lables are string that can be separated by |. Translation will occur on each subsection and
	 *     replace the pipes by " - "
	 *
		...
		foreach ($parts as &$part) {
			if (preg_match('/^custom\#(.+)\#(.+)$/', $part, $matches)) {
				if (!$StructurePermissibleValuesCustom) {
					$StructurePermissibleValuesCustom = AppModel::getInstance("", "StructurePermissibleValuesCustom",
						true);
				}
				$translated_value = $StructurePermissibleValuesCustom->getTranslatedCustomDropdownValue($matches[1],
					$matches[2]);
	...
	private function buildBufferedSearchParameters($primary_node_ids, $order) {
		...
		array_unshift($fields,
			'CONCAT("", ' . $node[self::MODEL]->name . "." . $node[self::USE_KEY] . ') AS `' . $node[self::MODEL]->name . '`');
	...
	public function initDataLoad(array $browsing, $merge_to, array $primary_node_ids, $order = null) {
		...
		foreach ($nodes_to_fetch as $node) {
			...
			$current_model = AppModel::getInstance($current_browsing['DatamartStructure']['plugin'],
				$current_browsing['DatamartStructure']['model'], true);
			...
			if ($last_browsing != null) {
				...
				$browsing_control = self::$browsing_control_model->find('first', array(
					'conditions' => array(
						'id1' => $last_browsing['DatamartStructure']['id'],
						'id2' => $current_browsing['DatamartStructure']['id']
					)
				));
				if (empty($browsing_control)) {
					...
					$browsing_control = self::$browsing_control_model->find('first', array(
						'conditions' => array(
							'id2' => $last_browsing['DatamartStructure']['id'],
							'id1' => $current_browsing['DatamartStructure']['id']
						)
					));
			...
			//update header
			$count = $current_model->find('count', array(
				'conditions' => array($current_model->name . "." . $current_model->primaryKey => $ids),
				'recursive' => -1
			));
		...
		$this->count = $this->nodes[0][self::MODEL]->find('count', array(
				'joins' => $this->search_parameters['joins'],
	...
	public function getDataChunk($chunk_size) {
		...
		if (empty($this->rows_buffer)) {
			...
			foreach ($this->models_buffer as $model_index => $model_ids) {
				...
				$model_data = AppController::defineArrayKey($model_data, $prefix . $node[self::MODEL]->name,
					$node[self::USE_KEY]);
	...
	function buildAdvancedSearchParameters(array &$params) {
		...
		foreach ($params['adv_struct']['Sfs'] as $field) {
			if ($field['field'] != 'browsing_filter' && isset($params['data'][$field['model']][$field['field']]) && $params['data'][$field['model']][$field['field']]) {
				...
				if ($adv_field && isset($adv_field[$params['data'][$curr_model->name][$field['field']]])) {
					...
					while ($parent_browsing = array_pop($path)) {
						if (!array_key_exists($parent_browsing['DatamartStructure']['model'], $joined_models)) {
							$control = $browsing_control_model->find('first', array(
								'conditions' => array(
									'BrowsingControl.id1' => $curr_id,
									'BrowsingControl.id2' => $parent_browsing['DatamartStructure']['id']
								)
							));
							$parent_browsing_model = AppModel::getInstance($parent_browsing['DatamartStructure']['plugin'],
								$parent_browsing['DatamartStructure']['control_master_model'] ?: $parent_browsing['DatamartStructure']['model'],
								true);

							if ($parent_browsing_model->name == $adv_field['model'] && !$parent_browsing_model->schema($adv_field['field'])) {
								//error, the field doesn't exist
								AppController::getInstance()->redirect('/Pages/err_internal?p[]=model+field+not+found',
									null, true);
							}
							...
							if ($control) {
								...
								$control = $browsing_control_model->find('first', array(
									'conditions' => array(
										'BrowsingControl.id2' => $curr_id,
										'BrowsingControl.id1' => $parent_browsing['DatamartStructure']['id']
									)
								));
								...
								$distinct_count = $parent_browsing_model->find('first', array(
									'conditions' => array(
										$parent_ref_key => explode(',', $parent_browsing['BrowsingResult']['id_csv'])
									),
				...
				$cond = sprintf('%s.%s %s %s.%s', $params['browsing_model']->name, $field['field'],
					$adv_field['relation'], $adv_field['model'], $adv_field['field']);
	...
	function createNode($params) {
		...
		$model_to_search = AppModel::getInstance($browsing['DatamartStructure']['plugin'],
			$browsing['DatamartStructure']['model'], true);
		$use_sub_model = null;
		$joins = array();
		...
		if ($params['sub_struct_ctrl_id'] && $ctrl_model = $model_to_search->getControlName()) {
			//sub structure
			$model_to_search = AppModel::getInstance($browsing['DatamartStructure']['plugin'],
				$browsing['DatamartStructure']['control_master_model'], true);
			$alternate_info = Browser::getAlternateStructureInfo($browsing['DatamartStructure']['plugin'], $ctrl_model,
				$params['sub_struct_ctrl_id']);
		...
		if ($params['node_id'] != 0) {
			...
			$parent = $browsing_result_model->find('first',
				array('conditions' => array("BrowsingResult.id" => $params['node_id'])));
			$control_data = $browsing_ctrl_model->find('first', array(
				'conditions' => array(
					'BrowsingControl.id1' => $parent['DatamartStructure']['id'],
					'BrowsingControl.id2' => $browsing['DatamartStructure']['id']
				)
			));
			$parent_model = AppModel::getInstance($parent['DatamartStructure']['plugin'],
				$parent['DatamartStructure']['control_master_model'] ?: $parent['DatamartStructure']['model'], true);
			if (!empty($control_data)) {
				$to_join = array(
					...
					'type' => 'INNER',
					'conditions' => array(
						$parent_model->name . '_2.' . $control_data['BrowsingControl']['use_field'] . ' = ' . $select_key,
						$parent_model->name . '_2.' . $parent_model->primaryKey => explode(',',
							$parent['BrowsingResult']['id_csv'])
					)
				);
				if ($params['parent_child'] == 'c') {
					...
					$to_join['conditions'] = array(
						//WRONG KEY
						$parent_model->name . '_2.' . $control_data['BrowsingControl']['use_field'] => explode(',',
							$parent['BrowsingResult']['id_csv']),
				...
				assert($control_data);
				$search_conditions[$model_to_search->name . '.' . $control_data['BrowsingControl']['use_field']] = explode(',',
					$parent['BrowsingResult']['id_csv']);
			}
		}
		...
		if ($browsing['DatamartStructure']['adv_search_structure_alias']) {
			$advanced_structure = $controller->Structures->get('form',
				$browsing['DatamartStructure']['adv_search_structure_alias']);
			$adv_params = array(
			...
			if ($error_model_display_name != null) {
				...
				$controller->flash(__("a special parameter could not be applied because relations between %s and its children node are shared",
					__($error_model_display_name)), 'javascript:history.back()');
				return false;
			}
		...
		if (isset($data[0])) {
			...
			if ($possible_counters_conditions) {
				...
				foreach ($possible_counters_conditions as $name => $val) {
					...
					if (!$found) {
						//join on it
						$joins[] = $browsing_control_model->getInnerJoinArray($model_to_search->name,
							$browsing_result['BrowsingResult']['browsing_structures_id'],
							explode(',', $browsing_result['BrowsingResult']['id_csv']));
					}
					$model = $datamart_structure_model->getModel($browsing_result['BrowsingResult']['browsing_structures_id']);
					if ($val) {
						$org_search_conditions['counters'][] = array(
							'browsing_structures_id' => $browsing_result['BrowsingResult']['browsing_structures_id'],
		...
		if ($browsing_filter && $save_ids) {
			...
			if ($model_to_search->schema($browsing_filter['field'] . '_accuracy')) {
				...
				$select_field = sprintf(AppModel::ACCURACY_REPLACE_STR, $browsing_filter['field'],
					$browsing_filter['field'] . '_accuracy', $browsing_filter['attribute'] == 'MAX' ? '"\n"' : '"A"',
					//non year
			...
			$query = sprintf('CREATE TEMPORARY TABLE %1$s (SELECT %2$s, %3$s AS order_field, " " AS accuracy FROM %4$s WHERE %5$s GROUP BY %2$s)',
				$temporary_table, $browsing_filter['group by'], $select_field, $model_to_search->table,
			...
			if ($model_to_search->schema($browsing_filter['field'] . '_accuracy')) {
				...
				if ($org_field_info['type'] == 'date') {
					$model_to_search->tryCatchQuery(sprintf($query, 4, '-01-01', 'y',
							5) . ' AND INSTR(order_field, "' . ($browsing_filter['attribute'] == 'MAX' ? '\t' : 'B') . '")!=0');
					$model_to_search->tryCatchQuery(sprintf($query, 4, '-01-01', 'm', 5));
					$model_to_search->tryCatchQuery(sprintf($query, 7, '-01', 'd', 8));
				} else {
					//datetime
					$model_to_search->tryCatchQuery(sprintf($query, 4, '-01-01 00:00:00', 'y',
							5) . ' AND INSTR(order_field, "' . ($browsing_filter['attribute'] == 'MAX' ? '\t' : 'B') . '")!=0');
			...
			if ($model_to_search->schema($browsing_filter['field'] . '_accuracy')) {
				$joins[0]['conditions'][] = sprintf('TmpTable.accuracy = %1$s.%2$s', $model_to_search->name,
		...
		if (strlen($save_ids) == 0) {
			...
			if ($params['last']) {
				...
				//go to the last node
				$controller->flash(__("you cannot browse to the requested entities because there is no [%s] matching your request",
					$browsing['DatamartStructure']['display_name']), "/Datamart/Browser/browse/" . $node_id . "/", 5);
			}
			return false;
		}
	...
	function buildDrillDownIfNeeded($data, &$node_id) {
		...
		if (isset($data[$parent['DatamartStructure']['model']]) && isset($data['Browser'])) {
			$parent_model = AppModel::getInstance($parent['DatamartStructure']['plugin'],
				$parent['DatamartStructure']['model'], true);
			...
			if (!$parent['BrowsingResult']['raw']) {
				...
				$parent = $browsing_result_model->find('first',
					array('conditions' => array("BrowsingResult.id" => $parent['BrowsingResult']['parent_id'])));

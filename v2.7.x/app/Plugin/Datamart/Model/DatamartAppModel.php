...
class DatamartAppModel extends AppModel {
	...
	function getDropdownOptions(
		...
		$compatible_batch_sets = $batch_set_model->getCompatibleBatchSets($plugin_name, $model_name,
			$datamart_structure_id, $batch_set_id);
		...
		foreach ($compatible_batch_sets as $batch_set) {
			$batch_set_label = str_replace(array('  ', "\t", "\n", "\r"), ' ',
				$batch_set_model->getBatchSetLabel($batch_set['BatchSet']));
			$add_to_batch_set_menu[] = array(
				'value' => '0',
		...
		$functions = $structure_functions->find('all', array(
			'conditions' => array(
				'DatamartStructureFunction.datamart_structure_id' => $datamart_structure_id,

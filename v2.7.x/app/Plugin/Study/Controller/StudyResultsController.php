...
class StudyResultsController extends StudyAppController {
	...
	function listall($study_summary_id) {
		...
		// MANAGE DATA
		$study_results_data = $this->StudySummary->find('first',
			array('conditions' => array('StudySummary.id' => $study_summary_id), 'recursive' => '-1'));
		if (empty($study_results_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		}

		$this->request->data = $this->paginate($this->StudyResult,
			array('StudyResult.study_summary_id' => $study_summary_id));
	...
	function detail($study_summary_id, $study_results_id) {
		...
		// MANAGE DATA
		$study_results_data = $this->StudyResult->find('first', array(
			'conditions' => array(
				'StudyResult.id' => $study_results_id,
				'StudyResult.study_summary_id' => $study_summary_id
			)
		));
		if (empty($study_results_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		...
		$this->set('atim_menu_variables',
			array('StudySummary.id' => $study_summary_id, 'StudyResult.id' => $study_results_id));
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyResult->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/Study/StudyResults/detail/' . $study_summary_id . '/' . $this->StudyResult->id);
				}
			}
		}
	}
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyResult->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/Study/StudyResults/detail/' . $study_summary_id . '/' . $study_results_id);
				}
			}
		}
	}
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			if ($this->StudyResult->atimDelete($study_results_id)) {
				...
				$this->atimFlash(__('error deleting data - contact administrator.'),
					'/Study/StudyResults/listall/' . $study_summary_id);
			}
		} else {

...
class StudyContactsController extends StudyAppController {
	...
	function listall($study_summary_id) {
		...
		// MANAGE DATA
		$study_contact_data = $this->StudySummary->find('first',
			array('conditions' => array('StudySummary.id' => $study_summary_id), 'recursive' => '-1'));
		if (empty($study_contact_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		}

		$this->request->data = $this->paginate($this->StudyContact,
			array('StudyContact.study_summary_id' => $study_summary_id));
	...
	function detail($study_summary_id, $study_contact_id) {
		...
		// MANAGE DATA
		$study_contact_data = $this->StudyContact->find('first', array(
			'conditions' => array(
				'StudyContact.id' => $study_contact_id,
				'StudyContact.study_summary_id' => $study_summary_id
			)
		));
		if (empty($study_contact_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		...
		$this->set('atim_menu_variables',
			array('StudySummary.id' => $study_summary_id, 'StudyContact.id' => $study_contact_id));
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyContact->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/Study/StudyContacts/detail/' . $study_summary_id . '/' . $this->StudyContact->id);
				}
			}
		}
	}
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyContact->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/Study/StudyContacts/detail/' . $study_summary_id . '/' . $study_contact_id);
				}
			}
		}
	}
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			if ($this->StudyContact->atimDelete($study_contact_id)) {
				...
				$this->atimFlash(__('error deleting data - contact administrator.'),
					'/Study/StudyContacts/listall/' . $study_summary_id);
			}
		} else {

...
class StudyFundingsController extends StudyAppController {
	...
	function listall($study_summary_id) {
		...
		// MANAGE DATA
		$study_funding_data = $this->StudySummary->find('first',
			array('conditions' => array('StudySummary.id' => $study_summary_id), 'recursive' => '-1'));
		if (empty($study_funding_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		}

		$this->request->data = $this->paginate($this->StudyFunding,
			array('StudyFunding.study_summary_id' => $study_summary_id));
	...
	function detail($study_summary_id, $study_funding_id) {
		...
		// MANAGE DATA
		$study_funding_data = $this->StudyFunding->find('first', array(
			'conditions' => array(
				'StudyFunding.id' => $study_funding_id,
				'StudyFunding.study_summary_id' => $study_summary_id
			)
		));
		if (empty($study_funding_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		...
		$this->set('atim_menu_variables',
			array('StudySummary.id' => $study_summary_id, 'StudyFunding.id' => $study_funding_id));
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyFunding->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/Study/StudyFundings/detail/' . $study_summary_id . '/' . $this->StudyFunding->id);
				}
			}
		}
	}
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyFunding->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/Study/StudyFundings/detail/' . $study_summary_id . '/' . $study_funding_id);
				}
			}
		}
	}
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			if ($this->StudyFunding->atimDelete($study_funding_id)) {
				...
				$this->atimFlash(__('error deleting data - contact administrator.'),
					'/Study/StudyFundings/listall/' . $study_summary_id);
			}
		} else {
			$this->atimFlash(__($arr_allow_deletion['msg']),
				'/Study/StudyFundings/detail/' . $study_summary_id . '/' . $study_fundings_id);
		}
	}
}

		...
		if( $hook_link ) { require($hook_link); }
			...
			if($arr_allow_deletion['allow_deletion']) {
				...
				if( $this->StudyRelated->atimDelete( $study_related_id ) ) {
					...
					$this->atimFlash(__('error deleting data - contact administrator.'), '/Study/StudyRelated/listall/'.$study_summary_id );
				}
			}else {
					$this->atimFlash(__($arr_allow_deletion['msg']), '/Study/StudyRelated/detail/'.$study_summary_id.'/'.$study_related_id);
			}
	}
}

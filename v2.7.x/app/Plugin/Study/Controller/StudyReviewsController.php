		...
		if( $hook_link ) { require($hook_link); }
			...
			if($arr_allow_deletion['allow_deletion']) {
				...
				if( $this->StudyReview->atimDelete( $study_reviews_id ) ) {
					...
					$this->atimFlash(__('error deleting data - contact administrator.'), '/Study/StudyReviews/listall/'.$study_summary_id );
				}
			}else {
					$this->atimFlash(__($arr_allow_deletion['msg']), '/Study/StudyReviews/detail/'.$study_summary_id.'/'.$study_reviews_id);
			}
	}
}

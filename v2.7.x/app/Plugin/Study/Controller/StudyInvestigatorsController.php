...
class StudyInvestigatorsController extends StudyAppController {
	...
	function listall($study_summary_id) {
		...
		// MANAGE DATA
		$study_investigator_data = $this->StudySummary->find('first',
			array('conditions' => array('StudySummary.id' => $study_summary_id), 'recursive' => '-1'));
		if (empty($study_investigator_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		}

		$this->request->data = $this->paginate($this->StudyInvestigator,
			array('StudyInvestigator.study_summary_id' => $study_summary_id));
	...
	function detail($study_summary_id, $study_investigator_id) {
		...
		if (!$study_investigator_id) {
			$this->redirect('/Pages/err_plugin_funct_param_missing?method=' . __METHOD__ . ',line=' . __LINE__, null,
				true);
		}

		// MANAGE DATA
		$study_investigator_data = $this->StudyInvestigator->find('first', array(
			'conditions' => array(
				'StudyInvestigator.id' => $study_investigator_id,
				'StudyInvestigator.study_summary_id' => $study_summary_id
			)
		));
		if (empty($study_investigator_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		...
		$this->set('atim_menu_variables',
			array('StudySummary.id' => $study_summary_id, 'StudyInvestigator.id' => $study_investigator_id));
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyInvestigator->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/Study/StudyInvestigators/detail/' . $study_summary_id . '/' . $this->StudyInvestigator->id);
				}
			}
		}
	}
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudyInvestigator->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/Study/StudyInvestigators/detail/' . $study_summary_id . '/' . $study_investigator_id);
				}
			}
		}
	}
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			if ($this->StudyInvestigator->atimDelete($study_investigator_id)) {
				$this->atimFlash(__('your data has been deleted'),
					'/Study/StudyInvestigators/listall/' . $study_summary_id);
			} else {

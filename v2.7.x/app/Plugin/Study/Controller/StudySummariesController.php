...
class StudySummariesController extends StudyAppController {
			...
			if ($submitted_data_validates) {
				if ($this->StudySummary->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/Study/StudySummaries/detail/' . $this->StudySummary->id);
				}
			}
		}
	}
			...
			if ($submitted_data_validates) {
				...
				if ($this->StudySummary->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/Study/StudySummaries/detail/' . $study_summary_id);
				}
			}
		}
	}
	...
	function delete($study_summary_id) {
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			if ($this->StudySummary->atimDelete($study_summary_id)) {
				...
				$this->atimFlash(__('error deleting data - contact administrator'), '/Study/StudySummaries/search/');
			}
		} else {
			$this->atimFlash(__($arr_allow_deletion['msg']), '/Study/StudySummaries/detail/' . $study_summary_id);
		}
	}
	...
	function listAllLinkedRecords($study_summary_id, $specific_list_header = null) {
		if (!$this->request->is('ajax')) {
			$this->set('atim_menu',
				$this->Menus->get('/Study/StudySummaries/listAllLinkedRecords/%%StudySummary.id%%/'));
			$this->set('atim_menu_variables', array('StudySummary.id' => $study_summary_id));
		}
		...
		if (!$specific_list_header) {
			...
			if ($linked_records_properties[$specific_list_header]) {
				...
				$this->request->data = $this->paginate($this->{$model},
					array("$model.$foreign_key" => $study_summary_id));

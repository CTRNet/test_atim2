...
class StorageControlsController extends AdministrateAppController {
	...
	public function listAll() {
		...
		if (empty($list_args)) {
			if (!isset($_SESSION['StorageCtrl']['ListAllArgs'])) {
				$_SESSION['StorageCtrl']['ListAllArgs'] = array();
			}
		} else {
	...
	public function add($storage_category, $duplicated_parent_storage_control_id = null) {
		...
		if (!$duplicated_parent_storage_control_id && !empty($this->request->data)) {
			...
			if ($submitted_data_validates) {
				...
				if ($this->StorageCtrl->save($this->request->data)) {
					$control_data = $this->StructurePermissibleValuesCustomControl->find('first',
						array('conditions' => array('StructurePermissibleValuesCustomControl.name' => 'storage types')));
					...
					$this->atimFlash(__('your data has been saved') . '<br>' . __('please use custom drop down list administration tool to add storage type translations'),
						'/Administrate/StorageControls/listAll/');
				}
			}
		}
	}

	public function edit($storage_control_id) {

		$storage_control_data = $this->StorageCtrl->getOrRedirect($storage_control_id);
		if ($storage_control_data['StorageCtrl']['flag_active']) {
			$this->atimFlash(__('you are not allowed to work on active storage type'),
				'/Administrate/StorageControls/listAll/');
			return;
		} else {
			if ($this->StorageMaster->find('count', array(
				'conditions' => array(
					...
					'StorageMaster.deleted' => array('0', '1')
				)
			))
			) {
				$this->atimFlash(__('this storage type has already been used to build a storage in the past - properties can not be changed anymore'),
					'/Administrate/StorageControls/listAll/');
				return;
			}
		}
	...
	public function changeActiveStatus($storage_control_id) {
		$storage_control_data = $this->StorageCtrl->getOrRedirect($storage_control_id);

		$new_data = array();
		if ($storage_control_data['StorageCtrl']['flag_active']) {
			// Check no Storage Master use it
			$existing_storage_count = $this->StorageMaster->find('count',
				array('conditions' => array('StorageMaster.storage_control_id' => $storage_control_id)));
			if ($existing_storage_count) {
				$this->atimFlash(__('this storage type has already been used to build a storage - active status can not be changed'),
					'/Administrate/StorageControls/listAll/');
				return;
			}
	...
	public function seeStorageLayout($storage_control_id) {
		$storage_control_data = $this->StorageCtrl->getOrRedirect($storage_control_id);
		$storage_category = $this->StorageCtrl->getStorageCategory($storage_control_data);
		if ($storage_category == 'no_d') {
			...
			if ($storage_control_data['StorageCtrl']['coord_x_type'] == 'list') {
				$this->atimFlash(__('custom layout will be built adding coordinates to a created storage'),
					'/Administrate/StorageControls/listAll/');
				return;
			}
		}
		$translated_storage_type = $this->StructurePermissibleValuesCustom->getTranslatedCustomDropdownValue('storage types',
			$storage_control_data['StorageCtrl']['storage_type']);

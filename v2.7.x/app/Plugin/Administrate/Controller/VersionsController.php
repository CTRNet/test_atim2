...
class VersionsController extends AdministrateAppController {
	...
	public $paginate = array('Version' => array('limit' => PAGINATION_AMOUNT, 'order' => 'Version.version_number'));
	...
	public function test() {
		...
		foreach ($to_test as $unit) {
			...
			foreach ($control_data as $data) {
				...
				$master->find("all",
					array('conditions' => array($master_name . "." . $master->belongsTo[$control_name]["foreignKey"] => $data[$control_name]['id'])));
				echo("</li>");
			}
			echo("</ul></li>");
		}

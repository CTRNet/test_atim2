...
class GroupsController extends AdministrateAppController {
	...
	public $uses = array('Group', 'Aco', 'Aro', 'User');

	public $paginate = array('Group' => array('limit' => PAGINATION_AMOUNT, 'order' => 'Group.name ASC'));
	...
	public function add() {
		...
		if (!empty($this->request->data)) {
			$group_data = $this->Group->find('first',
				array('conditions' => array('Group.name' => $this->request->data['Group']['name'])));
			if (empty($group_data)) {
				if ($this->Group->save($this->request->data)) {
					...
					$aro_data = $this->Aro->find('first',
						array('conditions' => 'Aro.model="Group" AND Aro.foreign_key = "' . $group_id . '"'));
	...
	public function edit($group_id) {
		if ($group_id == 1) {
			$this->Flash->error(__('the group administrators cannot be edited'), '/Administrate/Groups/detail/1');
		}
		...
		if (!empty($this->request->data)) {
			$duplicated_name = $this->Group->find('count', array(
				'conditions' => array(
					"Group.id != $group_id",
		...
		if (empty($this->request->data)) {
			...
			$this->request->data = $this->Group->find('first',
				array('conditions' => 'Group.id="' . $group_id . '"', 'recursive' => 2));
		}
	...
	public function delete($group_id) {
		if ($group_id == 1) {
			$this->Flash->error(__('the group administrators cannot be deleted'), '/Administrate/Groups/detail/1');
		}
		...
		if (empty($this->request->data)) {
			...
			$this->Flash->error(__('this group is being used and cannot be deleted'),
				'/Administrate/Groups/detail/' . $group_id . "/");
		}
	}
}
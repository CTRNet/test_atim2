<?php
/**
 * Increments/decrements the var according to the reverseOrder option and returns true/false based on reverseOrder and
 * the limit
 *
...
if ($data['parent']['StorageControl']['coord_x_type'] == 'list') {
	...
	foreach ($data['parent']['list'] as $list_item) {
		$csv_layout_data[][0] = array(
			'x_y' => $list_item['StorageCoordinate']['id'] . '_1',
			'display_value' => $list_item['StorageCoordinate']['coordinate_value'],
	...
	if ((strlen($x_size) == 0 || strlen($y_size) == 0) && ($data['parent']['StorageControl']['display_x_size'] > 0 || $data['parent']['StorageControl']['display_y_size'] > 0)) {
		...
		if (max(1, $data['parent']['StorageControl']['coord_x_size']) * max(1,
				$data['parent']['StorageControl']['coord_y_size']) != $x_size * $y_size
		) {
			...
			$errors[] = array(
				"Real storage cells: " . (($data['parent']['StorageControl']['coord_x_size']) * max(1,
						$data['parent']['StorageControl']['coord_y_size']))
			);
		...
		while (axisLoopCondition($j, $data['parent']['StorageControl']['reverse_y_numbering'], $use_height)) {
			...
			while (axisLoopCondition($i, $data['parent']['StorageControl']['reverse_x_numbering'], $use_width)) {
				...
				$csv_layout_line_data[] = array(
					...
					'display_value_x' => $display_value_x,
					'display_value_y' => $display_value_y
				);
			}
...
if (!$errors) {
	...
	foreach ($data['children'] as $new_content) {
		$x_y_values_to_cell_content_labels[$new_content['DisplayData']['x'] . "_" . $new_content['DisplayData']['y']][] = $new_content['DisplayData']['label'];
	}
	...
	if ($data['parent']['StorageControl']['coord_y_type']) {
		...
		foreach ($csv_layout_data as $csv_layout_line_data) {
			...
			foreach ($csv_layout_line_data as $csv_layout_cell_data) {
				...
				$csv_line_content_data[] = isset($x_y_values_to_cell_content_labels[$csv_layout_cell_data['x_y']]) ? implode(' / ',
					$x_y_values_to_cell_content_labels[$csv_layout_cell_data['x_y']]) : '';
				unset($x_y_values_to_cell_content_labels[$csv_layout_cell_data['x_y']]);
			}
			if ($first_line_record) {
				$csv_data[] = $x_headers;
			}
		...
		if (sizeof($csv_layout_data) == 1) {
			...
			foreach ($csv_layout_data[0] as $csv_layout_cell_data) {
				...
				//Content
				$csv_data[1][] = isset($x_y_values_to_cell_content_labels[$csv_layout_cell_data['x_y']]) ? implode(' / ',
					$x_y_values_to_cell_content_labels[$csv_layout_cell_data['x_y']]) : '';
				unset($x_y_values_to_cell_content_labels[$csv_layout_cell_data['x_y']]);
			}
		} else {
			if (sizeof($csv_layout_data[0]) == 1) {
				//One dimension layout display as a column with only x coordinates tracked
				foreach ($csv_layout_data as $csv_layout_line_data) {
					$csv_layout_cell_data = $csv_layout_line_data[0];
					$csv_data[] = array(
						$csv_layout_cell_data['display_value'],
						(isset($x_y_values_to_cell_content_labels[$csv_layout_cell_data['x_y']]) ? implode(' / ',
							$x_y_values_to_cell_content_labels[$csv_layout_cell_data['x_y']]) : '')
					);
					unset($x_y_values_to_cell_content_labels[$csv_layout_cell_data['x_y']]);
				}
	...
	if ($x_y_values_to_cell_content_labels) {
		...
		foreach ($x_y_values_to_cell_content_labels as $new_labels) {
			$unclassified_labels = array_merge($unclassified_labels, $new_labels);
		}
...
$this->Csv->addRow(array(''));
foreach ($csv_data as $new_data_line) {
	$this->Csv->addRow($new_data_line);
}
echo $this->Csv->render(true,
	isset(AppController::getInstance()->csv_config) ? AppController::getInstance()->csv_config['define_csv_encoding'] : CSV_ENCODING);
exit;

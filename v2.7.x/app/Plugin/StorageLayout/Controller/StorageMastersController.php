...
class StorageMastersController extends StorageLayoutAppController {
	...
	function search($search_id = 0, $from_layout_page = false) {
		...
		if ($from_layout_page) {
			...
			$this->searchHandler($search_id, $model_to_use, $structure_alias, $structure_index, false, 21);
			if (count($this->request->data) > 20) {
				...
				foreach ($this->request->data as $key => $data) {
					if ($data['StorageControl']['coord_x_type'] == null) {
						...
						if ($data['StorageControl']['coord_x_type'] == 'list' && !$this->StorageCoordinate->find('count',
								array(
									'conditions' => array('StorageCoordinate.storage_master_id' => $data['StorageMaster']['id']),
									'recursive' => '-1'
								))
						) {
							unset($this->request->data[$key]);
			...
			$this->searchHandler($search_id, $model_to_use, $structure_alias, $structure_index);
		}
	...
	function detail($storage_master_id, $is_from_tree_view_or_layout = 0, $storage_category = null) {
		...
		$parent_storage_data = $this->StorageMaster->find('first',
			array('conditions' => array('StorageMaster.id' => $parent_storage_id)));
		...
		$display_layout = true;
		if (!$this->StorageControl->allowCustomCoordinates($data['StorageControl']['id'],
			array('StorageControl' => $data['StorageControl']))
		) {
			...
			if (!$this->StorageCoordinate->find('count', array(
				'conditions' => array('StorageCoordinate.storage_master_id' => $storage_master_id),
				'recursive' => '-1'
			))
			) {
		...
		if (!$is_from_tree_view_or_layout && $display_layout) {
			if (empty($data['StorageControl']['coord_y_type'])) {
				...
				if ($this->StorageMaster->find('count', array(
						...
						'conditions' => array(
							'AliquotMaster.storage_master_id' => $storage_master_id,
							'OR' => array('AliquotMaster.storage_coord_x' => '', 'AliquotMaster.storage_coord_y' => '')
						)
					))
				) {
					AppController::addWarningMsg(__('at least one stored element is not displayed in layout'));
	...
	function add($storage_control_id, $predefined_parent_storage_id = null) {
		...
		if (!$storage_control_data['StorageControl']['flag_active']) {
		...
		if (!is_null($predefined_parent_storage_id)) {
			$predefined_parent_storage_data = $this->StorageMaster->find('first', array(
				'conditions' => array(
					'StorageMaster.id' => $predefined_parent_storage_id,
			...
			$this->set('predefined_parent_storage_selection_label',
				$this->StorageMaster->getStorageLabelAndCodeForDisplay($predefined_parent_storage_data));
		...
		if (!empty($this->request->data)) {
			...
			if ($submitted_data_validates) {
				...
				if ($bool_save_done) {
					...
					$view_storage_master_model->manageViewUpdate('view_storage_masters', 'StorageMaster.id',
						array($storage_master_id), $view_storage_master_model::$table_query);
				}
				...
				if ($bool_save_done) {
					$this->atimFlash(__('your data has been saved'),
						'/StorageLayout/StorageMasters/detail/' . $storage_master_id);
				}
			}
		}
	}
	...
	function edit($storage_master_id) {
		...
		if (!empty($storage_data['StorageMaster']['parent_id'])) {
			$predefined_parent_storage_data = $this->StorageMaster->find('first', array(
				'conditions' => array(
					'StorageMaster.id' => $storage_data['StorageMaster']['parent_id'],
					'StorageControl.is_tma_block' => '0'
				)
			));
			if (empty($predefined_parent_storage_data)) {
		...
		if (!$this->StorageControl->allowCustomCoordinates($storage_data['StorageControl']['id'],
			array('StorageControl' => $storage_data['StorageControl']))
		) {
			// Check storage supports custom coordinates and disable access to coordinates menu option if required
			$atim_menu = $this->inactivateStorageCoordinateMenu($atim_menu);
		}
		...
		if (empty($this->request->data)) {
			...
			if ($submitted_data_validates) {
				...
				// Set storage temperature information
				$this->StorageMaster->manageTemperature($this->request->data,
					array('StorageControl' => $storage_data['StorageControl']));
			}
			...
			if ($submitted_data_validates) {
				...
				if ($this->StorageMaster->save($this->request->data, false)) {
					...
					$storage_temperature = (array_key_exists('temperature',
						$this->request->data['StorageMaster'])) ? $this->request->data['StorageMaster']['temperature'] : $storage_data['StorageMaster']['temperature'];
					$storage_temp_unit = (array_key_exists('temp_unit',
						$this->request->data['StorageMaster'])) ? $this->request->data['StorageMaster']['temp_unit'] : $storage_data['StorageMaster']['temp_unit'];
					$this->StorageMaster->updateChildrenSurroundingTemperature($storage_master_id, $storage_temperature,
						$storage_temp_unit);

					// Manage children selection label
					if (strcmp($this->request->data['StorageMaster']['selection_label'],
							$storage_data['StorageMaster']['selection_label']) != 0
					) {
						$this->StorageMaster->updateChildrenStorageSelectionLabel($storage_master_id,
							$this->request->data);
					}
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			if ($atim_flash) {
				...
				$this->atimFlash(__('error deleting data - contact administrator'),
					'/StorageLayout/StorageMasters/search/');
			}
		} else {
	...
	function contentTreeView($storage_master_id = 0, $is_ajax = false) {
		...
		if ($storage_master_id) {
			$storage_data = $this->StorageMaster->getOrRedirect($storage_master_id);
			$tree_data = $this->StorageMaster->find('all',
				array('conditions' => array('StorageMaster.parent_id' => $storage_master_id), 'recursive' => '0'));
			...
			if (sizeof($tree_data) > $storages_nbr_limit) {
				$tree_data = array(array('Generated' => array('storage_tree_view_item_summary' => __('storage contains too many children storages for display') . ' (' . sizeof($tree_data) . ')')));
			}
			$aliquots = $this->AliquotMaster->find('all', array(
				'conditions' => array('AliquotMaster.storage_master_id' => $storage_master_id),
				'recursive' => '0'
			));
			$aliquots = $this->StorageMaster->contentNatCaseSort($fields_to_sort_on['AliquotMaster'], $aliquots);
			if (sizeof($aliquots) > $aliquots_nbr_limit) {
				$aliquots = array(array('Generated' => array('storage_tree_view_item_summary' => __('storage contains too many aliquots for display') . ' (' . sizeof($aliquots) . ')')));
			}
			$tree_data = array_merge($tree_data, $aliquots);
			$tma_slides = $this->TmaSlide->find('all',
				array('conditions' => array('TmaSlide.storage_master_id' => $storage_master_id), 'recursive' => '0'));
			$tma_slides = $this->StorageMaster->contentNatCaseSort($fields_to_sort_on['TmaSlide'], $tma_slides);
			if (sizeof($tma_slides) > $tma_slides_nbr_limit) {
				$tma_slides = array(array('Generated' => array('storage_tree_view_item_summary' => __('storage contains too many tma slides for display') . ' (' . sizeof($tma_slides) . ')')));
			}
			...
			$tree_data = $this->StorageMaster->find('all', array(
				'conditions' => array('StorageMaster.parent_id IS NULL'),
				'order' => 'CAST(StorageMaster.parent_storage_coord_x AS signed), CAST(StorageMaster.parent_storage_coord_y AS signed)',
				'recursive' => '0'
			));
			$tree_data = $this->StorageMaster->contentNatCaseSort($fields_to_sort_on['InitialStorageMaster'],
				$tree_data);
			if (sizeof($tree_data) > $storages_nbr_limit) {
				$this->atimFlash(__('there are too many main storages for display'),
					'/StorageLayout/StorageMasters/search/');
				return;
			}
			$atim_menu = $this->Menus->get('/StorageLayout/StorageMasters/search');
		...
		foreach ($tree_data as &$data_unit) {
			...
			$data_unit['children'] = isset($data_unit['StorageMaster']) && !isset($data_unit['TmaSlide']) && !isset($data_unit['AliquotMaster']) && array_key_exists($data_unit['StorageMaster']['id'],
					$ids);
		}
	...
	function storageLayout($storage_master_id, $is_ajax = false, $csv_creation = false) {
		...
		if ($storage_data['StorageControl']['coord_x_type'] == "list") {
			$coordinate_tmp = $this->StorageCoordinate->find('all', array(
				'conditions' => array('StorageCoordinate.storage_master_id' => $storage_master_id),
				'recursive' => '-1',
			...
			if (empty($parent_coordinate_list)) {
				if ($is_ajax) {
					...
					$this->atimFlash(__('no layout exists - add coordinates first'),
						'/StorageLayout/StorageMasters/detail/' . $storage_master_id);
					return;
				}
			}
		...
		if (empty($storage_data['StorageControl']['coord_x_type'])) {
			if ($is_ajax) {
				...
				$this->atimFlash(__('no storage layout is defined for this storage type'),
					'/StorageLayout/StorageMasters/detail/' . $storage_master_id);
				return;
			}
		}
		...
		if (!empty($this->request->data)) {
			if ($csv_creation) {
				...
				if ($storage_data[$second_storage_id]['StorageControl']['coord_x_type'] == "list") {
					$coordinate_tmp = $this->StorageCoordinate->find('all', array(
						'conditions' => array('StorageCoordinate.storage_master_id' => $second_storage_id),
						'recursive' => '-1',
						'order' => 'StorageCoordinate.order ASC'
					));
					foreach ($coordinate_tmp as $key => $value) {
				...
				$storages_initial_data = isset($data['StorageMaster']) ? $this->StorageMaster->find('all',
					array('conditions' => array('StorageMaster.id' => array_keys($data['StorageMaster'])))) : array();
				$aliquots_initial_data = isset($data['AliquotMaster']) ? $this->AliquotMaster->find('all',
					array('conditions' => array('AliquotMaster.id' => array_keys($data['AliquotMaster'])))) : array();
				$tmas_initial_data = isset($data['TmaSlide']) ? $this->TmaSlide->find('all',
					array('conditions' => array('TmaSlide.id' => array_keys($data['TmaSlide'])))) : array();

				//manual validate/alteration of positions based on position conflict checks
				$storage_config = array();
				$conflicts_found = $this->StorageMaster->checkBatchLayoutConflicts($data, 'StorageMaster',
					'selection_label', $storage_config);
				$conflicts_found = $this->StorageMaster->checkBatchLayoutConflicts($data, 'AliquotMaster', 'barcode',
						$storage_config) || $conflicts_found;
				$conflicts_found = $this->StorageMaster->checkBatchLayoutConflicts($data, 'TmaSlide', 'barcode',
						$storage_config) || $conflicts_found;
				...
				$this->StorageMaster->updateAndSaveDataArray($storages_initial_data, "StorageMaster",
					"parent_storage_coord_x", "parent_storage_coord_y", "parent_id", $data, $this->StorageMaster,
					$storage_data);

				//Update AliquotMaster
				$this->AliquotMaster->check_writable_fields = false;
				$this->StorageMaster->updateAndSaveDataArray($aliquots_initial_data, "AliquotMaster", "storage_coord_x",
					"storage_coord_y", "storage_master_id", $data, $this->AliquotMaster, $storage_data);

				//Update TmaSlide
				$this->TmaSlide->check_writable_fields = false;
				$this->StorageMaster->updateAndSaveDataArray($tmas_initial_data, "TmaSlide", "storage_coord_x",
					"storage_coord_y", "storage_master_id", $data, $this->TmaSlide, $storage_data);
				...
				if ($conflicts_found) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/StorageLayout/StorageMasters/storageLayout/' . $storage_master_id);
					return;
				}
			}
		}
		...
		$storage_master_c = $this->StorageMaster->find('all',
			array('conditions' => array('StorageMaster.parent_id' => $storage_master_id)));
		$storage_master_c = $this->StorageMaster->contentNatCaseSort($fields_to_sort_on['StorageMaster'],
			$storage_master_c, true);
		$aliquot_master_c = $this->AliquotMaster->find('all',
			array('conditions' => array('AliquotMaster.storage_master_id' => $storage_master_id), 'recursive' => '-1'));
		$aliquot_master_c = $this->StorageMaster->contentNatCaseSort($fields_to_sort_on['AliquotMaster'],
			$aliquot_master_c, true);
		$tma_slide_c = $this->TmaSlide->find('all',
			array('conditions' => array('TmaSlide.storage_master_id' => $storage_master_id), 'recursive' => '-1'));
		...
		if (!$storage_data['StorageControl']['is_tma_block']) {
			$this->set('storage_types_from_id', $storage_types_from_id);
		}
		...
		foreach ($data['children'] as &$children_array) {
			if (isset($children_array['StorageMaster'])) {
				...
				$this->StorageMaster->buildChildrenArray($children_array, "StorageMaster", "parent_storage_coord_x",
					"parent_storage_coord_y", "selection_label", $rkey_coordinate_list, $link, "storage");
			} else {
				if (isset($children_array['AliquotMaster'])) {
					...
					$this->StorageMaster->buildChildrenArray($children_array, "AliquotMaster", "storage_coord_x",
						"storage_coord_y", "barcode", $rkey_coordinate_list, $link, "aliquot");
				} else {
					if (isset($children_array['TmaSlide'])) {
						...
						$this->StorageMaster->buildChildrenArray($children_array, "TmaSlide", "storage_coord_x",
							"storage_coord_y", "barcode", $rkey_coordinate_list, $link, "slide");
					}
				}
			}
		}
	...
	function autocompleteLabel() {
		...
		foreach ($storage_masters as $storage_master) {
			...
			$result .= '"' . $storage_master['StorageMaster']['selection_label'] . ' [' . $storage_master['StorageMaster']['code'] . '] / ' . (isset($storage_types_from_id[$storage_control_id]) ? $storage_types_from_id[$storage_control_id] : $storage_master['StorageControl']['storage_type']) . '", ';
			++$count;
			if ($count > 9) {
				break;
			}
	...
	function contentListView($storage_master_id, $model = null) {
		...
		if (!$this->StorageControl->allowCustomCoordinates($storage_master_data['StorageControl']['id'],
			array('StorageControl' => $storage_master_data['StorageControl']))
		) {
			// Check storage supports custom coordinates and disable access to coordinates menu option if required
			$atim_menu = $this->inactivateStorageCoordinateMenu($atim_menu);
		}
		...
		if (!$model) {
			...
			if ($storage_master_data['StorageControl']['is_tma_block']) {
				...
				$this->set('models_to_dispay',
					array('StorageMaster' => 'storages', 'AliquotMaster' => 'aliquots', 'TmaSlide' => 'tma slides'));
			}
			if (!$storage_master_data['StorageControl']['is_tma_block']) {
				// Get all storage control types to build the add to selected button
				$this->set('storage_types_from_id', $this->StorageControl->getStorageTypePermissibleValues());
			...
			switch ($model) {
				case 'StorageMaster':
					$this->request->data = $this->paginate($this->StorageMaster,
						array('StorageMaster.parent_id' => $storage_master_id));
					...
					break;
				case 'AliquotMaster':
					$this->request->data = $this->paginate($this->AliquotMaster,
						array('AliquotMaster.storage_master_id' => $storage_master_id));
					...
					break;
				case 'TmaSlide':
					$this->request->data = $this->paginate($this->TmaSlide,
						array('TmaSlide.storage_master_id' => $storage_master_id));

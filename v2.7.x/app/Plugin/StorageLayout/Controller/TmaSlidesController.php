...
class TmaSlidesController extends StorageLayoutAppController {
	...
	function listAll($tma_block_storage_master_id) {
		...
		$this->request->data = $this->paginate($this->TmaSlide,
			array('TmaSlide.tma_block_storage_master_id' => $tma_block_storage_master_id));
	...
	function add($tma_block_storage_master_id = null) {
		...
		if ($tma_block_storage_master_id != null) {
			...
			if (isset($this->request->data['ViewStorageMaster']['id'])) {
				...
				if ($this->request->data['ViewStorageMaster']['id'] == 'all' && isset($this->request->data['node'])) {
					...
					$this->request->data['ViewStorageMaster']['id'] = explode(",",
		...
		$tma_blocks = $this->StorageMaster->find('all',
			array('conditions' => array('StorageMaster.id' => $tma_block_ids), 'recursive' => '0'));
		...
		foreach ($tma_blocks as &$tma_block_data) {
			...
			if (!$tma_block_data['StorageControl']['is_tma_block']) {
		...
		if ($initial_display) {
			...
			foreach ($previous_data as $key_block_storage_master_id => $data_unit) {
				...
				if (!array_key_exists($key_block_storage_master_id, $tma_blocks_from_id)) {
			...
			if (empty($errors)) {
				...
				if ($tma_block_storage_master_id != null) {
					$this->atimFlash(__('your data has been saved'),
						'/StorageLayout/StorageMasters/detail/' . $tma_block_storage_master_id);
				} else {
					//batch
				...
				foreach ($errors as $field => $msg_and_lines) {
					foreach ($msg_and_lines as $msg => $lines) {
						$this->TmaSlide->validationErrors[$field][] = __($msg) . (($record_counter != 1) ? ' - ' . str_replace('%s',
	...
	function detail($tma_block_storage_master_id, $tma_slide_id, $is_from_tree_view_or_layout = 0) {
		...
		$tma_slide_data = $this->TmaSlide->find('first', array(
			'conditions' => array(
				'TmaSlide.id' => $tma_slide_id,
			...
			if ($submitted_data_validates) {
				...
				if ($this->TmaSlide->save($this->request->data, false)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/StorageLayout/TmaSlides/detail/' . $tma_block_storage_master_id . '/' . $tma_slide_id);
				}
			}
		}
	}
	...
	function editInBatch($tma_block_storage_master_id = null) {
		...
		$slides_data = $this->TmaSlide->find('all',
			array('conditions' => $criteria, 'order' => 'TmaSlide.barcode ASC'));
		if (empty($slides_data)) {
			$this->atimFlash(__('no slide to update'), $url_to_cancel);
		}
		if ($tma_slide_ids_for_sorting) {
			$this->TmaSlide->sortForDisplay($slides_data, $tma_slide_ids_for_sorting);
		}
		$display_limit = Configure::read('TmaSlideCreation_processed_items_limit');
		...
		if ($initial_display) {
			...
			foreach ($this->request->data as $key => $new_studied_tma) {
				$record_counter++;
				// Get order item id
				if (!isset($slide_id_by_barcode[$new_studied_tma['TmaSlide']['barcode']])) {
				...
				foreach ($this->TmaSlide->validationErrors as $field => $msgs) {
					$msgs = is_array($msgs) ? $msgs : array($msgs);
					foreach ($msgs as $msg) {
						$errors['TmaSlide'][$field][$msg][] = $record_counter;
					}
				}
				// Reset data
					...
					foreach ($field_messages as $field => $messages) {
						foreach ($messages as $message => $lines_nbr) {
							if (!array_key_exists($field, $this->{$model}->validationErrors)) {
								...
								$this->{$model}->validationErrors[][] = __($message) . ' - ' . str_replace('%s',
										implode(',', $lines_nbr), __('see line %s'));
							}
						}
					}

...
class StorageMaster extends StorageLayoutAppModel {
	...
	function summary($variables = array()) {
		...
		if (isset($variables['StorageMaster.id'])) {
			$result = $this->find('first',
				array('conditions' => array('StorageMaster.id' => $variables['StorageMaster.id'])));
			$title = '';
			if ($result['StorageControl']['is_tma_block']) {
				$title = __('TMA-blc');
			} else {
				$StructurePermissibleValuesCustom = AppModel::getInstance("", "StructurePermissibleValuesCustom", true);
				$translated_storage_type = $StructurePermissibleValuesCustom->getTranslatedCustomDropdownValue('storage types',
					$result['StorageControl']['storage_type']);
	...
	function validates($options = array()) {
		if (!(array_key_exists('FunctionManagement',
				$this->data) && array_key_exists('recorded_storage_selection_label', $this->data['FunctionManagement']))
		) {
		...
		$parent_storage_selection_results = $this->validateAndGetStorageData($this->data['FunctionManagement']['recorded_storage_selection_label'],
		...
		if (array_key_exists('id', $this->data['StorageMaster']) && $this->insideItself()) {
			...
			if (!empty($parent_storage_data) && ($parent_storage_data['StorageControl']['is_tma_block'])) {
				...
				if (empty($this->validationErrors['recorded_storage_selection_label']) && empty($this->validationErrors['parent_storage_coord_x']) && empty($this->validationErrors['parent_storage_coord_y']) && isset($parent_storage_selection_results['storage_data']['StorageControl']) && $parent_storage_selection_results['storage_data']['StorageControl']['check_conflicts'] && (strlen($this->data['StorageMaster']['parent_storage_coord_x']) > 0 || strlen($this->data['StorageMaster']['parent_storage_coord_y']) > 0)) {
					...
					$position_status = $this->positionStatusQuick($parent_storage_selection_results['storage_data']['StorageMaster']['id'],
						array(
		...
		if (!empty($storage_having_duplicated_barcode)) {
			foreach ($storage_having_duplicated_barcode as $duplicate) {
				if ((!array_key_exists('id',
						$storage_data['StorageMaster'])) || ($duplicate['StorageMaster']['id'] != $storage_data['StorageMaster']['id'])
				) {
	 ...
	 *        // array_to_display should be used to get list of allowed coordinates and set drop down list for position
	 *     selection
	...
	function buildAllowedStoragePosition($storage_data, $coord) {
		...
		if (!empty($storage_data['StorageControl']['coord_' . $coord . '_type'])) {
			if (!empty($storage_data['StorageControl']['coord_' . $coord . '_size'])) {
				...
				if ((strcmp($storage_data['StorageControl']['coord_' . $coord . '_type'],
							'list') == 0) && (strcmp($coord, 'x') == 0)
				) {
					$storage_coordinate = AppModel::getInstance("StorageLayout", "StorageCoordinate", true);
					$coordinates = $storage_coordinate->atim_list(array(
						'conditions' => array(
							'StorageCoordinate.storage_master_id' => $storage_data['StorageMaster']['id'],
	 ...
	 *
	 * @return storage data (array('StorageMaster' => array(), 'StorageControl' => array()) when found, array('error'
	 *     => message) otherwise
	 */
	...
	function getStorageDataFromStorageLabelAndCode($storage_label_and_code) {
		...
		if (preg_match_all("/([^\b]+)\[([^\[]+)\]/", $storage_label_and_code, $matches, PREG_SET_ORDER) > 0) {
			...
			$selected_storages = $this->find('all', array(
				'conditions' => array(
			...
			$selected_storages = $this->find('all',
				array('conditions' => array('StorageMaster.selection_label' => $storage_label_and_code)));
		}
		...
		if (sizeof($selected_storages) == 1) {
			...
			if (sizeof($selected_storages) > 1) {
				return array(
					'error' => str_replace('%s', $storage_label_and_code,
						__('more than one storages matche the selection label [%s]'))
				);
			}
		}
	...
	function hasChild(array $storage_master_ids) {
		...
		$result = array_filter($this->find('list', array(
			'fields' => array("StorageMaster.parent_id"),
			'conditions' => array('StorageMaster.parent_id' => $storage_master_ids),
			'group' => array('StorageMaster.parent_id')
		)));
		...
		return array_merge($result, array_filter($aliquot_master->find('list', array(
			'fields' => array('AliquotMaster.storage_master_id'),
			'conditions' => array('AliquotMaster.storage_master_id' => $storage_master_ids),
			'group' => array('AliquotMaster.storage_master_id')
		))));
	}
	...
	function allowDeletion($storage_master_id) {
		...
		$nbr_children_storages = $this->find('count',
			array('conditions' => array('StorageMaster.parent_id' => $storage_master_id), 'recursive' => '-1'));
		...
		$nbr_storage_aliquots = $aliquot_master_model->find('count',
		...
		$nbr_tma_slides = $tma_slide_model->find('count', array(
			'conditions' => array('TmaSlide.tma_block_storage_master_id' => $storage_master_id),
			'recursive' => '-1'
		));
		if ($nbr_tma_slides > 0) {
		...
		$nbr_children_storages = $tma_slide_model->find('count',
	...
	function manageTemperature(&$storage_data, $storage_control_data) {
		if ($storage_data['StorageMaster']['storage_control_id'] != $storage_control_data['StorageControl']['id']) {
			AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__,
				null, true);
		}
		...
		if (!$storage_control_data['StorageControl']['set_temperature']) {
			if (!empty($storage_data['StorageMaster']['parent_id'])) {
				$parent_storage_data = $this->find('first', array(
					'conditions' => array('StorageMaster.id' => $storage_data['StorageMaster']['parent_id']),
					'recursive' => '-1'
				));
				if (empty($parent_storage_data)) {
					AppController::getInstance()->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__,
						null, true);
				}
	...
	function updateChildrenStorageSelectionLabel($parent_storage_id, $parent_storage_data) {
		...
		while (!empty($arr_studied_parents_data)) {
			...
			foreach ($children_storage_to_update as $new_children_to_update) {
				...
				$storage_data_to_update['StorageMaster']['selection_label'] = $this->createSelectionLabel($new_children_to_update,
					$parent_storage_data);
	 ...
	 */
	function updateAndSaveDataArray(
		$data_array,
		$type,
		$x_key,
		$y_key,
		...
		$rcv_data,
		$updater_model,
		array $storage_controls
	) {
		foreach ($data_array as &$init_data_unit) {
			$init_data_id = $init_data_unit[$type]['id'];
			if (($init_data_unit[$type][$x_key] != $rcv_data[$type][$init_data_id]['x'] && !(in_array($rcv_data[$type][$init_data_id]['x'],
							array(
								'u',
								't'
							)) && $init_data_unit[$type][$x_key] == '')) || ($init_data_unit[$type][$y_key] != $rcv_data[$type][$init_data_id]['y'] && !(in_array($rcv_data[$type][$init_data_id]['y'],
							array(
								'u',
								't'
							)) && $init_data_unit[$type][$y_key] == '')) || $init_data_unit[$type][$storage_parent_key] != $rcv_data[$type][$init_data_id]['s']
			) {
				...
				if ($type == "StorageMaster") {
					...
					if ($this->insideItself()) {
						...
						AppController::addWarningMsg(__('trying to put storage [%s] within itself failed',
								$this->getLabel($data, 'StorageMaster',
									'selection_label')) . ' ' . __('storage parent defined to none'));
					}
				}
				...
				if ($update_temp_and_label) {
					...
					if (!$init_data_unit['StorageControl']['set_temperature']) {
						$this->updateChildrenSurroundingTemperature($init_data_id,
							$init_data_unit['StorageMaster']['temperature'],
							$init_data_unit['StorageMaster']['temp_unit']);
					}
				}
			}
		}
	...
	function positionStatusQuick($storage_master_id, array $position, array $exception = array()) {
		...
		$this->used_storage_pos[$storage_master_id][$position['x']] = array_key_exists('y',
			$position) ? array($position['y']) : null;
		...
		if (isset($data[$model_name])) {
			foreach ($data[$model_name] as &$model_data) {
				...
				if (!array_key_exists($storage_id, $cumul_storage_data)) {
					...
					$cumul_storage_data[$storage_id]['printed_label'] = $this->getLabel($storage, 'StorageMaster',
						'selection_label');
				}
				...
				if (isset($cumul_storage_data[$storage_id]['pos'][$model_data['x']][$model_data['y']])) {
					$msg = __('conflict detected in storage [%s] at position [%s, %s]',
						$cumul_storage_data[$storage_id]['printed_label'], $model_data['x'], $model_data['y']);
					//react
					...
					$cumul_storage_data[$storage_id]['pos'][$model_data['x']][$model_data['y']] = $this->getLabel($model_full_data,
	...
	function contentNatCaseSort($models_and_fields, $contents_to_sort, $desc_order = false) {
		...
		foreach ($contents_to_sort as $key => $new_content) {
			...
			foreach ($models_and_fields as $model_and_field) {
				...
				if ($first_test) {
					if (!array_key_exists($model, $new_content) || !array_key_exists($field, $new_content[$model])) {
						AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__,
							null, true);
					}
					$first_test = false;
		...
		if ($desc_order) {
			$values_to_sort = array_reverse($values_to_sort);
		}
		$sorted_contents = array();
		foreach ($values_to_sort as $sorted_value) {
			if (!array_key_exists($sorted_value, $value_to_key)) {
				AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__,
					null, true);
			}
			foreach ($value_to_key[$sorted_value] as $key) {
				if (!array_key_exists($key, $contents_to_sort)) {
					AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__,
						null, true);
				}

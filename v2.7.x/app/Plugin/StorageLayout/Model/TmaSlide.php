...
class TmaSlide extends StorageLayoutAppModel {
	...
	function validateAndUpdateTmaSlideStorageData() {
		...
		if (array_key_exists('FunctionManagement',
				$tma_slide_data) && array_key_exists('recorded_storage_selection_label',
				$tma_slide_data['FunctionManagement'])
		) {
			// Check the tma slide storage definition
			$arr_storage_selection_results = self::$storage->validateAndGetStorageData($tma_slide_data['FunctionManagement']['recorded_storage_selection_label'],
				$tma_slide_data['TmaSlide']['storage_coord_x'], $tma_slide_data['TmaSlide']['storage_coord_y']);
			...
			if (empty($this->validationErrors['recorded_storage_selection_label']) && empty($this->validationErrors['storage_coord_x']) && empty($this->validationErrors['storage_coord_y']) && isset($arr_storage_selection_results['storage_data']['StorageControl']) && $arr_storage_selection_results['storage_data']['StorageControl']['check_conflicts'] && (strlen($tma_slide_data['TmaSlide']['storage_coord_x']) > 0 || strlen($tma_slide_data['TmaSlide']['storage_coord_y']) > 0)) {
				...
				if ($msg != null) {
					$msg = sprintf($msg,
						...
						$this->data['TmaSlide']['storage_coord_x'], $this->data['TmaSlide']['storage_coord_y']);
			...
			if ((array_key_exists('storage_coord_x',
					$tma_slide_data['TmaSlide'])) || (array_key_exists('storage_coord_y', $tma_slide_data['TmaSlide']))
			) {
	...
	function isDuplicatedTmaSlideBarcode($tma_slide_data) {
		...
		$slides_having_duplicated_barcode = $this->find('all',
			array('conditions' => array('TmaSlide.barcode' => $barcode), 'recursive' => -1));;
		if (!empty($slides_having_duplicated_barcode)) {
			foreach ($slides_having_duplicated_barcode as $duplicate) {
				if ((!array_key_exists('id',
						$tma_slide_data['TmaSlide'])) || ($duplicate['TmaSlide']['id'] != $tma_slide_data['TmaSlide']['id'])
				) {
					$this->validationErrors['barcode'][] = str_replace('%s', $barcode,

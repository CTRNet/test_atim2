...
class MaterialsController extends MaterialAppController {
	...
	function detail($material_id = null) {
		...
		$this->request->data = $this->Material->find('first',
			array('conditions' => array('Material.id' => $material_id)));
	...
	function delete($material_id = null) {
		...
		if ($this->Material->atimDelete($material_id)) {
			...
			$this->atimFlash(__('error deleting data - contact administrator'), '/material/materials/listall/');
		}
	}

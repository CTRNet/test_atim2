 ...
 */
	...
	function edit($template_id) {
		...
		if (empty($tmp_template)) {
			$this->atimFlash(__('you do not own that template'), '/Tools/Template/index/');
			return;
		}
		...
		$sample_controls = $sample_control_model->find('all',
			array('fields' => array('id', 'sample_type'), 'recursive' => -1));
		...
		if (!empty($this->request->data)) {
			...
			if ($this->request->is('ajax')) {
				...
				//TODO validate this section is useful or not
				...
				$nodes_to_delete = $this->TemplateNode->find('list', array(
					...
					'conditions' => array(
						...
						'NOT' => array('TemplateNode.id' => $found_nodes)
					)
				));

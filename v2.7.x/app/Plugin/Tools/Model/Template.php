...
class Template extends AppModel {
	...
	function init() {
		...
		$tree = $template_node_model->find('all',
			array('conditions' => array('TemplateNode.Template_id' => $this->id)));
		$result[''] = array(
			'id' => 0,
			'parent_id' => null,
			'control_id' => '0',
	...
	function setOwnerAndVisibility(&$tempate_data, $created_by = null) {
		...
		if ($created_by && $created_by != $template_user_id) {
			...
			$template_user_data = $user_model->find('first',
				array('conditions' => array('User.id' => $created_by, 'User.deleted' => array(0, 1))));

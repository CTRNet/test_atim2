...
class LabBookMastersController extends LabBookAppController {
	...
	function search($search_id = 0) {
		...
		$this->set('lab_book_controls_list',
			$this->LabBookControl->find('all', array('conditions' => array('LabBookControl.flag_active' => '1'))));
	...
	function detail($lab_book_master_id, $full_detail_screen = true) {
		if (!$lab_book_master_id) {
			...
			if ($lab_book_master_id == '-1') {
				$this->atimFlash(__('no lab book is linked to this record'), "javascript:history.back()", 5);
				return;
			}
		}
		...
		if (empty($this->request->data)) {
			...
			if ($submitted_data_validates) {
				...
				if ($this->LabBookMaster->save($this->request->data)) {
					...
					$this->LabBookMaster->synchLabbookRecords($lab_book_master_id,
						$this->request->data['LabBookDetail']);
					$this->atimFlash(__('your data has been updated'),
						'/labbook/LabBookMasters/detail/' . $lab_book_master_id);
				}
			}
		}
	}
			...
			if ($submitted_data_validates) {
				if (isset($this->request->data['derivative'])) {
					...
					foreach ($this->request->data['derivative'] as $new_record) {
						...
						if (!$this->DerivativeDetail->save(array('DerivativeDetail' => $new_record['DerivativeDetail']),
							false)
						) {
	...
	function delete($lab_book_master_id) {
		...
		if ($arr_allow_deletion['allow_deletion']) {
			if ($this->LabBookMaster->atimDelete($lab_book_master_id, true)) {
				...
				$this->atimFlash(__('error deleting data - contact administrator'), '/labbook/LabBookMasters/index/');
			}
		} else {
			$this->atimFlash(__($arr_allow_deletion['msg']), '/labbook/LabBookMasters/detail/' . $lab_book_master_id);
		}
	}

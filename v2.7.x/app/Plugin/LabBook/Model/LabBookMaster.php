...
class LabBookMaster extends LabBookAppModel {
	...
	function summary($variables = array()) {
		...
		if (isset($variables['LabBookMaster.id'])) {
			$result = $this->find('first',
				array('conditions' => array('LabBookMaster.id' => $variables['LabBookMaster.id'])));
	...
	function getLabBookPermissibleValuesFromId($lab_book_control_id = null) {
		...
		$available_books = $this->find('all',
			array('conditions' => $conditions, 'order' => 'LabBookMaster.created DESC'));
	...
	public function getIdFromCode($code) {
		$lb = $this->find('list',
			array('fields' => array('LabBookMaster.id'), 'conditions' => array('LabBookMaster.code' => $code)));
	...
	function allowLabBookDeletion($lab_book_master_id) {
		...
		$nbr_derivatives = $derivative_detail_model->find('count',
			array('conditions' => array('DerivativeDetail.lab_book_master_id' => $lab_book_master_id)));
		...
		$nbr_realiquotings = $realiquoting_model->find('count',
			array('conditions' => array('Realiquoting.lab_book_master_id' => $lab_book_master_id)));
	...
	function getLabBookDerivativesList($lab_book_master_id) {
		...
		return $sample_master_model->find('all',
			array('conditions' => array('DerivativeDetail.lab_book_master_id' => $lab_book_master_id)));
	}
	...
	function getLabBookRealiquotingsList($lab_book_master_id) {
		...
		$realiquoting_models_list = $realiquoting_model->find('all',
			array('conditions' => array('Realiquoting.lab_book_master_id' => $lab_book_master_id)));
		foreach ($realiquoting_models_list as $key => $realiquoting_model_data) {
			...
			$realiquoting_models_list[$key] = array_merge($sample_master_from_ids[$realiquoting_model_data['AliquotMaster']['sample_master_id']],
				$realiquoting_model_data);
		}

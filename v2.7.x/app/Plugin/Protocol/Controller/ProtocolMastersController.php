...
class ProtocolMastersController extends ProtocolAppController {
	...
	function search($search_id = 0) {
		...
		$this->set('protocol_controls',
			$this->ProtocolControl->find('all', array('conditions' => array('flag_active' => '1'))));
	...
	function add($protocol_control_id) {
		$protocol_control_data = $this->ProtocolControl->find('first',
			array('conditions' => array('ProtocolControl.id' => $protocol_control_id)));
		...
		if (empty($this->request->data)) {
			...
			if ($submitted_data_validates && $this->ProtocolMaster->save($this->request->data)) {
				...
				$this->atimFlash(__('your data has been updated'),
					'/Protocol/ProtocolMasters/detail/' . $this->ProtocolMaster->getLastInsertId());
			}
		}
	}
	...
	function detail($protocol_master_id) {
		...
		$this->set('display_precisions',
			(empty($protocol_data['ProtocolControl']['protocol_extend_control_id']) ? false : true));
	...
	function delete($protocol_master_id) {
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			$this->atimFlash(__($arr_allow_deletion['msg']), '/Protocol/ProtocolMasters/detail/' . $protocol_master_id);
		}
	}
}

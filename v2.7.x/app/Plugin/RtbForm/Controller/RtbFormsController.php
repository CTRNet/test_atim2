...
class RtbformsController extends RtbformAppController {
	...
	function edit($rtbform_id = null) {
		...
		if (!empty($this->request->data)) {
			...
			$this->request->data = $this->Rtbform->find('first',
				array('conditions' => array('Rtbform.id' => $rtbform_id)));
		}
	}
	...
	function delete($rtbform_id = null) {
		...
		if ($this->Rtbform->atimDelete($rtbform_id)) {
			...
			$this->atimFlash(__('error deleting data - contact administrator'), '/rtbform/rtbforms/search/');
		}
	}

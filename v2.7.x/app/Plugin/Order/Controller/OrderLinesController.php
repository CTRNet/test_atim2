...
class OrderLinesController extends OrderAppController {
	...
	function listall($order_id) {
		...
		// Set data
		$this->request->data = $this->paginate($this->OrderLine,
			array('OrderLine.order_id' => $order_id, 'OrderLine.deleted' => 0));
	...
	function add($order_id) {
		...
		$this->set('override_data', array(
			...
			'OrderLine.date_required' => $order_data['Order']['default_required_date'] ? substr($order_data['Order']['default_required_date'],
				0, (str_replace(array('', 'y', 'm', 'd', 'c'), array('c', 4, 4, 7, 10),
					$order_data['Order']['default_required_date_accuracy']))) : ''
		));
			...
			if (empty($errors_tracking)) {
				...
				foreach ($errors_tracking as $field => $msg_and_lines) {
					foreach ($msg_and_lines as $msg => $lines) {
						$this->OrderLine->validationErrors[$field][] = $msg . ' - ' . str_replace('%s',
	...
	function edit($order_id, $order_line_id) {
		...
		$order_line_data = $this->OrderLine->find('first',
			array('conditions' => array('OrderLine.id' => $order_line_id, 'OrderLine.order_id' => $order_id)));
			...
			if ($submitted_data_validates) {
				...
				if ($this->OrderLine->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/Order/OrderLines/detail/' . $order_id . '/' . $order_line_id);
				}
			}
		}
	}
	...
	function detail($order_id, $order_line_id) {
		...
		$shipments_list = $this->Shipment->find('all',
			array('conditions' => array('Shipment.order_id' => $order_id), 'recursive' => '-1'));
	...
	function delete($order_id, $order_line_id) {
		...
		if ($arr_allow_deletion['allow_deletion']) {
			if ($this->OrderLine->atimDelete($order_line_id)) {
				...
				$this->atimFlash(__('error deleting data - contact administrator'), 'javascript:history.go(-1)');
			}
		} else {

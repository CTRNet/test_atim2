...
class ShipmentsController extends OrderAppController {
	...
	function add($order_id, $copied_shipment_id = null) {
		...
		if (empty($this->request->data)) {
			if ($copied_shipment_id) {
				$this->request->data = $this->Shipment->find('first',
					array('conditions' => array('Shipment.id' => $copied_shipment_id), 'recursive' => '-1'));
			}
			...
			if ($submitted_data_validates && $this->Shipment->save($this->request->data)) {
				...
				$this->atimFlash(__('your data has been saved'),
					'/Order/Shipments/addToShipment/' . $order_id . '/' . $this->Shipment->getLastInsertId());
			}
		}
	}
	...
	function edit($order_id = null, $shipment_id = null) {
		...
		// Get shipment data
		$shipment_data = $this->Shipment->find('first',
			array('conditions' => array('Shipment.id' => $shipment_id, 'Shipment.order_id' => $order_id)));
	...
	function addToShipment($order_id, $shipment_id) {
		...
		$available_order_items = $this->OrderItem->find('all', array(
			...
			'order' => 'OrderItem.date_added DESC, OrderLine.id'
		));
		if (empty($available_order_items)) {
			$this->atimFlash(__('no new item could be actually added to the shipment'),
				'/Order/Shipments/detail/' . $order_id . '/' . $shipment_id);
		}
		...
		if (empty($this->request->data)) {
			...
			if (empty($data_to_save)) {
				...
				if (sizeof($data_to_save) > $aliquot_limit) {
					$this->atimFlash(__("batch init - number of submitted records too big") . " (>$aliquot_limit)",
						'/Order/Shipments/detail/' . $order_id . '/' . $shipment_id, 5);
					return;
				}
			}
	...
	function deleteFromShipment($order_id, $order_item_id, $shipment_id) {
		...
		$order_item_data = $this->OrderItem->find('first', array(
			'conditions' => array('OrderItem.id' => $order_item_id, 'OrderItem.shipment_id' => $shipment_id),
			'recursive' => '-1'
		));

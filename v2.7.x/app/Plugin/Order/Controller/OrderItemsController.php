...
class OrderItemsController extends OrderAppController {
	...
	function listall($order_id, $order_line_id = null) {
		...
		if (!$order_line_id) {
			$order_data = $this->Order->find('first',
				array('conditions' => array('Order.id' => $order_id), 'recursive' => '-1'));
			if (empty($order_data)) {
				$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
			}
		} else {
			$order_line_data = $this->OrderLine->find('first', array(
				'conditions' => array('OrderLine.id' => $order_line_id, 'OrderLine.order_id' => $order_id),
				'recursive' => '-1'
			));
		...
		$aliquots_for_batchset = $this->OrderItem->find('all',
			array('fields' => array('AliquotMaster.id'), 'conditions' => $conditions));
	...
	function add($order_id, $order_line_id = null) {
		...
		$this->set('atim_menu',
			$this->Menus->get($order_line_id ? '/Order/OrderLines/detail/%%Order.id%%/%%OrderLine.id%%/' : '/Order/OrderLines/detail/%%Order.id%%/'));
		$this->set('atim_menu_variables', array('Order.id' => $order_id, 'OrderLine.id' => $order_line_id));
		...
		if (empty($this->request->data)) {
			...
			foreach ($this->request->data as &$data_unit) {
				...
				$aliquot_data = $this->AliquotMaster->find('first', array(
					'conditions' => array('AliquotMaster.barcode' => $data_unit['AliquotMaster']['barcode']),
					'recursive' => '-1'
				));
				if (!$aliquot_data) {
				...
				if ($aliquot_data && $this->OrderItem->find('count', array(
						'conditions' => array('OrderItem.aliquot_master_id' => $aliquot_data['AliquotMaster']['id']),
						'recursive' => '-1'
					))
				) {
					$errors_tracking['barcode']['an aliquot can only be added once to an order'][] = $row_counter;
				}
				if (in_array($data_unit['AliquotMaster']['barcode'], $barcodes_recorded)) {
					$errors_tracking['barcode']['an aliquot can only be added once to an order'][] = $row_counter;
				}
			...
			if (empty($errors_tracking)) {
				...
				$this->atimFlash(__('your data has been saved'),
					$order_line_id ? '/Order/OrderLines/detail/' . $order_id . '/' . $order_line_id . '/' : '/Order/Orders/detail/' . $order_id);
			} else {
				$this->OrderItem->validationErrors = array();
				foreach ($errors_tracking as $field => $msg_and_lines) {
					foreach ($msg_and_lines as $msg => $lines) {
						$this->OrderItem->validationErrors[$field][] = __($msg) . ' - ' . str_replace('%s',
	...
	function addAliquotsInBatch($aliquot_master_id = null) {
		...
		if (isset($this->request->data['0']['aliquot_ids_to_add'])) {
			...
			if (!empty($aliquot_master_id)) {
				...
				if (!is_array($studied_aliquot_master_ids) && strpos($studied_aliquot_master_ids, ',')) {
					...
					$this->atimFlash(__("batch init - number of submitted records too big"),
				...
				$aliquots_count = $this->AliquotMaster->find('count', array(
					'conditions' => array('AliquotMaster.id' => $studied_aliquot_master_ids),
					'recursive' => '-1'
				));
			...
			if ($submitted_aliquots_validates) {
				...
				$existing_order_aliquots = $this->OrderItem->find('all', array(
					'conditions' => array('OrderItem.aliquot_master_id' => $studied_aliquot_master_ids),
					'recursive' => '0'
				));
				if (!empty($existing_order_aliquots)) {
					$aliquots_list_for_display = '';
					foreach ($existing_order_aliquots as $new_record) {
						$aliquots_list_for_display .= '<br> - ' . $new_record['AliquotMaster']['barcode'];
					}
					$submitted_aliquots_validates = false;
		...
		$aliquots_data = $this->ViewAliquot->find('all',
			array('conditions' => array('ViewAliquot.aliquot_master_id' => $aliquot_ids_to_add)));
		...
		$order_data_tmp = $this->Order->find('all', array(
			'conditions' => array('NOT' => array('Order.processing_status' => array('completed'))),
		...
		$order_line_data_tmp = $this->OrderLine->find('all', array(
			'conditions' => array('OrderLine.order_id' => array_keys($order_and_order_line_data)),
		...
		if ($launch_save_process) {
			...
			if (empty($this->request->data['FunctionManagement']['selected_order_and_order_line_ids'])) {
				...
				if (preg_match('/^([0-9]+)\|([0-9]*)$/',
					$this->request->data['FunctionManagement']['selected_order_and_order_line_ids'],
					$order_and_order_line_ids)) {
			...
			if ($submitted_data_validates) {
				...
				foreach ($aliquot_ids_to_add as $added_aliquot_master_id) {
					...
					$new_order_item_data['OrderItem'] = array_merge($new_order_item_data['OrderItem'],
				...
				// Redirect
				$this->atimFlash(__('your data has been saved'),
					(!$order_line_id) ? "/Order/Orders/detail/$order_id/" : "/Order/OrderLines/detail/$order_id/$order_line_id/");
			}
		}
	}
	...
	function edit($order_id, $order_line_id = null) {
		...
		$items_data = $this->OrderItem->find('all',
			array('conditions' => $criteria, 'order' => 'AliquotMaster.barcode ASC', 'recursive' => '0'));

		if (empty($items_data)) {
			$this->atimFlash(__($order_line_id ? 'no unshipped item exists into this order line' : 'no unshipped item exists into this order'),
				(!$order_line_id) ? "/Order/Orders/detail/$order_id/" : '/Order/OrderLines/detail/' . $order_id . '/' . $order_line_id . '/');
		}
		...
		if (empty($this->request->data)) {
			...
			foreach ($this->request->data as $key => $new_studied_item) {
				$record_counter++;
				// Get order item id
				if (!isset($order_item_id_by_barcode[$new_studied_item['AliquotMaster']['barcode']])) {
					$this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null,
						true);
				}
				...
				foreach ($this->OrderItem->validationErrors as $field => $msgs) {
					$msgs = is_array($msgs) ? $msgs : array($msgs);
					foreach ($msgs as $msg) {
						$errors['OrderItem'][$field][$msg][] = $record_counter;
					}
				}
				// Reset data
					...
					foreach ($field_messages as $field => $messages) {
						foreach ($messages as $message => $lines_nbr) {
							if (!array_key_exists($field, $this->{$model}->validationErrors)) {
								$this->{$model}->validationErrors[$field][] = $message . ' - ' . str_replace('%s',
										implode(',', $lines_nbr), __('see line %s'));
							} else {
								$this->{$model}->validationErrors[][] = $message . ' - ' . str_replace('%s',
										implode(',', $lines_nbr), __('see line %s'));
							}
						}
					}
	...
	function delete($order_id, $order_item_id) {
		...
		// Get data
		$order_item_data = $this->OrderItem->find('first',
			array('conditions' => array('OrderItem.id' => $order_item_id, 'OrderItem.order_id' => $order_id)));
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			if ($this->OrderItem->atimDelete($order_item_id)) {
				...
				if ($order_line_id) {
					// Update order line status
					$new_status = 'pending';
					$order_item_count = $this->OrderItem->find('count',
						array('conditions' => array('OrderItem.order_line_id' => $order_line_id), 'recursive' => '-1'));
					if ($order_item_count != 0) {
						$order_item_not_shipped_count = $this->OrderItem->find('count', array(
							'conditions' => array(
				...
				// Redirect
				$this->atimFlash(__('your data has been deleted - update the aliquot in stock data'),
					'/Order/Orders/detail/' . $order_id);
			} else {

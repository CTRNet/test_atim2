...
class SopMaster extends SopAppModel {
	...
	function allowDeletion($sop_master_id) {
		...
		$ctrl_value = $ctrl_model->find('count',
			array('conditions' => array('TmaSlide.sop_master_id' => $sop_master_id), 'recursive' => '-1'));
		if ($ctrl_value > 0) {
		...
		$ctrl_value = $block_model->find('count',
			array('conditions' => array('StorageDetail.sop_master_id' => $sop_master_id), 'recursive' => '-1'));
		if ($ctrl_value > 0) {
		...
		$ctrl_value = $ctrl_model->find('count',
			array('conditions' => array('Collection.sop_master_id' => $sop_master_id), 'recursive' => '-1'));
		if ($ctrl_value > 0) {
		...
		$ctrl_value = $ctrl_model->find('count',
			array('conditions' => array('SampleMaster.sop_master_id' => $sop_master_id), 'recursive' => '-1'));
		if ($ctrl_value > 0) {
		...
		$ctrl_value = $ctrl_model->find('count',
			array('conditions' => array('AliquotMaster.sop_master_id' => $sop_master_id), 'recursive' => '-1'));
		if ($ctrl_value > 0) {

...
$structure_links['bottom'] = array(
	...
	'print barcodes' => array(
		'link' => '/InventoryManagement/AliquotMasters/printBarcodes/model:SampleMaster/id:' . $atim_menu_variables['SampleMaster.id'],
		'icon' => 'barcode'
	),
		...
		foreach ($aliquots_data as $aliquot_control_id => $aliquots) {
			...
			$final_options = array(
				...
				'settings' => array(
					...
					'pagination' => false,
					'batchset' => array(
						'link' => '/InventoryManagement/SampleMasters/detail/' . $atim_menu_variables['Collection.id'] . '/' . $atim_menu_variables['SampleMaster.id'],
						'var' => 'aliquots_data',
						'ctrl' => $aliquot_control_id
					)
				)
			);
			$is_first = false;

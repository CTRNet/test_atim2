...
$empty_structure_options['extras'] = '<input type="hidden" name="data[ids]" value="' . $parent_aliquots_ids . '"/>
...
foreach ($this->request->data as $aliquot) {
	...
	if ($element_nbr == $counter) {
		...
		if (empty($aliquot_id)) {
			$final_children_options['settings']['confirmation_msg'] = __('multi_entry_form_confirmation_msg');
		}
	}
...
")); ?>";
    ...
    var labBookFields = ["<?php echo implode('", "', $lab_book_fields); ?>"];

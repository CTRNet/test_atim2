...
$structure_links['bottom']['print barcode'] = array(
	'link' => '/InventoryManagement/AliquotMasters/printBarcodes/model:AliquotMaster/id:' . $atim_menu_variables['AliquotMaster.id'],
	'icon' => 'barcode'
);
...
if (!empty($aliquot_storage_data)) {
	$structure_links['bottom']['storage'] = array(
		'plugin storagelayout access to storage' => array(
			"link" => '/StorageLayout/StorageMasters/detail/' . $aliquot_storage_data['StorageMaster']['id'],
			"icon" => "storage"
		),
		'remove from storage' => array(
			"link" => '/InventoryManagement/AliquotMasters/removeAliquotFromStorage/' . $col_id_samp_id_al_id,
			"icon" => "storage"
		)
	);
}
...
if (isset($order_line_id) && isset($order_id)) {
	$structure_links['bottom']['access to order'] = array(
		"link" => '/Order/OrderLines/detail/' . $order_id . '/' . $order_line_id . '/',
		"icon" => "order"
	);
} else {
	if (isset($order_id)) {
		$structure_links['bottom']['access to order'] = array(
			"link" => '/Order/Orders/detail/' . $order_id,
			"icon" => "order"
		);
	} else {
		$structure_links['bottom']['add to order'] = array(
			"link" => '/Order/OrderItems/addAliquotsInBatch/' . $atim_menu_variables['AliquotMaster.id'] . '/',
			"icon" => "order"
		);
	}
}
...
$structure_links['bottom']['add uses/events'] = array(
	"link" => '/InventoryManagement/AliquotMasters/addAliquotInternalUse/' . $atim_menu_variables['AliquotMaster.id'],
	"icon" => "use"
);

$structure_links['bottom']['realiquoting'] = array(
	'realiquot' => array(
		"link" => '/InventoryManagement/AliquotMasters/realiquotInit/creation/' . $atim_menu_variables['AliquotMaster.id'],
		"icon" => "aliquot"
	),
	'define realiquoted children' => array(
		"link" => '/InventoryManagement/AliquotMasters/realiquotInit/definition/' . $atim_menu_variables['AliquotMaster.id'],
		"icon" => "aliquot"
	)
);
...
if ($is_from_tree_view_or_layout == 1) {
	...
	if ($is_from_tree_view_or_layout == 2) {
		...
		if ($aliquot_master_data['Collection']['participant_id']) {
			$structure_links['bottom']['access to participant'] = '/ClinicalAnnotation/Participants/profile/' . $aliquot_master_data['Collection']['participant_id'];
		}
...
if ($is_from_tree_view_or_layout) {
	...
	// BUILD FORM
	$this->Structures->build($final_atim_structure, $final_options);
	$data_url = sprintf('InventoryManagement/AliquotMasters/listallUses/%d/%d/%d/',
	...
	// 3- STORAGE HISTORY
	$data_url = sprintf('InventoryManagement/AliquotMasters/storageHistory/%d/%d/%d/',
		$atim_menu_variables['Collection.id'], $atim_menu_variables['SampleMaster.id'],
		$atim_menu_variables['AliquotMaster.id']);
	...
	$data_url = sprintf('InventoryManagement/AliquotMasters/listAllRealiquotedParents/%d/%d/%d/',
		$atim_menu_variables['Collection.id'], $atim_menu_variables['SampleMaster.id'],
		$atim_menu_variables['AliquotMaster.id']);
	unset($structure_links['index']);
	$final_atim_structure = $empty_structure;

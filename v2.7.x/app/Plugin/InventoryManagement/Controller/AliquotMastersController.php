...
class AliquotMastersController extends InventoryManagementAppController {
	...
	function search($search_id = 0) {
		...
		$this->searchHandler($search_id, $this->ViewAliquot, 'view_aliquot_joined_to_sample_and_collection',
			'/InventoryManagement/AliquotMasters/search');

		$help_url = $this->ExternalLink->find('first',
			array('conditions' => array('name' => 'inventory_elements_defintions')));
	...
	function addInit() {
		...
		if (isset($this->request->data['BatchSet']) || isset($this->request->data['node'])) {
			...
			if (isset($this->request->data['node']) && $this->request->data[$model][$key] == 'all') {
				...
				$browsing_result = $this->BrowsingResult->find('first',
					array('conditions' => array('BrowsingResult.id' => $this->request->data['node']['id'])));
		...
		$init_data = $this->batchInit($this->SampleMaster, $model, $key, 'sample_control_id', $this->AliquotControl,
			'sample_control_id', 'you cannot create aliquots with this sample type');
		if (array_key_exists('error', $init_data)) {
			$this->atimFlash(__($init_data['error']), $url_to_cancel, 5);
			return;
		}
	...
	function add($sample_master_id = null, $aliquot_control_id = null, $quantity = 1) {
		...
		if (!empty($sample_master_id) && !empty($aliquot_control_id)) {
			...
			if (empty($this->request->data)) {
				$this->flash((__('you have been redirected automatically') . ' (#' . __LINE__ . ')'), $url_to_cancel,
					5);
				return;
			}
		}
		...
		if ($this->request->data[0]['realiquot_into'] == "") {
			$this->atimFlash(__('you need to select an aliquot type'), $url_to_cancel);
			return;
		}
		...
		if ($is_intial_display) {
			...
			if (!empty($this->request->data)) {
				...
				if (empty($sample_master_id)) {
					$this->flash((__('you have been redirected automatically') . ' (#' . __LINE__ . ')'),
						"javascript:history.back();", 5);
					return;
				}
				$sample_master_ids = array($sample_master_id);
			}
		}
		$samples = $this->ViewSample->find('all',
			array('conditions' => array('sample_master_id' => $sample_master_ids), 'recursive' => -1));
		$display_limit = Configure::read('AliquotCreation_processed_items_limit');
		if (sizeof($samples) > $display_limit) {
			$this->atimFlash(__("batch init - number of submitted records too big") . " (>$display_limit)",
				$url_to_cancel, 5);
			return;
		}
		...
		foreach ($samples as $sample_master_data) {
			if (is_null($sample_control_id)) {
				...
				if ($sample_master_data['ViewSample']['sample_control_id'] != $sample_control_id) {
					$this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null,
						true);
				}
			}
		...
		// set structure
		$this->Structures->set($aliquot_control['AliquotControl']['form_alias'], 'atim_structure',
			array('model_table_assoc' => array('AliquotDetail' => $aliquot_control['AliquotControl']['detail_tablename'])));
		if ($is_batch_process) {
		...
		$override_data = array(
			...
			'AliquotMaster.storage_datetime' => ($is_batch_process ? date('Y-m-d G:i') : $this->AliquotMaster->getDefaultStorageDate($this->SampleMaster->find('first',
				array('conditions' => array('SampleMaster.id' => $sample_master_id))))),
		...
		if ($is_intial_display) {
			...
			if (empty($prev_data)) {
				$this->atimFlash(__("at least one data has to be created"), "javascript:history.back();", 5);
				return;
			}
			$this->request->data = array();
			$record_counter = 0;
			foreach ($prev_data as $sample_master_id => $created_aliquots) {
				...
				if (!isset($samples_from_id[$sample_master_id])) {
					$this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null,
						true);
				}
				...
				foreach ($created_aliquots as $key => $aliquot) {
					...
					if (array_key_exists('initial_volume', $aliquot['AliquotMaster'])) {
						if (empty($aliquot_control['AliquotControl']['volume_unit'])) {
							$this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__,
								null, true);
						}
				...
				$this->request->data[] = array(
					'parent' => $sample_master_data,
					'children' => $created_aliquots
			...
			$this->AliquotMaster->addWritableField(array('aliquot_master_id'),
				$aliquot_control['AliquotControl']['detail_tablename']);
			...
			if (empty($errors)) {
				...
				foreach ($this->request->data as $created_aliquots) {
					foreach ($created_aliquots['children'] as $new_aliquot) {
						...
						if (!$this->AliquotMaster->save($new_aliquot, false)) {
							$this->redirect('/Pages/err_plugin_record_err?method=' . __METHOD__ . ',line=' . __LINE__,
								null, true);
						}
				...
				if ($is_batch_process) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/Datamart/BatchSets/listall/' . $batch_set_model->getLastInsertId());
				} else {
					if ($this->request->is('ajax')) {
						ob_end_clean();
						...
						$this->atimFlash(__('your data has been saved'),
							'/InventoryManagement/SampleMasters/detail/' . $samples[0]['ViewSample']['collection_id'] . '/' . $sample_master_id);
					}
				}
	...
	function detail($collection_id, $sample_master_id, $aliquot_master_id, $is_from_tree_view_or_layout = 0) {
		...
		// Set storage data
		$this->set('aliquot_storage_data',
			empty($aliquot_data['StorageMaster']['id']) ? array() : array('StorageMaster' => $aliquot_data['StorageMaster']));

		// MANAGE FORM, MENU AND ACTION BUTTONS

		// Get the current menu object.
		$atim_menu_link = ($aliquot_data['SampleControl']['sample_category'] == 'specimen') ? '/InventoryManagement/AliquotMasters/detail/%%Collection.id%%/%%SampleMaster.initial_specimen_sample_id%%/%%AliquotMaster.id%%' : '/InventoryManagement/AliquotMasters/detail/%%Collection.id%%/%%SampleMaster.id%%/%%AliquotMaster.id%%';
		$this->set('atim_menu', $this->Menus->get($atim_menu_link));
		$this->set('atim_menu_variables', array(
		...
		$order_item = $this->OrderItem->find('first',
			array('conditions' => array('OrderItem.aliquot_master_id' => $aliquot_master_id)));
		if (!empty($order_item)) {
			$this->set('order_line_id', $order_item['OrderItem']['order_line_id']);
			$this->set('order_id', $order_item['OrderItem']['order_id']);
		}

		$sample_master = $this->SampleMaster->find('first',
			array('conditions' => array('SampleMaster.id' => $sample_master_id), 'recursive' => -1));
		$ptdsc_model = AppModel::getInstance('InventoryManagement', 'ParentToDerivativeSampleControl', true);
		$ptdsc = $ptdsc_model->find('first', array(
			'conditions' => array('ParentToDerivativeSampleControl.parent_sample_control_id' => $sample_master['SampleMaster']['sample_control_id']),
			'recursive' => -1
		));
	...
	function edit($collection_id, $sample_master_id, $aliquot_master_id) {
		...
		// Set structure
		$this->Structures->set($aliquot_data['AliquotControl']['form_alias'], 'atim_structure',
			array('model_table_assoc' => array('AliquotDetail' => $aliquot_data['AliquotControl']['detail_tablename'])));
		...
		if (empty($this->request->data)) {
			...
			//Update data
			if (array_key_exists('initial_volume',
					$this->request->data['AliquotMaster']) && empty($aliquot_data['AliquotControl']['volume_unit'])
			) {
				$this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null,
					true);
			}
			...
			if ($submitted_data_validates) {
				...
				$this->atimFlash(__('your data has been updated'),
					'/InventoryManagement/AliquotMasters/detail/' . $collection_id . '/' . $sample_master_id . '/' . $aliquot_master_id);
				return;
			}
		}
	}
	...
	function removeAliquotFromStorage($collection_id, $sample_master_id, $aliquot_master_id) {
		if ((!$collection_id) || (!$sample_master_id) || (!$aliquot_master_id)) {
			$this->redirect('/Pages/err_plugin_funct_param_missing?method=' . __METHOD__ . ',line=' . __LINE__, null,
				true);
		}
	...
	function delete($collection_id, $sample_master_id, $aliquot_master_id) {
		...
		if ($arr_allow_deletion['allow_deletion']) {
			...
			$this->atimFlash(__($arr_allow_deletion['msg']),
				'/InventoryManagement/AliquotMasters/detail/' . $collection_id . '/' . $sample_master_id . '/' . $aliquot_master_id);
		}
	}
	...
	function addAliquotInternalUse($aliquot_master_id = null) {
		...
		if ($aliquot_master_id != null) {
			...
			if (empty($this->request->data) && $submited_request_data_empty) {
			...
			if (isset($this->request->data['ViewAliquot']['aliquot_master_id'])) {
				if ($this->request->data['ViewAliquot']['aliquot_master_id'] == 'all' && isset($this->request->data['node'])) {
					...
					$browsing_result = $this->BrowsingResult->find('first',
						array('conditions' => array('BrowsingResult.id' => $this->request->data['node']['id'])));
					$this->request->data['ViewAliquot']['aliquot_master_id'] = explode(",",
						$browsing_result['BrowsingResult']['id_csv']);
				}
		...
		$aliquot_data = $this->AliquotMaster->find('all',
			array('conditions' => array('AliquotMaster.id' => $aliquot_ids)));
		$display_limit = Configure::read('AliquotInternalUseCreation_processed_items_limit');
		if (empty($aliquot_data)) {
			$this->flash((__('you have been redirected automatically') . ' (#' . __LINE__ . ')'), $url_to_cancel, 5);
			return;
		} else {
			if (sizeof($aliquot_data) > $display_limit) {
				$this->atimFlash(__("batch init - number of submitted records too big") . " (>$display_limit)",
					$url_to_cancel, 5);
				return;
			}
		...
		if ($aliquot_master_id != null) {
			...
			$atim_menu_link = ($aliquot_data[0]['SampleControl']['sample_category'] == 'specimen') ? '/InventoryManagement/AliquotMasters/detail/%%Collection.id%%/%%SampleMaster.initial_specimen_sample_id%%/%%AliquotMaster.id%%' : '/InventoryManagement/AliquotMasters/detail/%%Collection.id%%/%%SampleMaster.id%%/%%AliquotMaster.id%%';
			$this->set('atim_menu_variables', array(
		...
		$this->Structures->set('used_aliq_in_stock_details,used_aliq_in_stock_detail_volume',
			'aliquots_volume_structure');
		$this->set('display_batch_process_aliq_storage_and_in_stock_details', (sizeof(array_filter($aliquot_ids)) > 1));
		$this->Structures->set('batch_process_aliq_storage_and_in_stock_details',
			'batch_process_aliq_storage_and_in_stock_details');
		$this->Structures->set('aliquotinternaluses', 'aliquotinternaluses_structure');
		$this->Structures->set('aliquotinternaluses_volume,aliquotinternaluses',
			'aliquotinternaluses_volume_structure');
		...
		if ($initial_display) {
			...
			foreach ($previous_data as $key_aliquot_master_id => $data_unit) {
				...
				if (!$this->AliquotMaster->validates()) {
					foreach ($this->AliquotMaster->validationErrors as $field => $msgs) {
						$msgs = is_array($msgs) ? $msgs : array($msgs);
						foreach ($msgs as $msg) {
							$errors[$field][$msg][$record_counter] = $record_counter;
						}
					}
				}
				$aliquot_data_to_save[] = array(
			...
			if (empty($errors)) {
				...
				foreach ($aliquot_data_to_save as $new_aliquot_to_save) {
					...
					if (!$this->AliquotMaster->save($new_aliquot_to_save, false)) {
						$this->redirect('/Pages/err_plugin_record_err?method=' . __METHOD__ . ',line=' . __LINE__, null,
							true);
					}
				}
				...
				foreach ($errors as $field => $msg_and_lines) {
					foreach ($msg_and_lines as $msg => $lines) {
						$this->AliquotMaster->validationErrors[$field][] = __($msg) . (($record_counter != 1) ? ' - ' . str_replace('%s',
									implode(",", $lines), __('see # %s')) : '');
					}
				}
			}
		}
	...
	function detailAliquotInternalUse($aliquot_master_id, $aliquot_use_id) {
		...
		$use_data = $this->AliquotInternalUse->find('first', array(
				'fields' => array('*'),
				'conditions' => array(
					'AliquotInternalUse.aliquot_master_id' => $aliquot_master_id,
					'AliquotInternalUse.id' => $aliquot_use_id
				),
				'joins' => array(
		...
		$sample_data = $this->SampleMaster->find('first', array(
			'conditions' => array(
				'SampleMaster.collection_id' => $use_data['AliquotMaster']['collection_id'],
		...
		// Get the current menu object.
		$atim_menu_link = ($sample_data['SampleControl']['sample_category'] == 'specimen') ? '/InventoryManagement/AliquotMasters/detail/%%Collection.id%%/%%SampleMaster.initial_specimen_sample_id%%/%%AliquotMaster.id%%' : '/InventoryManagement/AliquotMasters/detail/%%Collection.id%%/%%SampleMaster.id%%/%%AliquotMaster.id%%';
		$this->set('atim_menu', $this->Menus->get($atim_menu_link));
		$this->set('atim_menu_variables', array(
				'Collection.id' => $use_data['AliquotMaster']['collection_id'],
				'SampleMaster.id' => $use_data['AliquotMaster']['sample_master_id'],
	...
	function editAliquotInternalUse($aliquot_master_id, $aliquot_use_id) {
		...
		if (empty($this->request->data)) {
			...
			if ((!empty($this->request->data['AliquotInternalUse']['used_volume'])) && empty($use_data['AliquotControl']['volume_unit'])) {
				...
				if (empty($this->request->data['AliquotInternalUse']['used_volume'])) {
					// Change '0' to null
			...
			if ($submitted_data_validates) {
				...
				if ($this->AliquotInternalUse->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/InventoryManagement/AliquotMasters/detailAliquotInternalUse/' . $aliquot_master_id . '/' . $aliquot_use_id . '/');
				}
			}
		}
	}
	...
	function deleteAliquotInternalUse($aliquot_master_id, $aliquot_use_id) {
		...
		// -> Delete use
		if ($deletion_done) {
			if (!$this->AliquotInternalUse->atimDelete($aliquot_use_id)) {
				$deletion_done = false;
			}
		}
		...
		if ($deletion_done) {
			...
			$this->atimFlash(__('error deleting data - contact administrator'), $flash_url);
		}
	}
	...
	function addInternalUseToManyAliquots() {
		...
		$studied_aliquot_nbrs = $this->AliquotMaster->find('count',
			array('conditions' => array('AliquotMaster.id' => $aliquot_ids), 'recursive' => '-1'));
		...
		if (!$studied_aliquot_nbrs) {
			...
			if ($studied_aliquot_nbrs > $display_limit) {
				$this->flash((__("batch init - number of submitted records too big") . " (>$display_limit)"),
					$url_to_cancel, 5);
				return;
			}
		}
		...
		foreach ($this->AliquotMaster->find('all', array(
			...
			'fields' => array('DISTINCT AliquotMaster.aliquot_control_id'),
			'recursive' => '-1'
		)) as $new_ctrl) {
			$aliquot_control_ids[] = $new_ctrl['AliquotMaster']['aliquot_control_id'];
		}
		$all_volume_units = $this->AliquotControl->find('all', array(
			'conditions' => array('AliquotControl.id' => $aliquot_control_ids),
			'fields' => array('DISTINCT AliquotControl.volume_unit'),
			'recursive' => '-1'
		));
		...
		if ($initial_display) {
			...
			list($aliquot_master_data_to_update, $validates, $position_deletion_warning_message) = $this->AliquotMaster->validateAliquotMasterDataUpdateInBatch($this->request->data['FunctionManagement'],
				$this->request->data['AliquotMaster'], $aliquot_ids);
			if (!$validates) {
				$submitted_data_validates = false;
			}
			...
			if ($submitted_data_validates) {
				...
				foreach ($aliquot_ids as $aliquot_master_id) {
					...
					if (!$this->AliquotInternalUse->save($aliquot_internal_use_data, false)) {
						$this->redirect('/Pages/err_plugin_record_err?method=' . __METHOD__ . ',line=' . __LINE__, null,
							true);
					}
					if (!$this->AliquotMaster->updateAliquotUseAndVolume($aliquot_master_id, true, true)) {
				...
				if ($position_deletion_warning_message) {
					AppController::addWarningMsg(__($position_deletion_warning_message));
				}
				$this->atimFlash(__('your data has been saved'),
					'/Datamart/BatchSets/listall/' . $batch_set_model->getLastInsertId());
			}
	...
	function addSourceAliquots($collection_id, $sample_master_id) {
		...
		$existing_source_aliquots = $this->SourceAliquot->find('all',
			array('conditions' => array('SourceAliquot.sample_master_id' => $sample_master_id), 'recursive' => '-1'));
		...
		$available_sample_aliquots_wo_volume = $this->AliquotMaster->find('all',
			array('conditions' => $criteria, 'order' => 'AliquotMaster.barcode ASC', 'recursive' => '0'));
		unset($criteria['OR']);
		$criteria['NOT']['OR'] = array(
			array('AliquotControl.volume_unit' => ''),
			array('AliquotControl.volume_unit' => null)
		);
		$available_sample_aliquots_w_volume = $this->AliquotMaster->find('all',
			array('conditions' => $criteria, 'order' => 'AliquotMaster.barcode ASC', 'recursive' => '0'));

		if (empty($available_sample_aliquots_w_volume) && empty($available_sample_aliquots_wo_volume)) {
			$this->atimFlash(__('no new sample aliquot could be actually defined as source aliquot'),
				'/InventoryManagement/SampleMasters/detail/' . $collection_id . '/' . $sample_master_id);
		}
		...
		// MANAGE FORM, MENU AND ACTION BUTTONS
		$this->set('atim_menu',
			$this->Menus->get('/InventoryManagement/SampleMasters/detail/%%Collection.id%%/%%SampleMaster.id%%'));
			...
			foreach ($unified_data_pointers as &$studied_aliquot_pointer) {
				...
				if ($studied_aliquot_pointer['FunctionManagement']['use']) {
					...
					if ((!empty($studied_aliquot_pointer['SourceAliquot']['used_volume'])) && empty($studied_aliquot_pointer['AliquotControl']['volume_unit'])) {
						...
						if (empty($studied_aliquot_pointer['SourceAliquot']['used_volume'])) {
							// Change '0' to null
					...
					foreach ($this->AliquotMaster->validationErrors as $field => $msgs) {
						$msgs = is_array($msgs) ? $msgs : array($msgs);
						foreach ($msgs as $msg) {
							$errors['AliquotMaster'][$field][$msg][] = $line_counter;
						}
					}
					...
					foreach ($this->SourceAliquot->validationErrors as $field => $msgs) {
						$msgs = is_array($msgs) ? $msgs : array($msgs);
						foreach ($msgs as $msg) {
							$errors['SourceAliquot'][$field][$msg][] = $line_counter;
						}
					}
				...
				foreach ($errors as $model => $field_messages) {
					...
					foreach ($field_messages as $field => $messages) {
						foreach ($messages as $message => $lines) {
							$this->{$model}->validationErrors[$field][] = __($message) . ' - ' . str_replace('%s',
									implode(",", $lines), __('see line %s'));
						}
					}
				}
	...
	function editSourceAliquot($sample_master_id, $aliquot_master_id) {
		...
		$source_data = $this->SourceAliquot->find('first', array(
			'conditions' => array(
				'SourceAliquot.sample_master_id' => $sample_master_id,
				'SourceAliquot.aliquot_master_id' => $aliquot_master_id
			)
		));
		...
		if ($this->request->data) {
			...
			if ($this->SourceAliquot->save($this->request->data)) {
				$this->AliquotMaster->updateAliquotUseAndVolume($source_data['AliquotMaster']['id'], true, false,
					false);
				...
				$this->atimFlash(__('your data has been saved'),
					'/InventoryManagement/SampleMasters/detail/' . $source_data['SampleMaster']['collection_id'] . '/' . $source_data['SampleMaster']['id']);
			}
		} else {
	...
	function deleteSourceAliquot($sample_master_id, $aliquot_master_id) {
		if ((!$sample_master_id) || (!$aliquot_master_id)) {
			$this->redirect('/Pages/err_plugin_funct_param_missing?method=' . __METHOD__ . ',line=' . __LINE__, null,
				true);
		}
		...
		// -> Update volume
		if ($deletion_done) {
			$deletion_done = $this->AliquotMaster->updateAliquotUseAndVolume($source_data['AliquotMaster']['id'], true,
				true);
		}
	...
	function realiquotInit($process_type, $aliquot_id = null) {
		...
		if (!empty($aliquot_id)) {
			...
			$this->request->data['url_to_cancel'] = sprintf('/InventoryManagement/AliquotMasters/detail/%d/%d/%d',
				$aliquot['collection_id'], $aliquot['sample_master_id'], $aliquot['id']);
			...
			if (!is_array($ids) && strpos($ids, ',')) {
				...
				$this->atimFlash(__("batch init - number of submitted records too big"), "javascript:history.back();",
					5);
				return;
			}
		...
		if (count($aliquots) > 1) {
			foreach ($aliquots as $aliquot) {
				if (($aliquot['AliquotMaster']['aliquot_control_id'] != $aliquot_ctrl_id) || ($aliquot['SampleMaster']['sample_control_id'] != $sample_ctrl_id)) {
					$this->atimFlash(__("you cannot realiquot those elements together because they are of different types"),
		...
		if (empty($possible_ctrl_ids)) {
			$this->atimFlash(__("you cannot realiquot those elements"), "javascript:history.back();", 5);
			return;
		}
	...
	function realiquotInit2($process_type, $aliquot_id = null) {
		...
		if (!isset($this->request->data['sample_ctrl_id']) || !isset($this->request->data['realiquot_from']) || !isset($this->request->data[0]['realiquot_into']) || !isset($this->request->data[0]['ids'])) {
			...
			if ($this->request->data[0]['realiquot_into'] == '') {
				$this->atimFlash(__("you must select an aliquot type"), "javascript:history.back();", 5);
				return;
			}
		}
		...
		$aliquot_data = $this->AliquotMaster->find('first',
			array('conditions' => array('AliquotMaster.id' => $this->request->data[0]['ids'])));
		$sample_ctrl_id = $aliquot_data['SampleMaster']['sample_control_id'];
		if ($sample_ctrl_id != $this->request->data['sample_ctrl_id']) {
			$this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		}
		$lab_book_ctrl_id = $this->RealiquotingControl->getLabBookCtrlId($sample_ctrl_id,
			$this->request->data['realiquot_from'], $this->request->data[0]['realiquot_into']);
	...
	function realiquot($aliquot_id = null) {
		...
		if (empty($this->request->data)) {
			...
			if (isset($this->request->data[0]) && isset($this->request->data[0]['ids'])) {
				if ($this->request->data[0]['realiquot_into'] == '') {
					$this->atimFlash(__("you must select an aliquot type"), "javascript:history.back();", 5);
					return;
				}
		...
		if (isset($this->request->data['Realiquoting']) && isset($this->request->data['Realiquoting']['lab_book_master_code']) && (strlen($this->request->data['Realiquoting']['lab_book_master_code']) > 0 || $this->request->data['Realiquoting']['sync_with_lab_book'])) {
			...
			$lab_book_expected_ctrl_id = $this->RealiquotingControl->getLabBookCtrlId($sample_ctrl_id,
				$parent_aliquot_ctrl_id, $child_aliquot_ctrl_id);
			$sync_response = $lab_book->syncData($this->request->data, array(),
				$this->request->data['Realiquoting']['lab_book_master_code'], $lab_book_expected_ctrl_id);
			if (is_numeric($sync_response)) {
				...
				$this->atimFlash(__($sync_response), "javascript:history.back()", 5);
				return;
			}
		}
		...
		if (empty($aliquot_id)) {
			...
			$parent = $this->AliquotMaster->find('first',
				array('conditions' => array('AliquotMaster.id' => $aliquot_id), 'recursive' => '0'));
			if (empty($parent)) {
				$this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null,
					true);
			}
		...
		$this->Structures->set('used_aliq_in_stock_details', 'in_stock_detail',
			array('model_table_assoc' => array('AliquotDetail' => 'tmp_detail_table')));
		$this->set('display_batch_process_aliq_storage_and_in_stock_details',
			(sizeof(array_filter(explode(',', $parent_aliquots_ids))) > 1));
		$this->Structures->set('batch_process_aliq_storage_and_in_stock_details',
			'batch_process_aliq_storage_and_in_stock_details');
		...
		$this->Structures->set($child_aliquot_ctrl['AliquotControl']['form_alias'] . (empty($parent_aliquot_ctrl['AliquotControl']['volume_unit']) ? ',realiquot_without_vol' : ',realiquot_with_vol'),
			'atim_structure',
			array('model_table_assoc' => array('AliquotDetail' => $child_aliquot_ctrl['AliquotControl']['detail_tablename'])));
		...
		if ($initial_display) {
			...
			if (sizeof($parent_aliquots) > $display_limit) {
				$this->atimFlash(__("batch init - number of submitted records too big") . " (>$display_limit)",
					$this->request->data['url_to_cancel'], 5);
				return;
			}
			...
			foreach ($this->request->data as $parent_id => $parent_and_children) {
				...
				if ($used_aliquot_data_to_apply_to_all) {
					$parent_and_children = array_replace_recursive($parent_and_children,
						$used_aliquot_data_to_apply_to_all);
				}
			...
			$child_writable_fields['aliquot_masters']['addgrid'] = array_merge($child_writable_fields['aliquot_masters']['addgrid'],
				array(
					'collection_id',
					'sample_master_id',
					'aliquot_control_id',
			...
			$child_writable_fields['realiquotings']['addgrid'] = array_merge($child_writable_fields['realiquotings']['addgrid'],
				array(
			...
			if (empty($errors)) {
				...
				foreach ($this->request->data as $parent_id => $parent_and_children) {
					...
					foreach ($storage_writable_fields as $new_writable_field) {
						AppModel::$writableFields['aliquot_masters']['edit'][] = $new_writable_field;
					}
				...
				if (empty($aliquot_id)) {
					...
					$aliquot = $aliquot['AliquotMaster'];
					$this->atimFlash(__('your data has been saved') . '<br>' . __('aliquot storage data were deleted (if required)'),
						'/InventoryManagement/AliquotMasters/detail/' . $aliquot['collection_id'] . '/' . $aliquot['sample_master_id'] . '/' . $aliquot['id']);
				}
				...
				foreach ($errors as $field => $msg_and_lines) {
					foreach ($msg_and_lines as $msg => $lines) {
						...
						$this->AliquotMaster->validationErrors[$field][] = __($msg) . (empty($aliquot_id) ? ' - ' . str_replace('%s',
									implode(",", $lines), __('see # %s')) : '');
					}
				}
			}
	...
	function defineRealiquotedChildren($aliquot_master_id = null) {
		...
		if (empty($aliquot_master_id)) {
			...
			$parent = $this->AliquotMaster->find('first',
				array('conditions' => array('AliquotMaster.id' => $aliquot_master_id), 'recursive' => '0'));
			if (empty($parent)) {
				$this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null,
					true);
			}
		...
		if (empty($parent_aliquot_ctrl['AliquotControl']['volume_unit'])) {
			...
			$this->Structures->set('children_aliquots_selection,children_aliquots_selection_volume',
				'atim_structure_for_children_aliquots_selection');
		}
		$this->Structures->set('empty', 'empty_structure');
		...
		if ($initial_display) {
			...
			$parent_aliquots = $this->AliquotMaster->find('all',
				array('conditions' => array('AliquotMaster.id' => explode(",", $parent_aliquots_ids))));
			$this->AliquotMaster->sortForDisplay($parent_aliquots, $parent_aliquots_ids);
			if (empty($parent_aliquots)) {
				$this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null,
					true);
			...
			foreach ($parent_aliquots as $parent_aliquot_data) {
				...
				if (empty($aliquot_data_for_selection)) {
					...
					// Set data
					$this->request->data[] = array(
						'parent' => $parent_aliquot_data,
						'children' => $aliquot_data_for_selection
					);
				}
			}
			...
			if (!empty($excluded_parent_aliquot)) {
				...
				$msg = __('no new aliquot could be actually defined as realiquoted child for the following parent aliquot(s)') . ': [' . implode(",",
						$tmp_barcode) . ']';

				if (empty($this->request->data)) {
					$this->atimFlash(__($msg), "javascript:history.back()", 5);
					return;
				} else {
					...
					if (is_numeric($tmp_id)) {
						if ($children_aliquot['FunctionManagement']['use']) {
							...
							if (isset($relations[$children_aliquot['AliquotMaster']['id']])) {
								$errors[][__("circular assignation with [%s]",
									$children_aliquot['AliquotMaster']['barcode'])][] = $record_counter;
							}
				...
				foreach ($this->request->data as $parent_id => $parent_and_children) {
					...
					foreach ($parent_and_children['children'] as $children_aliquot_to_save) {
						if ($children_aliquot_to_save['FunctionManagement']['use']) {
							...
							if (!$this->Realiquoting->save(array('Realiquoting' => $children_aliquot_to_save['Realiquoting']),
								false)
							) {
								$this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__,
									null, true);
	...
	function listAllRealiquotedParents($collection_id, $sample_master_id, $aliquot_master_id) {
		...
		// Get the aliquot data
		$current_aliquot_data = $this->AliquotMaster->find('first', array(
			'conditions' => array(
				'AliquotMaster.collection_id' => $collection_id,
				'AliquotMaster.sample_master_id' => $sample_master_id,
				'AliquotMaster.id' => $aliquot_master_id
		...
		foreach ($this->request->data as &$new_record) {
			...
			if (array_key_exists('lab_book_master_id',
					$new_record['Realiquoting']) && !empty($new_record['Realiquoting']['lab_book_master_id'])
			) {
	...
	function editRealiquoting($realiquoting_id) {
		...
		if ($this->request->data) {
			...
			if ($this->Realiquoting->save($this->request->data)) {
				...
				$this->atimFlash(__('your data has been saved'),
					'/InventoryManagement/AliquotMasters/detail/' . $data['AliquotMasterChildren']['collection_id'] . '/' . $data['AliquotMasterChildren']['sample_master_id'] . '/' . $data['AliquotMasterChildren']['id']);
			}
		} else {
	...
	function deleteRealiquotingData($parent_id, $child_id, $source) {
		...
		// Get the realiquoting data
		$realiquoting_data = $this->Realiquoting->find('first', array(
			'conditions' => array(
				'Realiquoting.parent_aliquot_master_id' => $parent_id,
				'Realiquoting.child_aliquot_master_id' => $child_id
			)
		));
		...
		if ($arr_allow_deletion['allow_deletion']) {
			if ($this->Realiquoting->atimDelete($realiquoting_data['Realiquoting']['id'])) {
				...
				if ($this->AliquotMaster->updateAliquotUseAndVolume($realiquoting_data['AliquotMaster']['id'], true,
					true)
				) {
					$this->atimFlash(__('your data has been deleted - update the aliquot in stock data'), $flash_url);
				} else {
			...
			$this->atimFlash(__($arr_allow_deletion['msg']), $flash_url);
		}
	}
	...
	function contentTreeView($collection_id, $aliquot_master_id, $is_ajax = false) {
		...
		$atim_structure['AliquotMaster'] = $this->Structures->get('form',
			'aliquot_masters_for_collection_tree_view,realiquoting_data_for_collection_tree_view');
		...
		$this->AliquotMaster->unbindModel(array(
			'belongsTo' => array('Collection', 'SampleMaster'),
			'hasOne' => array('SpecimenDetail')
		), false);
		...
		foreach ($this->Realiquoting->find('all', array(
			'conditions' => array('Realiquoting.parent_aliquot_master_id' => $aliquot_master_id),
			'recursive' => '-1'
		)) as $new_realiquoting_data) {
			$realiquoting_data_from_child_ids[$new_realiquoting_data['Realiquoting']['child_aliquot_master_id']] = $new_realiquoting_data;
		}
		$this->request->data = $this->AliquotMaster->find('all', array(
			'conditions' => array(
				'AliquotMaster.id' => array_keys($realiquoting_data_from_child_ids),
				'AliquotMaster.collection_id' => $collection_id
			)
		));
		foreach ($this->request->data as &$new_children_aliquot_data) {
			$new_children_aliquot_data = array_merge($new_children_aliquot_data,
				$realiquoting_data_from_child_ids[$new_children_aliquot_data['AliquotMaster']['id']]);
		}
		...
		$tmp_aliquot_uses = $this->ViewAliquotUse->find('all', array(
			'conditions' => array(
				'ViewAliquotUse.aliquot_master_id' => $aliquot_master_id,
				"ViewAliquotUse.use_definition !='realiquoted to'"
			),
			'order' => array('ViewAliquotUse.use_datetime ASC')
		));
		$aliquot_uses = array();
		foreach ($tmp_aliquot_uses as $new_aliquot_use) {
			...
			switch ($new_aliquot_use['ViewAliquotUse']['use_definition']) {
				...
				case 'aliquot shipment':
					...
					break;
				default:
					$model = preg_match('/^sample\ derivative\ creation.+$/',
						$new_aliquot_use['ViewAliquotUse']['use_definition']) ? 'SampleMaster' : 'AliquotInternalUse';
			}
		...
		foreach ($this->request->data as $new_record) {
			...
			if (isset($new_record['ViewAliquotUse']['use_datetime'])) {
				...
				$new_record['ViewAliquotUse']['use_datetime_accuracy'] = str_replace(array('', 'c', 'i'),
					array('h', 'h', 'h'), $new_record['ViewAliquotUse']['use_datetime_accuracy']);
			} else {
				if (isset($new_record['Realiquoting']['realiquoting_datetime'])) {
					$date_key = $new_record['Realiquoting']['realiquoting_datetime'] . $date_key;
					$new_record['Realiquoting']['realiquoting_datetime_accuracy'] = str_replace(array('', 'c', 'i'),
						array('h', 'h', 'h'), $new_record['Realiquoting']['realiquoting_datetime_accuracy']);
				} else {
	...
	function listallUses($collection_id, $sample_master_id, $aliquot_master_id, $is_from_tree_view = false) {
		...
		$this->request->data = $this->ViewAliquotUse->find('all',
			array('conditions' => array('ViewAliquotUse.aliquot_master_id' => $aliquot_master_id)));
	...
	function printBarcodes() {
		...
		switch ($this->passedArgs['model']) {
			case 'Collection':
				...
				if ($conditions['AliquotMaster.collection_id'] == 'all' && isset($this->request->data['node'])) {
					...
					$browsing_result = $this->BrowsingResult->find('first',
						array('conditions' => array('BrowsingResult.id' => $this->request->data['node']['id'])));
					$conditions['AliquotMaster.collection_id'] = explode(",",
						$browsing_result['BrowsingResult']['id_csv']);
				}
				break;
			case 'SampleMaster':
				...
				if ($conditions['AliquotMaster.sample_master_id'] == 'all' && isset($this->request->data['node'])) {
					...
					$browsing_result = $this->BrowsingResult->find('first',
						array('conditions' => array('BrowsingResult.id' => $this->request->data['node']['id'])));
					$conditions['AliquotMaster.sample_master_id'] = explode(",",
						$browsing_result['BrowsingResult']['id_csv']);
				}
				break;
			case 'AliquotMaster':
		...
		$at_least_once = false;
		$aliquots_count = $this->AliquotMaster->find('count',
			array('conditions' => $conditions, 'limit' => 1000, 'offset' => $offset));
		...
		while ($this->request->data = $this->AliquotMaster->find('all',
			array('conditions' => $conditions, 'limit' => 300, 'offset' => $offset))) {
		...
		if ($at_least_once) {
			...
			$this->atimFlash(__('there are no barcodes to print'), 'javascript:history.back();');
		}
	}
}

...
class SampleMastersController extends InventoryManagementAppController {
	...
	function search($search_id = 0) {
		...
		$this->searchHandler($search_id, $this->ViewSample, 'view_sample_joined_to_collection',
			'/InventoryManagement/SampleMasters/search');
	...
	function contentTreeView($collection_id, $sample_master_id = 0, $is_ajax = false) {
		...
		if ($sample_master_id == 0) {
			...
			foreach ($this->request->data as &$new_sample) {
				$new_sample['DerivativeDetail']['creation_datetime_accuracy'] = str_replace(array('', 'c', 'i'),
					array('h', 'h', 'h'), $new_sample['DerivativeDetail']['creation_datetime_accuracy']);
			}
		}
		...
		if ($sample_master_id != 0) {
			...
			$aliquot_ids = $this->AliquotMaster->find('list', array(
				'fields' => array('AliquotMaster.id'),
				'conditions' => array(
			...
			$aliquot_ids = array_diff($aliquot_ids, $this->Realiquoting->find('list', array(
				'fields' => array('Realiquoting.child_aliquot_master_id'),
				'conditions' => array("Realiquoting.child_aliquot_master_id" => $aliquot_ids),
				'group' => array("Realiquoting.child_aliquot_master_id")
			)));
			$aliquot_ids_has_child = array_flip($this->AliquotMaster->hasChild($aliquot_ids));

			$aliquot_ids[] = 0;//counters Eventum 1353
			$aliquots = $this->AliquotMaster->find('all',
				array('conditions' => array("AliquotMaster.id" => $aliquot_ids), 'recursive' => 0));
	...
	function listAllDerivatives($collection_id, $specimen_sample_master_id) {
		if ((!$collection_id) || (!$specimen_sample_master_id)) {
			$this->redirect('/Pages/err_plugin_funct_param_missing?method=' . __METHOD__ . ',line=' . __LINE__, null,
				true);
		}

		$specimen_data = $this->SampleMaster->find('first', array(
			'conditions' => array(
				'SampleMaster.collection_id' => $collection_id,
				'SampleMaster.id' => $specimen_sample_master_id
			),
			'recursive' => -1
		));
		...
		$derivatives_data = $this->SampleMaster->find('all', array(
			'conditions' => array(
				'SampleMaster.collection_id' => $collection_id,
				'SampleMaster.initial_specimen_sample_id' => $specimen_sample_master_id,
		...
		if (empty($derivatives_data)) {
			...
			// Data
			$derivatives_data = AppController::defineArrayKey($derivatives_data, "SampleMaster", "sample_control_id",
				false);
			...
			foreach ($derivatives_data as $sample_control_id => $derivatives) {
				$derivatives_structures[$sample_control_id] = $this->Structures->get('form',
					$derivatives[0]['SampleControl']['form_alias']);
			}
	...
	function detail($collection_id, $sample_master_id, $is_from_tree_view = 0) {
		...
		// Get the sample data
		$sample_data = $this->ViewSample->find('first', array(
			'conditions' => array(
				'ViewSample.collection_id' => $collection_id,
				'ViewSample.sample_master_id' => $sample_master_id
			)
		));
		if (empty($sample_data)) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		}
		if (array_key_exists('coll_to_rec_spent_time_msg', $sample_data['ViewSample'])) {
			$sample_data['SampleMaster']['coll_to_rec_spent_time_msg'] = $sample_data['ViewSample']['coll_to_rec_spent_time_msg'];
		}
		if (array_key_exists('coll_to_creation_spent_time_msg', $sample_data['ViewSample'])) {
		...
		$parent_sample_data = $this->SampleMaster->find('first', array(
			'conditions' => array(
				'SampleMaster.collection_id' => $collection_id,
				'SampleMaster.id' => $parent_sample_master_id
			),
			'recursive' => '0'
		));
		...
		$this->set('parent_sample_data_for_display',
			$this->SampleMaster->formatParentSampleDataForDisplay($parent_sample_data));
		...
		if (!$is_from_tree_view) {
			$aliquots_data = $this->AliquotMaster->find('all', array(
				'conditions' => array(
					'AliquotMaster.collection_id' => $collection_id,
					'AliquotMaster.sample_master_id' => $sample_master_id
				)
			));
			$aliquots_data = AppController::defineArrayKey($aliquots_data, "AliquotMaster", "aliquot_control_id",
				false);
		...
		if (!$is_specimen) {
			...
			$tmp_data = $this->ParentToDerivativeSampleControl->find('first', array(
				'conditions' => array(
		...
		if (!$is_from_tree_view) {
			...
			foreach ($aliquots_data as $aliquot_control_id => $aliquots) {
				$aliquots_structures[$aliquot_control_id] = $this->Structures->get('form',
					$aliquots[0]['AliquotControl']['form_alias']);
			}
		...
		$this->set('allowed_derivative_type',
			$this->SampleControl->getPermissibleSamplesArray($sample_data['SampleControl']['id']));

		// Get all aliquot control types to build the add to selected button
		$this->set('allowed_aliquot_type',
			$this->AliquotControl->getPermissibleAliquotsArray($sample_data['SampleControl']['id']));
	...
	function add($collection_id, $sample_control_id, $parent_sample_master_id = 0) {
		...
		if ($parent_sample_master_id == 0) {
			...
			$parent_to_derivative_sample_control = $this->ParentToDerivativeSampleControl->find('first',
		...
		$atim_menu_variables = (empty($parent_sample_data) ? array('Collection.id' => $collection_id) : array(
			'Collection.id' => $collection_id,
			'SampleMaster.initial_specimen_sample_id' => $parent_sample_data['SampleMaster']['initial_specimen_sample_id']
		));
		...
		$this->Structures->set($structure_name, 'atim_structure',
			array('model_table_assoc' => array('SampleDetail' => $sample_control_data['SampleControl']['detail_tablename'])));
		...
		if (empty($this->request->data)) {
			...
			if ($is_specimen && !isset(AppController::getInstance()->passedArgs['templateInitId'])) {
				...
				if ($this->SampleMaster->find('count',
						array('conditions' => array('SampleMaster.collection_id' => $collection_id))) == 0
				) {
					$collection = $this->Collection->find('first',
						array('conditions' => array('Collection.id' => $collection_id)));
					...
					$sample = $this->SampleMaster->find('first', array(
						...
						'order' => array('SpecimenDetail.reception_datetime DESC')
					));
			...
			$this->SampleMaster->addWritableField(array('sample_master_id'),
				$sample_control_data['SampleControl']['detail_tablename']);
			$this->SampleMaster->addWritableField(array('sample_master_id'),
				$is_specimen ? 'specimen_details' : 'derivative_details');
			...
			if ($submitted_data_validates) {
				...
				if ($this->SampleMaster->save($this->request->data, false)) {
					...
					$this->SampleMaster->tryCatchQuery(str_replace("sample_masters", "sample_masters_revs",
						$query_to_update));
	...
	function edit($collection_id, $sample_master_id) {
		...
		if (!$is_specimen) {
			...
			$lab_book = AppModel::getInstance("LabBook", "LabBookMaster", true);
			$lab_book_ctrl_id = $this->ParentToDerivativeSampleControl->getLabBookControlId($parent_sample_data['SampleMaster']['sample_control_id'],
				$sample_data['SampleMaster']['sample_control_id']);
			$lab_book_fields = $lab_book->getFields($lab_book_ctrl_id);

			// Set lab book code for initial display
			if (empty($this->request->data) && !empty($sample_data['DerivativeDetail']['lab_book_master_id'])) {
				$previous_labook = $lab_book->find('first', array(
					'conditions' => array('id' => $sample_data['DerivativeDetail']['lab_book_master_id']),
					'recursive' => '-1'
				));
		...
		$this->Structures->set($structure_name, 'atim_structure',
			array('model_table_assoc' => array('SampleDetail' => $sample_data['SampleControl']['detail_tablename'])));
		...
		if (empty($this->request->data)) {
			...
			//Update data	
			if (isset($this->request->data['SampleMaster']['parent_id']) && ($sample_data['SampleMaster']['parent_id'] !== $this->request->data['SampleMaster']['parent_id'])) {
				$this->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__, null,
					true);
			}
			...
			if ($is_specimen) {
				...
				if (array_key_exists('sync_with_lab_book_now', $this->request->data)) {
					$msg = $this->SampleMaster->validateLabBook($this->request->data, $lab_book, $lab_book_ctrl_id,
						$this->request->data[0]['sync_with_lab_book_now']);
					if (strlen($msg) > 0) {
						$this->DerivativeDetail->validationErrors['lab_book_master_code'][] = $msg;
						$submitted_data_validates = false;
					}
		...
		if ($arr_allow_deletion['allow_deletion']) {
			if ($this->SampleMaster->atimDelete($sample_master_id)) {
				...
				$this->atimFlash(__('your data has been deleted'),
					'/InventoryManagement/Collections/detail/' . $collection_id);
	...
	function batchDerivativeInit($aliquot_master_id = null) {
					...
					if (isset($this->request->data['ViewAliquot']) || isset($this->request->data['AliquotMaster'])) {
						...
						foreach ($aliquot_data as $aliquot_unit) {
							if ($aliquot_unit['AliquotMaster']['aliquot_control_id'] != $expected_ctrl_id) {
								$this->atimFlash(__("you must select elements with a common type"), $url_to_cancel, 5);
							}
		...
		// Manage data	
		$init_data = $this->batchInit($this->SampleMaster, $model, $key, "sample_control_id",
			$this->ParentToDerivativeSampleControl, "parent_sample_control_id",
	...
	function batchDerivativeInit2($aliquot_master_id = null) {
		if (!isset($this->request->data['SampleMaster']['ids']) || !isset($this->request->data['SampleMaster']['sample_control_id']) || !isset($this->request->data['ParentToDerivativeSampleControl']['parent_sample_control_id'])) {
			...
			if ($this->request->data['SampleMaster']['sample_control_id'] == '') {
				$this->atimFlash(__("you must select a derivative type"), "javascript:history.back();", 5);
				return;
			}
		}
		...
		$this->set('parent_sample_control_id',
			$this->request->data['ParentToDerivativeSampleControl']['parent_sample_control_id']);
		$this->setUrlToCancel();
	...
	function batchDerivative($aliquot_master_id = null) {
		...
		if (!isset($this->request->data['SampleMaster']['sample_control_id']) || !isset($this->request->data['ParentToDerivativeSampleControl']['parent_sample_control_id'])) {
			...
			if ($this->request->data['SampleMaster']['sample_control_id'] == '') {
				$this->atimFlash(__("you must select a derivative type"), $url_to_cancel, 5);
				return;
			}
		}
		...
		if (isset($this->request->data['DerivativeDetail']['lab_book_master_code']) && !empty($this->request->data['DerivativeDetail']['lab_book_master_code'])) {
			...
			$lab_book = AppModel::getInstance("LabBook", "LabBookMaster", true);
			$lab_book_expected_ctrl_id = $this->ParentToDerivativeSampleControl->getLabBookControlId($parent_sample_control_id,
				$this->request->data['SampleMaster']['sample_control_id']);
			$foo = array();
			...
			if (is_numeric($result)) {
				...
				$this->atimFlash(__($result), $url_to_cancel, 5);
				return;
			}
		...
		$ids = array_key_exists('ids',
			$this->request->data['SampleMaster']) ? $this->request->data['SampleMaster']['ids'] : $this->request->data['sample_master_ids'];
		...
		$this->Structures->set(str_replace(",derivative_lab_book", "",
			$children_control_data['SampleControl']['form_alias']), 'derivative_structure',
			array('model_table_assoc' => array('SampleDetail' => $children_control_data['SampleControl']['detail_tablename'])));
		$this->Structures->set(str_replace(",derivative_lab_book", "",
				$children_control_data['SampleControl']['form_alias']) . ",sourcealiquots_volume_for_batchderivative",
			'derivative_volume_structure');
		...
		$this->set('created_sample_override_data',
			array('SampleControl.sample_type' => $children_control_data['SampleControl']['sample_type']));
		...
		if (isset($this->request->data['SampleMaster']['ids'])) {
			...
			$this->set('parent_sample_data_for_display',
				$this->SampleMaster->formatParentSampleDataForDisplay($parent_sample_data_for_display));
			...
			foreach ($prev_data as $parent_id => &$children) {
				...
				if (isset($children['AliquotMaster'])) {
					if ($used_aliquot_data_to_apply_to_all) {
						$children = array_replace_recursive($children, $used_aliquot_data_to_apply_to_all);
					}
					...
					$parent = $this->ViewSample->find('first',
						array('conditions' => array('ViewSample.sample_master_id' => $parent_id), 'recursive' => -1));
				}
			...
			if (empty($errors)) {
				...
				$this->SampleMaster->addWritableField(array('sample_master_id'),
					$children_control_data['SampleControl']['detail_tablename']);
				...
				if (is_null($unique_aliquot_master_data)) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/InventoryManagement/SampleMasters/detail/' . $unique_aliquot_master_data['AliquotMaster']['collection_id'] . '/' . $child_ids[0] . '/');
				}
				...
				foreach ($errors as $field => $msg_and_lines) {
					foreach ($msg_and_lines as $msg => $lines) {
						$this->SampleMaster->validationErrors[$field][] = __($msg) . ' - ' . str_replace('%s',
		...
		$this->set('display_batch_process_aliq_storage_and_in_stock_details',
			$display_batch_process_aliq_storage_and_in_stock_details);

...
class AliquotReviewMaster extends InventoryManagementAppModel {
	...
	function getAliquotListForReview($sample_master_id = null, $specific_aliquot_type = null) {
		...
		if (!empty($sample_master_id)) {
			...
			foreach ($this->AliquotMaster->find('all', array(
				...
				$result[$new_aliquot['AliquotMaster']['id']] = $this->generateLabelOfReviewedAliquot($new_aliquot['AliquotMaster']['id'],
					$new_aliquot);
			}
		}
	...
	function afterFind($results, $primary = false) {
		...
		foreach ($results as &$new_review) {
			$new_review['Generated']['reviewed_aliquot_label_for_display'] = (isset($new_review['AliquotMaster']) && $new_review['AliquotMaster']['id']) ? $this->generateLabelOfReviewedAliquot($new_review['AliquotMaster']['id'],
				$new_review) : '';
		}
		return $results;
	}

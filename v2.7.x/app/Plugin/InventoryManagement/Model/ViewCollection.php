...
class ViewCollection extends InventoryManagementAppModel {
	...
	function summary($variables = array()) {
		...
		if (isset($variables['Collection.id'])) {
			$collection_data = $this->find('first', array(
				'conditions' => array('ViewCollection.collection_id' => $variables['Collection.id']),
				'recursive' => '-1'
			));
				...
				if (!$collection_data['ViewCollection']['participant_id']) {
					...
					if ($consent_status[$variables['Collection.id']] == null) {
						...
						if (AppController::checkLinkPermission('/ClinicalAnnotation/ClinicalCollectionLinks/detail/')) {
							$link = sprintf(' <a href="%sClinicalAnnotation/ClinicalCollectionLinks/detail/%d/%d">%s</a>',
								AppController::getInstance()->request->webroot,
								$collection_data['ViewCollection']['participant_id'],
								$collection_data['ViewCollection']['collection_id'], __('click here to access it'));
						}
						AppController::addWarningMsg(__('no consent is linked to the current participant collection') . '.' . $link);
					} else {
						AppController::addWarningMsg(__('the linked consent status is [%s]',
							__($consent_status[$variables['Collection.id']])));
					}
				}
			}
		}
	...
	function getUnconsentedParticipantCollections(array $collection) {
		...
		if (array_key_exists('id', $collection)) {
			$data = $this->find('all',
				array('conditions' => array('ViewCollection.collection_id' => $collection['id']), 'recursive' => '-1'));
		} else {
			$data = array_key_exists('ViewCollection',
				$collection['data']) ? array($collection['data']) : $collection['data'];
		}
		...
		if (!empty($consents_to_fetch)) {
			...
			$consent_data = $consent_model->find('all', array(
					...
					'conditions' => array(
						...
						'NOT' => array('ConsentMaster.consent_status' => 'obtained')
					),
					'recursive' => -1
				));

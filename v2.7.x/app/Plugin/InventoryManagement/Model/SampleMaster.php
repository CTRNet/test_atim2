...
class SampleMaster extends InventoryManagementAppModel {
	...
	static public $join_sample_control_on_dup = array(
		'table' => 'sample_controls',
		'alias' => 'SampleControl',
		'type' => 'LEFT',
		'conditions' => array('sample_masters_dup.sample_control_id =SampleControl.id')
	);
	...
	function specimenSummary($variables = array()) {
		...
		if (isset($variables['Collection.id']) && isset($variables['SampleMaster.initial_specimen_sample_id'])) {
			...
			$sample_precision = '';
			if (array_key_exists('blood_type',
					$specimen_data['SampleDetail']) && $specimen_data['SampleDetail']['blood_type']
			) {
				$query = array(
					'recursive' => 2,
	...
	function hasChild(array $sample_master_ids) {
		...
		$result = array_filter($this->find('list', array(
			'fields' => array("SampleMaster.parent_id"),
			'conditions' => array('SampleMaster.parent_id' => $sample_master_ids),
			'group' => array('SampleMaster.parent_id')
		)));
		...
		return array_merge($result, array_filter($aliquot_master->find('list', array(
			...
			'conditions' => array('AliquotMaster.sample_master_id' => $sample_master_ids),
	...
	function validateLabBook(array &$data, $lab_book, $lab_book_ctrl_id, $sync) {
		...
		if (array_key_exists('lab_book_master_code',
				$data['DerivativeDetail']) && strlen($data['DerivativeDetail']['lab_book_master_code']) > 0
		) {
			$result = $lab_book->syncData($data, $sync ? array('DerivativeDetail') : array(),
				$data['DerivativeDetail']['lab_book_master_code'], $lab_book_ctrl_id);
			...
			if ((array_key_exists('sync_with_lab_book',
						$data['DerivativeDetail']) && $this->data['DerivativeDetail']['sync_with_lab_book']) || (isset($data[0]['sync_with_lab_book_now']) && $data[0]['sync_with_lab_book_now'])
			) {
	...
	function allowDeletion($sample_master_id) {
		...
		$returned_nbr = $this->find('count',
			array('conditions' => array('SampleMaster.parent_id' => $sample_master_id), 'recursive' => '-1'));
		if ($returned_nbr > 0) {
		...
		$returned_nbr = $aliquot_master_model->find('count',
			array('conditions' => array('AliquotMaster.sample_master_id' => $sample_master_id), 'recursive' => '-1'));
		if ($returned_nbr > 0) {
		...
		$returned_nbr = $quality_ctrl_model->find('count',
			array('conditions' => array('QualityCtrl.sample_master_id' => $sample_master_id), 'recursive' => '-1'));
		if ($returned_nbr > 0) {
		...
		$returned_nbr = $specimen_review_master_model->find('count', array(
			'conditions' => array('SpecimenReviewMaster.sample_master_id' => $sample_master_id),
			'recursive' => '-1'
		));
		if ($returned_nbr > 0) {
	...
	function formatParentSampleDataForDisplay($parent_sample_data) {
		...
		if (!empty($parent_sample_data)) {
			if (isset($parent_sample_data['SampleMaster'])) {
				$formatted_data[$parent_sample_data['SampleMaster']['id']] = $parent_sample_data['SampleMaster']['sample_code'] . ' [' . __($parent_sample_data['SampleControl']['sample_type'],
						true) . ']';
			} else {
				if (isset($parent_sample_data[0]['ViewSample'])) {
					foreach ($parent_sample_data as $new_parent) {
						$formatted_data[$new_parent['ViewSample']['sample_master_id']] = $new_parent['ViewSample']['sample_code'] . ' [' . __($new_parent['ViewSample']['sample_type'],
								true) . ']';
					}
				}
			}
	...
	function atimDelete($model_id, $cascade = true) {
		if (parent::atimDelete($model_id, $cascade)) {
			...
			$source_aliquots = $source_aliquot_model->find('all',
				array('conditions' => array('SourceAliquot.sample_master_id' => $model_id)));
			$sources = array();
	...
	static function joinOnSampleDup($on_field) {
		return array(
			...
			'type' => 'LEFT',
			'conditions' => array($on_field . ' = sample_masters_dup.id')
		);
	}
}

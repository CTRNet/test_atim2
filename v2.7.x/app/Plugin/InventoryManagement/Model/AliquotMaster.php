...
class AliquotMaster extends InventoryManagementAppModel {
	...
	static public $volume_condition = array(
		'OR' => array(
	...
	static public $join_aliquot_control_on_dup = array(
		'table' => 'aliquot_controls',
		'alias' => 'AliquotControl',
		'type' => 'LEFT',
		'conditions' => array('aliquot_masters_dup.aliquot_control_id = AliquotControl.id')
	);
	...
	function updateAliquotUseAndVolume(
		$aliquot_master_id,
		$update_current_volume = true,
		$update_uses_counter = true,
		$remove_from_stock_if_empty_volume = false
	) {
		if (empty($aliquot_master_id)) {
			AppController::getInstance()->redirect('/Pages/err_plugin_funct_param_missing?method=' . __METHOD__ . ',line=' . __LINE__,
				null, true);
		}
		...
		if ($update_current_volume) {
			...
			if (empty($initial_volume)) {
				...
				$aliquot_uses = $this->tryCatchQuery(str_replace('%%WHERE%%',
					"AND AliquotMaster.id= $aliquot_master_id", $view_aliquot_use::$table_query));
		...
		foreach ($aliquot_data_to_save as $key_to_save => $value_to_save) {
			if ($key_to_save == "current_volume") {
				$this->data['AliquotMaster'][$key_to_save] = str_replace('0.00000', '0',
					$this->data['AliquotMaster'][$key_to_save]);
			}
	...
	function validateAndUpdateAliquotStorageData() {
		...
		// Launch validation		
		if (array_key_exists('FunctionManagement',
				$aliquot_data) && array_key_exists('recorded_storage_selection_label',
				$aliquot_data['FunctionManagement'])
		) {
			...
			$arr_storage_selection_results = self::$storage->validateAndGetStorageData($aliquot_data['FunctionManagement']['recorded_storage_selection_label'],
				$aliquot_data['AliquotMaster']['storage_coord_x'], $aliquot_data['AliquotMaster']['storage_coord_y'],
				$is_sample_core);
			...
			if ($set_storage) {
				...
				if (empty($this->validationErrors['storage_coord_x']) && empty($this->validationErrors['storage_coord_y']) && array_key_exists('StorageControl',
						$arr_storage_selection_results['storage_data']) && $arr_storage_selection_results['storage_data']['StorageControl']['check_conflicts'] && (strlen($aliquot_data['AliquotMaster']['storage_coord_x']) > 0 || strlen($aliquot_data['AliquotMaster']['storage_coord_y']) > 0)
				) {
					$exception = $this->id ? array('AliquotMaster' => $this->id) : array();
					$position_status = $this->StorageMaster->positionStatusQuick($arr_storage_selection_results['storage_data']['StorageMaster']['id'],
						array(
			...
			if ((array_key_exists('storage_coord_x',
						$aliquot_data['AliquotMaster']) && !empty($aliquot_data['AliquotMaster']['storage_coord_x'])) || (array_key_exists('storage_coord_y',
						$aliquot_data['AliquotMaster']) && !empty($aliquot_data['AliquotMaster']['storage_coord_y']))
			) {
				AppController::getInstance()->redirect('/Pages/err_plugin_system_error?method=' . __METHOD__ . ',line=' . __LINE__,
					null, true);
			}
	...
	function checkDuplicatedAliquotBarcode($aliquot_data) {
		...
		if (!strlen($barcode)) {
			...
			if (isset($this->barcodes[$barcode])) {
				$this->validationErrors['barcode'][] = str_replace('%s', $barcode,
					__('you can not record barcode [%s] twice'));
			} else {
		...
		$aliquots_having_duplicated_barcode = $this->find('all',
			array('conditions' => array('AliquotMaster.barcode' => $barcode), 'recursive' => -1));;
		if (!empty($aliquots_having_duplicated_barcode)) {
			foreach ($aliquots_having_duplicated_barcode as $duplicate) {
				if ((!array_key_exists('id',
						$aliquot_data['AliquotMaster'])) || ($duplicate['AliquotMaster']['id'] != $aliquot_data['AliquotMaster']['id'])
				) {
					$this->validationErrors['barcode'][] = str_replace('%s', $barcode,
						__('the barcode [%s] has already been recorded'));
				}
			}
		}
	}
	...
	function hasChild(array $aliquot_master_ids) {
		...
		return array_filter($ViewAliquotUse->find('list', array(
			'fields' => array('ViewAliquotUse.aliquot_master_id'),
			'conditions' => array('ViewAliquotUse.aliquot_master_id' => $aliquot_master_ids),
			'group' => array('ViewAliquotUse.aliquot_master_id')
		)));
	}
	...
	function getDefaultStorageDate($sample_master_data) {
		...
		switch ($sample_master_data['SampleControl']['sample_category']) {
			...
			case 'derivative':
				...
				$derivative_detail_data = $derivative_detail_model->find('first', array(
					'conditions' => array('DerivativeDetail.sample_master_id' => $sample_master_data['SampleMaster']['id']),
					'recursive' => '-1'
				));
				if (empty($derivative_detail_data)) {
					$this->redirect('/Pages/err_plugin_funct_param_missing?method=' . __METHOD__ . ',line=' . __LINE__,
						null, true);
				}
	...
	function allowDeletion($aliquot_master_id) {
		...
		$returned_nbr = $aliquot_internal_use_model->find('count', array(
			'conditions' => array('AliquotInternalUse.aliquot_master_id' => $aliquot_master_id),
			'recursive' => '-1'
		));
		if ($returned_nbr > 0) {
		...
		$returned_nbr = $realiquoting_model->find('count', array(
		...
		$returned_nbr = $aliquot_review_master_model->find('count', array(
			'conditions' => array('AliquotReviewMaster.aliquot_master_id' => $aliquot_master_id),
			'recursive' => '-1'
		));
		if ($returned_nbr > 0) {
		...
		$returned_nbr = $order_item_model->find('count',
			array('conditions' => array('OrderItem.aliquot_master_id' => $aliquot_master_id), 'recursive' => '-1'));
		if ($returned_nbr > 0) {
		...
		$returned_nbr = $quality_ctrl_model->find('count',
			array('conditions' => array('QualityCtrl.aliquot_master_id' => $aliquot_master_id), 'recursive' => '-1'));
		if ($returned_nbr > 0) {
		...
		$returned_nbr = $source_aliquot_model->find('count',
			array('conditions' => array('SourceAliquot.aliquot_master_id' => $aliquot_master_id), 'recursive' => '-1'));
		if ($returned_nbr > 0) {
	...
	function atimDelete($model_id, $cascade = true) {
		if (parent::atimDelete($model_id, $cascade)) {
			...
			$realiquoting_model = AppModel::getInstance('InventoryManagement', 'Realiquoting', true);
			$realiquotings = $realiquoting_model->find('all',
				array('conditions' => array('Realiquoting.child_aliquot_master_id' => $model_id)));
			$parents = array();
	...
	static function joinOnAliquotDup($on_field) {
		return array(
			...
			'type' => 'LEFT',
			'conditions' => array($on_field . ' = aliquot_masters_dup.id')
		);
	}

	function validateAliquotMasterDataUpdateInBatch(
		$function_management_data,
		$submitted_aliquot_master_data,
		$aliquot_ids
	) {
		//Set in_stock value	
		...
		if (strlen($function_management_data['recorded_storage_selection_label']) && (($function_management_data['remove_from_storage'] == '1') || ($submitted_aliquot_master_data['in_stock'] == 'no'))) {
			...
			if ($submitted_aliquot_master_data['in_stock'] == 'no') {
		...
		foreach ($submitted_aliquot_master_data as $key => $value) {
			if (strlen($submitted_aliquot_master_data[$key]) && array_key_exists('remove_' . $key,
					$function_management_data) && $function_management_data['remove_' . $key] == '1'
			) {
				$validates = false;
		...
		if ($validates) {
			...
			if ($function_management_data['recorded_storage_selection_label']) {
				...
				if (empty($submitted_aliquot_master_data['in_stock'])) {
					...
					$aliquot_not_in_stock = $this->find('count',
						array('conditions' => $condtions, 'recursive' => '-1'));
			...
			// Work on other data
			foreach ($submitted_aliquot_master_data as $key => $value) {
				if (array_key_exists('remove_' . $key,
						$function_management_data) && $function_management_data['remove_' . $key] == '1'
				) {
		...
		if ($validates) {
			...
			$not_core_nbr = $this->find('count', array(
				'conditions' => array(
		...
		if (sizeof($aliquot_master_data_to_update['AliquotMaster']) == '1' && array_key_exists('__validated__',
				$aliquot_master_data_to_update['AliquotMaster'])
		) {
			$aliquot_master_data_to_update['AliquotMaster'] = array();
		}    //No data to save

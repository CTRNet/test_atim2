...
class ClinicalCollectionLinksController extends ClinicalAnnotationAppController {
	...
	public function beforeFilter() {
		...
		if ($error) {
			...
			$this->flash(__('you need privileges on the following modules to manage participant inventory: %s',
				implode(', ', $error)), 'javascript:history.back()');
		}
	}

	//var $paginate = array('Collection' => array('limit' => PAGINATION_AMOUNT,'order'=>'Collection.acquisition_label ASC'));

	public function listall($participant_id) {
		...
		$joins = array(
			...
			array(
				...
				'type' => 'LEFT',
				'conditions' => array('Collection.treatment_master_id = treatment_masters_dup.id')
			),
			array(
				'table' => 'treatment_controls',
				'alias' => 'TreatmentControl',
				'type' => 'LEFT',
				'conditions' => array('treatment_masters_dup.treatment_control_id = TreatmentControl.id')
			),
			array(
				'table' => 'event_masters',
				'alias' => 'event_masters_dup',
				'type' => 'LEFT',
				'conditions' => array('Collection.event_master_id = event_masters_dup.id')
			),
			array(
				'table' => 'event_controls',
				'alias' => 'EventControl',
				'type' => 'LEFT',
				'conditions' => array('event_masters_dup.event_control_id = EventControl.id')
			)
		);
		...
		if (!$this->request->is('ajax')) {
			...
			$tree_view_atim_structure['Collection'] = $this->Structures->get('form',
				'collections_for_collection_tree_view');
	...
	public function detail($participant_id, $collection_id) {
		...
		$this->set('atim_menu_variables',
			array('Participant.id' => $participant_id, 'Collection.id' => $collection_id));
	...
	public function add($participant_id) {
		$participant_data = $this->Participant->getOrRedirect($participant_id);

		// Set collections list
		$this->set('collection_id',
			isset($this->request->data['Collection']['id']) ? $this->request->data['Collection']['id'] : null);

		// Set consents list
		$consent_data = $this->ConsentMaster->find('all',
			array('conditions' => array('ConsentMaster.participant_id' => $participant_id)));
		$consent_found = false;
		if (isset($this->request->data['Collection']['consent_master_id'])) {
			$consent_found = $this->setForRadiolist($consent_data, 'ConsentMaster', 'id', $this->request->data,
		...
		// Set diagnoses list
		$diagnosis_data = $this->DiagnosisMaster->find('threaded',
			array('conditions' => array('DiagnosisMaster.participant_id' => $participant_id)));
		$found_dx = false;
		if (isset($this->request->data['Collection']['diagnosis_master_id'])) {
			$found_dx = $this->DiagnosisMaster->arrangeThreadedDataForView($diagnosis_data,
				$this->request->data['Collection']['diagnosis_master_id'], 'Collection');
		}
		$this->set('diagnosis_data', $diagnosis_data);
		$this->set('found_dx', $found_dx);

		//set tx list
		$tx_data = $this->TreatmentMaster->find('all', array(
			'conditions' => array(
		...
		if (isset($this->request->data['Collection']['treatment_master_id'])) {
			$found_tx = $this->setForRadiolist($tx_data, 'TreatmentMaster', 'id', $this->request->data, 'Collection',
				'treatment_master_id');
		}
		...
		//set event list
		$event_data = $this->EventMaster->find('all', array(
			'conditions' => array(
		...
		$found_event = false;
		if (isset($this->request->data['Collection']['treatment_master_id'])) {
			$found_event = $this->setForRadiolist($event_data, 'EventMaster', 'id', $this->request->data, 'Collection',
				'event_master_id');
		}
			...
			if ($submitted_data_validates && $this->Collection->save($this->request->data, true, $fields)) {
				...
				if (isset($this->request->data['Collection']['deleted'])) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/ClinicalAnnotation/ClinicalCollectionLinks/detail/' . $participant_id . '/' . $this->Collection->id);
				}
				return;
			}
		}
	...
	public function edit($participant_id, $collection_id) {
		$collection_data = $this->Collection->getOrRedirect($collection_id);
		if ($collection_data['Collection']['participant_id'] != $participant_id) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		}
		...
		foreach (array(
			...
			if (!array_key_exists($foreign_key, $data_for_form['Collection'])) {
				$data_for_form['Collection'][$foreign_key] = null;
			}
		}

		// Set consents list
		$consent_data = $this->ConsentMaster->find('all', array(
			'conditions' => array(
		...
		$consent_found = $this->setForRadiolist($consent_data, 'ConsentMaster', 'id', $data_for_form, 'Collection',
			'consent_master_id');
		$this->set('consent_data', $consent_data);
		$this->set('consent_found', $consent_found);

		// Set diagnoses list
		$diagnosis_data = $this->DiagnosisMaster->find('threaded', array(
			'conditions' => array(
		...
		$found_dx = $this->DiagnosisMaster->arrangeThreadedDataForView($diagnosis_data,
			(isset($data_for_form['Collection']['diagnosis_master_id']) ? $data_for_form['Collection']['diagnosis_master_id'] : null),
			'Collection');
		...
		$found_tx = $this->setForRadiolist($tx_data, 'TreatmentMaster', 'id', $data_for_form, 'Collection',
			'treatment_master_id');
		$this->set('tx_data', $tx_data);
		$this->set('found_tx', $found_tx);
		...
		$found_event = $this->setForRadiolist($event_data, 'EventMaster', 'id', $data_for_form, 'Collection',
			'event_master_id');
		$this->set('event_data', $event_data);
		$this->set('found_event', $found_event);
		...
		if (!empty($this->request->data)) {
			...
			if ($submitted_data_validates && $this->Collection->save($this->request->data, true, $fields)) {
				...
				$this->atimFlash(__('your data has been updated'),
					'/ClinicalAnnotation/ClinicalCollectionLinks/detail/' . $participant_id . '/' . $collection_id);
				return;
			}
		} else {
	...
	public function delete($participant_id, $collection_id) {
		$collection_data = $this->Collection->getOrRedirect($collection_id);
		if ($collection_data['Collection']['participant_id'] != $participant_id) {
			$this->redirect('/Pages/err_plugin_no_data?method=' . __METHOD__ . ',line=' . __LINE__, null, true);
		}
		...
		if ($arr_allow_deletion['allow_deletion']) {
			$collection_data_to_update = array(
				'Collection' => array(
					'participant_id' => null,
			...
			if ($this->Collection->save($collection_data_to_update, false)) {
				...
				$this->atimFlash(__('your data has been deleted') . '<br>' . __('use inventory management module to delete the entire collection'),
					'/ClinicalAnnotation/ClinicalCollectionLinks/listall/' . $participant_id . '/');
			} else {

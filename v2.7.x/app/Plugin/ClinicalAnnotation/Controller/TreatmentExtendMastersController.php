...
class TreatmentExtendMastersController extends ClinicalAnnotationAppController {
	...
	function add($participant_id, $tx_master_id) {
		...
		if(!$tx_master_data['TreatmentControl']['treatment_extend_control_id']){
			$this->atimFlash(__('no additional data has to be defined for this type of treatment'), '/ClinicalAnnotation/TreatmentMasters/detail/'.$participant_id.'/'.$tx_master_id);
			return;
		}
	...
	function delete($participant_id, $tx_master_id, $tx_extend_id) {
		...
		if($arr_allow_deletion['allow_deletion']) {		
			if( $this->TreatmentExtendMaster->atimDelete( $tx_extend_id ) ) {
				...
				$this->atimFlash(__('error deleting data - contact administrator'), '/ClinicalAnnotation/TreatmentMasters/detail/'.$participant_id.'/'.$tx_master_id);
			}	
		} else {
	...
	function importDrugFromChemoProtocol($participant_id, $tx_master_id){
		...
		if(is_numeric($tx_master_data['TreatmentMaster']['protocol_master_id'])){
			...
			if(empty($prot_extend_data)){
				$this->atimFlash(__('there is no drug defined in the associated protocol'), '/ClinicalAnnotation/TreatmentMasters/detail/'.$participant_id.'/'.$tx_master_id);
			}else{
				...
				if($this->TreatmentExtendMaster->saveAll($data)){
					...
					$this->atimFlash(__('unknown error'), '/ClinicalAnnotation/TreatmentMasters/detail/'.$participant_id.'/'.$tx_master_id);
				}
			}
		}else{
			$this->atimFlash(__('there is no protocol associated with this treatment'), '/ClinicalAnnotation/TreatmentMasters/detail/'.$participant_id.'/'.$tx_master_id);
		}
	}
}

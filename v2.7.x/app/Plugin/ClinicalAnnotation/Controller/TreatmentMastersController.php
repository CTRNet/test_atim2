...
class TreatmentMastersController extends ClinicalAnnotationAppController {
	...
	public function listall($participant_id, $treatment_control_id = null) {
		// MANAGE DATA
		...
		if (!$treatment_control_id) {
			// 1 - MANAGE DISPLAY
			$treatment_controls = $this->TreatmentControl->find('all',
				array('conditions' => array('TreatmentControl.flag_active' => '1')));
			...
			if ($treatment_control_id == '-1') {
				...
				self::buildDetailBinding($this->TreatmentMaster, $search_criteria,
					$control_data['TreatmentControl']['form_alias']);
			}
		}

		// MANAGE DATA
		$this->request->data = $treatment_control_id ? $this->paginate($this->TreatmentMaster,
			$search_criteria) : array();
	...
	public function detail($participant_id, $tx_master_id) {
		// MANAGE DATA
		$treatment_master_data = $this->TreatmentMaster->find('first', array(
			'conditions' => array(
		...
		$this->set('diagnosis_data',
			$this->DiagnosisMaster->getRelatedDiagnosisEvents($this->request->data['TreatmentMaster']['diagnosis_master_id']));

		// MANAGE FORM, MENU AND ACTION BUTTONS
		$this->set('atim_menu_variables',
			array('Participant.id' => $participant_id, 'TreatmentMaster.id' => $tx_master_id));
		...
		if ($treatment_master_data['TreatmentControl']['treatment_extend_control_id']) {
			...
			$this->set('tx_extend_data', $this->TreatmentExtendMaster->find('all', array(
				'conditions' => array(
					'TreatmentExtendMaster.treatment_master_id' => $tx_master_id,
					'TreatmentExtendMaster.treatment_extend_control_id' => $treatment_master_data['TreatmentControl']['treatment_extend_control_id']
				)
			)));
			$this->Structures->set($treatment_extend_control_data['TreatmentExtendControl']['form_alias'],
				'extend_form_alias');
			if (!empty($treatment_master_data['TreatmentControl']['extended_data_import_process'])) {
				$this->set('extended_data_import_process',
					$treatment_master_data['TreatmentControl']['extended_data_import_process']);
			}
		}
	...
	public function edit($participant_id, $tx_master_id) {
		// MANAGE DATA
		$treatment_master_data = $this->TreatmentMaster->find('first', array(
			'conditions' => array(
				'TreatmentMaster.id' => $tx_master_id,
		...
		$dx_data = $this->DiagnosisMaster->find('threaded', array(
			'conditions' => array('DiagnosisMaster.participant_id' => $participant_id),
			...
			if ($submitted_data_validates) {
				...
				if ($this->TreatmentMaster->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/ClinicalAnnotation/TreatmentMasters/detail/' . $participant_id . '/' . $tx_master_id);
				}
			}
		}
	}

	public function add($participant_id, $tx_control_id, $diagnosis_master_id = null) {
		...
		if (isset($this->request->data['TreatmentMaster']['diagnosis_master_id'])) {
			$this->DiagnosisMaster->arrangeThreadedDataForView($dx_data,
				$this->request->data['TreatmentMaster']['diagnosis_master_id'], 'TreatmentMaster');
		} else {
		...
		// MANAGE FORM, MENU AND ACTION BUTTONS
		$this->set('atim_menu_variables',
			array('Participant.id' => $participant_id, 'TreatmentControl.id' => $tx_control_id));
		...
		// Set trt header
		$this->set('tx_header',
			__($tx_control_data['TreatmentControl']['tx_method']) . (empty($tx_control_data['TreatmentControl']['disease_site']) ? '' : ' - ' . __($tx_control_data['TreatmentControl']['disease_site'])));

		// set DIAGANOSES radio list form
		$this->Structures->set('view_diagnosis', 'diagnosis_structure');
		$this->Structures->set($tx_control_data['TreatmentControl']['form_alias'], 'atim_structure',
			array('model_table_assoc' => array('TreatmentDetail' => $tx_control_data['TreatmentControl']['detail_tablename'])));
		$this->Structures->Set('empty', 'empty_structure');
		...
		if (empty($this->request->data)) {
			...
			if (!$tx_control_data['TreatmentControl']['use_addgrid']) {
				...
				$this->TreatmentMaster->addWritableField('treatment_master_id',
					$tx_control_data['TreatmentControl']['detail_tablename']);
				...
				if ($submitted_data_validates) {
					if ($this->TreatmentMaster->save($this->request->data)) {
						...
						$this->atimFlash(__('your data has been saved'),
							'/ClinicalAnnotation/TreatmentMasters/detail/' . $participant_id . '/' . $this->TreatmentMaster->getLastInsertId());
					}
				}
				...
				$diagnosis_master_id = (array_key_exists('TreatmentMaster',
						$this->request->data) && array_key_exists('diagnosis_master_id',
						$this->request->data['TreatmentMaster'])) ? $this->request->data['TreatmentMaster']['diagnosis_master_id'] : null;
				...
				if (empty($this->request->data)) {
					...
					if (empty($errors_tracking)) {
						...
						$this->atimFlash(__('your data has been updated'),
							'/ClinicalAnnotation/TreatmentMasters/listall/' . $participant_id . '/');
					} else {
						$this->TreatmentMaster->validationErrors = array();
						foreach ($errors_tracking as $field => $msg_and_lines) {
							foreach ($msg_and_lines as $msg => $lines) {
								$this->TreatmentMaster->validationErrors[$field][] = $msg . ' - ' . str_replace('%s',
	...
	public function delete($participant_id, $tx_master_id) {

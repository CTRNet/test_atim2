...
class MiscIdentifiersController extends ClinicalAnnotationAppController {
	...
	function add( $participant_id, $misc_identifier_control_id) {
		...
		if($controls['MiscIdentifierControl']['flag_once_per_participant']) {
			...
			if($already_exist) {
				$this->atimFlash(__('this identifier has already been created for this participant'),'/ClinicalAnnotation/Participants/profile/'.$participant_id.'/' );
				return;
			}
		}
	...
	function delete( $participant_id, $misc_identifier_id ) {
		...
		if($arr_allow_deletion['allow_deletion']) {
			...
			if($deletion_worked){
				...
				$this->atimFlash(__('error deleting data - contact administrator'), '/ClinicalAnnotation/Participants/profile/'.$participant_id.'/' );
			}
		} else {

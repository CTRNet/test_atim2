...
class ParticipantsController extends ClinicalAnnotationAppController {
	...
	public function profile($participant_id) {
		// MANAGE DATA
		$this->request->data = $this->Participant->getOrRedirect($participant_id);

		// Set data for identifier list
		$participant_identifiers_data = $this->paginate($this->MiscIdentifier,
			array('MiscIdentifier.participant_id' => $participant_id));
		...
		foreach ($identifier_controls_list as &$unit) {
			if (!empty($unit['MiscIdentifierControl']['autoincrement_name']) && array_key_exists($unit['MiscIdentifierControl']['id'],
					$reusable)
			) {
				$unit['reusable'] = true;
			}
			...
			if ($submitted_data_validates) {
				if ($this->Participant->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been saved'),
						'/ClinicalAnnotation/Participants/profile/' . $this->Participant->getLastInsertID());
				}
			}
		}
	}
			...
			if ($submitted_data_validates) {
				...
				if ($this->Participant->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/ClinicalAnnotation/Participants/profile/' . $participant_id);
				}
			}
		}
	}
		...
		if ($arr_allow_deletion['allow_deletion']) {
			if ($this->Participant->atimDelete($participant_id)) {
				...
				$this->atimFlash(__('error deleting data - contact administrator'),
					'/ClinicalAnnotation/Participants/search/');
			}
		} else {
	...
	public function chronology($participant_id) {
		$tmp_array = array();
		$this->set('atim_menu_variables', array('Participant.id' => $participant_id));
		...
		$this->StructurePermissibleValuesCustom = AppModel::getInstance("", "StructurePermissibleValuesCustom",
			true); // Use of $StructurePermissibleValuesCustom->getTranslatedCustomDropdownValue()
		...
		$participant = $this->Participant->find('first',
			array('conditions' => array('Participant.id' => $participant_id)));
		...
		if ($chronolgy_data_participant_birth) {
			$add_to_tmp_array($chronolgy_data_participant_birth);
		}
		if ($chronolgy_data_participant_death) {
			$add_to_tmp_array($chronolgy_data_participant_death);
		}

		// 2-Consent

		$consent_status = $this->StructureValueDomain->find('first',
			array('conditions' => array('StructureValueDomain.domain_name' => 'consent_status'), 'recursive' => 2));
		...
		$consents = $this->ConsentMaster->find('all',
		...
		foreach ($consents as $consent) {
			...
			if ($chronolgy_data_consent) {
				$add_to_tmp_array($chronolgy_data_consent);
			}
		}

		// 2-Diagnosis

		$dxs = $this->DiagnosisMaster->find('all',
		...
		foreach ($dxs as $dx) {
			...
			if ($chronolgy_data_diagnosis) {
				$add_to_tmp_array($chronolgy_data_diagnosis);
			}
		}

		// 3-Event

		$annotations = $this->EventMaster->find('all',
			array('conditions' => array('EventMaster.participant_id' => $participant_id)));
		foreach ($annotations as $annotation) {
			...
			if ($chronolgy_data_annotation) {
				$add_to_tmp_array($chronolgy_data_annotation);
			}
		}

		// 4-Treatment

		$txs = $this->TreatmentMaster->find('all',
			array('conditions' => array('TreatmentMaster.participant_id' => $participant_id)));
		foreach ($txs as $tx) {
			...
			if ($chronolgy_data_treatment_start) {
				$add_to_tmp_array($chronolgy_data_treatment_start);
			}
			if ($chronolgy_data_treatment_finish) {
				$add_to_tmp_array($chronolgy_data_treatment_finish);
			}
		}

		$collection_model = AppModel::getInstance('InventoryManagement', 'Collection', true);
		$collections = $collection_model->find('all',
			array('conditions' => array('Collection.participant_id' => $participant_id), 'recursive' => -1));
		foreach ($collections as $collection) {
			...
			if ($chronolgy_data_collection) {
				$add_to_tmp_array($chronolgy_data_collection);
			}
		}
		...
		if (isset($this->request->data['Participant']['id']) && (is_array($this->request->data['Participant']['id']) || $this->request->data['Participant']['id'] == 'all')) {
			...
			if (isset($this->request->data[0]['ids']) && strlen($this->request->data[0]['ids'])) {
				...
				$participants = $this->Participant->find('all',
					array('conditions' => array('Participant.id' => explode(",", $this->request->data[0]['ids']))));
				...
				if ($submitted_data_validates) {
					...
					foreach ($ids as $id) {
						...
						$this->Participant->save($this->request->data['Participant'],
							array('validate' => false, 'fieldList' => array_keys($this->request->data['Participant'])));
					}

...
class DiagnosisMastersController extends ClinicalAnnotationAppController {
	...
	public function listall($participant_id, $parent_dx_id = null, $is_ajax = 0) {
		// MANAGE DATA
		...
		$can_have_child = $this->DiagnosisMaster->find('all', array(
			...
			'conditions' => array(
				'DiagnosisControl.category' => array('primary', 'secondary'),
				'DiagnosisMaster.participant_id' => $participant_id
			),
			'recursive' => 0
		));
		...
		$this->set('diagnosis_controls_list',
			$this->DiagnosisControl->find('all', array('conditions' => array('DiagnosisControl.flag_active' => 1))));
		$this->set('treatment_controls_list',
			$this->TreatmentControl->find('all', array('conditions' => array('TreatmentControl.flag_active' => 1))));
		$this->set('event_controls_list',
			$this->EventControl->find('all', array('conditions' => array('EventControl.flag_active' => 1))));
		...
		$help_url = $external_link_model->find('first',
			array('conditions' => array('name' => 'diagnosis_module_wiki')));
	...
	public function detail($participant_id, $diagnosis_master_id) {
		// MANAGE DATA
		...
		$dx_control_data = $this->DiagnosisControl->find('first',
			array('conditions' => array('DiagnosisControl.id' => $dx_master_data['DiagnosisMaster']['diagnosis_control_id'])));
	...
	public function setDiagnosisMenu($dx_master_data, $additional_menu_variables = array()) {
	...
	function add($participant_id, $dx_control_id, $parent_id) {
		...
		if ($parent_id == 0) {
			if ($dx_ctrl['DiagnosisControl']['category'] != 'primary') {
				...
				$this->atimFlash(__('invalid control id'), 'javascript:history.back();');
			}
		} else {
		...
		$dx_control_data = $this->DiagnosisControl->find('first',
			array('conditions' => array('DiagnosisControl.id' => $dx_control_id)));
		$this->Structures->set($dx_control_data['DiagnosisControl']['form_alias'] . "," . ($parent_id == 0 ? "dx_origin_primary" : "dx_origin_wo_primary"),
			'atim_structure',
			array('model_table_assoc' => array('DiagnosisDetail' => $dx_control_data['DiagnosisControl']['detail_tablename'])));
		$this->Structures->set('empty', 'empty_structure');
		...
		if (!empty($this->request->data)) {
			...
			$this->DiagnosisMaster->addWritableField('diagnosis_master_id',
				$dx_control_data['DiagnosisControl']['detail_tablename']);
			...
			if ($submitted_data_validates) {
				if ($this->DiagnosisMaster->save($this->request->data)) {
					...
					if ($parent_id == 0) {
						...
						$this->DiagnosisMaster->tryCatchQuery(str_replace("diagnosis_masters", "diagnosis_masters_revs",
							$query_to_update));
					}
					...
					$this->atimFlash(__('your data has been saved'),
						'/ClinicalAnnotation/DiagnosisMasters/detail/' . $participant_id . '/' . $diagnosis_master_id . '/');
				}
			}
		}
	}

	public function edit($participant_id, $diagnosis_master_id, $redefined_primary_control_id = null) {
		...
		if (!is_null($redefined_primary_control_id)) {
			...
			$new_primary_ctrl = $this->DiagnosisControl->find('first',
				array('conditions' => array('DiagnosisControl.id' => $redefined_primary_control_id)));
	...
	public function delete($participant_id, $diagnosis_master_id) {
		// MANAGE DATA
		...
		if ($arr_allow_deletion['allow_deletion']) {
			if ($this->DiagnosisMaster->atimDelete($diagnosis_master_id)) {
				$this->atimFlash(__('your data has been deleted'),
					'/ClinicalAnnotation/DiagnosisMasters/listall/' . $participant_id);
			} else {

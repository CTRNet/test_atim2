...
class EventMastersController extends ClinicalAnnotationAppController {
	...
	public function beforeFilter() {
		parent::beforeFilter();
		$this->set('atim_menu',
			$this->Menus->get('/' . $this->params['plugin'] . '/' . $this->params['controller'] . '/' . $this->params['action'] . '/' . $this->params['pass'][0]));
	}

	public function listall($event_group, $participant_id, $event_control_id = null) {
		$participant_data = $this->Participant->getOrRedirect($participant_id);

		$search_criteria = array();
		if (!$event_control_id) {
			// 1 - MANAGE DISPLAY
			$event_controls = $this->EventControl->find('all', array(
				'conditions' => array(
			...
			if ($event_control_id == '-1') {
				...
				self::buildDetailBinding($this->EventMaster, $search_criteria,
					$control_data['EventControl']['form_alias']);
			}
		}
		...
		$this->set('atim_menu_variables',
			array('EventMaster.event_group' => $event_group, 'Participant.id' => $participant_id));
	...
	public function detail($participant_id, $event_master_id, $is_ajax = 0) {
		// MANAGE DATA
		...
		// MANAGE FORM, MENU AND ACTION BUTTONS
		$this->set('atim_menu',
			$this->Menus->get('/' . $this->params['plugin'] . '/' . $this->params['controller'] . '/listall/' . $event_group));
		...
		if (isset($diagnosis_data[0]) && strpos($this->request->data['EventControl']['event_type'],
				'cap report - ') !== false && $diagnosis_data[0]['DiagnosisControl']['flag_compare_with_cap']
		) {
	...
	public function add($participant_id, $event_control_id, $diagnosis_master_id = null) {
		...
		if (!empty($this->request->data) && isset($this->request->data['EventMaster']['diagnosis_master_id'])) {
			$this->DiagnosisMaster->arrangeThreadedDataForView($dx_data,
				$this->request->data['EventMaster']['diagnosis_master_id'], 'EventMaster');
		} else {
		...
		$this->set('atim_menu_variables', array(
			'EventControl.event_group' => $event_group,
			'Participant.id' => $participant_id,
			'EventControl.id' => $event_control_id
		));
		$this->set('ev_header',
			__($event_control_data['EventControl']['event_type']) . (empty($event_control_data['EventControl']['disease_site']) ? '' : ' - ' . __($event_control_data['EventControl']['disease_site'])));
			...
			if (!$event_control_data['EventControl']['use_addgrid']) {
				...
				if ($submitted_data_validates && $this->EventMaster->save($this->request->data)) {
					...
					$this->atimFlash(__('your data has been updated'),
						'/ClinicalAnnotation/EventMasters/detail/' . $participant_id . '/' . $this->EventMaster->getLastInsertId());
				}
				...
				$diagnosis_master_id = (array_key_exists('EventMaster',
						$this->request->data) && array_key_exists('diagnosis_master_id',
						$this->request->data['EventMaster'])) ? $this->request->data['EventMaster']['diagnosis_master_id'] : null;
				...
				if (empty($this->request->data)) {
					...
					if (empty($errors_tracking)) {
						...
						$this->atimFlash(__('your data has been updated'),
							'/ClinicalAnnotation/EventMasters/listall/' . $event_group . '/' . $participant_id . '/');
					} else {
						$this->EventMaster->validationErrors = array();
						foreach ($errors_tracking as $field => $msg_and_lines) {
							foreach ($msg_and_lines as $msg => $lines) {
								$this->EventMaster->validationErrors[$field][] = $msg . ' - ' . str_replace('%s',
	...
	public function edit($participant_id, $event_master_id) {
		if (!AppController::checkLinkPermission('/ClinicalAnnotation/DiagnosisMasters/listall/')) {
			$this->atimFlash(__('you need privileges to access this page'), 'javascript:history.back()');
		}

		// MANAGE DATA
		$event_master_data = $this->EventMaster->find('first', array(
			'conditions' => array(
				'EventMaster.id' => $event_master_id,
				'EventMaster.participant_id' => $participant_id
			)
		...
		if (!empty($this->request->data)) {
			...
			if ($submitted_data_validates && $this->EventMaster->save($this->request->data)) {
				...
				$this->atimFlash(__('your data has been updated'),
					'/ClinicalAnnotation/EventMasters/detail/' . $participant_id . '/' . $event_master_id);
			}
		} else {
	...
	public function delete($participant_id, $event_master_id) {
		$event_master_data = $this->EventMaster->find('first', array(
			'conditions' => array(
				'EventMaster.id' => $event_master_id,
				'EventMaster.participant_id' => $participant_id

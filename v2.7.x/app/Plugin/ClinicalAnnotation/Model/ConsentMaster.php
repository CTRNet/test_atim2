...
class ConsentMaster extends ClinicalAnnotationAppModel {
	...
	static public $join_consent_control_on_dup = array(
		'table' => 'consent_controls',
		'alias' => 'ConsentControl',
		'type' => 'LEFT',
		'conditions' => array('consent_masters_dup.consent_control_id = ConsentControl.id')
	);
	...
	function allowDeletion($consent_master_id) {
		...
		$returned_nbr = $collection_model->find('count',
			array('conditions' => array('Collection.consent_master_id' => $consent_master_id)));
		if ($returned_nbr > 0) {
	...
	static function joinOnConsentDup($on_field) {
		return array(
			...
			'type' => 'LEFT',
			'conditions' => array($on_field . ' = consent_masters_dup.id')
		);
	}
}

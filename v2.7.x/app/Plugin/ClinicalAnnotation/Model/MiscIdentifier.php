...
class MiscIdentifier extends ClinicalAnnotationAppModel {
	...
	function beforeFind($queryData) {
		if (!AppController::getInstance()->Session->read('flag_show_confidential') && is_array($queryData['conditions']) && AppModel::isFieldUsedAsCondition("MiscIdentifier.identifier_value",
	...
	function find($type = 'first', $query = array()) {
		if (isset($query['conditions'])) {
			...
			if ($gt_key || $lt_key) {
				$inf_value = $gt_key ? str_replace(',', '.',
					$query['conditions']['MiscIdentifier.identifier_value >=']) : '';
				$sup_value = $lt_key ? str_replace(',', '.',
					$query['conditions']['MiscIdentifier.identifier_value <=']) : '';
				...
				if (isset($results[0]) && isset($results[0]['MiscIdentifier'])) {
					if (isset($results[0]['MiscIdentifier']['misc_identifier_control_id'])) {
						foreach ($results as &$result) {
							if (in_array($result['MiscIdentifier']['misc_identifier_control_id'],
								$confidential_control_ids)) {
	...
	function validates($options = array()) {
		...
		if (!isset($this->data['MiscIdentifier']['deleted']) || $this->data['MiscIdentifier']['deleted'] == 0) {
			...
			if ($rule && isset($this->data['MiscIdentifier']['identifier_value']) && !preg_match('/' . $rule . '/',
					$this->data['MiscIdentifier']['identifier_value'])
			) {
				$msg = __('the format of the identifier is incorrect');
				if (!empty($current['MiscIdentifierControl']['user_readable_format'])) {
					$msg .= ' ' . __('expected misc identifier format is %s',
							$current['MiscIdentifierControl']['user_readable_format']);
				}

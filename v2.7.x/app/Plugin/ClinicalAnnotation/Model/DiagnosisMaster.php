...
class DiagnosisMaster extends ClinicalAnnotationAppModel {
	...
	static public $join_diagnosis_control_on_dup = array(
		'table' => 'diagnosis_controls',
		'alias' => 'DiagnosisControl',
		'type' => 'LEFT',
		'conditions' => array('diagnosis_masters_dup.diagnosis_control_id = DiagnosisControl.id')
	);
	...
	function summary($diagnosis_master_id = null) {
		...
		if (!is_null($diagnosis_master_id)) {
			$result = $this->find('first',
				array('conditions' => array('DiagnosisMaster.id' => $diagnosis_master_id), 'recursive' => 0));
			...
			$return = array(
				'menu' => array(
					null,
					__($result['DiagnosisControl']['category'],
						true) . ' - ' . __($result['DiagnosisControl']['controls_type'], true)
				),
	...
	function patchIcd10NullValues(&$participant_array) {
		if (array_key_exists('icd10_code',
				$participant_array['DiagnosisMaster']) && strlen(trim($participant_array['DiagnosisMaster']['icd10_code'])) == 0
		) {
	...
	function allowDeletion($diagnosis_master_id) {
		...
		$nbr_primary = $this->find('count', array(
			'conditions' => array(
		...
		// Check for existing records linked to the participant. If found, set error message and deny delete
		$nbr_parent = $this->find('count',
			array('conditions' => array('DiagnosisMaster.parent_id' => $diagnosis_master_id), 'recursive' => '-1'));
		if ($nbr_parent > 0) {
			$arr_allow_deletion['allow_deletion'] = false;
		...
		$nbr_linked_collection = $collection_model->find('count',
			array('conditions' => array('Collection.diagnosis_master_id' => $diagnosis_master_id)));
		...
		$nbr_events = $event_master_model->find('count',
			array('conditions' => array('EventMaster.diagnosis_master_id' => $diagnosis_master_id)));
		if ($nbr_events > 0) {
			$arr_allow_deletion['allow_deletion'] = false;
		...
		$nbr_treatment = $treatment_master_model->find('count',
			array('conditions' => array('TreatmentMaster.diagnosis_master_id' => $diagnosis_master_id)));
		if ($nbr_treatment > 0) {
			$arr_allow_deletion['allow_deletion'] = false;
	...
	function getExistingDx($participant_id, $current_dx_id = '0', $current_dx_primary_number = '') {
		$existing_dx = $this->find('all', array(
			'conditions' => array(
				'DiagnosisMaster.participant_id' => $participant_id,
				'DiagnosisMaster.id != ' . $current_dx_id
			)
		));
		//sort by dx number
	...
	function arrangeThreadedDataForView(array &$threaded_dx_data, $seeking_dx_id, $seeking_model_name) {
		...
		foreach ($threaded_dx_data as &$data) {
			...
			if (isset($data['children']) && !empty($data['children'])) {
				if ($found_dx = $this->arrangeThreadedDataForView($data['children'], $seeking_dx_id,
					$seeking_model_name)
				) {
					break;
				}
	...
	function getRelatedDiagnosisEvents($diagnosis_master_id) {
		...
		if (!empty($diagnosis_master_id)) {
			...
			$related_diagnosis_data[] = array_merge(array('Generated' => array('diagnosis_event_relation_type' => 'diagnosis event')),
				$event_diagnosis_data);

			$history_diagnosis_data = array();
			if ($event_diagnosis_data['DiagnosisMaster']['id'] != $event_diagnosis_data['DiagnosisMaster']['primary_id']) {
				$history_diagnosis_data = $this->find('all', array(
					'conditions' => array(
						'DiagnosisMaster.id' => array(
							$event_diagnosis_data['DiagnosisMaster']['primary_id'],
							$event_diagnosis_data['DiagnosisMaster']['parent_id']
						)
					),
				...
				foreach ($history_diagnosis_data as $new_diag) {
					$related_diagnosis_data[] = array_merge(array('Generated' => array('diagnosis_event_relation_type' => 'diagnosis history')),
						$new_diag);
				}
			}
		}
	...
	static function joinOnDiagnosisDup($on_field) {
		return array(
			...
			'type' => 'LEFT',
			'conditions' => array($on_field . ' = diagnosis_masters_dup.id')
		);
	}
}

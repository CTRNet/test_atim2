...
class StructurePermissibleValuesCustom extends AppModel {
	...
	public function getTranslatedCustomDropdownValue($control_name, $value) {
		$lang = self::getLanguage();
		...
		if (isset($this->data['StructurePermissibleValuesCustom']['control_id'])) {
			...
			if ($this->id) {
				$control_id = $this->find('first', array(
					'conditions' => array('StructurePermissibleValuesCustom.id' => $this->id),
					'fields' => array('StructurePermissibleValuesCustom.control_id')
				));
				$control_id = $control_id['StructurePermissibleValuesCustom']['control_id'];
			}
		}
		if ($control_id) {
			$values_counter = $this->find('count',
				array('conditions' => array('StructurePermissibleValuesCustom.control_id' => $control_id)));
			$values_used_as_input_counter = $this->find('count', array(
				'conditions' => array(
					'StructurePermissibleValuesCustom.control_id' => $control_id,
					'StructurePermissibleValuesCustom.use_as_input' => '1'
				)
			));
			$StructurePermissibleValuesCustomControl = AppModel::getInstance('',
				'StructurePermissibleValuesCustomControl');
			$this->tryCatchQuery("UPDATE structure_permissible_values_custom_controls
									SET values_counter = $values_counter,
										values_used_as_input_counter = $values_used_as_input_counter 
									WHERE id = $control_id;");
		}

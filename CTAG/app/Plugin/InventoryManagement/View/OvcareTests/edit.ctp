<?php 
	$structure_links = array(
		'top'=>'/InventoryManagement/OvcareTests/edit/'
			.$atim_menu_variables['Collection.id'].'/'
			.$atim_menu_variables['SampleMaster.id'].'/'
			.$atim_menu_variables['OvcareTest.id'].'/',
		'bottom'=>array(
			'cancel'=>'/InventoryManagement/OvcareTests/detail/'
				.$atim_menu_variables['Collection.id'].'/'
				.$atim_menu_variables['SampleMaster.id'].'/'
				.$atim_menu_variables['OvcareTest.id'].'/',
		)
	...
	//aliquot part
	$aliquot_master_id = $this->request->data['OvcareTest']['aliquot_master_id'];
	$na_checked = true;
	foreach($aliquot_data_vol as &$aliquot_data_unit){
		if($aliquot_data_unit['AliquotMaster']['id'] == $aliquot_master_id){
			$aliquot_data_unit['OvcareTest']['aliquot_master_id'] = $aliquot_master_id;
			$na_checked = false;
			break;
		}
	}
	...
	$structure_links['radiolist'] = array('OvcareTest.aliquot_master_id' => '%%AliquotMaster.id%%');
	...
	?>
		<table class="structure"><tbody><tr>
			<td style='text-align: left; padding-left: 10px;'>
 				...
 				<?php echo $this->Form->radio('OvcareTest.aliquot_master_id', array('na' => 'N/A'), array('legend'=>false, 'value'=>false, 'checked' => $na_checked)); ?>

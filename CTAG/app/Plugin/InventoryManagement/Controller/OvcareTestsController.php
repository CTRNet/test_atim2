...
class OvcareTestsController extends InventoryManagementAppController {
	...
	var $uses = array(
		...
		'InventoryManagement.OvcareTest'
	);
	
	var $paginate = array('OvcareTest' => array('limit' => pagination_amount, 'order' => 'OvcareTest.date ASC'));
	...
	function listAll($collection_id, $sample_master_id) {
		...
		$this->request->data = $this->paginate($this->OvcareTest, array('OvcareTest.sample_master_id'=>$sample_master_id));
		...
		$this->set('atim_menu', $this->Menus->get('/InventoryManagement/OvcareTests/listAll/%%Collection.id%%/' . $sample_id_parameter));		
	...
	function add($sample_master_id = null){
		...
		$this->Structures->set('ovcaretests', 'ov_test_structure');
		$this->Structures->set('ovcaretests,ovaretests_volume', 'ov_test_volume_structure');
		...
		if($sample_master_id != null){
			...
			$cancel_button = '/InventoryManagement/OvcareTests/listAll/'.$menu_data['collection_id'].'/'.$sample_master_id;
		...
		if(isset($this->request->data['ViewAliquot']) || isset($this->request->data['ViewSample'])){
			...
			foreach($this->request->data as $key => $data_unit){
				...
				if(empty($data_unit)){
					...
					foreach($data_unit as $ovcare_test){
						$this->OvcareTest->data = null;
						$this->OvcareTest->set($ovcare_test);
						if(!$this->OvcareTest->validates()){
							foreach($this->OvcareTest->validationErrors as $field => $msgs){
								$msgs = is_array($msgs)? $msgs : array($msgs);
								foreach($msgs as $msg) $errors[$field][$msg][] = $record_counter;
							}
						}
						$ovcare_test = $this->OvcareTest->data; 
						$ovcare_test['OvcareTest']['aliquot_master_id'] = $aliquot_master_id;
						$ovcare_test['OvcareTest']['sample_master_id'] = $studied_sample_master_id;
						$ov_test_data_to_save[] = $ovcare_test;
					}
				}
			}
			...
			//save
			if(empty($errors) && !empty($ov_test_data_to_save)){
				$this->OvcareTest->addWritableField(array('sample_master_id', 'aliquot_master_id'));
				$this->OvcareTest->writable_fields_mode = 'addgrid';		
				$this->OvcareTest->saveAll($ov_test_data_to_save, array('validate' => false));
				$last_ov_test_id = $this->OvcareTest->getLastInsertId();
				
				$this->OvcareTest->generateOvTestCode();
				...
				if($sample_master_id != null){
					...
					$batch_set_data = array('BatchSet' => array(
						'datamart_structure_id' => $datamart_structure->getIdByModelName('OvcareTest'),
						'flag_tmp' => true
					));
					$batch_set_model->saveWithIds($batch_set_data, range($last_ov_test_id - count($ov_test_data_to_save) + 1, $last_ov_test_id));
				...
				$this->OvcareTest->validationErrors = array();
				if(!empty($errors)) {
					foreach($errors as $field => $msg_and_lines) {
						foreach($msg_and_lines as $msg => $lines) {
							...
							$this->OvcareTest->validationErrors[$field][] = $msg;
						}
					}
				}
	...
	function detail($collection_id, $sample_master_id, $ovcare_test_id) {
		if((!$collection_id) || (!$sample_master_id) || (!$ovcare_test_id)) { 
		...
		$ovcare_test_data = $this->OvcareTest->find('first',array(
			'fields' => array('*'),
			'conditions'=>array('OvcareTest.id' => $ovcare_test_id,'SampleMaster.collection_id' => $collection_id,'SampleMaster.id' => $sample_master_id),
			'joins' => array(
				AliquotMaster::joinOnAliquotDup('OvcareTest.aliquot_master_id'), 
				AliquotMaster::$join_aliquot_control_on_dup,
				SampleMaster::joinOnSampleDup('OvcareTest.sample_master_id'), 
				SampleMaster::$join_sample_control_on_dup)
		));
		if(empty($ovcare_test_data)) { 
			$this->redirect('/Pages/err_plugin_no_data?method='.__METHOD__.',line='.__LINE__, null, true); 
		}
		
		$structure_to_load = 'ovcaretests';
		if(!empty($ovcare_test_data['AliquotControl']['volume_unit'])){
			$structure_to_load .= ",ovcaretests_volume_for_detail";
		}
		$this->Structures->set($structure_to_load);
		
		// Set aliquot data
		$this->set('ovcare_test_data', $ovcare_test_data);
		$this->request->data = array();
		
		// MANAGE FORM, MENU AND ACTION BUTTONS
		
		$sample_id_parameter = ($ovcare_test_data['SampleControl']['sample_category'] == 'specimen')? '%%SampleMaster.initial_specimen_sample_id%%': '%%SampleMaster.id%%';
		$this->set('atim_menu', $this->Menus->get('/InventoryManagement/OvcareTests/detail/%%Collection.id%%/' . $sample_id_parameter . '/%%OvcareTest.id%%'));	
		$this->set( 'atim_menu_variables', 
			array('Collection.id' => $ovcare_test_data['SampleMaster']['collection_id'], 
			'SampleMaster.id' => $ovcare_test_data['OvcareTest']['sample_master_id'],
			'SampleMaster.initial_specimen_sample_id' => $ovcare_test_data['SampleMaster']['initial_specimen_sample_id'],
			'OvcareTest.id' => $ovcare_test_id) );
	...
	function edit($collection_id, $sample_master_id, $ovcare_test_id) {
		...
		// MANAGE DATA
		$this->SampleMaster;//lazy load
		$ov_test_data = $this->OvcareTest->find('first',array(
			'fields' => array('*'),
			'conditions'=>array('OvcareTest.id'=>$ovcare_test_id, 'SampleMaster.collection_id' => $collection_id, 'SampleMaster.id' => $sample_master_id),
			'joins' => array(
				SampleMaster::joinOnSampleDup('OvcareTest.sample_master_id'), 
		...
		if(empty($ov_test_data)) { 
			$this->redirect('/Pages/err_plugin_no_data?method='.__METHOD__.',line='.__LINE__, null, true); 
		}
		
		// MANAGE FORM, MENU AND ACTION BUTTONS
				
		$sample_id_parameter = ($ov_test_data['SampleControl']['sample_category'] == 'specimen')? '%%SampleMaster.initial_specimen_sample_id%%': '%%SampleMaster.id%%';
		$this->set('atim_menu', $this->Menus->get('/InventoryManagement/OvcareTests/detail/%%Collection.id%%/' . $sample_id_parameter . '/%%OvcareTest.id%%'));	
		
		$this->set( 'atim_menu_variables', 
			array('Collection.id' => $ov_test_data['SampleMaster']['collection_id'], 
			'SampleMaster.id' => $ov_test_data['OvcareTest']['sample_master_id'],
			'SampleMaster.initial_specimen_sample_id' =>  $ov_test_data['SampleMaster']['initial_specimen_sample_id'],
			'OvcareTest.id' => $ovcare_test_id) );
		...
		if ( empty($this->request->data) ) {
			$this->request->data = $ov_test_data;
			...
			if(is_numeric($this->request->data['OvcareTest']['aliquot_master_id'])){
				$update_new_aliquot_id = $this->request->data['OvcareTest']['aliquot_master_id'];
				$aliquot_data = $this->AliquotMaster->findById($this->request->data['OvcareTest']['aliquot_master_id']);
				if((empty($aliquot_data) || empty($aliquot_data['AliquotControl']['volume_unit'])) && !empty($this->request->data['OvcareTest']['used_volume'])){
					$this->request->data['OvcareTest']['used_volume'] = null;
					AppController::addWarningMsg(__('this aliquot has no recorded volume').". ".__('the inputed volume was automatically removed').".");
				}
			}else{
				$this->request->data['OvcareTest']['aliquot_master_id'] = null;
			}
			
			if(!empty($ov_test_data['OvcareTest']['aliquot_master_id']) && $ov_test_data['OvcareTest']['aliquot_master_id'] != $this->request->data['OvcareTest']['aliquot_master_id']){
				//the aliquot changed, update the old one afterwards
				$update_old_aliquot_id = $ov_test_data['OvcareTest']['aliquot_master_id'];
			}
			
			if(!array_key_exists('used_volume', $this->request->data['OvcareTest'])){
				$this->request->data['OvcareTest']['used_volume'] = null;
			}
			
			// Save data
			$this->OvcareTest->id = $ovcare_test_id;
			$this->OvcareTest->addWritableField(array('aliquot_master_id'));
			if ($submitted_data_validates && $this->OvcareTest->save( $this->request->data )) {
				...
				$this->atimFlash( 'your data has been saved', '/InventoryManagement/OvcareTests/detail/'.$collection_id.'/'.$sample_master_id.'/'.$ovcare_test_id.'/' );
			}
		}
	...
	function delete($collection_id, $sample_master_id, $ovcare_test_id) {
		if((!$collection_id) || (!$sample_master_id) || (!$ovcare_test_id)) { 
			$this->redirect('/Pages/err_plugin_funct_param_missing?method='.__METHOD__.',line='.__LINE__, null, true); 
		}		

		$ov_test_data = $this->OvcareTest->find('first',array('conditions'=>array('OvcareTest.id'=>$ovcare_test_id, 'SampleMaster.collection_id' => $collection_id, 'SampleMaster.id' => $sample_master_id)));
		if(empty($ov_test_data)) { 
			$this->redirect('/Pages/err_plugin_no_data?method='.__METHOD__.',line='.__LINE__, null, true); 
		}

		// Check deletion is allowed
		$arr_allow_deletion = $this->OvcareTest->allowDeletion($ovcare_test_id);
		...
		if($arr_allow_deletion['allow_deletion']) {
			if($this->OvcareTest->atimDelete($ovcare_test_id)) {
				if($ov_test_data['OvcareTest']['aliquot_master_id'] != null){
					$this->AliquotMaster->updateAliquotUseAndVolume($ov_test_data['OvcareTest']['aliquot_master_id'], true, true, false);
				}
				$this->atimFlash( 'your data has been deleted', 
						'/InventoryManagement/OvcareTests/listAll/'
						.$ov_test_data['SampleMaster']['collection_id'].'/'
						.$ov_test_data['OvcareTest']['sample_master_id'].'/');
			} else {
				$this->flash('error deleting data - contact administrator', '/InventoryManagement/OvcareTests/listAll/' . $collection_id . '/' . $sample_master_id);
			}
		} else {
			$this->flash($arr_allow_deletion['msg'], '/InventoryManagement/OvcareTests/detail/' . $collection_id . '/' . $sample_master_id . '/' . $ovcare_test_id);
		}
	}
}
?>
